...
def eligible_mapping(
    ...
    excluded = sorted(fact_id for fact_id in mapping if partitions[fact_id] == "held-out")
    eligible = {
        fact_id: target
...
def population_mapping(population: dict[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(outcomes, list):
        raise ValueError("population must contain an outcomes array")  # noqa: TRY004
    mapping = {}
    for row in outcomes:
        if (
...
def measure(
    ...
    if population_path is not None:
        ...
        mapping = {fact_id: target for fact_id, target in mapping.items() if fact_id in selected}
        mismatched = sorted(
            fact_id for fact_id, target in mapping.items() if selected_mapping[fact_id] != target
        )
        if mismatched:
            raise ValueError(f"population target definitions disagree with mapping: {mismatched}")
        population_source = {
    ...
    if must_decline_path is not None:
        ...
        for entry in entries:
            if not isinstance(entry, dict) or not isinstance(entry.get("fact_id"), str):
                raise ValueError(  # noqa: TRY004
                    "every must-decline entry must name a string fact_id"
                )
    ...
    if ranking_path is not None:
        ...
        for goal in ranking.get("goals", []):
            if not isinstance(goal, dict) or not isinstance(goal.get("fact_id"), str):
                raise ValueError(  # noqa: TRY004
                    "every ranking goal must name a string fact_id"
                )
            ...
            if not isinstance(candidates, list):
                raise ValueError(  # noqa: TRY004
                    f"ranking goal has no candidate list: {fact_id}"
                )
            ranked_candidates[fact_id] = tuple(
                row["kernel_declaration_id"]
                for row in candidates
                if isinstance(row, dict) and isinstance(row.get("kernel_declaration_id"), str)
            )
    ...
    for fact_id, target_definition in sorted(mapping.items()):
        ...
        row: dict[str, Any] = {
            ...
            "evaluation_class": (
                "must-decline-control" if fact_id in must_decline_ids else "positive-target"
            ),
        }
        ...
        try:
            imported = producers.import_candidate_statement_ndjson(
                ...
                target_definition,
                list(
                    CANDIDATES if transport_native_candidates or modeq_family else candidate_names
                ),
            )
            ...
            if modeq_family:
                ...
                for candidate_name in candidate_names:
                    if kernel.get_declaration(candidate_name) is not None:
                        executable_candidates.append(kernel.name(candidate_name, must_exist=True))
                        ...
                        continue
                    try:
                        transported = producers.transport_native_candidate(imported, candidate_name)
            ...
            if modeq_family:
                ...
                search_metrics = {"inductions_used": candidate.inductions_used}
            else:
    ...
    decline_reasons = Counter(row["reason_kind"] for row in outcomes if row["result"] == "declined")
    rejection_declarations = Counter(
        row.get("trusted_declaration") or row.get("candidate_declaration") or "<unclassified>"
        for row in outcomes
        if row["result"] == "import_rejected"
    )
    class_counts = Counter(row["evaluation_class"] for row in outcomes)
    result_class_counts = Counter((row["result"], row["evaluation_class"]) for row in outcomes)
    transport_counts = Counter(
        transport["result"] for row in outcomes for transport in row.get("candidate_transport", [])
    )
    ...
    result: dict[str, Any] = {
        ...
        "strategy": {
            ...
            "candidate_declarations": (
                [] if modeq_family else (None if ranking_path is not None else list(CANDIDATES))
            ),
        ...
        "census": {
            ...
            "conversion_percent": round(100 * counts["accepted"] / len(outcomes), 1),
    ...
    if population_source is not None:
        source["population_filter"] = population_source
    if must_decline_source is not None:
        source["must_decline_population"] = must_decline_source
    if ranking_source is not None:
        source["candidate_ranking"] = ranking_source
    return result
