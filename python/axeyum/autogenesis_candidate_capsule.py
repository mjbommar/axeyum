...
def materialize(
    ...
    source.add_declaration(
        Declaration.definition(target_name, [], source.sort_zero(), target_declaration.ty)
    )
    ...
    kernel.add_declaration(Declaration.theorem(admitted_name, [], imported.goal(), proposal.proof))
    proof_text = kernel.render_lean(proposal.proof)
    theorem_dependencies = cast(list[str], kernel.theorem_dependencies(admitted_name))
...
def main() -> int:
    ...
    print(
        ...
        f"dependencies={','.join(cast(list[str], report['theorem_dependencies']))}"
    )
    return 0
