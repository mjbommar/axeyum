# Summary

<!-- What does this change and why? Link the roadmap phase or ADR it serves. -->

## Checklist

- [ ] `just check` passes (fmt, clippy, test, doc, links)
- [ ] New transformations ship with their check (evaluator equivalence,
      round trip, lift map) per the evidence rules
- [ ] No architectural decision made silently — open questions closed via
      ADR in `docs/research/09-decisions/`
- [ ] `PLAN.md` status updated if this changes project state
- [ ] Default build remains free of C/C++ dependencies
