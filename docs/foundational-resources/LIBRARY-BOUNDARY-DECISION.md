...
| At least 12 validated example packs | 73 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 61 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 137 expected-result rows
- `replay-only`: 158 expected-result rows
- `lean-horizon`: 36 expected-result rows
