...
| At least 12 validated example packs | 83 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 71 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 169 expected-result rows
- `replay-only`: 175 expected-result rows
- `lean-horizon`: 46 expected-result rows
