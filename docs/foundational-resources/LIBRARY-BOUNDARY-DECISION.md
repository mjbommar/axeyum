...
| At least 12 validated example packs | 155 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 155 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 155 promoted packs. |
...
- `checked`: 381 expected-result rows
- `replay-only`: 509 expected-result rows
- `lean-horizon`: 118 expected-result rows
- `not-run`: 118 expected-result rows
- `expected_result`: 512 `sat`, 378 `unsat`, 118 `not-run`
- row labels: 90 checked witnesses, 290 checked refutations, 422 finite
  witness replays, 87 finite rejection replays, and 118 theorem horizons
- pack labels: 155 checked evidence packs, 144 mixed-trust packs, and 118 packs
  with theorem boundaries
- `solver_reuse`: 155 promoted packs and 0 unclassified packs
