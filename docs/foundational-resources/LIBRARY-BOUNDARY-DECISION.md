...
| At least 12 validated example packs | 64 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 52 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 128 expected-result rows
- `replay-only`: 128 expected-result rows
- `lean-horizon`: 27 expected-result rows
