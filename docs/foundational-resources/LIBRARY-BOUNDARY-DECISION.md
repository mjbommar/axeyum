...
| At least 12 validated example packs | 113 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 113 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 113 promoted packs. |
...
- `checked`: 339 expected-result rows
- `replay-only`: 304 expected-result rows
- `lean-horizon`: 76 expected-result rows
- `not-run`: 76 expected-result rows
- `expected_result`: 349 `sat`, 294 `unsat`, 76 `not-run`
- row labels: 90 checked witnesses, 249 checked refutations, 259 finite
  witness replays, 45 finite rejection replays, and 76 theorem horizons
- pack labels: 113 checked evidence packs, 102 mixed-trust packs, and 76 packs
  with theorem boundaries
- `solver_reuse`: 113 promoted packs and 0 unclassified packs
