...
Reviewed: 2026-06-30; counts refreshed after 78 promoted solver-reuse packs
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 78 promoted packs. |

The current pack-level evidence mix is still intentionally conservative:

- `checked`: 198 expected-result rows
- `replay-only`: 171 expected-result rows
- `lean-horizon`: 47 expected-result rows
- `not-run`: 47 expected-result rows
- `solver_reuse`: 78 promoted packs, 6 non-benchmark-horizon packs, and 0
  unclassified packs
