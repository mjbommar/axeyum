...
| At least 12 validated example packs | 86 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 86 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 86 promoted packs. |

The current pack-level evidence mix is still intentionally conservative:

- `checked`: 206 expected-result rows
- `replay-only`: 177 expected-result rows
- `lean-horizon`: 49 expected-result rows
- `not-run`: 49 expected-result rows
- `solver_reuse`: 86 promoted packs, 0 non-benchmark-horizon packs, and 0
  unclassified packs
