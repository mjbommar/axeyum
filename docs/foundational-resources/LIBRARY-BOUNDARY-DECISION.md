...
| At least 12 validated example packs | 153 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 153 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 153 promoted packs. |
...
- `checked`: 379 expected-result rows
- `replay-only`: 501 expected-result rows
- `lean-horizon`: 116 expected-result rows
- `not-run`: 116 expected-result rows
- `expected_result`: 506 `sat`, 374 `unsat`, 116 `not-run`
- row labels: 90 checked witnesses, 289 checked refutations, 416 finite
  witness replays, 85 finite rejection replays, and 116 theorem horizons
- pack labels: 153 checked evidence packs, 142 mixed-trust packs, and 116 packs
  with theorem boundaries
- `solver_reuse`: 153 promoted packs and 0 unclassified packs
