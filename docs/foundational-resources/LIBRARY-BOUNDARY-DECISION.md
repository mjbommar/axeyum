...
| At least 12 validated example packs | 154 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 154 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 154 promoted packs. |
...
- `checked`: 380 expected-result rows
- `replay-only`: 505 expected-result rows
- `lean-horizon`: 117 expected-result rows
- `not-run`: 117 expected-result rows
- `expected_result`: 509 `sat`, 376 `unsat`, 117 `not-run`
- row labels: 90 checked witnesses, 290 checked refutations, 419 finite
  witness replays, 86 finite rejection replays, and 117 theorem horizons
- pack labels: 154 checked evidence packs, 143 mixed-trust packs, and 117 packs
  with theorem boundaries
- `solver_reuse`: 154 promoted packs and 0 unclassified packs
