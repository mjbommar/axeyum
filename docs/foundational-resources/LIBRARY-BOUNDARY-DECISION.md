...
Reviewed: 2026-06-30; counts refreshed after all 103 current math packs carry
...
|---|---|
| At least 40 validated concept rows | 106 atlas rows: 23 curriculum rows, 18 field rows, 60 bridge-concept rows, and 5 example-family rows. |
| At least 12 validated example packs | 103 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 103 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 103 promoted packs. |
...
- `checked`: 223 expected-result rows
- `replay-only`: 232 expected-result rows
- `lean-horizon`: 66 expected-result rows
- `not-run`: 66 expected-result rows
- `solver_reuse`: 103 promoted packs, 0 non-benchmark-horizon packs, and 0
  unclassified packs
