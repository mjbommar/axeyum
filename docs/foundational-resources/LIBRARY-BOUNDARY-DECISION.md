...
|---|---|
| At least 40 validated concept rows | 128 atlas rows: 23 curriculum rows, 18 field rows, 82 bridge-concept rows, and 5 example-family rows. |
| At least 12 validated example packs | 164 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 164 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 164 promoted packs. |
...
- `checked`: 390 expected-result rows
- `replay-only`: 551 expected-result rows
- `lean-horizon`: 127 expected-result rows
- `not-run`: 127 expected-result rows
- `expected_result`: 545 `sat`, 396 `unsat`, 127 `not-run`
- row labels: 90 checked witnesses, 300 checked refutations, 455 finite
  witness replays, 96 finite rejection replays, and 127 theorem horizons
- pack labels: 164 checked evidence packs, 153 mixed-trust packs, and 127 packs
  with theorem boundaries
- `solver_reuse`: 164 promoted packs and 0 unclassified packs
