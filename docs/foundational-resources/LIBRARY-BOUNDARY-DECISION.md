...
|---|---|
| At least 40 validated concept rows | 134 atlas rows: 23 curriculum rows, 18 field rows, 88 bridge-concept rows, and 5 example-family rows. |
| At least 12 validated example packs | 170 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 170 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 170 promoted packs. |
...
- `checked`: 396 expected-result rows
- `replay-only`: 581 expected-result rows
- `lean-horizon`: 133 expected-result rows
- `not-run`: 133 expected-result rows
- `expected_result`: 569 `sat`, 408 `unsat`, 133 `not-run`
- row labels: 90 checked witnesses, 306 checked refutations, 479 finite
  witness replays, 102 finite rejection replays, and 133 theorem horizons
- pack labels: 170 checked evidence packs, 159 mixed-trust packs, and 133 packs
  with theorem boundaries
- `solver_reuse`: 170 promoted packs and 0 unclassified packs
