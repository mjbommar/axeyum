...
| At least 12 validated example packs | 74 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 62 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 138 expected-result rows
- `replay-only`: 162 expected-result rows
- `lean-horizon`: 37 expected-result rows
