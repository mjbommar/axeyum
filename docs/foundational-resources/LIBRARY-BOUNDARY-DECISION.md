...
Reviewed: 2026-06-30; counts refreshed after all 106 current math packs carry
...
|---|---|
| At least 40 validated concept rows | 109 atlas rows: 23 curriculum rows, 18 field rows, 63 bridge-concept rows, and 5 example-family rows. |
| At least 12 validated example packs | 106 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 106 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 106 promoted packs. |
...
- `checked`: 229 expected-result rows
- `replay-only`: 240 expected-result rows
- `lean-horizon`: 69 expected-result rows
- `not-run`: 69 expected-result rows
- `solver_reuse`: 106 promoted packs, 0 non-benchmark-horizon packs, and 0
  unclassified packs
