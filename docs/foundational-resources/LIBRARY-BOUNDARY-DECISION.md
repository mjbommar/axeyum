...
| At least 12 validated example packs | 69 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 57 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 133 expected-result rows
- `replay-only`: 143 expected-result rows
- `lean-horizon`: 32 expected-result rows
