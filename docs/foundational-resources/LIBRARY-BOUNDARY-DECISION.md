...
| At least 12 validated example packs | 109 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 109 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 326 expected-result rows
- `replay-only`: 295 expected-result rows
- `lean-horizon`: 72 expected-result rows
- `not-run`: 72 expected-result rows
- `expected_result`: 338 `sat`, 283 `unsat`, 72 `not-run`
- row labels: 86 checked witnesses, 240 checked refutations, 252 finite
  witness replays, 43 finite rejection replays, and 72 theorem horizons
- pack labels: 109 checked evidence packs, 98 mixed-trust packs, and 72 packs
  with theorem boundaries
- `solver_reuse`: 108 promoted packs, 1 non-benchmark-horizon pack, and 0
  unclassified packs
