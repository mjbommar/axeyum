...
| At least 12 validated example packs | 67 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 55 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 131 expected-result rows
- `replay-only`: 137 expected-result rows
- `lean-horizon`: 30 expected-result rows
