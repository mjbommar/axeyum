...
| At least 12 validated example packs | 68 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 56 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 132 expected-result rows
- `replay-only`: 140 expected-result rows
- `lean-horizon`: 31 expected-result rows
