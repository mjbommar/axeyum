...
Reviewed: 2026-06-30; counts refreshed after all 97 current math packs carry
...
| At least 12 validated example packs | 97 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 97 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 97 promoted packs. |

The current pack-level evidence mix is still intentionally conservative:

- `checked`: 217 expected-result rows
- `replay-only`: 212 expected-result rows
- `lean-horizon`: 60 expected-result rows
- `not-run`: 60 expected-result rows
- `solver_reuse`: 97 promoted packs, 0 non-benchmark-horizon packs, and 0
  unclassified packs
