...
| At least 12 validated example packs | 75 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 63 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 139 expected-result rows
- `replay-only`: 166 expected-result rows
- `lean-horizon`: 38 expected-result rows
