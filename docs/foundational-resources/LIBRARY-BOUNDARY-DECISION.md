...
| At least 12 validated example packs | 79 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 67 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 152 expected-result rows
- `replay-only`: 173 expected-result rows
- `lean-horizon`: 42 expected-result rows
