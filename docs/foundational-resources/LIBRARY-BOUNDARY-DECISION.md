...
| At least 12 validated example packs | 81 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 69 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 161 expected-result rows
- `replay-only`: 175 expected-result rows
- `lean-horizon`: 44 expected-result rows
