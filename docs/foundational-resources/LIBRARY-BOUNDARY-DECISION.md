...
Reviewed: 2026-06-30; counts refreshed after 36 promoted solver-reuse packs.
...
probability/measure QF_LRA/Farkas rows, but this is still an in-repository
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 36 promoted packs. |
...
- `solver_reuse`: 36 promoted packs and 48 unclassified packs
