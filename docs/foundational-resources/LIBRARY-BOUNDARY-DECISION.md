...
|---|---|
| At least 40 validated concept rows | 135 atlas rows: 23 curriculum rows, 18 field rows, 89 bridge-concept rows, and 5 example-family rows. |
| At least 12 validated example packs | 171 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 171 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 171 promoted packs. |
...
- `checked`: 397 expected-result rows
- `replay-only`: 586 expected-result rows
- `lean-horizon`: 134 expected-result rows
- `not-run`: 134 expected-result rows
- `expected_result`: 573 `sat`, 410 `unsat`, 134 `not-run`
- row labels: 90 checked witnesses, 307 checked refutations, 483 finite
  witness replays, 103 finite rejection replays, and 134 theorem horizons
- pack labels: 171 checked evidence packs, 160 mixed-trust packs, and 134 packs
  with theorem boundaries
- `solver_reuse`: 171 promoted packs and 0 unclassified packs
