...
| At least 12 validated example packs | 80 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 68 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 157 expected-result rows
- `replay-only`: 174 expected-result rows
- `lean-horizon`: 43 expected-result rows
