...
| At least 12 validated example packs | 70 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 58 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 134 expected-result rows
- `replay-only`: 146 expected-result rows
- `lean-horizon`: 33 expected-result rows
