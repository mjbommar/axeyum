...
|---|---|
| At least 40 validated concept rows | 133 atlas rows: 23 curriculum rows, 18 field rows, 87 bridge-concept rows, and 5 example-family rows. |
| At least 12 validated example packs | 169 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 169 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 169 promoted packs. |
...
- `checked`: 395 expected-result rows
- `replay-only`: 576 expected-result rows
- `lean-horizon`: 132 expected-result rows
- `not-run`: 132 expected-result rows
- `expected_result`: 565 `sat`, 406 `unsat`, 132 `not-run`
- row labels: 90 checked witnesses, 305 checked refutations, 475 finite
  witness replays, 101 finite rejection replays, and 132 theorem horizons
- pack labels: 169 checked evidence packs, 158 mixed-trust packs, and 132 packs
  with theorem boundaries
- `solver_reuse`: 169 promoted packs and 0 unclassified packs
