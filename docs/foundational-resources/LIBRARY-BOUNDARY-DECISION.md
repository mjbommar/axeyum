...
|---|---|
| At least 40 validated concept rows | 126 atlas rows: 23 curriculum rows, 18 field rows, 80 bridge-concept rows, and 5 example-family rows. |
| At least 12 validated example packs | 162 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 162 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 162 promoted packs. |
...
- `checked`: 388 expected-result rows
- `replay-only`: 541 expected-result rows
- `lean-horizon`: 125 expected-result rows
- `not-run`: 125 expected-result rows
- `expected_result`: 537 `sat`, 392 `unsat`, 125 `not-run`
- row labels: 90 checked witnesses, 298 checked refutations, 447 finite
  witness replays, 94 finite rejection replays, and 125 theorem horizons
- pack labels: 162 checked evidence packs, 151 mixed-trust packs, and 125 packs
  with theorem boundaries
- `solver_reuse`: 162 promoted packs and 0 unclassified packs
