...
|---|---|
| At least 40 validated concept rows | 127 atlas rows: 23 curriculum rows, 18 field rows, 81 bridge-concept rows, and 5 example-family rows. |
| At least 12 validated example packs | 163 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 163 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 163 promoted packs. |
...
- `checked`: 389 expected-result rows
- `replay-only`: 546 expected-result rows
- `lean-horizon`: 126 expected-result rows
- `not-run`: 126 expected-result rows
- `expected_result`: 541 `sat`, 394 `unsat`, 126 `not-run`
- row labels: 90 checked witnesses, 299 checked refutations, 451 finite
  witness replays, 95 finite rejection replays, and 126 theorem horizons
- pack labels: 163 checked evidence packs, 152 mixed-trust packs, and 126 packs
  with theorem boundaries
- `solver_reuse`: 163 promoted packs and 0 unclassified packs
