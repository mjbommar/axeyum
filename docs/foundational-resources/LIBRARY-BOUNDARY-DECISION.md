...
| At least 12 validated example packs | 76 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 64 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 140 expected-result rows
- `replay-only`: 170 expected-result rows
- `lean-horizon`: 39 expected-result rows
