...
|---|---|
| At least 40 validated concept rows | 132 atlas rows: 23 curriculum rows, 18 field rows, 86 bridge-concept rows, and 5 example-family rows. |
| At least 12 validated example packs | 168 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 168 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 168 promoted packs. |
...
- `checked`: 394 expected-result rows
- `replay-only`: 571 expected-result rows
- `lean-horizon`: 131 expected-result rows
- `not-run`: 131 expected-result rows
- `expected_result`: 561 `sat`, 404 `unsat`, 131 `not-run`
- row labels: 90 checked witnesses, 304 checked refutations, 471 finite
  witness replays, 100 finite rejection replays, and 131 theorem horizons
- pack labels: 168 checked evidence packs, 157 mixed-trust packs, and 131 packs
  with theorem boundaries
- `solver_reuse`: 168 promoted packs and 0 unclassified packs
