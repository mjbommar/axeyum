...
| At least 12 validated example packs | 82 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 70 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 165 expected-result rows
- `replay-only`: 175 expected-result rows
- `lean-horizon`: 45 expected-result rows
