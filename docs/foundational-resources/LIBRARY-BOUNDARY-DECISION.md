...
| At least 12 validated example packs | 66 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 54 non-template packs contain at least one `checked` expected-result row. |
...
- `checked`: 130 expected-result rows
- `replay-only`: 134 expected-result rows
- `lean-horizon`: 29 expected-result rows
