...
Reviewed: 2026-06-30; counts refreshed after 81 promoted solver-reuse packs
and 3 explicit non-benchmark-horizon packs.
...
| At least 6 packs with checked proof/evidence routes | 81 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 81 promoted packs. |

The current pack-level evidence mix is still intentionally conservative:

- `checked`: 201 expected-result rows
- `replay-only`: 171 expected-result rows
- `lean-horizon`: 47 expected-result rows
- `not-run`: 47 expected-result rows
- `solver_reuse`: 81 promoted packs, 3 non-benchmark-horizon packs, and 0
  unclassified packs
