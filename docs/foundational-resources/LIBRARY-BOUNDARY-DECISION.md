...
| At least 12 validated example packs | 112 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 112 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 112 promoted packs. |
...
- `checked`: 338 expected-result rows
- `replay-only`: 300 expected-result rows
- `lean-horizon`: 75 expected-result rows
- `not-run`: 75 expected-result rows
- `expected_result`: 346 `sat`, 292 `unsat`, 75 `not-run`
- row labels: 90 checked witnesses, 248 checked refutations, 256 finite
  witness replays, 44 finite rejection replays, and 75 theorem horizons
- pack labels: 112 checked evidence packs, 101 mixed-trust packs, and 75 packs
  with theorem boundaries
- `solver_reuse`: 112 promoted packs and 0 unclassified packs
