...
|---|---|
| At least 40 validated concept rows | 129 atlas rows: 23 curriculum rows, 18 field rows, 83 bridge-concept rows, and 5 example-family rows. |
| At least 12 validated example packs | 165 non-template math packs are listed through the atlas data contract. |
| At least 6 packs with checked proof/evidence routes | 165 non-template packs contain at least one `checked` expected-result row. |
...
| At least one consumer can read promoted solver-reuse rows | `scripts/query-foundational-resources.py packs --solver-reuse promoted --require-any` is part of `scripts/check-foundational-resources.sh` and currently finds 165 promoted packs. |
...
- `checked`: 391 expected-result rows
- `replay-only`: 556 expected-result rows
- `lean-horizon`: 128 expected-result rows
- `not-run`: 128 expected-result rows
- `expected_result`: 549 `sat`, 398 `unsat`, 128 `not-run`
- row labels: 90 checked witnesses, 301 checked refutations, 459 finite
  witness replays, 97 finite rejection replays, and 128 theorem horizons
- pack labels: 165 checked evidence packs, 154 mixed-trust packs, and 128 packs
  with theorem boundaries
- `solver_reuse`: 165 promoted packs and 0 unclassified packs
