...
| Rayleigh quotients, eigenpairs, singular values, and matrix invariants | `bridge_eigenpair` | `Farkas` | `checks --concept bridge_eigenpair --route Farkas --proof-status checked`; `checks --pack finite-singular-value-shadow-v0 --route Farkas --proof-status checked` |
...
Display checked Rayleigh, eigenpair, singular-value, or matrix-invariant rows:
...
- general rank-nullity, spectral, SVD, Hilbert-space, Chebyshev, Smith-normal-form,
