...
- 960 expected checks: 488 `sat`, 362 `unsat`, and 110 `not-run`.
...
| `real_analysis` | rational analysis, metric continuity, compactness shadows, Simpson-rule quadrature, optimization shadows | QF_LRA/Farkas, QF_NRA/RCF, Lean horizon | delta-epsilon/metric-ball, integration, quadrature, and convergence-boundary paths |
...
| `numerical_analysis` | residuals, Simpson-rule quadrature, explicit and implicit Euler steps, Adams-Bashforth and BDF2 multistep rows, interval/error recurrence, singular-value and Jordan-chain shadows, finite algorithms | QF_LRA/Farkas, replay, Lean horizon | pivoting/stability, quadrature metadata, and exact-vs-floating examples |
...
2. Landed: [Learner Coverage Audit](LEARNER-COVERAGE-AUDIT.md) records that
   the current 147 non-template packs are all focused-lesson linked, with no
