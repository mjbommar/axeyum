...
- 955 expected checks: 486 `sat`, 360 `unsat`, and 109 `not-run`.
...
| `numerical_analysis` | residuals, explicit and implicit Euler steps, Adams-Bashforth and BDF2 multistep rows, interval/error recurrence, singular-value and Jordan-chain shadows, finite algorithms | QF_LRA/Farkas, replay, Lean horizon | pivoting/stability metadata and exact-vs-floating examples |
...
2. Landed: [Learner Coverage Audit](LEARNER-COVERAGE-AUDIT.md) records that
   the current 146 non-template packs are all focused-lesson linked, with no
