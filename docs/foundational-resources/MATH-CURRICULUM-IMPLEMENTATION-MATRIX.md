...
| `geometry` | reals, polynomials, linear algebra | landed coordinate/incidence/rigid/affine/oriented replay plus finite circle/inversion/cyclic replay bridge rows; add only distinct angle variants beyond the current square dot-product witness, Ptolemy shadows, circle-line correspondence, or higher-degree polynomial-geometry pressure when reused | QF_LRA/NRA, replay |
