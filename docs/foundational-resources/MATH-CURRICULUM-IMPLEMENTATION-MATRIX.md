...
| `reals` | RCF shadow, bounded real analysis, metric continuity, finite root finding, finite separation, finite KKT, finite SDP, finite gradient descent | Add concept rows for balls, limits, continuity, compactness, root-finding, separation, KKT, SDP, gradient descent, and completeness/convergence horizons. | QF_LRA/Farkas for bounded bad-delta, bad-iterate, bad-separator, bad-stationarity, bad-objective, and bad-decrease rows, QF_LRA/NRA for algebraic shadows; Lean horizon for completeness/general topology, separation, KKT sufficiency, SDP duality, descent-rate, and convergence. | Each epsilon-delta/root-finding/separation/KKT/SDP/descent pack says fixed rational instance vs theorem, and metric-continuity, finite-root-finding, finite-separation, finite-KKT, finite-SDP, plus finite-gradient-descent now have checked finite bad-row routes. |
...
| `linear-algebra` | rational matrices, finite vector/dual/module/tensor, spectral, invariants, separation, KKT, SDP, gradient descent | Landed matrix-computation bridge rows plus algebra-map rows for kernel/image, quotient maps, module actions, tensor bilinearity, separating-hyperplane replay, KKT stationarity replay, finite SDP slack/objective replay, and finite gradient-step replay; next split dual/projection maps only when reuse demands it. | QF_LRA/Farkas, finite-field replay, QF_UF/Alethe for algebraic table conflicts. | Matrix, dot-product, stationarity, SDP objective, and descent-step rows can become solver regressions with source-pack back-links. |
| `calculus` | algebraic calculus, Riemann sums, multivariable rational calculus, finite root finding | Add derivative/integral/convergence theorem horizon rows plus exact algebraic and algorithm-step shadows. | LRA/NRA for polynomial shadows; Lean horizon for FTC, differentiability, convergence. | Calculus packs never conflate finite symbolic or iterative replay with analytic theorem proof. |
...
| `optimization_and_convexity` | rationals, reals, linear algebra | landed LP objective/Farkas, rational convexity/gradient bridge rows, finite root-finding step replay, finite hyperplane-separation replay, finite KKT replay, finite SDP replay, and finite gradient-descent replay; add narrower duality, active-set, higher-dimensional SDP, line-search/projected-gradient, or convergence rows only when reused | QF_LRA/Farkas, NRA shadows |
| `numerical_analysis` | linear algebra, calculus | residual bounds, Euler steps, interval boxes, exact error recurrences, root-finding iterations | QF_LRA, replay, numerical-honesty metadata |
...
32. `finite-gradient-descent-v0` now links `bad-descent-value-rejected` to
    ...
    checked by the `math_resource_lra_routes` QF_LRA/Farkas regression.
33. Revisit crate/repo boundaries only after three real consumers or repeated
