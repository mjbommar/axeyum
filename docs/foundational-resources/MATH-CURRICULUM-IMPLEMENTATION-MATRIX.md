...
| `reals` | RCF shadow, bounded real analysis, metric continuity, finite root finding, finite separation, finite KKT | Add concept rows for balls, limits, continuity, compactness, root-finding, separation, KKT, and completeness/convergence horizons. | QF_LRA/Farkas for bounded bad-delta, bad-iterate, bad-separator, and bad-stationarity rows, QF_LRA/NRA for algebraic shadows; Lean horizon for completeness/general topology, separation, KKT sufficiency, and convergence. | Each epsilon-delta/root-finding/separation/KKT pack says fixed rational instance vs theorem, and metric-continuity, finite-root-finding, finite-separation, plus finite-KKT now have checked finite bad-row routes. |
...
| `linear-algebra` | rational matrices, finite vector/dual/module/tensor, spectral, invariants, separation, KKT | Landed matrix-computation bridge rows plus algebra-map rows for kernel/image, quotient maps, module actions, tensor bilinearity, separating-hyperplane replay, and KKT stationarity replay; next split dual/projection maps only when reuse demands it. | QF_LRA/Farkas, finite-field replay, QF_UF/Alethe for algebraic table conflicts. | Matrix, dot-product, and stationarity rows can become solver regressions with source-pack back-links. |
...
| `optimization_and_convexity` | rationals, reals, linear algebra | landed LP objective/Farkas, rational convexity/gradient bridge rows, finite root-finding step replay, finite hyperplane-separation replay, and finite KKT replay; add narrower duality, active-set, SDP, or convergence rows only when reused | QF_LRA/Farkas, NRA shadows |
...
30. `finite-kkt-v0` now links `bad-kkt-stationarity-rejected` to
    ...
    checked by the `math_resource_lra_routes` QF_LRA/Farkas regression.
31. Revisit crate/repo boundaries only after three real consumers or repeated
