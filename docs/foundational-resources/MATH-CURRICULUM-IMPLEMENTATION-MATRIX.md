...
| `reals` | RCF shadow, bounded real analysis, metric continuity, finite root finding, finite separation, finite KKT, finite SDP | Add concept rows for balls, limits, continuity, compactness, root-finding, separation, KKT, SDP, and completeness/convergence horizons. | QF_LRA/Farkas for bounded bad-delta, bad-iterate, bad-separator, bad-stationarity, and bad-objective rows, QF_LRA/NRA for algebraic shadows; Lean horizon for completeness/general topology, separation, KKT sufficiency, SDP duality, and convergence. | Each epsilon-delta/root-finding/separation/KKT/SDP pack says fixed rational instance vs theorem, and metric-continuity, finite-root-finding, finite-separation, finite-KKT, plus finite-SDP now have checked finite bad-row routes. |
...
| `linear-algebra` | rational matrices, finite vector/dual/module/tensor, spectral, invariants, separation, KKT, SDP | Landed matrix-computation bridge rows plus algebra-map rows for kernel/image, quotient maps, module actions, tensor bilinearity, separating-hyperplane replay, KKT stationarity replay, and finite SDP slack/objective replay; next split dual/projection maps only when reuse demands it. | QF_LRA/Farkas, finite-field replay, QF_UF/Alethe for algebraic table conflicts. | Matrix, dot-product, stationarity, and SDP objective rows can become solver regressions with source-pack back-links. |
...
| `optimization_and_convexity` | rationals, reals, linear algebra | landed LP objective/Farkas, rational convexity/gradient bridge rows, finite root-finding step replay, finite hyperplane-separation replay, finite KKT replay, and finite SDP replay; add narrower duality, active-set, higher-dimensional SDP, or convergence rows only when reused | QF_LRA/Farkas, NRA shadows |
...
31. `finite-sdp-v0` now links `bad-sdp-objective-rejected` to
    ...
    checked by the `math_resource_lra_routes` QF_LRA/Farkas regression.
32. Revisit crate/repo boundaries only after three real consumers or repeated
