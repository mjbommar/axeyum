...
| `linear_algebra` | fields, polynomials, relations | LU/QR/Cholesky exact replay plus checked bad product-entry proof rows, rank/nullity, residual, condition-number, singular-value/SVD, Jordan-chain, spectral, tensor and module rows | QF_LRA/Farkas, finite-field replay |
...
| `numerical_analysis` | linear algebra, calculus | maintain landed finite dynamics/Euler bridge alongside residual bounds, interval boxes, exact condition-number/perturbation shadows, singular-value/SVD norm shadows, exact Jordan-chain shadows, exact error recurrences, root-finding, LU/QR/Cholesky factorization replay, active-set QP, gradient-descent, Armijo/Wolfe line-search descent-direction and accepted-candidate arithmetic, projected-gradient, and proximal-gradient composite-decrease iterations | QF_LRA, replay, numerical-honesty metadata |
...
89. Landed: add the finite Jordan-chain resource.
    ...
    claims.
90. Revisit crate/repo boundaries only after three real consumers or repeated
