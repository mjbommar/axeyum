...
| `calculus` | algebraic calculus, Riemann sums, finite Simpson-rule quadrature, multivariable rational calculus, finite root finding, finite active-set QP, finite line search, finite Wolfe line search, finite projected gradient, finite proximal gradient | Add derivative/integral/quadrature/convergence theorem horizon rows plus exact algebraic, quadrature, and algorithm-step shadows. | LRA/NRA for polynomial shadows; QF_LRA/Farkas for bad finite quadrature values; Lean horizon for FTC, differentiability, Simpson/Newton-Cotes quadrature theory, active-set/Wolfe/line-search/projected/proximal-gradient convergence, and convergence. | Calculus packs never conflate finite symbolic, quadrature, or iterative replay with analytic theorem proof. |
...
| `real_analysis` | rationals, reals, sequences, calculus | balls, bounded epsilon-delta, Simpson-rule quadrature, compactness/continuity horizons | QF_LRA/Farkas, QF_LRA/NRA, Lean horizon |
...
118. Revisit crate/repo boundaries only after three real consumers or repeated
