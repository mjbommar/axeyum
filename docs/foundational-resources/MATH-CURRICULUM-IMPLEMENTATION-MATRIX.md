...
| `reals` | RCF shadow, bounded real analysis, metric continuity, finite root finding, finite separation, finite KKT, finite SDP, finite gradient descent, finite line search, finite projected gradient, finite proximal gradient | Add concept rows for balls, limits, continuity, compactness, root-finding, separation, KKT, SDP, gradient descent, line search, projected gradient, proximal gradient, and completeness/convergence horizons. | QF_LRA/Farkas for bounded bad-delta, bad-iterate, bad-separator, bad-stationarity, bad-objective, bad-decrease, bad-Armijo, bad-projection, and bad-proximal-point rows, QF_LRA/NRA for algebraic shadows; Lean horizon for completeness/general topology, separation, KKT sufficiency, SDP duality, descent-rate, line-search/projected/proximal-gradient convergence, and convergence. | Each epsilon-delta/root-finding/separation/KKT/SDP/descent/line-search/projected/proximal-gradient pack says fixed rational instance vs theorem, and metric-continuity, finite-root-finding, finite-separation, finite-KKT, finite-SDP, finite-gradient-descent, finite-line-search, finite-projected-gradient, plus finite-proximal-gradient now have checked finite bad-row routes. |
...
| `linear-algebra` | rational matrices, finite vector/dual/module/tensor, spectral, invariants, separation, KKT, SDP, gradient descent, line search, projected gradient, proximal gradient | Landed matrix-computation bridge rows plus algebra-map rows for kernel/image, quotient maps, module actions, tensor bilinearity, separating-hyperplane replay, KKT stationarity replay, finite SDP slack/objective replay, finite gradient-step replay, finite Armijo line-search replay, finite projected-gradient interval replay, and finite proximal-gradient soft-threshold replay; next split dual/projection maps only when reuse demands it. | QF_LRA/Farkas, finite-field replay, QF_UF/Alethe for algebraic table conflicts. | Matrix, dot-product, stationarity, SDP objective, descent-step, line-search, projected-gradient, and proximal-gradient rows can become solver regressions with source-pack back-links. |
| `calculus` | algebraic calculus, Riemann sums, multivariable rational calculus, finite root finding, finite line search, finite projected gradient, finite proximal gradient | Add derivative/integral/convergence theorem horizon rows plus exact algebraic and algorithm-step shadows. | LRA/NRA for polynomial shadows; Lean horizon for FTC, differentiability, line-search/projected/proximal-gradient convergence, and convergence. | Calculus packs never conflate finite symbolic or iterative replay with analytic theorem proof. |
...
| `optimization_and_convexity` | rationals, reals, linear algebra | landed LP objective/Farkas, rational convexity/gradient bridge rows, finite root-finding step replay, finite hyperplane-separation replay, finite KKT replay, finite SDP replay, finite gradient-descent replay, finite Armijo line-search replay, finite projected-gradient interval replay, and finite proximal-gradient soft-threshold replay; add narrower duality, active-set, higher-dimensional SDP, box-plus-L1, Wolfe/exact-line-search, or stochastic/convergence rows only when reused | QF_LRA/Farkas, NRA shadows |
| `numerical_analysis` | linear algebra, calculus | residual bounds, Euler steps, interval boxes, exact error recurrences, root-finding, gradient-descent, line-search, projected-gradient, and proximal-gradient iterations | QF_LRA, replay, numerical-honesty metadata |
...
34. `finite-projected-gradient-v0` now links `bad-projected-point-rejected` to
    ...
    checked by the `math_resource_lra_routes` QF_LRA/Farkas regression.
35. `finite-proximal-gradient-v0` now links `bad-proximal-point-rejected` to
    `artifacts/examples/math/finite-proximal-gradient-v0/smt2/bad-proximal-point-farkas-conflict.smt2`,
    checked by the `math_resource_lra_routes` QF_LRA/Farkas regression.
36. Revisit crate/repo boundaries only after three real consumers or repeated
