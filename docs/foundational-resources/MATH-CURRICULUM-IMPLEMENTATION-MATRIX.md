...
| `sequences-and-limits` | sequence-limit shadow, bounded monotone sequence, real-analysis, generating functions | Bounded Cauchy-tail and monotone-prefix bad-bound rows landed; add recurrence or convergence-horizon rows only when reused. | Finite replay/LRA for bounded tails; Lean horizon for general convergence. | Lessons keep finite prefix evidence separate from convergence theorems. |
...
28. Revisit crate/repo boundaries only after three real consumers or repeated
