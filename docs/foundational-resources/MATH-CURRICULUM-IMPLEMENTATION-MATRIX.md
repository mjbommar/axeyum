...
| `reals` | RCF shadow, bounded real analysis, metric continuity, finite root finding | Add concept rows for balls, limits, continuity, compactness, root-finding, and completeness/convergence horizons. | QF_LRA/Farkas for bounded bad-delta and bad-iterate rows, QF_LRA/NRA for algebraic shadows; Lean horizon for completeness/general topology and convergence. | Each epsilon-delta/root-finding pack says fixed rational instance vs theorem, and metric-continuity plus finite-root-finding now have checked finite bad-row routes. |
...
| `optimization_and_convexity` | rationals, reals, linear algebra | landed LP objective/Farkas, rational convexity/gradient bridge rows, and finite root-finding step replay; add narrower duality, KKT, SDP, or convergence rows only when reused | QF_LRA/Farkas, NRA shadows |
...
28. `finite-root-finding-v0` now links `bad-newton-step-rejected` to
    ...
    checked by the `math_resource_lra_routes` QF_LRA/Farkas regression.
29. Revisit crate/repo boundaries only after three real consumers or repeated
