...
| `reals` | RCF shadow, bounded real analysis, metric continuity, finite root finding, finite separation, finite KKT, finite SDP, finite gradient descent, finite line search | Add concept rows for balls, limits, continuity, compactness, root-finding, separation, KKT, SDP, gradient descent, line search, and completeness/convergence horizons. | QF_LRA/Farkas for bounded bad-delta, bad-iterate, bad-separator, bad-stationarity, bad-objective, bad-decrease, and bad-Armijo rows, QF_LRA/NRA for algebraic shadows; Lean horizon for completeness/general topology, separation, KKT sufficiency, SDP duality, descent-rate, line-search convergence, and convergence. | Each epsilon-delta/root-finding/separation/KKT/SDP/descent/line-search pack says fixed rational instance vs theorem, and metric-continuity, finite-root-finding, finite-separation, finite-KKT, finite-SDP, finite-gradient-descent, plus finite-line-search now have checked finite bad-row routes. |
...
| `linear-algebra` | rational matrices, finite vector/dual/module/tensor, spectral, invariants, separation, KKT, SDP, gradient descent, line search | Landed matrix-computation bridge rows plus algebra-map rows for kernel/image, quotient maps, module actions, tensor bilinearity, separating-hyperplane replay, KKT stationarity replay, finite SDP slack/objective replay, finite gradient-step replay, and finite Armijo line-search replay; next split dual/projection maps only when reuse demands it. | QF_LRA/Farkas, finite-field replay, QF_UF/Alethe for algebraic table conflicts. | Matrix, dot-product, stationarity, SDP objective, descent-step, and line-search rows can become solver regressions with source-pack back-links. |
| `calculus` | algebraic calculus, Riemann sums, multivariable rational calculus, finite root finding, finite line search | Add derivative/integral/convergence theorem horizon rows plus exact algebraic and algorithm-step shadows. | LRA/NRA for polynomial shadows; Lean horizon for FTC, differentiability, line-search convergence, and convergence. | Calculus packs never conflate finite symbolic or iterative replay with analytic theorem proof. |
...
| `optimization_and_convexity` | rationals, reals, linear algebra | landed LP objective/Farkas, rational convexity/gradient bridge rows, finite root-finding step replay, finite hyperplane-separation replay, finite KKT replay, finite SDP replay, finite gradient-descent replay, and finite Armijo line-search replay; add narrower duality, active-set, higher-dimensional SDP, projected-gradient, Wolfe/exact-line-search, or convergence rows only when reused | QF_LRA/Farkas, NRA shadows |
| `numerical_analysis` | linear algebra, calculus | residual bounds, Euler steps, interval boxes, exact error recurrences, root-finding, gradient-descent, and line-search iterations | QF_LRA, replay, numerical-honesty metadata |
...
33. `finite-line-search-v0` now links `bad-armijo-acceptance-rejected` to
    ...
    checked by the `math_resource_lra_routes` QF_LRA/Farkas regression.
34. Revisit crate/repo boundaries only after three real consumers or repeated
