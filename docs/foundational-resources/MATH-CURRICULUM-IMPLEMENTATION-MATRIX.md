...
| `polynomials` | identities, rational factorization, generating functions, finite root finding, finite circle geometry | Add coefficient-ring, polynomial-division, finite root-finding, and finite polynomial-geometry reusable rows. | Finite replay, QF_LIA/LRA coefficient constraints, QF_LRA/Farkas for linearized exact conflicts, Lean horizon for general factorization, circle geometry, and convergence. | Factorization, root-finding, and circle rows replay product, degree/leading constraints, polynomial values, exact iterates, squared radii, tangent lines, and chord dot products. |
...
| `geometry` | reals, polynomials, linear algebra | landed coordinate, incidence, rigid-configuration, affine, oriented-area, and finite circle-geometry shadows; add only distinct isometry, inversion, cyclic-configuration, or higher-degree polynomial-geometry pressure when reused | QF_LRA/NRA, replay |
...
4. R1 bridge-concept rows landed for probability/statistics finite-table
   ...
   rows now also land for coordinate/incidence/rigid/oriented/circle geometry replay and complex
...
38. `finite-circle-geometry-v0` now links `bad-circle-radius-rejected` to
    ...
    checked by the `math_resource_lra_routes` QF_LRA/Farkas regression.
39. Revisit crate/repo boundaries only after three real consumers or repeated
