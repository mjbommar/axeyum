...
| `reals` | RCF shadow, bounded real analysis, metric continuity, finite root finding, finite separation | Add concept rows for balls, limits, continuity, compactness, root-finding, separation, and completeness/convergence horizons. | QF_LRA/Farkas for bounded bad-delta, bad-iterate, and bad-separator rows, QF_LRA/NRA for algebraic shadows; Lean horizon for completeness/general topology, separation, and convergence. | Each epsilon-delta/root-finding/separation pack says fixed rational instance vs theorem, and metric-continuity, finite-root-finding, plus finite-separation now have checked finite bad-row routes. |
...
| `linear-algebra` | rational matrices, finite vector/dual/module/tensor, spectral, invariants, separation | Landed matrix-computation bridge rows plus algebra-map rows for kernel/image, quotient maps, module actions, tensor bilinearity, and separating-hyperplane replay; next split dual/projection maps only when reuse demands it. | QF_LRA/Farkas, finite-field replay, QF_UF/Alethe for algebraic table conflicts. | Matrix and dot-product rows can become solver regressions with source-pack back-links. |
...
| `optimization_and_convexity` | rationals, reals, linear algebra | landed LP objective/Farkas, rational convexity/gradient bridge rows, finite root-finding step replay, and finite hyperplane-separation replay; add narrower duality, KKT, SDP, or convergence rows only when reused | QF_LRA/Farkas, NRA shadows |
...
29. `finite-separation-v0` now links `bad-separator-rejected` to
    ...
    checked by the `math_resource_lra_routes` QF_LRA/Farkas regression.
30. Revisit crate/repo boundaries only after three real consumers or repeated
