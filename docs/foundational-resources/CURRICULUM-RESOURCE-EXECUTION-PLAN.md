...
- 121 atlas rows generated from curriculum, field data, 75 R1 bridge
  ...
  operator/Jordan/Chebyshev replay, Schur-complement replay, and Lean horizons, plus five route-specific
  example families.
- 121 non-template math example packs, plus the validating template pack.
...
- [Learner Coverage Audit](LEARNER-COVERAGE-AUDIT.md), which records that all
  121 current non-template math packs are focused-lesson linked with no
...
15. Add QF_UF/Alethe upgrade rows for equivalence, function, and finite algebra
   ...
   decision. The curriculum-pressure view now groups the 121 non-template packs
