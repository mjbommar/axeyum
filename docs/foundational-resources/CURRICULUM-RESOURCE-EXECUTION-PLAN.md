...
- 122 atlas rows generated from curriculum, field data, 76 R1 bridge
  ...
  example families.
- 162 non-template math example packs, plus the validating template pack.
...
- [Learner Coverage Audit](LEARNER-COVERAGE-AUDIT.md), which records that all
  162 current non-template math packs are focused-lesson linked with no
...
- Keep consumer field-readiness smokes representative: probability/Farkas now
  ...
  finite KKT stationarity/complementarity replay, active-set QP free-gradient,
  inactive-slack, and degenerate-multiplier conflicts, finite SDP objective/slack replay, finite
  gradient-descent step replay, finite
  line-search rejected-step, descent-direction, and accepted-candidate replay, finite Wolfe
...
- geometry/complex analysis: landed bridge rows for coordinate/incidence/
  rigid/oriented/circle/inversion/cyclic geometry replay and complex real-pair transform replay, backed by
  the coordinate, incidence, rigid-configuration, affine, orientation/area,
  finite-circle, finite-inversion, finite-cyclic, complex algebraic, and complex-plane transform packs while leaving synthetic/differential geometry,
...
| `probability_theory` | standalone finite probability mass-table and finite Naive Bayes lessons landed; maintain stochastic-process path through kernels/Markov chains |
| `statistics` | exact finite tests, regression, concentration, finite classifier shadows, and explicit numerical-honesty status |
...
15. Add QF_UF/Alethe upgrade rows for equivalence, function, and finite algebra
   ...
   decision. The curriculum-pressure view now groups the 162 non-template packs
...
12. Landed: add consumer-facing sample queries over the JSON data contract.
   ...
   descent-direction, and accepted-candidate replay, finite Wolfe line-search exact-minimizer
...
12. Landed: add the modular CRT/inverse witness bridge row and number-theory
    ...
    fixed-width nonunit-inverse plus Fermat-unit QF_BV evidence discoverable from the atlas without
