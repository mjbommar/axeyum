...
| `measure_theory` | Measure theory | Graduate analysis / probability foundation | Finite measures, sigma-algebra sanity checks, simple-function integrals, and exact finite probability | Lebesgue measure, convergence theorems, and almost-everywhere reasoning need Lean. |
| `probability_theory` | Probability theory | Undergraduate / graduate applied and pure math | Finite probability tables, conditional probability, Bayes rule, finite expectations, exact discrete distributions | Continuous distributions, stochastic processes, and limit theorems are mostly proof or numerical-honesty tracks. |
