...
1. `graph-coloring-v0` (landed), `graph-reachability-v0` (landed), and
   `graph-matching-v0` (landed): SAT colorings, non-colorability, finite
   reachability, traversal traces, cut separation, matching witnesses, and
   augmenting paths.
...
finite structures, and analysis/topology horizons. Phase M6 now has cookbook
links from all current non-template math example packs, and
...
slice (richer cut certificates or d-separation), by adding the next
