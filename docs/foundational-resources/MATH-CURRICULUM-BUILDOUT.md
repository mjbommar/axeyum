...
| `statistics` | probability, rationals, linear algebra | `descriptive-statistics-v0`, `least-squares-regression-v0`, `finite-ridge-regression-v0`, `finite-linear-discriminant-v0`, `finite-covariance-matrix-v0`, contingency tables, exact small tests, ordinary and ridge regression normal equations, finite linear-discriminant/classification replay, finite stochastic-kernel checks, finite hitting-time checks, finite martingale checks, finite concentration checks, finite covariance checks, and finite random-matrix moment checks. |
...
- At least 12 validated example packs.
  Status: 161 non-template math example packs validate.
- At least 6 packs with checked proof/evidence routes.
  Status: 161 non-template packs have at least one `checked` expected-result row.
