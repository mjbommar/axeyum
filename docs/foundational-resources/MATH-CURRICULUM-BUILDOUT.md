...
| `differential_equations_and_dynamical_systems` | calculus, linear algebra | `bounded-dynamics-v0`, `finite-euler-method-v0`, `finite-value-iteration-v0`, recurrence traces, Euler-method steps, threshold reachability, finite error replay, discounted Bellman backups, and invariant checks before continuous theory. |
...
- At least 12 validated example packs.
  Status: 172 non-template math example packs validate.
- At least 6 packs with checked proof/evidence routes.
  Status: 172 non-template packs have at least one `checked` expected-result row.
