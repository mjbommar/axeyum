...
40. `finite-proximal-gradient-v0`: exact smooth-gradient replay, ordinary
    trial-step arithmetic, L1 soft-threshold replay, composite descent, and
    checked QF_LRA/Farkas bad-proximal-point rejection, while composite
    convex-analysis, stochastic, active-set, and proximal-gradient convergence
    theorems remain horizon claims.
...
- At least 12 validated example packs.
  Status: 97 non-template math example packs validate.
- At least 6 packs with checked proof/evidence routes.
  Status: 97 non-template packs have at least one `checked` expected-result row.
...
proximal replay, composite descent, checked bad-proximal-point rejection with
QF_LRA/Farkas evidence, and the general proximal-gradient convergence Lean
horizon.
...
bounds, finite projected-gradient replay, finite proximal-gradient replay, and
...
line-search, projected-gradient convergence, proximal-gradient convergence, or
