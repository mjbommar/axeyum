...
| Bounded implementation equivalence | the Bool/QF_LIA, QF_LRA/Farkas, and category-equivalence packs have checked `implementation_equivalence` rows | `bridge_finite_image_preimage_inverse`, `bridge_qf_uf_alethe_anatomy`, `family_finite_algebra_alethe` | checked Bool/QF_LIA, QF_LRA/Farkas, or QF_UF/Alethe mismatch obligations plus executable replay | `python3 scripts/query-rules-as-code.py checks --text implementation_equivalence --require-any` |
