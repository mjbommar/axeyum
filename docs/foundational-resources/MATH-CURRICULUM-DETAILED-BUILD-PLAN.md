...
- 95 non-template math packs.
- 477 expected checks.
- 215 checked proof/evidence rows.
- 204 replay-only rows.
- 58 Lean-horizon rows.
- 95 promoted solver-reuse packs.
...
### Wave 1: Stabilize The Current 95 Packs
...
|---|---|
| `finite-line-search-v0` | added and promoted through a bad Armijo row with a source-linked QF_LRA/Farkas artifact and route regression |
...
| QF_LRA/Farkas | rationals, matrices, LP, probability tables, geometry, dynamics, root finding, separation, KKT, SDP, gradient descent | express exact rational conflict, emit/recheck Farkas certificate | source pack links artifact and learner page names trust boundary |
...
| `linear_algebra` | exact matrices, vector spaces, duals, modules, tensors, spectral rows, SDP rows, descent-step rows, line-search rows | make matrix rows queryable by computation type and solver route | QF_LRA/Farkas, finite replay, QF_UF/Alethe |
...
| `real_analysis` | bounded rational intervals, metric continuity, RCF shadows, calculus shadows, root-finding shadows, separation/KKT/SDP/gradient-descent/line-search shadows | keep bounded shadows distinct from completeness/convergence/separation/KKT/SDP/descent/line-search theorems | QF_LRA/Farkas, QF_NRA/RCF, Lean horizon |
...
| `optimization_and_convexity` | LP/Farkas, convexity, least squares, Hessians, root-finding steps, separation rows, KKT rows, SDP rows, gradient-descent rows, line-search rows | LP objective/Farkas, rational convexity/gradient bridge rows, finite root-finding step replay, finite hyperplane-separation replay, finite KKT replay, finite SDP primal/dual replay, finite gradient-descent replay, and finite Armijo line-search replay landed; add only distinct duality, active-set variants, higher-dimensional SDP, projected-gradient, Wolfe/exact-line-search, or stochastic/convergence pressure next | QF_LRA/Farkas, QF_NRA shadows |
| `numerical_analysis` | residuals, Euler steps, exact error recurrences, matrix algorithms, root-finding, gradient-descent, and line-search iterations | keep finite replay and numerical-honesty rows distinct from promoted exact residual/error certificates | QF_LRA/Farkas, replay, Lean horizon |
...
| `reals` | deepen | RCF shadow now has a source-linked QF_LRA/Farkas negative-discriminant row, root-finding has a source-linked bad-iterate row, separation has a source-linked bad-separator row, KKT has a source-linked bad-stationarity row, SDP has a source-linked bad-objective row, and gradient descent has a source-linked bad-decrease row; keep completeness, convergence, separation, KKT sufficiency, SDP duality, descent-rate, and broad CAD/SOS/RCF claims horizon |
...
| `linear-algebra` | deepen | matrix corpus notes, dot-product/separator/KKT/SDP/gradient-step/line-search rows, and route-specific regression back-links |
| `calculus` | deepen | one-variable false derivative, Riemann-sum false integral, multivariable bad-gradient, finite root-finding bad-step, finite KKT bad-stationarity, finite gradient-descent bad-decrease, and finite line-search bad-Armijo rows now have QF_LRA/Farkas regressions |
