...
| `real_analysis` | bounded rational intervals, metric continuity, RCF shadows, calculus shadows, root-finding shadows, separation/KKT/active-set/SDP/gradient-descent/line-search/Wolfe/projected-gradient/proximal-gradient shadows | keep bounded shadows distinct from completeness/convergence/separation/KKT/active-set/SDP/descent/line-search/Wolfe/projected/proximal-gradient theorems; finite KKT now has source-linked bad stationarity and bad complementarity rows, finite SDP now has source-linked bad objective and bad duality-gap rows, finite line search has source-linked bad Armijo and bad accepted-candidate rows, and finite Wolfe line search has source-linked bad minimizer and bad curvature rows | QF_LRA/Farkas, QF_NRA/RCF, Lean horizon |
