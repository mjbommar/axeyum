...
- 18 field rows.
- 89 bridge-concept rows.
- 5 example-family rows.
- 171 non-template math packs.
- 1117 expected checks.
- 397 checked proof/evidence rows.
- 586 replay-only rows.
- 134 Lean-horizon rows.
- 171 promoted solver-reuse packs.
- 0 non-benchmark-horizon solver-reuse packs.
- 0 unclassified solver-reuse packs.
- 171 focused learner-linked packs, with no path-only, index-only, or missing
...
### Wave 1: Stabilize The Current 171 Packs
...
all 171 current non-template packs satisfy the focused-lesson side of this
...
| `linear_algebra` | exact matrices, vector spaces, duals, modules, tensors, spectral rows, regression rows, active-set QP rows, SDP rows, descent-step rows, line-search rows, Wolfe line-search rows, projected-gradient rows, proximal-gradient rows | maintain matrix rows queryable by computation type and solver route; ordinary and ridge regression normal-equation rows now include source-linked QF_LRA/Farkas regressions | QF_LRA/Farkas, finite replay, QF_UF/Alethe |
...
| `real_analysis` | bounded rational intervals, metric continuity, RCF shadows, calculus, Simpson-rule quadrature, Romberg extrapolation, divided-difference and barycentric interpolation shadows, root-finding shadows, separation/KKT/active-set/SDP/gradient-descent/line-search/Wolfe/projected-gradient/proximal-gradient shadows | keep bounded shadows distinct from completeness/convergence/integration/quadrature/extrapolation/interpolation/separation/KKT/active-set/SDP/descent/line-search/Wolfe/projected/proximal-gradient theorems; metric continuity now has source-linked bad-delta and bad open-ball preimage rows, finite Simpson now has a source-linked bad quadrature-value row, finite Romberg now has a source-linked bad extrapolated-value row, finite divided differences now has a source-linked bad interpolation-value row, finite barycentric interpolation now has a source-linked bad barycentric-value row, finite KKT now has source-linked bad stationarity and bad complementarity rows, finite SDP now has source-linked bad objective, bad duality-gap, and bad slack-entry rows, finite gradient descent now has source-linked bad decrease, bad step-coordinate, and bad descent-bound rows, finite line search has source-linked bad Armijo, bad descent-direction, and bad accepted-candidate rows, finite Wolfe line search has source-linked bad minimizer, bad sufficient-decrease, and bad curvature rows, and finite proximal gradient has source-linked bad proximal-point, bad composite-decrease, and bad box-proximal-point rows | QF_LRA/Farkas, QF_NRA/RCF, Lean horizon |
...
| `probability_theory` | finite probability, kernels, Markov chains, martingales, hitting times, concentration, finite Naive Bayes replay, finite classifier metrics, finite ROC/AUC replay | standalone finite probability mass-table, finite Naive Bayes, finite classifier-metrics, and finite ROC/AUC lessons landed; keep table/classifier rows exact and route bad rows through LRA/LIA | QF_LRA/Farkas, QF_LIA, finite replay |
| `statistics` | descriptive stats, exact tests, ordinary and ridge regression, finite count tables, finite classifier shadows, finite classifier metrics, finite ROC/AUC replay | distinguish exact finite tests, exact rational regression/classifier replay, confusion-matrix metrics, ROC/AUC score-ranking rows, PCA/LDA/k-means shadows, and numerical/statistical inference | QF_LIA, QF_LRA/Farkas, replay |
...
| `polynomials` | deepen | generated `bridge_polynomial_coefficient_factor_replay` now groups fixed identities, factor/division witnesses, finite coefficient windows, root-finding steps, derivative shadows, divided-difference and barycentric interpolation, and rational polynomial-geometry obligations; polynomial identities have a QF_LIA false-root regression, factorization has a QF_LRA/Farkas discriminant regression, root-finding has exact polynomial evaluation plus replay-only bad source rows with separate QF_LRA/Farkas bad-step and bad-width proof rows, and interpolation rows now have checked bad-value Farkas proof rows |
...
185. Landed: split `bounded-dynamics-v0` transition-step, threshold-step, and
     ...
     rows.
186. Landed: split `finite-recurrence-prefix-v0` bad Fibonacci value and bad
     affine-step proof-object checking into explicit `qf-lra-*` rows. Exact
     recurrence replay still owns the malformed source rows by computing
     `F_6 = 8` and `x_4 = 15` with transition residual `1`, while the
     QF_LRA/Farkas rows separately reject the fixed `F_6 = 9` and
     `transition_residual <= 0` conflicts through the existing
     `math_resource_lra_routes` regressions. The public query surface now
...
189. Landed: split `finite-separation-v0` bad convex-combination and bad
     separator proof-object checking into explicit `qf-lra-*` rows. Exact
     finite replay still owns the malformed source rows by computing point
     `(1/3, 1/3)`, x-coordinate error `1/6`, outside score `4`, and score
     excess `3`, while the QF_LRA/Farkas rows separately reject the fixed
     equality and separator-bound conflicts through the existing
     `math_resource_lra_routes` regressions. The public query surface now
...
271. Landed: add `finite-qr-decomposition-v0` as an exact rational matrix
     factorization resource. The pack replays `Q^T Q = I`, upper-triangular
     `R`, and `Q*R = A`, then adds `qf-lra-bad-qr-product-entry` as the
     source-linked Farkas row for the malformed bottom-right product-entry
...
274. Landed: add `finite-newton-step-v0` as an exact finite multivariable
     Newton-step resource. The pack replays the fixed quadratic's gradient,
     Hessian, positive leading minors, Hessian inverse, Newton direction,
     stationary next point, and objective decrease, then adds
     `qf-lra-bad-newton-coordinate` as the source-linked Farkas row for the
     malformed next-coordinate claim `3/2` versus the exact value `10/7`.
...
276. Landed: add `finite-singular-value-shadow-v0` as an exact finite
     singular-value/SVD-shadow resource. The pack replays `A^T A` for
     `A = [[3,0],[0,1]]`, singular-vector equations, `U*Sigma*V^T = A`,
     `||A||_2 = 3`, `||A||_F^2 = 10`, and `kappa_2(A) = 3`, then adds
     `qf-lra-bad-singular-value-bound` as the source-linked Farkas row for
     `sigma_max(A) <= 2`. The learner and query pages keep exact rational SVD
...
277. Landed: add `finite-jordan-chain-v0` as an exact finite Jordan-chain
     ...
     claims.
278. Landed: add `finite-schur-complement-v0` as an exact finite
     Schur-complement resource. The pack replays `A = [[4,2],[2,3]]`, the
     one-by-one leading block inverse, `S = D - C*B^-1*C^T = 2`, determinant
     factorization, two-sided inverse replay, positive-definite and
     conditional-variance shadows, then adds
     `qf-lra-bad-schur-complement` as the source-linked Farkas row for the
     false scalar claim `S = 3/2`. The learner and query pages keep exact
...
281. Landed: add `finite-conjugate-gradient-v0` as an exact finite
     conjugate-gradient resource. The pack replays
     `A = [[4,1],[1,3]]`, `b = [1,2]`, `x0 = [0,0]`, two residual updates,
     two step sizes, Fletcher-Reeves `beta`, residual orthogonality,
     A-conjugacy, and the exact solution `[1/11,7/11]`, then adds
     `qf-lra-bad-cg-alpha0` as the source-linked Farkas row for the false
     first step-size claim `1/3` versus exact `1/4`. The learner and query
     pages keep exact finite CG replay separate from Krylov minimization,
     finite-termination theorems, preconditioners, roundoff, and
     floating-point CG stability.
282. Landed: add `finite-arnoldi-iteration-v0` as an exact finite Arnoldi
     resource. The pack replays `A = [[1,2],[3,4]]`, `q1 = [1,0]`,
     `A*q1 = [1,3]`, `h11 = 1`, residual `[0,3]`, `h21 = 3`,
     `q2 = [0,1]`, the second projection column `h12 = 2`, `h22 = 4`,
     orthonormality, and the exact Hessenberg relation `A*Q = Q*H`, then adds
     `qf-lra-bad-arnoldi-h21` as the source-linked Farkas row for the false
     subdiagonal coefficient claim `h21 = 2` versus exact `3`. The learner and
...
288. Landed: add `finite-pivoted-lu-decomposition-v0` as an exact finite
     ...
     stability.
289. Landed: add `finite-ldlt-decomposition-v0` as an exact finite LDLT
     decomposition resource. The pack replays
     ...
     `[1,1]`, then adds `qf-lra-bad-ldlt-diagonal` as the source-linked
     Farkas row for the false diagonal-entry claim `D[1,1] = 3` versus exact
...
291. Landed: add `finite-real-schur-decomposition-v0` as an exact finite
     real-Schur resource. The pack replays
     `Q = [[3/5,4/5],[-4/5,3/5]]`, `T = [[1,2],[0,4]]`,
     `A = [[97/25,54/25],[4/25,28/25]]`, `Q^T*Q = I`,
     `Q*T*Q^T = A`, `A*Q = Q*T`, `trace(A) = 5`, and
     `det(A) = 4`, then adds `qf-lra-bad-schur-superdiagonal` as the
     source-linked Farkas row for the false superdiagonal claim
     `T[0,1] = 3` versus exact `2`. The learner and query pages keep exact
...
296. Landed: add `finite-interval-arithmetic-shadow-v0` as an exact finite
     rational interval-arithmetic resource. The pack replays
     `X = Y = [1, 10001/10000]`, endpoint-wise sum/product intervals, interval
     widths, and the second-order product-width term, then adds
     `qf-lra-bad-interval-product-upper` as the source-linked Farkas row for the
     false shortcut claim `product_upper <= 5001/5000`. The learner and query
...
298. Landed: add `finite-gmres-residual-shadow-v0` as an exact finite
     one-step GMRES residual-minimization resource. The pack replays
     `A=[[2,1],[1,2]]`, `b=[1,0]`, `x0=[0,0]`, initial residual
     `r0=[1,0]`, Krylov direction `A*r0=[2,1]`, minimizer coefficient
     `alpha=2/5`, residual `[1/5,-2/5]`, residual orthogonality, and
     residual-norm decrease, then adds `qf-lra-bad-gmres-alpha` as the
     source-linked Farkas row for the false one-step coefficient claim
     `alpha=1/2`. The learner and query pages keep this finite exact Krylov
...
299. Landed: add `finite-runge-kutta-midpoint-v0` as an exact finite
     ...
     control.
300. Landed: add `finite-heun-method-v0` as an exact finite explicit
     trapezoidal RK2 time-stepping resource. The pack replays `y' = 2t`,
     `y(0)=0`, `h=1/2`, predictor states `[0, 3/4, 2]`, endpoint
     derivatives `[1, 2, 3]`, averaged derivatives `[1/2, 3/2, 5/2]`,
     states `[0, 1/4, 1, 9/4]`, exact solution `[0, 1/4, 1, 9/4]`, and zero
     absolute error, then adds `qf-lra-bad-heun-step` as the source-linked
     Farkas row for the false first-step claim `1/2` versus exact `1/4`. The
     learner and query pages keep this finite exact Heun shadow separate from
     general RK2 order theory, consistency, convergence, stability regions,
     stiffness, and adaptive-step control.
301. Landed: add `finite-backward-euler-method-v0` as an exact finite
     implicit backward Euler time-stepping resource. The pack replays
     `y' = -y`, `y(0)=1`, `h=1/2`, endpoint derivatives
     `[-2/3, -4/9, -8/27]`, zero implicit residuals, geometric decay ratio
     `2/3`, and states `[1, 2/3, 4/9, 8/27]`, then adds
     `qf-lra-bad-backward-euler-step` as the source-linked Farkas row for the
     false first-step claim `1/2` versus exact `2/3`. The learner and query
     pages keep this finite exact implicit-step shadow separate from general
     backward Euler convergence, A-stability, stiffness, nonlinear solve,
     adaptive-step, floating-point, and PDE theory.
302. Landed: add `finite-crank-nicolson-method-v0` as an exact finite
     implicit trapezoid time-stepping resource. The pack replays
     `y' = -y`, `y(0)=1`, `h=1/2`, start derivatives, endpoint derivatives,
     averaged slopes, zero implicit residuals, geometric decay ratio `3/5`,
     and states `[1, 3/5, 9/25, 27/125]`, then adds
     `qf-lra-bad-crank-nicolson-step` as the source-linked Farkas row for the
     false first-step claim `1/2` versus exact `3/5`. The learner and query
     pages keep this finite exact implicit-trapezoid shadow separate from
     general Crank-Nicolson order, convergence, A-stability, stiffness,
     nonlinear solve, adaptive-step, floating-point, and PDE theory.
303. Landed: add `finite-adams-bashforth-method-v0` as an exact finite
     explicit two-step Adams-Bashforth resource. The pack replays `y' = 2t`,
     `y(0)=0`, `h=1/2`, exact starter `y_1=1/4`, derivative history, slopes
     `[3/2, 5/2]`, exact states `[0, 1/4, 1, 9/4]`, and zero error, then adds
     `qf-lra-bad-adams-bashforth-step` as the source-linked Farkas row for the
     false first multistep claim `3/4` versus exact `1`. The learner and query
...
305. Landed: add `finite-simpson-rule-v0` as an exact finite Simpson-rule
     quadrature resource. The pack replays single-panel Simpson nodes,
     weights, sample values, weighted sums, Simpson values, and exact
     polynomial integrals for `x^3` and `1+x^2` on `[0,2]`, then adds
     `qf-lra-bad-simpson-value` as the source-linked Farkas row for the false
     cubic quadrature claim `7/2` versus exact `4`. The learner and query
     pages keep this finite exact quadrature shadow separate from general
     Simpson degree-of-exactness, composite/adaptive quadrature convergence,
     error bounds, floating-point quadrature correctness, and numerical
     stability.
306. Landed: add `finite-divided-differences-v0` as an exact finite Newton
     interpolation resource. The pack replays divided-difference tables,
     Newton coefficients, basis products, terms, and interpolation values for
     `1+x^2` at nodes `0,1,2` and `x^3` at nodes `0,1,2,3`, then adds
     `qf-lra-bad-interpolation-value` as the source-linked Farkas row for the
     false interpolation claim `9` versus exact `10`. The learner and query
     pages keep this finite exact interpolation shadow separate from general
...
307. Landed: add `finite-barycentric-interpolation-v0` as an exact finite
     barycentric interpolation resource. The pack replays barycentric weights,
     regular numerator/denominator terms, interpolation values for `1+2*x` and
     `x^2`, and the explicit node-hit value for `x=1`, then adds
     `qf-lra-bad-barycentric-value` as the source-linked Farkas row for the
     false barycentric claim `5` versus exact `4`. The learner and query pages
     keep this finite exact interpolation shadow separate from
     ...
     floating-point interpolation correctness, and numerical stability.
308. Landed: add `finite-difference-derivatives-v0` as an exact finite
     difference derivative resource. The pack replays a forward affine
     first-derivative stencil, a central quadratic first-derivative stencil,
     and a central quadratic second-derivative stencil, then adds
     `qf-lra-bad-finite-difference-value` as the source-linked Farkas row for
     the false finite-difference claim `5` versus exact `4`. The learner and
     query pages keep this finite exact stencil shadow separate from
...
311. Landed: add `finite-cubic-spline-interpolation-v0` as an exact finite
     natural cubic spline interpolation resource. The pack replays two cubic
     pieces through knots `0, 1, 2`, sample values `0, 1, 0`, C1/C2 continuity
     at the interior knot, natural endpoint second derivatives, and midpoint
     values `11/16`, then adds `qf-lra-bad-spline-value` as the source-linked
     Farkas row for the false spline-value claim `3/4` versus exact `11/16`.
     The learner and query pages keep this finite exact spline shadow
     separate from spline existence, uniqueness, error estimates, convergence
     and stability, knot-selection, shape-preservation, and floating-point
     spline evaluation.
312. Landed: add `finite-romberg-extrapolation-v0` as an exact finite
     Romberg/Richardson extrapolation resource. The pack replays one-panel and
     two-panel composite trapezoid values, computes the extrapolated value
     `(4*T(h/2)-T(h))/3` for `x^2` and `x^4` on `[0,1]`, records exact
     quadratic error cancellation and the quartic residual `1/120`, then adds
     `qf-lra-bad-romberg-value` as the source-linked Farkas row for the false
     extrapolated-value claim `1/4` versus exact `1/3`. The learner, query,
     and calculus-boundary pages keep this finite exact extrapolation shadow
...
316. Landed: add `finite-ridge-regression-v0` as an exact finite
     regularized-regression resource. The pack replays the fixed design matrix
     `[[1,0],[1,1],[1,2]]`, response `[1,2,4]`, `lambda = 1`,
     regularized normal matrix `X^T X + I`, ridge coefficients
     `[4/5, 19/15]`, residuals, RSS, penalty, regularized objective,
     coefficient shrinkage, and objective comparison against ordinary least
     squares, then adds `qf-lra-bad-ridge-beta0` as the source-linked Farkas
     row for the false regularized coefficient claim `beta0 = 1` versus exact
     `4/5`. The learner and query pages keep this finite exact ridge shadow
     separate from general ridge optimality, regularization paths,
...
319. Landed: add `finite-k-means-clustering-v0` as an exact finite clustering
     resource. The pack replays a fixed four-point rational sample, a fixed
     two-cluster assignment, exact centroids, residuals, squared distances,
     WCSS, global centroid, total scatter, and between-cluster scatter, then
     adds `qf-lra-bad-centroid` as the source-linked Farkas row for the false
     centroid-coordinate claim `-1/2` versus exact `-1`. The learner and query
     pages keep this finite exact clustering shadow separate from Lloyd
     convergence, global optimality, clustering consistency, randomized
     initialization, floating-point k-means implementations, and statistical
     generalization.
320. Landed: add `finite-naive-bayes-classifier-v0` as an exact finite
     classifier resource. The pack replays a fixed six-row binary-feature
     training table, class counts, priors, Laplace-smoothed class-conditional
     likelihoods, class scores, posterior probabilities, score/posterior
     margins, and the positive decision, then adds `qf-lra-bad-posterior` as
     the source-linked Farkas row for the false posterior claim `2/3` versus
     exact `9/13`. The learner and query pages keep this finite exact Naive
     Bayes shadow separate from conditional-independence validity, Bayes
...
322. Landed: add `finite-roc-auc-v0` as an exact finite classifier-ranking
     resource. The pack replays a fixed six-row rational score table,
     descending score order, class totals, the `score >= 7/10` threshold
     operating point, TPR/FPR/precision/specificity rates, the ROC staircase,
     pairwise AUC, and trapezoid area, then adds `qf-lra-bad-auc` as the
     source-linked Farkas row for the false AUC claim `3/4` versus exact `2/3`.
     The learner and query pages keep this finite tie-free ROC/AUC shadow
