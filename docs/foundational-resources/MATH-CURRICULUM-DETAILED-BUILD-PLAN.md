...
### Wave 1: Stabilize The Current 94 Packs
...
|---|---|
| `finite-gradient-descent-v0` | added and promoted through a bad decrease row with a source-linked QF_LRA/Farkas artifact and route regression |
...
| `linear_algebra` | exact matrices, vector spaces, duals, modules, tensors, spectral rows, SDP rows, descent-step rows | make matrix rows queryable by computation type and solver route | QF_LRA/Farkas, finite replay, QF_UF/Alethe |
...
| `real_analysis` | bounded rational intervals, metric continuity, RCF shadows, calculus shadows, root-finding shadows, separation/KKT/SDP/gradient-descent shadows | keep bounded shadows distinct from completeness/convergence/separation/KKT/SDP/descent theorems | QF_LRA/Farkas, QF_NRA/RCF, Lean horizon |
...
| `optimization_and_convexity` | LP/Farkas, convexity, least squares, Hessians, root-finding steps, separation rows, KKT rows, SDP rows, gradient-descent rows | LP objective/Farkas, rational convexity/gradient bridge rows, finite root-finding step replay, finite hyperplane-separation replay, finite KKT replay, finite SDP primal/dual replay, and finite gradient-descent replay landed; add only distinct duality, active-set variants, higher-dimensional SDP, line-search/projected-gradient, or stochastic/convergence pressure next | QF_LRA/Farkas, QF_NRA shadows |
| `numerical_analysis` | residuals, Euler steps, exact error recurrences, matrix algorithms, root-finding and gradient-descent iterations | keep finite replay and numerical-honesty rows distinct from promoted exact residual/error certificates | QF_LRA/Farkas, replay, Lean horizon |
...
| `linear-algebra` | deepen | matrix corpus notes, dot-product/separator/KKT/SDP/gradient-step rows, and route-specific regression back-links |
| `calculus` | deepen | one-variable false derivative, Riemann-sum false integral, multivariable bad-gradient, finite root-finding bad-step, finite KKT bad-stationarity, and finite gradient-descent bad-decrease rows now have QF_LRA/Farkas regressions |
