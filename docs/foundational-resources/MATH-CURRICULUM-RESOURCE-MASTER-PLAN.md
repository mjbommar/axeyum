...
As of 2026-07-03, the committed resource query reports:
...
| `real_analysis` | rational intervals, metric continuity, sequences, compactness/connectedness, Riemann-sum and Simpson-rule integration shadows, root-finding, optimization shadows | bounded-vs-theorem bridge rows, theorem-horizon map for completeness, integration, quadrature, and convergence | QF_LRA/Farkas, QF_NRA/RCF shadows, Lean horizon | every lesson states finite/bounded shadow vs theorem |
...
| `numerical_analysis` | residuals, Simpson-rule quadrature, explicit and implicit Euler/Crank-Nicolson steps, Adams-Bashforth and BDF2 multistep rows, root-finding, finite optimization iterations, operator, singular-value, and Jordan-chain bounds | landed finite dynamics/Euler bridge plus exact condition-number, singular-value, Jordan-chain, and quadrature shadows; add only distinct implicit-solve, multistep-history, quadrature, pivoting/stability, or reproducible numerical metadata pressure | QF_LRA/Farkas, replay, Lean horizon | exact replay is separate from floating-point experiment claims |
...
16. Landed: add functional-analysis/operator consumer-query coverage through
   ...
   spectral/eigenpair witnesses, Jordan-chain shadows, and operator bridge
   rows visible through the public JSON contract.
