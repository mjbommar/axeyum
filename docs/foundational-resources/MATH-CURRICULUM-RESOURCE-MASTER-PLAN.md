...
Goal: make the current 114 packs a stable, queryable data product.
...
| `linear_algebra` | rational matrices, finite vector/dual/module/tensor, spectral/singular-value, invariant, optimization/numerical rows | matrix-computation index plus matrix-corpus boundary by LU/rank/nullity/projection/residual/eigen/singular-value/characteristic/random-moment; LU/nullspace/singular-bound Farkas rows and finite vector/module/dual Alethe closure-additivity rows now have source-linked checked evidence | QF_LRA/Farkas, finite replay, QF_UF/Alethe | solver regressions cite source pack and pack cites regression before benchmark claims |
...
| `numerical_analysis` | residuals, Euler steps, root-finding, finite optimization iterations, operator and singular-value bounds | landed finite dynamics/Euler bridge plus exact condition-number and singular-value shadows; add only distinct pivoting/stability or reproducible numerical metadata pressure | QF_LRA/Farkas, replay, Lean horizon | exact replay is separate from floating-point experiment claims |
...
| `functional_analysis_and_operator_theory` | finite operators, inner products, projections, singular-value shadows, Chebyshev slices, finite duals | norm variants, finite approximation/alternation, operator-spectrum/SVD rows only when they add distinct proof pressure | QF_LRA/Farkas, finite replay, Lean horizon | Banach/Hilbert, SVD theorem, and infinite-dimensional claims are not finite checks |
...
12. Landed: add
   ...
   as a rules/law pack that reuses exact rational allocation shares, budget
   balance, shelter/clinic floors, administrative caps, and QF_LRA/Farkas proof
...
15. Landed: add
   [`rules-law-trust-boundary.md`](../learn/rules-law-trust-boundary.md) so
   learners can trace the rules/law lane from source rule to formal model,
   replayed witness, checked obligation, and explicit horizon.
16. Landed: add functional-analysis/operator consumer-query coverage through
...
24. Landed: add the modular CRT/inverse witness bridge row and number-theory
   ...
   fixed-width nonunit-inverse and Fermat-unit QF_BV rows queryable from the atlas while keeping
...
27. Landed: add `grant-allocation-v0` as the rational-allocation rules/law
    crosswalk example, reusing exact shares, budget balance, minimum-share
    floors, administrative caps, finite replay, and checked QF_LRA/Farkas
    fixtures.
28. Landed: add `RULES-LAW-QUERIES.md` and
...
29. Landed: add `RULES-LAW-PATTERN-MATRIX.md` so the current rule-pattern
    surface is mapped to math concept rows and proof routes before a new
    rule ontology or rule-pack family is introduced.
30. Landed: add `docs/learn/rules-law-trust-boundary.md` as the educational
    trust-boundary page for the current rules/law packs.
31. Add future rules/law crosswalk examples only by reusing existing
