...
- 95 non-template math example packs.
- 477 expected checks.
...
| QF_LRA/Farkas | exact rational infeasibility, LP, residuals, root-finding steps, separation rows, KKT rows, SDP rows, gradient-descent rows, line-search rows, probability tables | Continue promoting bad table, bad bound, bad iterate, bad separator, bad stationarity, bad objective, bad decrease, and bad Armijo rows with independent Farkas checks. |
...
- Keep `sequence-limit-shadow-v0`'s promoted bounded Cauchy-tail row and
  ...
  tied to its source QF_LRA/Farkas artifact; and keep general
  ...
  convergence, asymptotics, and stability in the Lean-horizon lane.
...
- Landed bridge rows for LP objective-threshold/Farkas replay and rational
  ...
  Armijo trial rejection and accepted-backtrack replay. Add narrower rows only
  when multiple packs need distinct duality, active-set variants,
  higher-dimensional SDP, projected-gradient, Wolfe/exact-line-search, affine
...
- Promote small infeasible LP/convexity/root-finding/separation/KKT/SDP/descent
  and line-search rows through QF_LRA/Farkas.
...
66. Continue proof-route promotions or consumer-query examples; revisit the
