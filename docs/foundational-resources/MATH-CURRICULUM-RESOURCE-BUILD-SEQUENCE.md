...
- 129 concept rows: 23 curriculum nodes, 18 field rows, 83 bridge concepts, and
  5 example-family rows.
- 165 non-template math example packs.
- 1075 expected checks: 549 `sat`, 398 `unsat`, and 128 `not-run`.
- 391 checked proof/evidence rows, 556 replay-only rows, and 128 Lean-horizon
  rows.
...
Goal: make the existing 116 packs easy to discover by curriculum layer, field,
...
- finite quotient-topology replay: quotient map, fibers, representative
  consistency, saturated opens, preimage-open definition, and checked bad
...
1. Refresh stale planning counts and link this sequence from the resource
   indexes.
2. Audit learner coverage for the current 163 packs and record any remaining
