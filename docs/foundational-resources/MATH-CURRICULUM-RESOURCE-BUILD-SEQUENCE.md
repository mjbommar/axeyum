...
- 698 expected checks: 340 `sat`, 285 `unsat`, and 73 `not-run`.
- 330 checked proof/evidence rows, 295 replay-only rows, and 73 Lean-horizon
  rows.
...
Goal: make the existing 110 packs easy to discover by curriculum layer, field,
...
1. Refresh stale planning counts and link this sequence from the resource
   indexes.
2. Audit learner coverage for the current 110 packs and record any remaining
