...
- 125 concept rows: 23 curriculum nodes, 18 field rows, 79 bridge concepts, and
  5 example-family rows.
- 161 non-template math example packs.
- 1047 expected checks: 533 `sat`, 390 `unsat`, and 124 `not-run`.
- 387 checked proof/evidence rows, 536 replay-only rows, and 124 Lean-horizon
  rows.
...
1. Refresh stale planning counts and link this sequence from the resource
   indexes.
2. Audit learner coverage for the current 161 packs and record any remaining
