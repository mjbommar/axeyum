...
- 693 expected checks: 338 `sat`, 283 `unsat`, and 72 `not-run`.
- 326 checked proof/evidence rows, 295 replay-only rows, and 72 Lean-horizon
  rows.
...
Goal: make the existing 109 packs easy to discover by curriculum layer, field,
...
1. Refresh stale planning counts and link this sequence from the resource
   indexes.
2. Audit learner coverage for the current 109 packs and record any remaining
