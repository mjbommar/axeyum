...
- 126 concept rows: 23 curriculum nodes, 18 field rows, 80 bridge concepts, and
  5 example-family rows.
- 162 non-template math example packs.
- 1054 expected checks: 537 `sat`, 392 `unsat`, and 125 `not-run`.
- 388 checked proof/evidence rows, 541 replay-only rows, and 125 Lean-horizon
  rows.
...
1. Refresh stale planning counts and link this sequence from the resource
   indexes.
2. Audit learner coverage for the current 162 packs and record any remaining
