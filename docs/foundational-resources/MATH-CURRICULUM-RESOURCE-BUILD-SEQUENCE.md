...
- 124 concept rows: 23 curriculum nodes, 18 field rows, 78 bridge concepts, and
  5 example-family rows.
- 160 non-template math example packs.
- 1040 expected checks: 529 `sat`, 388 `unsat`, and 123 `not-run`.
- 386 checked proof/evidence rows, 531 replay-only rows, and 123 Lean-horizon
  rows.
...
1. Refresh stale planning counts and link this sequence from the resource
   indexes.
2. Audit learner coverage for the current 160 packs and record any remaining
