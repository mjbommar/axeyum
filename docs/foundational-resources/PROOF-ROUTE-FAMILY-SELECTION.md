...
| QF_UF/Alethe | equality-heavy finite structures and quotient maps | equivalence classes, function composition, finite homomorphisms, finite monoids/groups/actions including identity and compatibility conflicts, finite module scalar-closure membership, modules/tensors, continuous-map preimage membership, quotient topology representative/open conflicts, and cohomology shadows | Alethe adds value when table replay says what happened and congruence proof explains why an equality claim is impossible. | Prefer equality conflicts where table replay and congruence proof teach different things, such as algebra/topology map preservation beyond the current preimage, module scalar-closure, and quotient representative consistency rows. |
