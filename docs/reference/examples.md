...
| [`ordered_ring_refutation`](../../crates/axeyum-solver/examples/ordered_ring_refutation.rs) | `[--require-empty\|--dump-statement\|--footprint-table]`, requires `--features full` | Generalizes an LRA/Farkas or SOS refutation over the ordered-ring interface, λ-abstracting the 30 `Real` constants out of the proof term so the generalized theorem's `Kernel::axiom_footprint` is measured **empty** — the laws become the theorem's own hypotheses rather than assumptions — and re-applies it to the `Real` constants to recover the original, kernel-rechecked statement. Prints the non-empty original footprint alongside the empty generalized one as a negative control in the same output. Also reconstructs each fixture a second time with equality routed through the **equality slot** (ADR-0468 phase R3), generalizes it over **39** binders instead of 30, and specializes that back at `Eq`: the 30 non-slot binder types must come back node for node, and the setoid proof must mention no kernel `Eq` constant. `--require-empty` fails on any of those. |
