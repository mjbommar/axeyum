...
| [`creal_setoid_witness`](../../crates/axeyum-lean-kernel/examples/creal_setoid_witness.rs) | no arguments | ℝ **constructed** — a Bishop setoid of regular ℚ-sequences (ADR-0468 phase R1), with `CReal.Equiv`'s reflexivity, symmetry and transitivity all proved, at a measured trusted surface of **0**. Where [`creal_shape_probe`](../../crates/axeyum-lean-kernel/examples/creal_shape_probe.rs) priced the carrier parametrically before ℚ had an order, this is the real thing over the constructed `Rat`. The exit status depends on four findings, and the last two are what stop a green light being vacuous: every declaration checked and footprint-empty; the whole trusted surface still empty; `CReal.ofRat` present, so `CReal.Regular` has a **solution** and the carrier is not the empty type (all three laws would otherwise hold, footprint-free, of nothing); and `CReal.Equiv.not_zero_one` present, so `Equiv` is not the **total** relation, which is also an equivalence relation. Verified by mutation: deleting both witnesses leaves all eleven other rows green with empty footprints and still exits 1. Of the 22 ordered-ring laws, **7** now hold: the whole additive group in `Equiv` form (`add_comm`, `add_neg`, `add_zero`, `add_assoc` — the last two are not pointwise and close on regularity plus `1/(2n+2) + 1/(n+1) ≤ 2/(n+1)`) and three order laws **verbatim** (`le_refl`, `le_trans`, `add_le_add`, none of which mentions `Eq`). A fifth exit condition covers the order: `CReal.not_le_one_zero`, without which all three order laws hold footprint-free of the relation that relates every pair. |
