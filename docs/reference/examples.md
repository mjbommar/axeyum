...
| [`creal_setoid_witness`](../../crates/axeyum-lean-kernel/examples/creal_setoid_witness.rs) | no arguments | ℝ **constructed** — a Bishop setoid of regular ℚ-sequences (ADR-0468 phase R1/R2), with `CReal.Equiv`'s reflexivity, symmetry and transitivity all proved, at a measured trusted surface of **0**. Where [`creal_shape_probe`](../../crates/axeyum-lean-kernel/examples/creal_shape_probe.rs) priced the carrier parametrically before ℚ had an order, this is the real thing over the constructed `Rat`. The exit status depends on seven findings, and the last five are what stop a green light being vacuous: every declaration checked and footprint-empty; the whole trusted surface still empty; `CReal.ofRat` present, so `CReal.Regular` has a **solution** and the carrier is not the empty type (every law would otherwise hold, footprint-free, of nothing); `CReal.Equiv.not_zero_one` present, so `Equiv` is not the **total** relation, which is also an equivalence relation; `CReal.not_le_one_zero`, without which all three `le` laws hold footprint-free of the relation that relates every pair; `CReal.zero_lt_one` **plus** `CReal.lt_irrefl`, because six of the seven strict-order laws only CONSUME a `lt` and so all hold, footprint-free, of the EMPTY relation; and `CReal.ofRat_mul` **plus** `CReal.not_equiv_mul_one_one_zero`, because `mul_zero`, `mul_comm` and `sq_nonneg` all hold, footprint-free, of the product `fun _ _ => zero` — `ofRat_mul` pins the OPERATION on the embedded ℚ rather than asserting a property of it, and `not_equiv_mul_one_one_zero` refuses the degenerate product by computation. Verified by mutation: deleting any one witness leaves every other row green with empty footprints and still exits 1. Of the 22 ordered-ring laws, **19** now hold: seven in `Equiv` form (`add_comm`, `add_neg`, `add_zero`, `add_assoc`, `mul_comm`, `mul_zero`, `mul_one`) and twelve **verbatim** (`le_refl`, `le_trans`, `add_le_add`, `lt_irrefl`, `lt_trans`, `lt_of_lt_of_le`, `lt_of_le_of_lt`, `le_of_lt`, `zero_lt_one`, `add_lt_add_of_le_of_lt`, `mul_nonneg`, `sq_nonneg` — none of which mentions `Eq`). The 3 that remain (`mul_assoc`, `left_distrib`, `mul_le_mul_of_nonneg_left`) are exactly the 3 that compare two products sampled at DIFFERENT indices. |
