...
| [`creal_setoid_witness`](../../crates/axeyum-lean-kernel/examples/creal_setoid_witness.rs) | no arguments | ℝ **constructed** — a Bishop setoid of regular ℚ-sequences (ADR-0468 phase R1/R2), with `CReal.Equiv`'s reflexivity, symmetry and transitivity all proved, at a measured trusted surface of **0**. Where [`creal_shape_probe`](../../crates/axeyum-lean-kernel/examples/creal_shape_probe.rs) priced the carrier parametrically before ℚ had an order, this is the real thing over the constructed `Rat`. The exit status depends on six findings, and the last four are what stop a green light being vacuous: every declaration checked and footprint-empty; the whole trusted surface still empty; `CReal.ofRat` present, so `CReal.Regular` has a **solution** and the carrier is not the empty type (every law would otherwise hold, footprint-free, of nothing); `CReal.Equiv.not_zero_one` present, so `Equiv` is not the **total** relation, which is also an equivalence relation; `CReal.not_le_one_zero`, without which all three `le` laws hold footprint-free of the relation that relates every pair; and `CReal.zero_lt_one` **plus** `CReal.lt_irrefl`, because six of the seven strict-order laws only CONSUME a `lt` and so all hold, footprint-free, of the EMPTY relation — `zero_lt_one` exhibits an inhabitant and `lt_irrefl` refuses a pair. Verified by mutation: deleting either strict-order witness leaves every other row green with empty footprints and still exits 1. Of the 22 ordered-ring laws, **14** now hold: the whole additive group in `Equiv` form (`add_comm`, `add_neg`, `add_zero`, `add_assoc` — the last two are not pointwise and close on regularity plus `1/(2n+2) + 1/(n+1) ≤ 2/(n+1)`) and ten order laws **verbatim** (`le_refl`, `le_trans`, `add_le_add`, `lt_irrefl`, `lt_trans`, `lt_of_lt_of_le`, `lt_of_le_of_lt`, `le_of_lt`, `zero_lt_one`, `add_lt_add_of_le_of_lt` — none of which mentions `Eq`). The 8 that remain are exactly the 8 that mention `mul`. |
