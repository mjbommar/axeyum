# Diary: evidence-certification — the verdict is not the certificate

The working record for the `evidence-certification` lane, 2026-08-17. Kept here
rather than in `docs/plan/status/` because `check-plan-authority.py` budgets the
files `PLAN.md` is generated from and this narrative is journal, not status.
