...
4. Treat the accepted S4 root as immutable. S5/S5.1 admission and the repaired
   ...
   Lean owner must restore the quotient-package contract/implementation marker
   and regenerate the downstream `resume_fs.py` pin as needed. No live root or
   canonical acceptance record has been published. Revalidate the exact current `origin/main`,
