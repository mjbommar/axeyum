...
so the lane block stays inside the per-lane ceiling (ADR-0471). The decision
itself is
[ADR-0474](../../research/09-decisions/adr-0474-the-real-inverse-is-partial-and-its-modulus-is-data.md).
...
- **No `sqrt`, no completeness, no supremum.** Each is its own ADR. ℂ's `abs`
  needs `sqrt` needs completeness, so ADR-0472's gap is untouched by this lane.
