...
| [`corpus.json`](corpus.json) | the corpus: 128 entries, one per query, each with its area, module tag (if any), tier, expected value, and the method that established it |
...
Tiers: **117 `core`**, **11 `decline_expected`** (≥ 10 `decline_expected`
...
Total entries: **128** (was 71 before item 10 wave two, 119 after it,
123 after item 9 wave three, 126 after item 9 wave four, 128 after the item 1
wave four atom-key pair). The README's own count sat at 119 through wave
three and two lanes corrected it concurrently; the counts here are measured
from `corpus.json` at merge, not carried from either lane.
