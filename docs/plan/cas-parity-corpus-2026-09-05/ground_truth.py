...
def check_qe() -> None:
    section("qe")
    if need_sympy("qe1/qe2/qe3"):
        return
    x = sp.symbols("x", real=True)
    ok(sp.solve(x**2 - 2, x) != [], "qe1 (hand) exists x. x^2-2=0 is true, witnessed by sqrt(2)")
    ok(
        sp.ask(sp.Q.positive(x**2 + 1)) is not False,
        "qe2 (hand) x^2+1 > 0 for all real x (a square plus 1)",
    )
    witness = 10**15
...
def check_permgroup() -> None:
    section("permgroup")
    if need_sympy("pg1/pg2"):
        return
    from sympy.combinatorics import Permutation, PermutationGroup

    s3 = PermutationGroup([Permutation(0, 1, 2), Permutation(0, 1)])
    ok(s3.order() == 6, "pg1 |S3| = 6 (sympy.combinatorics)")
    c4 = PermutationGroup([Permutation(0, 1, 2, 3)])
    ok(c4.order() == 4, "pg2 |C4| = 4 (sympy.combinatorics, a single 4-cycle)")
...
def check_probability() -> None:
    section("probability")
    if need_sympy("prob1/prob2/prob3"):
        return
    # prob1: the sum of two independent Poisson variables is Poisson with the
    # summed rate -- classical (verified here by the pgf argument: the
    # probability generating function of Poisson(lam) is e^{lam(z-1)}, and
    # pgfs of independent sums multiply).
    z, l1, l2 = sp.symbols("z lambda1 lambda2", positive=True)
    pgf1 = sp.exp(l1 * (z - 1))
    pgf2 = sp.exp(l2 * (z - 1))
    combined = sp.simplify(pgf1 * pgf2 - sp.exp((l1 + l2) * (z - 1)))
    ok(combined == 0, "prob1 pgf(Poisson(l1))*pgf(Poisson(l2)) = pgf(Poisson(l1+l2))")
    # prob2: Poisson's total mass is provably 1 (classical: e^{-lam} sum
    # lam^k/k! = e^{-lam} e^{lam} = 1) -- the point of this entry is that
    # axeyum-cas's OWN machinery declines to certify this true fact via its
    # available route (lam^k/k! is not Gosper-summable), which is a fact
    # about the tool, not about the mathematics; the mathematics is settled.
    lam = sp.symbols("lam", positive=True)
    k = sp.symbols("k", integer=True, nonnegative=True)
    total = sp.summation(sp.exp(-lam) * lam**k / sp.factorial(k), (k, 0, sp.oo))
    ok(sp.simplify(total - 1) == 0, "prob2 (hand/sympy) Poisson total mass sums to 1 (true; the CAS route declines it)")
    # prob3: E[Binomial(n,p)] = n*p (classical; verified for n=4 symbolically).
    p = sp.symbols("p")
    n_val = 4
    mean = sp.simplify(
        sum(k_ * sp.binomial(n_val, k_) * p**k_ * (1 - p) ** (n_val - k_) for k_ in range(n_val + 1))
    )
    ok(sp.expand(mean - n_val * p) == 0, "prob3 E[Binomial(4,p)] = 4p")


def check_geometry_beyond() -> None:
    section("geometry_beyond")
    if need_sympy("gb1/gb2"):
        return
    x, y = sp.symbols("x y")
    ...
    conic = x**2 + y**2 - 1
    pts = [(1, 0), (0, 1), (-1, 0), (0, -1), (sp.Rational(3, 5), sp.Rational(4, 5))]
    ok(all(conic.subs({x: px, y: py}) == 0 for px, py in pts), "gb1 (hand) the five points lie on x^2+y^2-1=0")
    sixth = (sp.Rational(4, 5), sp.Rational(-3, 5))
    ok(conic.subs({x: sixth[0], y: sixth[1]}) == 0, "gb1 (hand) a 6th unit-circle point also satisfies x^2+y^2-1=0")
    off_circle = (sp.Rational(3, 5), sp.Rational(9, 5))
    ok(
        conic.subs({x: off_circle[0], y: off_circle[1]}) != 0,
        "gb1-ctrl (hand) the perturbed 5th point (3/5,9/5) is NOT on the unit circle",
    )
    # gb2: a 90-degree rotation matrix is orthogonal (classical: rotations
    # preserve distance because they are orthogonal transformations).
    rot = sp.Matrix([[0, -1], [1, 0]])
    ok(rot * rot.T == sp.eye(2), "gb2 (hand) the 90-degree rotation matrix is orthogonal (M M^T = I)")
    shear = sp.Matrix([[1, 1], [0, 1]])
    ok(shear * shear.T != sp.eye(2), "gb2-ctrl (hand) the shear matrix [[1,1],[0,1]] is NOT orthogonal")
