...
| **R4.6** | Finite sums `Σ` beyond the existing `geo`/`geo1`, including empty-range corners (`k = 3` in `thm:sharp`). | all three | **WIP:** generic `Nat.sumRange`, empty/successor equations, power reindexing, and scalar distribution landed under [ADR-0391](../research/09-decisions/adr-0391-generic-nat-finite-range-sums.md), [ADR-0393](../research/09-decisions/adr-0393-checked-geometric-sum-reindexing.md), and [ADR-0394](../research/09-decisions/adr-0394-checked-scalar-distribution-over-finite-sums.md); generic sum congruence remains |
