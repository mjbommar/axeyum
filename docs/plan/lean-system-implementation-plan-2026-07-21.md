...
| TL0.7.3 | PARTIAL | Implement the immutable checkpoint store and completion-last validator on registered storage classes. The [source-first plan](lean-execution-store-tl0.7.3-plan-2026-07-22.md) freezes two local storage classes, the accepted ADR-0344 no-replace primitive, exact Lean namespace/closure rules, eighteen mutation families, and a 16-cell dependency/completion × four-boundary × worktree/tmpfs `SIGKILL` matrix before implementation. | TL0.7.2 | M | Kill at every persistence boundary, resume without overwrite/duplication, quarantine conflicts, and reproduce the uninterrupted canonical result. |
...
20. TL0.7.2--TL0.7.4 then TL0.6.3 — implement the bounded process adapter,
    durable checkpoint store, and
...
21. TL3.1-TL3.3 — inventory/digest/classify and namespace all preludes.
22. TL3.4 — discharge the first five axioms.
23. TL3.6 — export minimal `Init` roots and regenerate blocker priority.
24. T6.1a — extract the reusable IR-to-CIC bridge in the parallel proof lane.
25. T6.2.1-T6.2.5 — establish one goal/metavariable state with delayed assignment.
26. TL6.1-TL6.4 — establish the source/syntax/parser substrate in the parallel
    frontend lane.
27. Recompute the critical path from the generated construct/root and
