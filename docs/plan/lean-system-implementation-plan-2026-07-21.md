...
| TL2.12 | WIP (M4 complete; M5 next) | Generalize direct, recursive-indexed, and reflexive/higher-order fields through proposed ADR-0353's one telescope-based IH/rule construction. [M0-M3](lean-recursive-induction-hypotheses-m3-2026-07-22.md) freeze the targets, close fourteen native rows/twelve mutation classes/768 recursive profiles, and complete both construct targets with exact recursor comparison. [M4](lean-recursive-induction-hypotheses-m4-2026-07-22.md) confirms pinned Lean and Axeyum computations twice at the exact registered normal forms and regenerates a machine-validated assurance matrix with four admitted, two computation-checked, and two declined rows. M5 final gates/disposition remain. | TL2.11 | L | [Preregistered plan](lean-recursive-induction-hypotheses-tl2.12-plan-2026-07-22.md): `Vector`- and `Acc`-shaped official streams complete twice, exact generated recursors compare, selected recursor applications compute, direct recursion/840-case positivity remain unchanged, and all rollback/resource gates pass. |
