...
21. **DONE:** TL0.7.4 — run the pinned-Lean preflight and official-export
    ...
    512 MiB task-stack policy. TL0.6.3 M0 R1 then retained one decided local
    failure after `LEAN_CC=/usr/bin/cc` selected the system linker. **NEXT:**
    implement the separately preregistered R2 removal of that override,
    commit/push it, then execute attempt 003 once;
