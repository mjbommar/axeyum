...
| TL2.12 | WIP (M0 complete; M1 next) | Generalize direct, recursive-indexed, and reflexive/higher-order fields through proposed ADR-0353's one telescope-based IH/rule construction. [M0](lean-recursive-induction-hypotheses-m0-2026-07-22.md) now freezes the baseline, actual explicit-recursor computation source, two byte-identical official root streams, exact metadata, and fail-closed machine contract with no Axeyum product observation. M1 next introduces the shared representation while preserving direct-recursive identity and both feature declines; M2-M4 own native semantics, exact importer comparison, generated mutations, and pinned computation evidence. | TL2.11 | L | [Preregistered plan](lean-recursive-induction-hypotheses-tl2.12-plan-2026-07-22.md): `Vector`- and `Acc`-shaped official streams complete twice, exact generated recursors compare, selected recursor applications compute, direct recursion/840-case positivity remain unchanged, and all rollback/resource gates pass. |
