...
| TL2.16 | PARTIAL | Generate the parsed/translated/admitted/dual-admitted construct and root matrix. The [selected-family seed](generated/lean-official-construct-matrix.md) is now generated from exact source, wire, and product freezes with implication tests, but it must not be mistaken for completion of the full format/root population. | TL0.6 | M | Matrix is generated from tests and exact fixtures, never hand-maintained. |
