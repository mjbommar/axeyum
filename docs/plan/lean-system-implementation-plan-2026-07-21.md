...
| TL0.7.4 | PARTIAL | Run one no-credit pinned-Lean preflight and one official-export control through the complete path, then accept or revise the policy. The [source-first plan](lean-execution-acceptance-tl0.7.4-plan-2026-07-22.md) freezes the exact Lean binary, official exporter source/build, flat source/reference oracle, empty selection, 4/8 GiB envelopes, immutable completion, sixteen mutation families, and zero-credit boundary before exporter build or execution. | TL0.7.3 | S | Exact runner/platform/resource/artifact evidence validates; exporter stdout equals the committed 65-line stream; TL0.6.3 may begin without promoting either control into U2 outcomes. |
