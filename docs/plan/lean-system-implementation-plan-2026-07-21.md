...
| TL0.6.5 | PARTIAL | Run matched native Axeyum cases incrementally and publish exact overlap, official-only, Axeyum-only, mismatch, unadjudicated, not-run, invalid-run, assurance, and resource fields. The [source-first matched-execution plan](lean-u2-matched-execution-tl0.6.5-plan-2026-07-23.md) now preregisters three separate denominators, independent official/native execution objects, typed absent/invalid states, layer-owned normalization plus mutation controls, exact pair-population authorities, non-vacuous G3 derivation, and M0-M6 execution/review gates. This is contract/schema progress only: the required TL0.6.3/TL0.6.4 parents, comparison-obligation authority, Axeyum process, outcomes, pairs, and credit remain absent. | TL0.6.3, TL0.6.4 | XL | Every paired row satisfies the terminal cell schema and independently checked accepted declarations/proofs retain trust evidence; every expected obligation is present, so omitted not-run/invalid rows cannot disappear from the denominator. |
