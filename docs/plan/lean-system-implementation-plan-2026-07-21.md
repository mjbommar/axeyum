...
| TL2.16 | TODO | Generate the parsed/translated/admitted/dual-admitted construct and root matrix. The [proposed selected-family seed](lean-official-construct-matrix-plan-2026-07-22.md) now has source, wire, and product freezes, but no generated assurance matrix; it must not be mistaken for completion of the full population. | TL0.6 | M | Matrix is generated from tests and exact fixtures, never hand-maintained. |
