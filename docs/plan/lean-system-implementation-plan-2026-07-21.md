...
| TL2.12 | WIP (M3 complete; M4 next) | Generalize direct, recursive-indexed, and reflexive/higher-order fields through proposed ADR-0353's one telescope-based IH/rule construction. [M0-M2](lean-recursive-induction-hypotheses-m2-2026-07-22.md) freeze the targets, install the shared representation, and close fourteen native rows, ten native mutations, 768 recursive profiles, and the retained positivity control. [M3](lean-recursive-induction-hypotheses-m3-2026-07-22.md) completes both construct targets twice with exact recursor comparison and closes metadata/publication mutations; the pre-elaborated well-founded stream also completes through `Acc.rec`, without a frontend claim. M4 owns official computation and assurance evidence. | TL2.11 | L | [Preregistered plan](lean-recursive-induction-hypotheses-tl2.12-plan-2026-07-22.md): `Vector`- and `Acc`-shaped official streams complete twice, exact generated recursors compare, selected recursor applications compute, direct recursion/840-case positivity remain unchanged, and all rollback/resource gates pass. |
