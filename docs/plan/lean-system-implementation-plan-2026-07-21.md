...
| TL2.12 | WIP (preregistered) | Generalize direct, recursive-indexed, and reflexive/higher-order fields through proposed ADR-0353's one telescope-based IH/rule construction. M0 freezes the machine contract and actual recursor-computation source before admission changes; M1 preserves direct-recursive identity under the shared representation; M2-M4 add native semantics, exact importer comparison, generated mutation coverage, and pinned-Lean computation evidence. | TL2.11 | L | [Preregistered plan](lean-recursive-induction-hypotheses-tl2.12-plan-2026-07-22.md): `Vector`- and `Acc`-shaped official streams complete twice, exact generated recursors compare, selected recursor applications compute, direct recursion/840-case positivity remain unchanged, and all rollback/resource gates pass. |
