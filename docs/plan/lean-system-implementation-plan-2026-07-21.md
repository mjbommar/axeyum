...
| TL0.3 | PARTIAL | Pin Lean, exporter, source hashes, fixture hashes, commands, limits, host-independent options, and the executable path used from every test working directory. The first corrected remote job failed before its representative sweep because `AXEYUM_LEAN_BIN` resolved to an unconfigured elan shim outside the repository directory. The workflow now resolves and preflights the installed versioned executable; remote confirmation remains open. | — | S | One manifest drives exporter, importer, and official-check tests; a retained remote run reaches the exact current 70/70 attestation. |
...
| TL0.6 | PARTIAL | Add generated A0-A11 construct, declaration-root, source, tactic, project, editor, runtime, ecosystem, and platform scoreboards over content-identified U0-U9 populations. The [v1 registry](lean-complete-parity-v1.json) and generated [Markdown](generated/lean-complete-parity.md)/[JSON](generated/lean-complete-parity.json) status seed exact ordered populations, axes, outcomes, and terminal gates; derive bounded K-profile/construct/task/axiom facts; reject incomplete denominators, illegal axis/gate credit, and premature public terminal claims. U2 now has a [bounded executable registration authority](lean-u2-test-authority-2026-07-22.md) for 3,678 default / 3,723 full-Lake CTest cases, an exact [official CI profile derivation](lean-u2-official-ci-profiles-tl0.6.2-2026-07-22.md) for 17 contexts, 153 cells, 111 not-run attempts, and eight selection sets, a bounded [native-surface harness floor](lean-u2-native-surface-classification-tl0.6.4-m0-result-2026-07-23.md), and an accepted [7,004-file tracked-content census](lean-u2-native-surface-classification-tl0.6.4-m1-result-2026-07-23.md) over all 3,723 cases. The registry now also requires independently sealed official/Axeyum execution records, a comparison that cites both side seals, a whole-cell seal, and exact per-population pair count/ID/cell-seal authorities, so neither a nonempty agreeing subset nor ID-preserving content substitution can satisfy G3. Exact dependency closure, complete official execution, Axeyum execution, all terminal U0-U9 authorities, and paired cells remain incomplete. | TL0.2 | M | Every profile has exact raw/normalized denominators, paired overlap, blocker categories, assurance, attempts, resources, and completion. |
| TL0.7 | DONE | Freeze resource envelopes and checkpoint policy, including 4 GiB default and 8 GiB official-export lanes. The [source-first execution plan](lean-execution-evidence-tl0.7-plan-2026-07-22.md) froze lane/run/attempt/case/completion identities before implementation. The [TL0.7.1 result](lean-execution-evidence-tl0.7.1-2026-07-22.md), [authority](lean-execution-evidence-v1.json), generated [summary](generated/lean-execution-evidence.md), and twelve tests close the two lane templates, twelve typed termination classes, five no-process controls, nineteen mutations, and zero-real-outcome boundary. The [TL0.7.2 result](lean-execution-process-tl0.7.2-2026-07-22.md) adds 8/8 retained synthetic process attempts, exact raw output, group cleanup, and evidence-backed exit/signal/timeout/memory/launch/preflight classes while preserving zero cases, completions, real outcomes, and parity credit. The [TL0.7.3 result](lean-execution-store-tl0.7.3-2026-07-22.md), [authority](lean-execution-store-v1.json), and 65 retained files add 16/16 local process-interruption cells with completion-last closure and uninterrupted projection equality across observed ext4/tmpfs. The [TL0.7.4 result](lean-execution-acceptance-tl0.7.4-2026-07-22.md), [authority](lean-execution-acceptance-v1.json), and 67 retained files close the real local path with one preserved failed compile and two completed no-credit controls: a 9,672-byte `.olean` under 4 GiB and an exact 3,849-byte/65-line official export under 8 GiB. Power/host/network/object durability and every U2/Axeyum/pair/performance/parity claim remain uncredited. | TL0.3 | S | OOM/signal/timeout/limit outcomes are typed and retained; local process-interruption checkpoint recovery and the two no-credit external acceptance controls are qualified. |
...
| TL0.6.3 | PARTIAL | Execute and retain the official profiles with executable/configuration/environment/resource/attempt/completion identities, JUnit, logs, artifacts, and typed invalid/incomplete outcomes. The source-first [M0 plan](lean-u2-official-execution-tl0.6.3-m0-plan-2026-07-22.md) selects `compile/534.lean` as a one-case child shard of the release-tag Linux-release 3,678-case selection. [Attempt 001](lean-u2-official-execution-tl0.6.3-m0-attempt-001-2026-07-22.md) retained an exited-8 thread-creation failure but no outcome. The [R1 result](lean-u2-official-execution-tl0.6.3-m0-r1-result-2026-07-22.md) corrected Lean-shell worker and artifact closure, then retained one decided local official failure after the adapter forced `leanc` through system `cc`; an exact same-C control links with the released bundled compiler. The [R2 result](lean-u2-official-execution-tl0.6.3-m0-r2-result-2026-07-22.md) removed only that override but retained attempt 003's pre-import direct-entry failure and no new outcome. The preregistered R3 correction then ran attempt 004 once and passed the same singleton with the released bundled compiler/linker. The accepted [R3 result](lean-u2-official-execution-tl0.6.3-m0-r3-result-2026-07-22.md) and [authority](lean-u2-official-execution-tl0.6.3-m0-r3-v1.json) retain four process attempts, two incomplete attempts, one failed and one passed official outcome for one unique case, 1/3,678 parent coverage, and zero provider/Axeyum/pair/performance/parity credit. The source-first [M1 plan](lean-u2-official-execution-tl0.6.3-m1-shard-plan-2026-07-22.md), [result](lean-u2-official-execution-tl0.6.3-m1-shard-result-2026-07-22.md), and [authority](lean-u2-official-child-shards-v1.json) now factor all eight selections into five exact ordered memberships and 289 bounded physical shards, but all 111 attempts remain not run. The pushed [M2 plan](lean-u2-official-execution-tl0.6.3-m2-shard-0001-plan-2026-07-22.md) selects the lowest-ordinal zero-history 64-case shard and freezes one local attempt. Its pushed [implementation checkpoint](lean-u2-official-execution-tl0.6.3-m2-implementation-2026-07-22.md) records commits `9783ba93` (pure contract), `57dcf343` (completion store), `d1f144d4` (case-before-post correction), and `431d3959` (one-shot runner). The first invocation stopped during selected-source preflight before any process because the official compile-bench runner is a pinned symlink. The source-first [M2 R1 correction](lean-u2-official-execution-tl0.6.3-m2-r1-symlink-preflight-plan-2026-07-22.md) and [implementation checkpoint](lean-u2-official-execution-tl0.6.3-m2-r1-symlink-preflight-implementation-2026-07-22.md) then bind pushed commit `9d5d40c8`, exact manifest-only link resolution, and fifteen mutation variants. Exact preflight passed and R1 ran once. Its [result](lean-u2-official-execution-tl0.6.3-m2-r1-result-2026-07-22.md) and [authority](lean-u2-official-execution-tl0.6.3-m2-r1-result-v1.json) retain an exited-8 process and exact 64-row JUnit (30 pass / 34 fail), but family-blind docparse artifact closure stopped before post/projection/completion. The attempt is consumed and grants zero M2 case/shard credit. Published commit `d9b286a1` binds the exact 83-file invalid evidence/result. The source-first [R2 diagnostic-closure plan](lean-u2-official-execution-tl0.6.3-m2-r2-diagnostic-closure-plan-2026-07-22.md) authorized only a zero-process, zero-credit append: bind all 124 generated rows, retain 67 outcome/log payloads, and keep 56 large reproducible C/executable intermediates metadata-only. The accepted [R2 result](lean-u2-official-execution-tl0.6.3-m2-r2-diagnostic-closure-result-2026-07-22.md) now records 69 new files / 159,346 bytes installed completion last, zero processes/outcomes, and unchanged invalid R1 credit. The separately source-first [R3 attempt-002 plan](lean-u2-official-execution-tl0.6.3-m2-r3-attempt-002-plan-2026-07-23.md) freezes new run/work/evidence roots, universal `LEAN_STACK_SIZE_KB=524288`, the unchanged 64-case/8 GiB/one-worker/hour lane, R2's family-specific tiered store, and at most one process after separately pushed implementation gates. No R3 harness, discovery, or process exists at preregistration, and the singleton must never be rerun for population credit. | TL0.6.2, TL0.7 | XL | Every selected official case has retained per-attempt evidence; aggregate totals reconstruct only from completed case records. |
| TL0.6.4 | PARTIAL | Classify every U2 case by required native Axeyum surface and dependency closure. The source-first [M0 plan](lean-u2-native-surface-classification-tl0.6.4-m0-plan-2026-07-23.md), accepted [bounded result](lean-u2-native-surface-classification-tl0.6.4-m0-result-2026-07-23.md), [authority](lean-u2-native-surface-classification-v1.json), and generated [summary](generated/lean-u2-native-surface-classification.md) give all 3,723 cases exactly one conservative harness floor across ten stable surfaces, fourteen family rules, and three exact overrides. The accepted [M1 result](lean-u2-native-surface-classification-tl0.6.4-m1-result-2026-07-23.md), [content authority](lean-u2-native-surface-content-v1.json), and generated [summary](generated/lean-u2-native-surface-content.md) now inspect all 7,004 tracked files, retain 90,909 exact/candidate spans, and project all 3,723 cases with 3,670 generated-wrapper residuals. All exact dependency closures and native outcomes remain `not-run`; M2-M3 must derive exact reachability and review every row. | TL0.6.1, TL1-TL9 | L | No case is silently delegated to official Lean or credited to a lower K/A profile; unsupported surfaces have stable owners and decline codes. |
| TL0.6.5 | PARTIAL | Run matched native Axeyum cases incrementally and publish exact overlap, official-only, Axeyum-only, mismatch, unadjudicated, not-run, invalid-run, assurance, and resource fields. The [source-first matched-execution plan](lean-u2-matched-execution-tl0.6.5-plan-2026-07-23.md) now preregisters three separate denominators, independent sealed official/native execution objects, typed absent/invalid states, layer-owned normalization plus mutation controls, exact pair-population count/ID/cell-seal authorities, non-vacuous G3 derivation, and M0-M6 execution/review gates. Accepted R2 adds typed complete-side result classes and distinct normalized-observable identities, then derives all eight outcomes; fully resealed semantic mutations cannot preserve stale agreement. R3 adds a nine-contract machine authority, 68 semantic fields, 18 explicit ignored rules, exact-field canonical projection, and paired-cell registration/layer/seal checks. This is contract/schema/projection progress only: the required TL0.6.3/TL0.6.4 parents, comparison-obligation authority, raw extractors, semantic canonicalizers, Axeyum process, outcomes, pairs, and credit remain absent. | TL0.6.3, TL0.6.4 | XL | Every paired row satisfies the terminal cell schema and independently checked accepted declarations/proofs retain trust evidence; every expected obligation is present, so omitted not-run/invalid rows cannot disappear from the denominator. |
...
surface classification. M0 accepted the full 3,723-case conservative harness
floor, and M1 now accepts the complete 7,004-file tracked-source census with
90,909 exact/candidate spans and zero native outcomes or pairs. Next run M2
exact dependency closure and M3 review; do not promote lexical signals into
reachability, native support, or parity credit.
...
| ID | Status | Task | Depends | Size | Exit artifact/gate |
|---|---|---|---|---|---|
| TL0.7.1 | DONE | Freeze the machine execution-evidence contract: 4 GiB standard and 8 GiB official-export lane templates, concrete run/resource identity, typed attempt/case/completion records, immutable completion-last checkpoints, credit predicates, and synthetic validation only. The [result](lean-execution-evidence-tl0.7.1-2026-07-22.md) closes the preregistered schema with five valid synthetic controls and nineteen mutation classes while every real counter remains zero. | TL0.3, TL0.6.2 | M | The generated authority and twelve tests represent clean, failed, interrupted/resumed, invalid, and incomplete synthetic bundles while committed real-outcome counters remain zero. |
| TL0.7.2 | DONE | Implement the bounded process adapter with explicit limits, exact command/environment/working-directory identity, process-group cleanup, raw output hashing, and evidence-backed exit/signal/timeout/limit classification. The [source-first plan](lean-execution-process-adapter-tl0.7.2-plan-2026-07-22.md) was pushed before implementation and probes. The [result](lean-execution-process-tl0.7.2-2026-07-22.md), [authority](lean-execution-process-v1.json), generated [summary](generated/lean-execution-process.md), 40 retained files, and eleven tests close all eight synthetic controls and sixteen mutation families with no guessed OOM, surviving descendant, case/completion, or parity credit. | TL0.7.1 | M | Forced exit, signal, timeout, cooperative memory-limit, and launch/preflight failures retain exact attempts without guessed OOM or completion. |
| TL0.7.3 | DONE | Implement the immutable checkpoint store and completion-last validator on registered storage classes. The [source-first plan](lean-execution-store-tl0.7.3-plan-2026-07-22.md) was pushed before implementation. The [result](lean-execution-store-tl0.7.3-2026-07-22.md), [authority](lean-execution-store-v1.json), generated [summary](generated/lean-execution-store.md), 65 retained files, and 22 contract tests close all 16 dependency/completion × four-boundary × observed ext4/tmpfs `SIGKILL` cells with exact recovery and uninterrupted projection equality. Claims remain bounded to local process interruption with zero real/U2/parity credit. | TL0.7.2 | M | Every worker is reaped by `SIGKILL`; resume never overwrites/duplicates, conflicts/orphans quarantine, completion installs last, and all 16 canonical projections equal their uninterrupted reference. |
| TL0.7.4 | DONE | Run one no-credit pinned-Lean preflight and one official-export control through the complete path, then accept or revise the policy. The [source-first plan](lean-execution-acceptance-tl0.7.4-plan-2026-07-22.md) froze the exact inputs before execution. [Attempt 001](lean-execution-acceptance-tl0.7.4-attempt-001-2026-07-22.md) failed before `.olean` creation when the default 1 GiB task stacks exceeded the 4 GiB address-space envelope; the exporter did not run. The source-first [R1 plan](lean-execution-acceptance-tl0.7.4-r1-plan-2026-07-22.md) froze `-s524288`, terminal-before-artifact failure closure, and failed-evidence retention. The [final result](lean-execution-acceptance-tl0.7.4-2026-07-22.md) retains one failed attempt plus two completed controls, including the 9,672-byte `.olean` and byte-equal 3,849-byte/65-line export, across 67 files / 142,523 bytes with zero U2/parity credit. | TL0.7.3 | S | Exact runner/platform/resource/artifact evidence validates; exporter stdout equals the committed 65-line stream; TL0.6.3 may begin without promoting either control into U2 outcomes. |
...
| TL2.9 | TODO | **Paused after pushed P0; resume from the [authoritative handoff](lean-string-literal-semantics-tl2.9-resume.md).** Implement checked String-literal typing and pinned Lean's Unicode-scalar `String.ofList` conversion across defeq, projection, recursors, and format-3.1 ingestion under [proposed ADR-0366](../research/09-decisions/adr-0366-preregister-lean-string-literal-semantics.md) and its [P0--M5 plan](lean-string-literal-semantics-tl2.9-plan-2026-07-23.md). P0 freezes the semantic/mutation/resource contract but adds no product or K1 credit. | TL2.7 | L | Exact String root is retained, advances past literals, and reports its exact next blocker; authorized pinned-Lean positive/false controls agree. |
| TL2.10 | WIP | The [offline M1--M3 result](lean-quotient-package-m1-m3-result-2026-07-23.md) under [proposed ADR-0365](../research/09-decisions/adr-0365-preregister-lean-quotient-package.md) now closes canonical `Eq` validation, atomic four-member admission, exact `Quot.lift`/`Quot.ind` reduction, the 576-row generated seam, official import/computation, additive identities, renderer non-laundering, and transactional mutations. M4's separately authorized pinned-Lean differential and final acceptance remain open. | TL1.7 | M | Official quotient closure admits; relation/proof mutations reject; authorized positive/false-control differential passes. |
...
| TL2.15 | PARTIAL | Run seam-first kernel fuzzing across Prop/elimination, universes/inductives, proof irrelevance/iota, literals/reduction, projections/eta, and quotients. The [T6.0.3 seed](lean-kernel-seam-fuzz-seed-2026-07-21.md) covers four semantic seams with 768 unique generated cases and deterministic summary replay. TL2.10 now adds a separate twice-identical 576-row quotient package/reduction grammar with frozen digest `649c98095f6e8d45`. Generated projection/reduction/eta remains open. | TL2.2-TL2.14 as applicable | L | Current four-seam `kernel accepts False` class plus quotient seam are retained; generated projection/reduction/eta remains mandatory. |
...
17. TL0.3 — retain the first true remote current 70/70 attestation for the
    locally verified changed-working-directory executable-identity repair.
...
19. **DONE:** TL0.7.1 — implement the preregistered machine execution-evidence
    contract with two lanes, twelve termination classes, five synthetic
    controls, nineteen mutations, and zero real outcomes.
20. **DONE:** TL0.7.2-TL0.7.3 — qualify bounded process behavior and the
    local process-interruption checkpoint store without creating U2 credit.
21. **DONE:** TL0.7.4 — run the pinned-Lean preflight and official-export
    controls through the complete path with zero credit and accept the revised
    512 MiB task-stack policy. TL0.6.3 M0 now retains four process attempts and
    two official outcomes for one unique case: R1's environment-induced failure
    ...
    aggregate U2 is 66 outcomes over 65 unique cases. TL0.6.4 M0 now classifies
    all 3,723 cases at a conservative harness floor. M1 now completes the
    7,004-file tracked-source census over all cases, while exact dependency,
    native-outcome, and pair fields remain unrun. **NEXT:** source-first the next
    child shard and TL0.6.4 M2 exact closure, followed by M3 review before any
    official/Axeyum pair.
    Do not rerun the singleton for coverage
    or promote an official-only pass into parity credit. Continue U0-U9
    authorities and A0-A11 paired scoreboards through the
    [complete-parity execution roadmap](lean4-complete-parity-roadmap-2026-07-22.md).
22. TL3.1-TL3.3 — inventory/digest/classify and namespace all preludes.
23. TL3.4 — discharge the first five axioms.
24. TL3.6 — export minimal `Init` roots and regenerate blocker priority.
25. T6.1a — extract the reusable IR-to-CIC bridge in the parallel proof lane.
26. T6.2.1-T6.2.5 — establish one goal/metavariable state with delayed assignment.
27. TL6.1-TL6.4 — establish the source/syntax/parser substrate in the parallel
    frontend lane.
28. Recompute the critical path from the generated construct/root and
