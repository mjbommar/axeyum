...
| | |
|---|---:|
| Established facts (`proved` or `computed`) | 160 |
| …via an operation covering **more than one** fact | **7** |
| …via a single-target operation (a capsule) | 21 |
| …with no registered operation (hand-constructed or imported) | 132 |
...
| `kernel-lean` | 7 | 20 | 71 | kernel proof, reconstructed and checked here |
...
Of the 160 established facts, 97 record an
