...
**Task 2 (workspace clippy cleanup)**: see below, updated after Task 1's
commit.
...
Task 2: fix the ~23-25 pre-existing `cargo clippy -p axeyum-lean-kernel
--all-targets -- -D warnings` failures (`doc_lazy_continuation`,
`unused_mut`, `large_stack_arrays`) mechanically, doc-comment/`mut`/`#[allow]`
only, skipping files with heavy uncommitted churn from other lanes.
