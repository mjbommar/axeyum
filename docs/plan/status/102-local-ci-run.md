...
`6389e0194` said the same of three of these suites on 2026-08-15. Re-pinned at
cause, green. The point is not the pins: **no pre-merge gate runs those four
`tests/*.rs` suites** — `--lib` skips integration targets — so the only reader
of those pins was the gate nobody ran.
...
- It gated the **WORKING TREE**, so a sibling lane's uncommitted work decided
  whether a SHA passed. Now gates a detached worktree at the commit —
...
binaries being 40% of the wall. And **nextest is 3.5x slower than `cargo test`
on the heaviest binary** (399 s vs 114 s measured), so the runner choice is
likely costing this gate real time. Next: a timer on s5/s7 — which **measured
today cannot run it** (no stable, no 1.88.0, no nextest; 342 and 422 commits
behind) — read by a freshness step inside `just check`, not by a dashboard.
