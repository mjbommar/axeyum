...
| LRA-MODEL-REPLAY | — (2139 unspent) | the 28 QF_LRA files stopping at "online CDCL(T) LRA model did not replay (arithmetic outside the incremental engine)" (`lra_theory.rs:491`) | census done, scoped down at the user's pause: **the label is wrong — 0 unsupported atoms across all 47 files that lose on it**; 27 have no tableau because admission still counts dense cells (`lra_online.rs:1308`, `simplex.rs:2090`) over the sparse storage ADR-2111 built, so the model falls to Fourier–Motzkin and declines; 11 are disequalities dropped at `lra_online.rs:2610` under a comment that is true of the offline driver and false of the CDCL(T) one (z3 splits eagerly, cvc5 on the model; our replay gate already detects the violation and answers `unknown` instead of a lemma); 7 are a deadline poll at pivot 0 (`simplex.rs:1441`) labelled as incompleteness; `distinct`/`to_real`/`to_int` occur in 0 of 200 files; merged with the day's batch |
