...
## The finding, reproduced independently

Real `select` + `guard`, in memory, nothing written, `FAMILY_MODULES` and
`FAMILY_ROUTES` asserted restored:

    before  A  four un-owned modules at the floor, no new constant
    ...
    after   A  REFUSED: R11 2 new held-out family/families publish mathematics
               ...
               natural-bitwise-core: topic -- Bitwise (published by
               natural-bitwise); natural-gcd-basic: topic -- GCD (published by
               integer-gcd), GCD (published by natural-gcd) | vocabulary: 10 of
               10 rows (allowance 5) | disclosure: 2 stem(s), no review recorded
            D  REFUSED at R5, unchanged
