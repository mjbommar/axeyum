...
- `scripts/check-merge-hygiene.sh` — first run **FAILED** (exit 1): one
  stale generated artifact, `docs/plan/generated/production-provenance-ledger.md`,
  stale because of the three new facts. Regenerated with
  `gen-production-provenance-ledger.py` and rerun: `MERGE_HYGIENE|markers=0|adr_index=ok|generated=current|…|PASS`, exit 0. `facts_via_multi_target` did
  NOT rise (34, unchanged) — the three facts land in `no_operation`, the honest
  bucket for a hand-written kernel module. Two guards printed **SKIPPED** and
  are not findings either way: `check-shape-duplicates.py --prebuilt`
  (no-binary) and the kernel-dependency-projection staleness check
  (not-answerable).
