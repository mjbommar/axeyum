...
passes through every box. For example, a DRAT proof can be elaborated to LRAT
for a more explicit propositional check — the LRAT checker and the *forward*
elaborator both handle RUP and RAT steps; only the separate *backward*,
core-first elaborator (`elaborate_drat_to_lrat_backward`, ADR-0382) still
declines a RAT core lemma. A linear-arithmetic certificate has a different
structure.
...
- `axeyum-cnf` provides DRAT, an LRAT checker and forward elaborator that both
  handle RUP and RAT steps (the separate backward elaborator does not yet
  elaborate RAT core lemmas), and a selected Alethe core.
