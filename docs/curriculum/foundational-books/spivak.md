...
| 8 | Least upper bounds | **X → K** | classical LUB unavailable; **Bishop completeness** proved instead (`creal/completeness.rs`): every regular sequence of reals has a limit, *constructed*. **Graded family stated**: [`graded-statement-families.md`](../graded-statement-families.md) §2 — the unavailability is asserted, not proved (no counterexample exists in this repository); `extremum::polynomial_extremum` gives row 3 for the polynomial-range special case |
