...
| **13** | **Integrals** | **K** | **`CReal.riemannSum_cauchy` proved** — five lanes, each stopping deliberately, and the shape compounded: (1) `samplePoint_reblock` **unconditional**; (2) `Nat.mul_succ_add_lt_of_le_of_lt`, no induction; (3) `reblockBlock_eq_fineBlockSum` an **exact `Equiv`**, so nothing accumulates; (4) `riemannSum_reblock_close` therefore carries one error term; (5) the Archimedean threshold discharged **unconditionally** via `Nat.le_add_right`, kernel rejecting nothing. The obstruction that shaped it: `CReal → CReal` is not automatically `Equiv`-respecting (ADR-0512). **`CReal.integral` remains blocked, and precisely so:** `riemannSum_cauchy` gives closeness at an *arbitrary shared* index; `RegularSeq` needs it at each term's *own canonical* index. Both routes (`CReal.mk`, and `completeness.rs`'s `limit`) converge on the same missing representative-index telescope — measured at 35–45 steps for the analogous `sumRange` case |
| 14 | Fundamental Theorem of Calculus | — | open, downstream of `CReal.integral` — which is itself one named telescope away, not open-ended |
