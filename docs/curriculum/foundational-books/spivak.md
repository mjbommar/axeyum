...
| **15–17** | **Trig, π irrational, planetary motion** | **K (opened)** | **This row's previous claim — "no transcendental functions exist" — is no longer true.** **`CReal.cosOne` is constructed**: `cos 1 = Σ(-1)^k/(2k)!`, built via `CReal.mk` on an explicit regular sequence, never `Exists`-elimination, mirroring `e`. Its index is doubled as `Nat.add k k` (**not** `Nat.mul 2 k`) so `CReal.pow_add` applies with zero reduction bookkeeping, and its domination series is *literally* `expDominant` — the same one `e` uses — so no new domination argument was needed. Two claims I briefed were wrong and the lane checked both: the absolute-convergence bridge is unnecessary (`sumRange_cauchy_of_dominated` never required nonnegativity, only a bound on `abs (f k)`, so it already covers a SIGNED series), and no parity case split is needed (`abs (pow (neg one) k) ≤ one` goes by induction). **Still out of reach: general `sin`/`cos : CReal → CReal`** (needs a bound depending on `\|x\|`, i.e. power series — see row 24) and **π** — **but see the dated correction that follows, which retracts the π half of that sentence.** **2026-08-31 correction.** This row previously read: *"**π**, which is downstream of a root of `cos` and therefore of the exact-root construction `creal/ivt.rs` **refutes**"*. That is **false**, and the way it is false is worth naming: it is a statement about ONE DEFINITION of π (twice the first positive root of `cos`) presented as a statement about π. The number is the sum of a rational series, and a series needs no root, no intermediate value theorem and no decision on a sign. **`CReal.pi` is constructed** (`crates/axeyum-lean-kernel/src/creal/pi.rs`), by `CReal.mk` on an explicit regular sequence exactly as `CReal.e` and `CReal.cosOne` are, from **Euler's transform of Leibniz**, `π/2 = Σ 2ᵏ(k!)²/(2k+1)!`. Three things made that the right series and none of them is convergence speed alone: the terms are defined by the RECURSION `t 0 = 1`, `t (k+1) = t k·(k+1)/(2k+3)`, so the ratio is definitional and no factorial identity is ever needed; `(k+1)/(2k+3) ≤ 1/2` holds with no case split, so `t k ≤ (1/2)ᵏ` is a short induction and the domination series is `CReal.e`'s own `expDominant` **reused unchanged**; and every term is positive, so there is no `(−1)ᵏ` factor and none of `creal/alternating.rs`'s pairing machinery is needed. Leibniz itself (`π/4 = Σ (−1)ᵏ/(2k+1)`) is the wrong choice here — its terms are dominated by no geometric series, so the one cheap Cauchy witness in the development does not apply to it. Landed with π: `CReal.piHalfConverges`, `CReal.piHalfLeTwo`, `CReal.piLeFour`, `CReal.twoLePi` and `CReal.threeLePi` — **3 ≤ π ≤ 4**, axiom-free. What remains genuinely out of reach is the *identification* of this π with a root of `cos`, which does need the refuted exact-root construction; the NUMBER never did |
