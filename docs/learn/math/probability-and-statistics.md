...
finite perceptron trace replay with exact dot products, mistake updates,
strict-margin convergence, replayed bad weight-update rejection, and checked
QF_LRA bad weight-update evidence,
...
claim.
For finite precision-recall classification, it recomputes exact score order,
...
For finite perceptron training, it replays every presented step's dot
product, mistake condition, and weight update, requires strictly positive
functional margins at the final weights, then emits checked `UnsatFarkas`
evidence for a malformed weight-update claim.
...
For exact finite perceptron training-trace replay, read
...
nearest-neighbor vote rows, finite perceptron traces, and random-matrix
moments first use
...
weight-update rows, and random-matrix moment/rank rows graduate through
...
tie policy, perceptron mistake bounds and convergence, causal
identification,
