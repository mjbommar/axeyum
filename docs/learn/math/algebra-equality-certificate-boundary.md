...
| `finite-group-actions-v0` | Identity and compatibility laws, orbit/stabilizer replay, Burnside average. | Bad identity and compatibility now have separate replay rows and explicit `qf-uf-bad-identity-action` / `qf-uf-bad-action-compatibility` equality-conflict rows. | orbit-stabilizer and Burnside/Cauchy-Frobenius in full generality |
...
| `finite-ideals-v0` | Ideal closure, generated ideal, ring-homomorphism kernel/image, quotient-ring tables. | Bad additive closure now has separate replay and `qf-uf-bad-ideal-additive-closure` membership rows; quotient representatives have `qf-uf-quotient-ring-representative-alethe`. | general ideal, quotient-ring, and correspondence theorems |
