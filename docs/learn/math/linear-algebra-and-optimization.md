...
and an explicit QF_UF/Alethe covector-additivity row. The exact rational
...
conditioning, or floating-point stability. The LDLT slice adds exact rational
unit-lower/diagonal factor replay, determinant and positive-minor checks,
triangular-solve replay, and checked QF_LRA/Farkas rejection of a bad diagonal
entry without claiming LDLT existence, pivoting strategy correctness,
indefinite variants, sparse algorithms, conditioning, or floating-point
stability. The QR slice adds exact rational
...
Gram-Schmidt slice adds exact rational projection coefficients,
normalization, residual orthogonality, orthonormality, upper-triangular factor
checking, `Q*R` replay, and checked QF_LRA/Farkas rejection of a bad
projection coefficient without claiming general Gram-Schmidt/QR correctness,
rank-deficient variants, conditioning, or floating-point stability. The
Givens slice adds exact rational rotation coefficients, coordinate zeroing,
...
covector row replays the failed additivity equation; the separate
`qf-uf-bad-covector-additivity` row links that equality conflict to checked
...
For one exact shifted QR step, `finite-shifted-qr-step-v0` adds a rational
shift to the same finite matrix story:
...
The validator recomputes `A0 - mu*I = Q*R`, `A1 = R*Q + mu*I`,
`Q^T*A0*Q = A1`, `trace(A0) = trace(A1) = 4`, and
`det(A0) = det(A1) = 8`. Its checked bad row rejects the claim that
`A1[1,1] = 2` after exact replay reads `8/5`; shift-selection theory,
deflation, QR convergence, Schur theorem reconstruction, loss-of-orthogonality
analysis, and floating-point stability stay in the horizon lane. For a focused
trace, read [End To End: Finite Shifted QR Step](shifted-qr-step-end-to-end.md).
...
Wolfe curvature slack, the bad minimizer claim for the full step `alpha = 1`,
and the bad-row curvature violation for that same full step. The final exact
...
transform coefficients, bad LU multipliers, bad pivot signs, bad LDLT diagonal entries, bad Gram-Schmidt projection coefficients, bad Givens coefficients, bad Householder entries, bad QR/Cholesky product entries, bad covariance
...
LU/LDLT pivoting and pivot-selection correctness, Gram-Schmidt/Givens/Householder/QR/Cholesky existence/algorithm/stability theorems, Schur-complement and block-inverse theorems, Gaussian-elimination pivoting/stability, numerical stability, SDP, general convex analysis, KKT sufficiency, constraint
