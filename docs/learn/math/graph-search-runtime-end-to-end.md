...
does not prove asymptotic BFS or DFS runtime, average-case search behavior,
heuristic-search guarantees, parallel-search properties, or graph lower
bounds.
...
the target is reached, and checks the visited-node counters. The finite rows
are checked evidence. The asymptotic runtime row is metadata for future Lean
work.
