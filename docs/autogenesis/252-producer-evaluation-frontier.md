...
groups. Nine are independently refuted, mandatory must-decline controls; the
remaining 89 are ordinary candidate facts. The generated view verifies that the
controls are a nonempty subset of this exact frontier. It names no held-out
fact. The remaining 34 held-out and nine out-of-population ready facts are only
aggregate exclusions; they are neither dispatched nor made visible to producer
code through this artifact.
...
all nine negative controls, and independently check/reproduce any proposed
proof. Admitting one control voids the entire census.
