...
- **incremental (push/pop, reset-assertions)** — push/pop and reset-assertions parsed (full `reset` is rejected); warm QF_BV/Bool with assumption-core pruning + all-SAT decides; supported BV-indexed Bool/BV array-symbol reads, scalar UFs, direct array equality, and bounded store/constant/ITE structural reads retain scalar owners/definitions in the persistent CNF with replay projection. Remaining array/UF theory defers; no DRAT/Alethe across push/pop. ADR-0009/0030/0086
