...
- **incremental (push/pop, reset-assertions)** — push/pop and reset-assertions parsed (full `reset` is rejected); warm QF_BV/Bool with assumption-core pruning + all-SAT decides; supported BV-indexed Bool/BV array reads, scalar UFs, bounded structural reads, scalar-keyed array-valued UF parents, projection-owned equality, and exact top-level structural diff witnesses retain private owners in the persistent engine. Exact transitive ROW summaries stay dormant until candidate violation; array-result reads/relations use conditional congruence, array-first/function-second projection, and replay. Positive structural equality and Boolean relation flags still defer; no DRAT/Alethe across push/pop. ADR-0009/0030/0086/0087/0088/0089
