...
- **incremental (push/pop, reset-assertions)** — push/pop and reset-assertions parsed (full `reset` is rejected); warm QF_BV/Bool with assumption-core pruning + all-SAT decides; supported BV-indexed Bool/BV array-symbol reads, scalar UFs, direct array equality, bounded store/constant/ITE structural reads, and scalar-keyed array-valued UF parents retain private owners in the persistent engine. Exact transitive ROW summaries stay dormant until candidate violation; array-result reads use conditional argument/index congruence and full-value function projection. Remaining array/UF theory defers; no DRAT/Alethe across push/pop. ADR-0009/0030/0086/0087/0088
