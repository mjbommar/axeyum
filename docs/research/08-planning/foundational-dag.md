...
| Arrays over BV | `select`/`store` done | Read-over-write done | Reader + writer done (non-extensional) | Direct Z3 array differential | Eager elimination to QF_BV (ADR-0010) plus a true abstraction-only boundary and replay-guided select interfaces on canonical `CdclT` (ADR-0071); lazy store axioms/extensionality remain | Model projection + evaluator replay; 768-case online/eager/front-door/Z3 matrix |
...
| QF_AUFBV / QF_AUFLIA (arrays + UF + ints) | Composed from arrays + EUF + LIA | Composed | Composed (all passes) | Direct Z3 QF_AUFBV differential; arithmetic mixes reduce first | QF_AUFBV now composes abstraction-only arrays → functions and refines both interface kinds on canonical `CdclT` (ADR-0071); QF_AUFLIA retains the stacked/lazy routes | Combined function-then-array projection + original replay; integer-bearing `unsat`/overflow → `unknown`; wide extensionality remains |
