...
| QF_AUFBV / QF_AUFLIA (arrays + UF + ints) | Composed from arrays + EUF + LIA | Composed | Composed (all passes) | Direct Z3 QF_AUFBV differential; arithmetic mixes reduce first | QF_AUFBV composes lazy ROW/base-select/equality arrays → functions, refines scalar interfaces, keeps array equality on live `EufTheory`, schedules explanation-guarded base-parent reads, projects direct-symbol classes, and admits Bool/BitVec components (ADR-0071/72/73/77/78/79); QF_AUFLIA retains the stacked/lazy routes | Combined function-then-array projection + original replay; integer-bearing `unsat`/overflow → `unknown`; structural-parent/non-symbol/warm depth remains |
