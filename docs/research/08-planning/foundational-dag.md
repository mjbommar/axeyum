...
| EUF | `declare_fun`/`apply` done (ADR-0013) | `Op::Apply` against a `FuncValue` model | Reader + writer round-trip done (`declare-fun` n-ary + applications, `QF_UFBV`) | Z3 rejects `Op::Apply` (eliminated first) | Eager Ackermann elimination plus abstraction-only lazy `QF_UFBV` solving done; canonical EUF+BV combination in progress | Model projection + replay; `function_catalog` and lazy-vs-eager differentials |
