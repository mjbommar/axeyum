...
| Arrays over finite scalars | `select`/`store` done for Bool/BitVec components | Read-over-write done | Reader + writer done (non-extensional) | Direct Z3 array differential | Eager elimination to QF_BV (ADR-0010) plus replay-guided select interfaces (ADR-0071), lazy ROW (ADR-0072), candidate-guided equality/diff observations (ADR-0073), majority-default models (ADR-0074), original equality flags plus direct-symbol class models (ADR-0077), explanation-guarded base/store-parent scheduling (ADR-0078/0080), Bool/BitVec component admission (ADR-0079), same-search local ROW insertion (ADR-0081), same-search pair-generated scalar interfaces over pre-observed terms (ADR-0082), array-valued UF result projection by final application class (ADR-0084), bounded structural store/ITE/constant class equations (ADR-0085), retained incremental structural-read owners (ADR-0086), candidate-triggered permanent transitive ROW summaries (ADR-0087), retained scalar-keyed warm array-valued UF parents (ADR-0088), retained projection equality/exact structural diff witnesses (ADR-0089), positive warm structural equality (ADR-0090), Boolean relation flags (ADR-0091), direct array-valued UF parameters (ADR-0092), supported structural array-valued UF parameters (ADR-0093), nested array-valued application keys (ADR-0094), and memory-aware BMC/k-induction via eager memory elimination; nested/extended arrays, certified memory reachability, and proofs remain | Model projection + evaluator replay; 2,784 canonical comparisons plus three 192-case warm/check-auto/Z3 matrices; memory BMC/k-induction gates; direct equal-array select congruence checks in-tree/Carcara/Lean without a reduction trust step (ADR-0075) |
| EUF | `declare_fun`/`apply` done for scalar and flat array results (ADR-0013/0084) | `Op::Apply` against a full-value `FuncValue` model | Reader + writer round-trip done (`declare-fun` n-ary + applications, including array results) | Direct Z3 comparison after canonical abstraction | Eager Ackermann elimination plus abstraction-only lazy solving; bounded scalar QF_UFBV combines the e-graph and warm BV solver through canonical `CdclT` (ADR-0066), exact ground-distinct pruning (ADR-0069), and replay-guided dynamic interface materialization (ADR-0070); array-result UFs use the canonical AUFBV bus while eager elimination declines them (ADR-0084), and scalar-keyed array results now retain warm parents with conditional read congruence (ADR-0088) | Model projection + original-query replay; `function_catalog`, eager/online, front-door, and Z3 differentials, including 288 canonical array-result comparisons plus 192 warm array-result comparisons |
| QF_AUFBV / QF_AUFLIA (arrays + UF + ints) | Composed from arrays + EUF + LIA | Composed | Composed (all passes) | Direct Z3 QF_AUFBV differential; arithmetic mixes reduce first | QF_AUFBV composes lazy ROW/base-select/equality arrays → functions, keeps array equality on live `EufTheory`, schedules explanation-guarded base/store/application-parent reads, appends candidate-violated scalar UF/select/extensionality interfaces inside one retained search, projects finite-scalar array-valued UF results by final e-class, realizes bounded structural class equations, retains candidate-triggered transitive summaries for bounded warm structural reads, retains scalar-keyed warm array-result parents, supports projection equality/exact structural diff witnesses, and retains direct, supported structural, plus nested array-valued application UF parameters (ADR-0071/72/73/77/78/79/80/81/82/84/85/86/87/88/89/90/91/92/93/94); QF_AUFLIA retains the stacked/lazy routes | Combined array-first/function-second projection + original replay; integer-bearing `unsat`/overflow → `unknown`; non-finite/nested/extended arrays and proofs remain |
...
| Quantifiers | `forall`/`exists` (named binders, ADR-0016) | Finite-domain enumeration (Bool/BV); infinite-domain ground evaluation declines | Reader + writer round-trip (binder form, fresh-symbol scoping) | Native oracles remain differential only | Finite expansion, equality-predicate-induced finite Int quotients, e-graph E-matching/MBQI/MBP, targeted CEGQI, closed-universal counterexample and negated-existential witness search, vacuous-existential-prefix universal counterexample search (ADR-0128), paired-existential witness-transfer search (ADR-0129), affine-LSB, signed-interval, and zero-product free-BV model search (ADR-0130/0131/0132), restricted `forall* exists` Skolem synthesis including exact BV identity witnesses (ADR-0121), exact vacuous outer-BV equality guards (ADR-0122), guarded unit-gap extraction, exact nested-XOR hierarchical instantiation, free-Boolean model search, finite counterexample-cover search for positive universal Bool/Int assertions, justified lazy equality-clause scheduling (ADR-0110), shared incremental compiled matching state (ADR-0111), revision-checked add-only root queues (ADR-0112), merge-incremental inverted-parent queues (ADR-0113), shared exact declaration/argument path tries (ADR-0114), exact e-class label/nullary ground-argument path filters (ADR-0115), generation-delta top-application queues (ADR-0116), source-bound checked detached equality-clause units (ADR-0117), bounded recursive generated-ground provenance (ADR-0118), checked clauses in retained CDCL(T)+EUF (ADR-0119), and scoped SAT-candidate equality e-matching (ADR-0120) | Finite original-formula replay; guarded-instantiation Alethe/Lean slices; exact Euclidean-residue, positive-slope affine-growth, nested-XOR, closed-universal, and equality-partition original-IR certificates with genuine quantified Lean routes (ADR-0095/0097/0099/0100--0106); complete typed witnesses for bounded closed Bool/BV negated existentials are evaluated directly against the untouched positive body (ADR-0126); leading Bool/BV existential binders may be removed only when an independent checker proves them absent from the closed universal body and directly evaluates a complete universal counterexample (ADR-0128), with genuine `Exists.rec`, typed universal application, and kernel-checked computational reduction for the public Lean route (ADR-0140); paired Bool/BV existential tuples transfer only under exact shared premises, alpha-aligned complete bodies, and per-conjunct source proof or checked signed no-wrap reasoning (ADR-0129); complete free-BV models certify only direct affine-LSB universals, evaluator-replayed negated-universal witnesses, directly negated existential implications whose ground facts replay and whose sole nonempty signed interval proves `lower <= upper <= cap`, and exact signed-nonnegative products whose direct binder-free `bvsdiv` factor evaluator-replays to zero under source caps (ADR-0130/0131/0132); arena-stable affine Skolem recipes checked by canonical prenex/unit-gap replay (ADR-0096/0098), with an exact same-width BV identity/relation subclass (ADR-0121); a dedicated outer-BV model certificate independently proves an exact equality antecedent false below direct Bool/BV alternation without inspecting the consequent (ADR-0122); free-Boolean SAT models checked by exact positive-universal closure, integer-ITE lifting, source-bound LIA-DPLL cores, and DRAT (ADR-0107); finite UNSAT covers independently bind every cube to an exact source instance, prove weakened-skeleton closure, and reconstruct one positive universal leaf through bounded kernel case analysis (ADR-0108); repeated closed proof DAG nodes export once through binder-safe Lean definitions (ADR-0109); lazy clause evaluation can propagate one detached equality/disequality literal only when a public checker reconstructs its exact source instance and replays every false sibling from named facts (ADR-0110/0117), generated premises recursively bind to exact instances or earlier checked propagations under explicit caps (ADR-0118), retained matching/index/path/filter/delta optimizations preserve that contract (ADR-0111 through ADR-0116), retained CDCL(T)+EUF accepts only checked clauses while ordinary QF replay guards each candidate online refutation (ADR-0119), and candidate equalities remain scoped search hints that produce only complete exact source instances after pop (ADR-0120). General free-BV models beyond ADR-0130/0131/0132, multi-constant partition proofs, MAM generation-cost scheduling/bytecode, general nested QE/QSAT and alternation, broader nonvacuous existential relations, premise-aware open-formula/function counterexamples, piecewise/general Skolem functions, non-equality online antecedents, high-frequency assignment callbacks, direct online proof serialization, open-context proof sharing, and broad proof reconstruction remain open; current bridge terms are all active-source relevant by construction |
...
the untouched closure true independently of every BV value; unresolved BV
closures are barred from the source-bound LIA fallback. Free-BV models beyond
the later ADR-0130 affine-LSB/direct-witness class and QSAT/QE remain open.

ADR-0124 refines the quantifier row's UNSAT boundary: one closed Bool/BV
`forall+ exists+` implication with an outer-only antecedent may carry concrete
outer bindings and a source-regenerated residual QF_BV DRAT. The checker repeats
prefix validation, substitution, existential freshening, CNF regeneration, and
proof replay. Broader alternation and Lean reconstruction remain open.

ADR-0125 scales only that row's total-binder cap from 128 to 1,024 for large
hardware state tuples, while retaining the 4,096-node matrix cap and the exact
source-bound replay contract. `bug802` uses 530 binders and now closes through
the same certificate; this does not add general QSAT.

ADR-0126 adds a separate source-direct route for one exact top-level
`not (exists+ body)` over Bool/BV. Search may freshen and solve the positive
body, but certification validates complete typed bindings and evaluates the
untouched original body to true under 128-binder/4,096-node caps. This closes
three minimal public UNSAT rows without trusting NNF conversion or adding
general QSAT; open bodies and Lean reconstruction remain separate boundaries.

ADR-0127 admits one unique universal reached only through a top-level Boolean
conjunction. Replacing it with a complete concrete Bool/BV instance weakens the
source assertion; replay regenerates that entire premise-aware QF_BV residual
and checks its DRAT/LRAT proof. This closes `cond-var-elim-binary` without
trusting polarity analysis beyond explicit conjunction or adding general QSAT.

ADR-0128 admits one exact nonempty `exists+ forall+` Bool/BV source only when
the checker proves every existential binder absent from the closed QF body.
Complete universal values are checked in source order and the untouched body
must evaluate to false under 128-binder/4,096-source-node caps. This closes
`issue2031-bv-var-elim` without trusting prefix rewriting, BV inversion, or
general QSAT. ADR-0140 gives that exact source a genuine kernel proof: nested
`Exists.rec` eliminates the vacuous prefix, exact typed values instantiate the
surviving universal, and computational AIG reduction closes `False`.

ADR-0129 admits one positive and one negated Bool/BV existential under exact
shared ground premises. Replay alpha-aligns equal typed prefixes and checks
every target conjunct by identity, a source-bound `QF_BV` implication proof, or
an exact signed-add monotonicity lemma whose no-wrap margin is revalidated. This
closes `nested9_true-unreach-call` without trusting witness normalization,
model-guided inversion/projection, modular arithmetic assumptions, or general
QSAT.

ADR-0130 admits complete free-BV models only for direct quantified assertions
whose untouched sources are proved by exact affine-LSB interpretation or a
complete evaluator-replayed witness for a directly negated universal. Admission
requires exact free-symbol coverage, unique Bool/BV binders, and
128-binder/4,096-node/256-depth caps. This closes `smtcomp-qbv-053118` without
trusting quantifier erasure, parity normalization, MBQI candidates, or a
solver-only witness.

ADR-0131 admits one directly negated existential implication only when a
complete free-BV model evaluator-replays every binder-free antecedent to true
and the untouched outer conclusion to false, and a separate arbitrary-width
signed comparison proves a nonempty `lower <= upper <= cap` interval for the
sole binder-dependent clause. This closes `intersection-example-onelane`
without trusting extracted QF obligations, signed-division normalization,
implication vacuity, or a solver-only model.

ADR-0132 admits a separate directly negated existential implication only when
the sole binder-dependent inner conclusion is an exact signed-nonnegative
binary product, one direct binder-free `bvsdiv` factor evaluator-replays to
zero, the other factor contains the unique binder, and the comparison bound is
a same-width literal zero. This closes `gn-wrong-091018` without interpreting
