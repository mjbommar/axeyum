...
| Arrays over BV | `select`/`store` done | Read-over-write done | Reader + writer done (non-extensional) | Direct Z3 array differential | Eager elimination to QF_BV (ADR-0010) plus replay-guided select interfaces (ADR-0071), lazy ROW (ADR-0072), candidate-guided equality/diff observations (ADR-0073), and majority-default finite-map models (ADR-0074) on canonical `CdclT`; merge-triggered/e-class depth remains | Model projection + evaluator replay; 768-case online/eager/front-door/Z3 matrix, half equality-bearing; direct equal-array select congruence checks in-tree/Carcara/Lean without a reduction trust step (ADR-0075) |
...
| QF_AUFBV / QF_AUFLIA (arrays + UF + ints) | Composed from arrays + EUF + LIA | Composed | Composed (all passes) | Direct Z3 QF_AUFBV differential; arithmetic mixes reduce first | QF_AUFBV composes lazy ROW/base-select/equality arrays → functions and refines all interface kinds on canonical `CdclT` (ADR-0071/72/73), then builds majority-default models (ADR-0074); QF_AUFLIA retains the stacked/lazy routes | Combined function-then-array projection + original replay; integer-bearing `unsat`/overflow → `unknown`; merge-triggered/e-class depth remains |
