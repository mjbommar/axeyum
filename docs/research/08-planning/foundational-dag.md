...
| QF_AUFBV / QF_AUFLIA (arrays + UF + ints) | Composed from arrays + EUF + LIA | Composed | Composed (all passes) | Direct Z3 QF_AUFBV differential; arithmetic mixes reduce first | QF_AUFBV composes lazy ROW/base-select/equality arrays → functions, keeps array equality on live `EufTheory`, schedules explanation-guarded base/store/application-parent reads, appends candidate-violated scalar UF/select/extensionality interfaces inside one retained search, projects finite-scalar array-valued UF results by final e-class, realizes bounded structural class equations, retains candidate-triggered transitive summaries for bounded warm structural reads, retains scalar-keyed warm array-result parents, and supports projection equality/exact structural diff witnesses (ADR-0071/72/73/77/78/79/80/81/82/84/85/86/87/88/89); QF_AUFLIA retains the stacked/lazy routes | Combined array-first/function-second projection + original replay; integer-bearing `unsat`/overflow → `unknown`; non-finite/nested/extended arrays, positive warm structural equality, Boolean relation flags, array-valued parameters, and proofs remain |
