...
| Arrays over BV | `select`/`store` done | Read-over-write done | Reader + writer done (non-extensional) | Direct Z3 array differential | Eager elimination to QF_BV (ADR-0010) plus replay-guided select interfaces (ADR-0071), lazy ROW (ADR-0072), candidate-guided equality/diff observations (ADR-0073), majority-default models (ADR-0074), and original equality flags plus direct-symbol class models on canonical `CdclT`/`EufTheory` (ADR-0077); parent-select/non-symbol/warm depth remains | Model projection + evaluator replay; 768-case online/eager/front-door/Z3 matrix, 456 equality-bearing; direct equal-array select congruence checks in-tree/Carcara/Lean without a reduction trust step (ADR-0075) |
...
| QF_AUFBV / QF_AUFLIA (arrays + UF + ints) | Composed from arrays + EUF + LIA | Composed | Composed (all passes) | Direct Z3 QF_AUFBV differential; arithmetic mixes reduce first | QF_AUFBV composes lazy ROW/base-select/equality arrays → functions, refines scalar interfaces, keeps array equality on live `EufTheory`, and projects direct-symbol classes through one model (ADR-0071/72/73/77); QF_AUFLIA retains the stacked/lazy routes | Combined function-then-array projection + original replay; integer-bearing `unsat`/overflow → `unknown`; parent-select/non-symbol/warm depth remains |
