...
| QF_AUFBV / QF_AUFLIA (arrays + UF + ints) | Composed from arrays + EUF + LIA | Composed | Composed (all passes) | Direct Z3 QF_AUFBV differential; arithmetic mixes reduce first | QF_AUFBV composes lazy ROW/base-select/equality arrays → functions, keeps array equality on live `EufTheory`, schedules explanation-guarded base/store-parent reads, and appends candidate-violated scalar UF/select/extensionality interfaces inside one retained search while projecting direct-symbol classes and admitting Bool/BitVec components (ADR-0071/72/73/77/78/79/80/81/82); QF_AUFLIA retains the stacked/lazy routes | Combined function-then-array projection + original replay; integer-bearing `unsat`/overflow → `unknown`; array-valued ITE/default/UF events and non-symbol/warm depth remain |
