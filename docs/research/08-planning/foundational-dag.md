...
| EUF | `declare_fun`/`apply` done (ADR-0013) | `Op::Apply` against a `FuncValue` model | Reader + writer round-trip done (`declare-fun` n-ary + applications, `QF_UFBV`) | Z3 rejects `Op::Apply` (eliminated first) | Eager Ackermann elimination plus abstraction-only lazy solving; bounded scalar QF_UFBV now combines the e-graph and warm BV solver through canonical `CdclT` (ADR-0066) | Model projection + original-query replay; `function_catalog`, eager/online, front-door, and Z3 differentials |
