...
| reachability | bounded model checking over a symbolic transition system (bounded_model_check / TransitionSystem; BV/Bool, array-free first slice) | validated | Reachable = replay-checked counterexample trace; UnreachableWithinBound is bounded only (interpolation = future work); unknown-safe | ADR-0009 |
