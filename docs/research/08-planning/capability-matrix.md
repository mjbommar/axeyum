...
| symbolic execution | DFS path explorer (SymbolicExecutor): assume / branch fork query / enter+backtrack / concrete test-input model | validated | models replay-checked vs path condition; three-valued PathStatus keeps unknown from wrongly pruning a live path | ADR-0009 |
