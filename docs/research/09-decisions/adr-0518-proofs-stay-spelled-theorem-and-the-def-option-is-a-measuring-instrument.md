...
ADR-0517 established that Lean has two checkers and that they disagree about a
...
(ADR-0514), resolved by `scripts/check-lean-gate.sh --print-toolchain`. Three
...
The four refusals on the carrier are exactly ADR-0517's four, re-measured:
...
both exit 0 today. ADR-0517's residue is real and it is *entirely outside the
shipped surface*: the only module the switch rescues is the whole carrier, which
ADR-0511 deliberately does not ship (the shared module is rooted at the reached
...
* `real_lean_wellfounded_elaborator_divergence` — **not** a re-bless. Its control
  ...
  the default does not merely break the check that pins ADR-0517's divergence —
...
The option ships anyway, off, because it is the instrument that makes ADR-0517's
...
* **A module rooted at the whole carrier is not.** We do not ship one (ADR-0511
...
- Nothing that ships moves. The emitted bytes on the default path are
  byte-identical (the carrier renders at 2,541,928 B, ADR-0517's figure to the
...
- ADR-0517's blast-radius argument is **narrower than it stated**, and that is
...
- **Flip it only for the carrier module.** Rejected: it would make the one
  ...
  the checker ADR-0517 decided to target, and it needs no re-spelling.
