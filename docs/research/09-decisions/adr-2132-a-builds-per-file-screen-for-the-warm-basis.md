...
is not the verdict table: it is 0 of 244 rows below the threshold opening it,
...
2. **`simplex_cold_builds` is not split per route**, and §4.2 measured the error
   ...
   derived, 6 of the 123 at or above it over the complete 400 -- all six on
   the HELD-OUT draw and all six `sc/*.induction3.cvc`, one subfamily rather
   than a scattering. The SCREEN is unaffected
