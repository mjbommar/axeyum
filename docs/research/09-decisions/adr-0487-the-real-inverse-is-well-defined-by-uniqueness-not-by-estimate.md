# ADR-0487: the real inverse is well-defined by uniqueness of inverses, and its congruence must range over the modulus
...
[ADR-0481](adr-0481-the-real-inverse-is-partial-and-its-modulus-is-data.md)
...
- **Hiding the modulus behind `Exists`.** Impossible, and that impossibility is
  ADR-0481's `pos_bound_of_lt`: `Exists` is a `Prop`, so `Exists.rec` eliminates
