...
| [0058](adr-0058-funded-nra-cad-nlsat-engine-arc.md) | The funded QF_NRA CAD/nlsat engine arc — commit a scoped multi-week decision-engine arc (Phase B: route DPLL cubes into the exact CAD + bignum coefficient path; C: ICP/transcendental with δ-sat⇒unknown; D: projection/cell scaling) for the 7/12 genuine-engine residue, with per-cell certificates designed to reconstruct to kernel-checked Lean `False`; pure Rust (no libpoly/C++), each phase dual-oracle DISAGREE=0. Ratify to accepted once Phase B's DPLL→CAD edge is scoped with a measured target | proposed |
