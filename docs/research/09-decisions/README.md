...
| [0060](adr-0060-arith-online-cdclt-default-dispatch.md) | Default dispatch for arithmetic online CDCL(T). Ratifies QF_UFLIA/QF_UFLRA online-first ordering and the 16M-step no-deadline belt. **2026-07-09 updates:** pure QF_LIA/QF_LRA lead with generic `CdclT` under caller-owned deadlines and a deterministic 1,024-LRA-atom cap; Boolean-structured QF_UFLIA/QF_UFLRA now drive their live combined theories through the same canonical `CdclT` API. Curated decided counts, zero disagreements, and replay safety are preserved. Importing the arithmetic-local driver's mature search heuristics into canonical `CdclT`, plus BV combination, remains open | accepted |
