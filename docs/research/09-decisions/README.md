...
| [0055](adr-0055-online-cdclt-dispatch.md) | Dispatch policy for the online CDCL(T) routes — QF_S online route default-on at the front door (ratifies the landed second-chance ordering; measured 52→58 with dual-oracle DISAGREE=0 and the 5707563b termination/expansion/polarity gate paid); QF_UF online stays opt-in as a parity twin until a measured criterion fires (win on a division, embedded-DPLL migration, or theory-combination need); new theories arrive online-first behind the same discipline | accepted |
...
| [0060](adr-0060-arith-online-cdclt-default-dispatch.md) | Default dispatch for the arith online CDCL(T) routes (QF_UFLIA/QF_UFLRA) — Gap 3 step 2b. RATIFIES the landed first-route ordering (the online EUF+linear-arith combination is already ahead of eager Ackermann in `check_auto`, gated on features not an opt-in flag; the "built but opt-in" framing was stale). Termination re-verify: deadline polled at the `lra_online::Dpll` loop head + every propagation fixpoint; the one asymmetry vs. ADR-0055 (no `DEFAULT_STEP_BUDGET` belt for the `deadline == None` / default-config case) FIXED — 16M-step belt added, mirroring `CdclT`, + a `step_budget_termination_tests` regression. Additive (belt never hit on in-corpus queries), re-measure DISAGREE=0 + model_replay_failures=0. Arrays-lazy port + #35 arith→`CdclT` migration stay OUT of scope | accepted |
