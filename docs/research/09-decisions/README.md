...
| [0071](adr-0071-replay-guided-array-interfaces.md) | Replay-guided array interfaces: a true abstraction-only array boundary plus candidate-violated select congruence on canonical `CdclT`, composed array-first/function-second with function-then-array projection and original replay | accepted |
| [0072](adr-0072-lazy-row-on-canonical-array-bus.md) | Lazy read-over-write on the canonical array bus: reuse the existing ROW abstraction, materialize only violated store hit/miss axioms, retain UF-bearing metadata roots, and accept SAT only after function-then-array replay | accepted |
...
| [0084](adr-0084-array-valued-uf-results-on-the-canonical-array-bus.md) | Array-valued UF results on the canonical array bus: retain semantic application parents on the e-graph, reconstruct fresh result arrays by final parent class, then project function tables and replay | proposed |
