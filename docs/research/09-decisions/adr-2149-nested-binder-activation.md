...
2. **At level 1 of ADR-2120's lever the crossed class is 1.6 M tuples on 9
   cores** (`ImportDeclVec.015` alone 380,131 in the interim read) — real, but
...
3. **What the budgets are spent on is duplicates.** A plain instance of
   `∀x⃗.M` exposes its nested universals at the same positions of `M` where the
   static walk already registered them with the same context and a strictly
   larger tuple set (the static one binds `x⃗ ∪ y⃗`, the discovered one `y⃗`
   with `x⃗` fixed; `positive_instance_formula` returns the same interned
   conclusion for both). Level 0 scans every instance anyway. And the matcher
   ingests the residual `forall` bodies of admitted instances, so triggers
   match their own bodies: 17,600 handoffs refused by the checker at level 1.
4. **The per-core table is in the README**; the cores whose crossed class is
   nonzero are the simplify/ESC-Java ones (`ImportDeclVec.015`,
   `Java2Html.832`, `NewInstanceExpr.271`, …), where a nested universal sits
   inside a registered universal's body.
...
- **Level 1.** An admitted instance is not scanned; a staged replacement is
  scanned only inside its replaced subtree (`NestedDiscovery::scan_replaced`,
  scope = the replacement's `PositiveContext.path`); a promoted universal is
  scanned whole, as before. A conclusion `produced` already holds no longer
  spends `MAX_POSITIVE_INSTANCES` — the matcher re-emits a registration's
  tuples after every rebuild, and at level 0 the counter moved before the
  dedup, which is how a fixture with 248 distinct replacements reached 4,096.
...
level 1 stops the budget spend (`instances_unscanned ≥ 300`,
`positive_instances < 4096`) but the flood remains and it is still `unknown` —
stated, not inferred; the cap fixture's SAT twin and the four SAT shapes at
every level of both levers through loop and front door; level 1 a strict
subset with what it skipped counted; level 2 hands off no self-binding tuple;
no-loss across levels; the certificate chain; the guard's thread-locality.
...
| level 1 keeps the universals inside the replaced subtree | 5 |
| level 1 does not scan admitted instances | 5 |
| a re-derived conclusion does not spend the positive-instance cap again | **1** — `level_1_stops_spending_the_budgets_on_duplicates_but_the_flood_remains` |
| level 2 adds a quantifier as an opaque leaf | 3 |

`--check-anchors`: `suites=205 anchors=1194 stale=0`. Two of seven kill exactly
