# ADR-0474: the real inverse is partial, its modulus is data, and both halves of that are theorems
...
[ADR-0472](adr-0472-complex-is-a-pair-setoid-over-creal-and-carries-no-order.md)
...
- **`CReal.no_total_inverse : ∀ (f : CReal → CReal), ¬ ∀ x, x · f x ≈ 1`.**
  Evaluate at `zero`. This is the field analogue of ADR-0472's
...
- **No `sqrt`, no completeness, no supremum.** Each is its own ADR. Note that
  ℂ's `abs` needs `sqrt` needs completeness, so ADR-0472's gap does not close
  here.
