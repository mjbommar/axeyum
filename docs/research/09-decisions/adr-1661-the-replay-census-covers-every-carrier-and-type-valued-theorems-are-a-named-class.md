...
Before landing, the sixteen nested per-carrier tests in `real_lean_replay_census_all` were put behind `#[ignore]` and `CHECK_FLOOR` set to 262 rather than 278: the gate runs only the `everything` carrier (the union, and the only row a headline may quote), because the suite's cost in the debug profile the push hook uses was not measured at the time (the lane's debug build waited 45 minutes in the cargo queue) and its release run alone took 738.79 s. **Measured afterwards, 2026-09-05, in the lane's worktree under host load ~20:** the `everything` census alone, debug profile, `1 passed … finished in 441.79s` (build 1m 19s on top). That is the gate's added cost: about 7.5 minutes per push battery, one Lean replay. The sixteen rows above are re-derived with `cargo test -p axeyum-lean-kernel --test real_lean_replay_census_all -- --ignored`; the measurement artifact is unchanged.
