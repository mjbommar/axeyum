...
ADR-0509 asserted that the telescope "is the interface, stated in the kernel and
...
ADR-0509 says "one deliberate `axiom` declared for the purpose … does the job
...
- **`arith_prelude_builds()` and `F:shipped-front-door-reaches-no-real-axiom`**
  ...
  number of ADR-0509 needs a new home;
...
None of this needs new mathematics — ADR-0509 was right about that, and the
...
the correction to ADR-0509's phrasing, which reads as though the 30 could be
thinned.

**Re-base `Real` on ℤ as definitions.** ADR-0509 rejects this because it names ℤ
