# ADR-0488: Lean has two checkers, and the kernel is the one we target
...
Related: [ADR-0482](adr-0482-the-shared-development-is-emitted-once-as-its-own-lean-module.md),
...
prelude module at the whole constructed-real carrier (ADR-0482) found that Lean
...
3. The shared prelude module (ADR-0482) stays rooted at the **reached union**,
...
- "Lean rejected our output" must always be qualified by *which Lean*. A
  ...
  not evidence — the same rule ADR-0485 applies to *which binary*.
