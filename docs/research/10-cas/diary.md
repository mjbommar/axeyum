...
**Certified positive-integer powers in rational-rate Bessel-J products** (Entry 37b01): wave nineteen probed the multiplicity seam left by direct products. `J₀(x)²`, `xJ₃(x)³`, `x²J₂(x³)²`, and a mixed `xJ₀(x)²J₁(x²)` all declined, as did a weighted powered rational argument and the correct borderline, supercritical, modified-I, and denominator controls. A positive integer power repeats the fixed-order envelope without introducing a new theorem: for exact rational-coefficient arguments `rᵢ(x)=Θ(|x|^dᵢ)`, positive integer multiplicities `mᵢ`, and an exact rational weight `w(x)=O(|x|^e)`, DLMF 10.17.3 and 10.11.1 give `w(x)∏Jₙᵢ(rᵢ(x))^mᵢ=O(|x|^(e-Σmᵢdᵢ/2))`. `limit` now records direct factors with multiplicity one, recognizes positive public powers without expansion, checked-multiplies each exact argument growth by its `u32` multiplicity, checked-adds the combined budget, and returns zero only under `2e<Σmᵢdᵢ`. Zero powers remain on the ordinary rational/simplification route and do not count as Bessel factors. Regressions cover squares, cubes, mixed powered/direct products, polynomial and rational arguments, reflected signs, symbolic outer factors, both infinities, order `u32::MAX`, and exponent `u32::MAX`; the latter exposed that generic rational normalization treated the Bessel head as an atom and could process the hostile exponent before the bounded theorem recognizer. Moving this fail-closed zero recognizer to the top of real-infinity dispatch makes that public path constant-time while unsupported shapes still fall through. Combined borderline and supercritical rates, modified-I powers, and powered denominators decline; the established `(J₀(1/x))²→1` composition remains intact. An independent 80-digit mpmath comparison of five admitted shapes at both directions against the product of their leading fixed-order asymptotics passes 10/10 at `|x|=200.125`, with maximum envelope-scaled error 0.00916891831061. SymPy independently returns zero for `J₀(x)²` and leaves the other four weighted or mixed prototypes as unevaluated `Limit` objects, so only the square is recorded as parity agreement. No expression head, public operator, backend, evidence format, or logic fragment changed, so no ADR is required. Source commit `8c82e7d1` was integrated exactly by merge `ac2571ce`. The thermally managed full gate passed 560 unit tests in 2415.34 s and 147 doctests with `RUST_TEST_THREADS=2` constrained to one CPU; the same process was repeatedly paused and resumed around external Lean/Python contention without restart. Workspace warning-denied all-target/all-feature Clippy (5.58 s warm), WASM (6.93 s), strict stable/nightly rustdoc (1.85/1.80 s), links, and whitespace checks all pass. The non-mutating single-file formatter audit proposed no owned-line changes, so no unrelated formatting was applied.

**Certified nested powers of pure rational-rate Bessel-J products** (Entry 37b02): wave twenty probed the remaining structural multiplicity seam. `(J₀(x)J₁(x²))²`, `x(J₀(x)J₃(x²))²`, `(J₀(x)²)³`, a mixed powered/direct product inside an outer cube, and a symbolic-constant powered product all declined. A pure product exponent of `u32::MAX` timed out before a result because generic rational normalization tried to expand the atom-based power. Internal variable-dependent weights, modified-I factors, Bessel denominators, and a zero outer power supplied the controls. The same fixed-order theorem requires no new analysis: an outer positive integer power distributes its multiplicity across every Bessel factor. `limit` now recognizes that structure only when the powered expression's complete variable-dependent part is a finite Bessel-J product; signs, variable-free factors, and variable-free denominators are harmless. A recursive collector composes nested multiplicities in `u128` without expanding the expression. Its saturation is proof-preserving on supported 32/64-bit targets because the compared doubled rational weight degree is derived from `usize` and therefore remains far below `u128::MAX`; once the true decay budget overflows `u128`, the strict inequality is already certain. The product-level exact rate test remains `2e<Σmᵢdᵢ`. Regressions cover product squares and cubes, nested and mixed inner powers, polynomial and rational arguments, reflected signs, symbolic constants, constant denominators, both infinities, a product exponent of `u32::MAX`, and three nested `u32::MAX` exponents, all promptly. Internal variable weights—including the mathematically subcritical but deliberately unsupported `(xJ₀(x³))²`—modified-I factors, powered Bessel denominators, and zero powers retain fail-closed or established exact behavior. An independent 80-digit mpmath comparison of five admitted shapes at both directions against their leading fixed-order product asymptotics passes 10/10 at `|x|=200.125`, with maximum envelope-scaled error 0.00000890279693823. SymPy independently returns zero for four prototypes and leaves only `x(J₀(x)J₃(x²))²` unevaluated, so the latter is recorded as a parity non-result. No expression head, public operator, backend, evidence format, or logic fragment changed, so no ADR is required. Source commit `02f99a79` was integrated exactly by merge `0e44005b`. The thermally managed full gate passed 561 unit tests in 2946.95 s and 147 doctests with `RUST_TEST_THREADS=2` constrained to one CPU; the same process was repeatedly paused and resumed around external Lean/Python contention without restart. Workspace warning-denied all-target/all-feature Clippy (6.13 s warm), WASM (32.75 s), strict stable/nightly rustdoc (1.89/1.86 s), links, and whitespace checks all pass. The non-mutating single-file formatter audit proposed no owned-line changes, so no unrelated formatting was applied.

**Certified rational weights inside powered rational-rate Bessel-J products** (Entry 37b03): wave twenty-one probed the internal rational-weight seam left by the pure-product recognizer. Representative subcritical squares and cubes, external plus internal weights, rational denominator weights, nested powers, and polynomial/rational Bessel arguments declined; `(xJ₀(x³)).pow(u32::MAX)` timed out after the earlier recognizer fell through to generic atom-based power normalization. The correct borderline and supercritical rates, Bessel denominators, modified-I mixtures, and symbolic-coefficient weights supplied fail-closed controls. No new asymptotic theorem is needed: for rational weights and fixed-order arguments, degree growth remains additive under multiplication and scales by every enclosing positive integer multiplicity. The structural collector now records each non-Bessel variable-dependent leaf together with that inherited `u128` multiplicity and its numerator/denominator orientation. Each leaf must independently normalize to a rational function univariate in the limit variable; its exact numerator and denominator degrees are checked-multiplied and checked-added to the external weight budget. Rational-budget overflow declines. The Bessel decay side retains proof-preserving saturation, and the final strict condition remains `2·max(0,total numerator degree−total denominator degree)<Σmᵢdᵢ`. No product or public power is expanded. Regressions cover internal polynomial and rational weights, rational Bessel arguments, numerator/denominator orientation, external/internal composition, nested powers, both infinities, and exponent `u32::MAX`; all intended cases return promptly, while the negative controls remain bounded. An independent 80-digit mpmath comparison of six admitted shapes at both directions against their leading fixed-order product asymptotics passes 12/12 at `|x|=200.125`, with maximum envelope-scaled error 0.00916891831060612. SymPy independently returns zero for the denominator-weight and decaying-weight shapes at both directions and leaves the other four prototypes unevaluated, so those are recorded as parity non-results rather than agreements. No expression head, public operator, backend, evidence format, or logic fragment changed, so no ADR is required. Source commit `9cb6f731` was integrated exactly by merge `53766474`. The thermally managed full gate passed 562 unit tests in 2183.78 s and 147 doctests with `RUST_TEST_THREADS=2` constrained to one CPU; the same process was paused and resumed around external Lean/Python contention without restart. Workspace warning-denied all-target/all-feature Clippy (44.94 s), WASM (6.44 s), strict stable/nightly rustdoc (1.90/1.80 s), links, and whitespace checks all pass. The non-mutating single-file formatter audit proposed no owned-line changes, so no unrelated formatting was applied.

**Exact-rational absolute-value limit continuity** (Entry 37b04): wave twenty-two probed continuity through the existing absolute-value head. Before the change, `|x|` at zero, `|x−1|` at zero, absolute values of two rational functions at real infinity, `|sin(x)/x|` at both directions, `|J₀(x)|`, `|xJ₀(x³)|`, and `|erf(x)|` all declined or left an unfurled exact absolute value. Oscillatory `|sin x|`, divergent `|x|`, reciprocal-Bessel, and symbolic finite-inner forms supplied the negative controls. Absolute value is continuous on the reals, so `limit` now composes only after the complete existing inner route returns a literal exact rational `q`, then returns the folded exact rational `|q|`. Restricting the composition result to a literal rational deliberately avoids symbolic sign analysis and complex-domain claims; absent, divergent, oscillatory, and symbolic inner results still decline or continue through existing dispatch. Ten admitted regressions cover finite points, rational functions, squeeze, Bessel-J, weighted Bessel-J, and error-function results at both infinity directions. SymPy independently agrees on nine of the ten admitted shapes and leaves only `|xJ₀(x³)|` at negative infinity unevaluated; its oscillatory and divergent controls return nonfinite set/infinity objects rather than false zeroes. No expression head, public operator, backend, evidence format, or logic fragment changed, so no ADR is required. Source commit `a6e79b08` was integrated exactly by merge `5b42a0ef`, with identical source blob and stable patch identity. The thermally managed full gate passed 563 unit tests in 6106.45 seconds wall time and 147 doctests. The same exact process was repeatedly stopped and resumed around external multi-threaded workspace contention without restart; the final unit duration therefore includes preserved pause time. Workspace warning-denied all-target/all-feature Clippy (13.07 s warm), WASM (7.00 s), strict stable/nightly rustdoc (2.00/1.79 s), links, whitespace, and the non-mutating owned-line formatter audit all pass.

**Exact continuous-head limits and sound asymptotic routing** (Entry 37b05): wave twenty-three probed thirteen existing real-continuous unary heads over exact-rational inner limits. The pre-change probe exposed a correctness defect before the intended parity gap: `arg_tends_to_infinity` inferred unboundedness from two large floating `evalf` samples, and unreliable Bessel samples made `atan(J₀(x))`, `erf(J₀(x))`, `Si(J₀(x))`, `FresnelS(J₀(x))`, and `FresnelC(J₀(x))` return false negative asymptotes at `+∞` (`−π/2`, `−1`, `−π/2`, `−1/2`, and `−1/2`). Numeric sampling has been removed from this proof boundary. Negative infinity is first reduced exactly by `x↦−x`; `algebraic_leading_at_infinity` must then provide a positive exact order, and the simplified leading coefficient must have a strict known sign under `Assumptions`. This preserves rational, polynomial, surd, and algebraic unbounded-argument routes while Bessel, oscillatory, bounded, and leading-cancellation shapes decline instead of guessing. Separately, `Sin`, `Cos`, `Atan`, `Erf`, `Si`, `Shi`, `FresnelS`, `FresnelC`, `Asinh`, `Ai`, `AiPrime`, `Bi`, and `BiPrime` now compose only after the complete existing inner route returns a literal exact rational. Seventy-eight admitted checks cover all thirteen heads across rational limits at both infinities, squeeze, unweighted and weighted Bessel limits, and a finite point; oscillatory and symbolic inner controls apply to every head, and the discontinuous sign head remains excluded. Isolated SymPy comparisons agree on all thirteen rational-inner cases and eight of thirteen Bessel-inner cases; the remaining five are unevaluated, for 21/26 agreements and no contradiction. No expression head, public operator, backend, evidence format, or logic fragment changed, so no ADR is required. Source commit `bd7d251a` was integrated exactly by merge `a317a502`, with identical source blob and stable patch identity. The thermally managed full gate passed 565 unit tests in 5671.04 seconds wall time and 147 doctests; the same exact process was repeatedly stopped and resumed around external workspace contention and finally governed by a five-second thermal loop, never restarted. Workspace warning-denied all-target/all-feature Clippy (7.86 s warm), WASM (7.21 s), strict stable/nightly rustdoc (3.23/1.98 s), links, whitespace, and the non-mutating owned-line formatter audit all pass.
