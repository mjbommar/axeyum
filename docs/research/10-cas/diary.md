...
**∫F(√x) — rational (and composite) functions of √x** (Entry 37ad9): probe find. `integrate_sqrt_rational_usub` substitutes `u=√x` (`x=u²`, `dx=2u du`): replace `√x→u`, `x→u²`, integrate `F(u)·2u`, back-substitute. Closes `∫1/(√x+1)=2√x−2ln(√x+1)`, `∫√x/(1+√x)`, `∫1/(√x(1+x))=2·atan(√x)`, `∫atan(√x)=(x+1)atan(√x)−√x`. Guards mirror the ln-substitution finder: fires only when the whole `x`-dependence is through `√x` (no residual `x`), and the `u`-integrand carries no `√u` (termination). Certified by differentiate-and-check.
