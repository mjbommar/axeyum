...
## Implemented (`crates/axeyum-cas` — pure Rust, WASM-safe, 518 unit + 147 doctests, clippy-clean)
...
| Summation (definite) | `definite_sum` (Σ over bounds — polynomial via telescoping, **geometric/hypergeometric via Gosper**: `Σ 2^k = 2^{n+1}−1`, `Σ k·2^k`; geometric base recovered from any exponent spelling — `2^{−k}`, `(½)^k`, `Σ_{k≥0}2^{−k}=2`); `prove_wz_sum` (symbolically checked Wilf–Zeilberger certificates for binomial moments, **Vandermonde** `ΣC(n,k)²=C(2n,n)`, fixed-shift convolutions `ΣC(n,k)C(n,k+r)=C(2n,n−r)` for `r=1,2,3,4`, and the first five squared-binomial moments through `Σk⁵C(n,k)²=n⁴(n+1)(n²+2n−5)C(2n,n)/(8(2n−3)(2n−1))`) | ✓ |
