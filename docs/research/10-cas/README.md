...
## Implemented (`crates/axeyum-cas` — pure Rust, WASM-safe, 529 unit + 147 doctests, clippy-clean)
...
| Transforms | `laplace_transform` (poly×{1,sin,cos} with the **s-shift rule** `L{e^{at}f}=F(s−a)` — `L{e^t sin t}`, `L{t·e^t·sin t}`) + `inverse_laplace` (simple/repeated real poles and **irreducible quadratics** → damped sinusoids; repeated rational-frequency quadratic powers through multiplicity 7, including shifted/mixed factors), `z_transform` (bounded `P(n)aⁿ`, rational `P`, `deg P≤32`, positive rational `a`) + `inverse_z_transform` (positive rational poles, multiplicity ≤32); inverse transforms are exact forward-round-trip-certified | ✓ |
