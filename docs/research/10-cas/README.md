...
## Implemented (`crates/axeyum-cas` — pure Rust, WASM-safe, 454 tests, clippy-clean)
...
| ODEs / recurrences | `dsolve_homogeneous`, `dsolve_inhomogeneous` (polynomial forcing), `dsolve_first_order_linear` (integrating factor), `dsolve_separable`, `dsolve_exact`, `dsolve_bernoulli`, `apply_initial_conditions` (IVPs — y″+y=0, y(0)=1,y′(0)=0 ⇒ cos x), `solve_recurrence` (rational **and** quadratic-irrational roots — incl. **Fibonacci**/Binet); `wronskian` | ✓ (substitute-and-check) |
