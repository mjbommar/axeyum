...
## Implemented (`crates/axeyum-cas` — pure Rust, WASM-safe, 504 unit + 147 doctests, clippy-clean)
...
| Summation (definite) | `definite_sum` (Σ over bounds — polynomial via telescoping, **geometric/hypergeometric via Gosper**: `Σ 2^k = 2^{n+1}−1`, `Σ k·2^k`; geometric base recovered from any exponent spelling — `2^{−k}`, `(½)^k`, `Σ_{k≥0}2^{−k}=2`); `prove_wz_sum` (symbolically checked Wilf–Zeilberger certificates for `ΣC(n,k)=2ⁿ`, `ΣkC(n,k)=n2ⁿ⁻¹`, `Σk²C(n,k)=n(n+1)2ⁿ⁻²`, and **Vandermonde** `ΣC(n,k)²=C(2n,n)`) | ✓ |
