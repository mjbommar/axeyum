...
## Implemented (`crates/axeyum-cas` — pure Rust, WASM-safe, 415 tests, clippy-clean)
...
| Special functions | `special`: `gamma`/`beta`, `zeta`/`dirichlet_eta`/`dirichlet_lambda`, `polygamma_at_one`, `gamma` at negative half-integers; **integral-defined heads** `erf`, `Si`/`Ci`/`Ei`, `li`, `Shi`/`Chi`, Fresnel `S`/`C` — with **certified defining integrals** (∫e^{−x²}=(√π/2)erf, ∫sin x/x=Si, ∫eˣ/x=Ei, ∫1/ln x=li, ∫sinh x/x=Shi, ∫sin(πx²/2)=S) + numeric `evalf`; `hyperbolic`: sinh/cosh/…/atanh (via exp tower) | ✓ (deriv-check / identities / Bernoulli) |
