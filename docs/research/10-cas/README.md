...
## Implemented (`crates/axeyum-cas` — pure Rust, WASM-safe, 516 unit + 147 doctests, clippy-clean)
...
| Summation (definite) | `definite_sum` (Σ over bounds — polynomial via telescoping, **geometric/hypergeometric via Gosper**: `Σ 2^k = 2^{n+1}−1`, `Σ k·2^k`; geometric base recovered from any exponent spelling — `2^{−k}`, `(½)^k`, `Σ_{k≥0}2^{−k}=2`); `prove_wz_sum` (symbolically checked Wilf–Zeilberger certificates for binomial moments, **Vandermonde** `ΣC(n,k)²=C(2n,n)`, fixed-shift convolutions `ΣC(n,k)C(n,k+r)=C(2n,n−r)` for `r=1,2,3`, and the first four squared-binomial moments through `Σk⁴C(n,k)²=n³(n³+n²−3n−1)C(2n,n)/(4(2n−3)(2n−1))`) | ✓ |
