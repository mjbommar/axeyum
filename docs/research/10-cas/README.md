...
## Implemented (`crates/axeyum-cas` — pure Rust, WASM-safe, 449 tests, clippy-clean)
...
fold** `(√u)²=u` for any `u` (not just constants — so `x/√x=√x` certifies), `ln(exp u)=u`, and the
