...
## Implemented (`crates/axeyum-cas` — pure Rust, WASM-safe, 508 unit + 147 doctests, clippy-clean)
...
| Summation (definite) | `definite_sum` (Σ over bounds — polynomial via telescoping, **geometric/hypergeometric via Gosper**: `Σ 2^k = 2^{n+1}−1`, `Σ k·2^k`; geometric base recovered from any exponent spelling — `2^{−k}`, `(½)^k`, `Σ_{k≥0}2^{−k}=2`); `prove_wz_sum` (symbolically checked Wilf–Zeilberger certificates for binomial moments, **Vandermonde** `ΣC(n,k)²=C(2n,n)`, adjacent convolution `ΣC(n,k)C(n,k+1)=C(2n,n−1)`, and squared-binomial moments `ΣkC(n,k)²=(n/2)C(2n,n)`, `Σk²C(n,k)²=n³C(2n,n)/(2(2n−1))`) | ✓ |
