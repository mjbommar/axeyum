...
## Implemented (`crates/axeyum-cas` — pure Rust, WASM-safe, 550 unit + 147 doctests, clippy-clean)
...
`∫J₁(u)dx=−J₀(u)/a` and `∫I₁(u)dx=I₀(u)/a`, plus the weighted pairs
`∫uJ₀(u)dx=uJ₁(u)/a`, `∫uI₀(u)dx=uI₁(u)/a`,
`∫u²J₁(u)dx=u²J₂(u)/a`, and `∫u²I₁(u)dx=u²I₂(u)/a` for `u=ax+b`.
They certify through bounded division-free polynomial recurrences through order
three; mismatched weights and higher requested orders still decline.

| Area | Functions | Certified |
|---|---|---|
| Core | `differentiate`/`differentiate_n`, `substitute`, `expand`, `collect` (group by powers), `simplify`, `trigsimp`, `normalize`, `equal` (zero-test w/ witness, **Euler-sound for related trig atoms**, **log arithmetic** `2ln2−ln3=ln(4/3)` via prime-basis expansion, bounded division-free **Bessel product recurrences** through order three) | equal ✓ |
