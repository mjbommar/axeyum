...
>
> The instrument is repaired (ADR-1906, commit `72a5adf6f`). Re-derived on the
...
> segment here contains the whole MBQI pass (ADR-1906).**
...
instrument (ADR-1906): the `q:uf-fmf-full` term above is MBQI plus the finder,
...
declared unsat. **(CORRECTED, ADR-1906: part of that 23 s is the full MBQI
...
- **25 files (C + D) are killed by the clock**, 8 of them inside a model finder
  running on a declared-unsat query (Finding 5). **(CORRECTED, ADR-1906: that
...
2. **Stop giving half the clock to a model finder on a declared-unsat query.**
   ...
   UF files and ~~56.2%~~ **38.4% (corrected, ADR-1906)** counting both
...
3. **Repair the two item-1.8 recording defects** named under Method, so the next
   ...
   rather than after it. **The second is DONE (ADR-1906), by a different route
