...
| **31 prelude assumptions** (real 30 + integer 0 + string `append` 1), runtime-inventoried and type-digested; `logic` and `nat` enumerate to zero. Carriers, operations, and laws include opaque `Declaration::Axiom` values; Lean accepts axioms **vacuously**; the ledger is an *inventory, not a validation* | **Inventory DONE (TL0.4); 34 integer rows retired as ℤ was constructed; ledger classes 3 derivable, 19 external, 9 primitive; counts generated, not transcribed (ADR-0465); accepted classification/discharge OPEN** — T6.0.6/TL3.2 |
