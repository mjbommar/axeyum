...
| **30 prelude assumptions** (real 30 + integer 0 + string 0), runtime-inventoried and type-digested; `logic`, `nat`, `integer` and `string` all enumerate to zero, so the whole trusted surface is the `real` prelude. Carriers, operations, and laws include opaque `Declaration::Axiom` values; Lean accepts axioms **vacuously**; the ledger is an *inventory, not a validation* | **Inventory DONE (TL0.4); 34 integer rows retired as ℤ was constructed; ledger classes 3 derivable, 19 external, 9 primitive; counts generated, not transcribed (ADR-0465); accepted classification/discharge OPEN** — T6.0.6/TL3.2 |
