...
That emphasis remains the honest limit of the **default** renderer, and it
directly qualifies the `PARITY-STATUS-AND-PATH.md:57-60` claim. The additive
`render_lean_module_with_inductives` path now emits selected flat,
non-parametric/non-indexed families as real `inductive` commands, skips their
kernel-generated constructor/recursor axioms, and applies Lean's regenerated
recursor explicitly with `@`. Families not selected, and parametric/indexed
families outside that exporter slice, retain the axiom representation.

So `#print axioms` reporting "clean" still means *no `sorryAx`* — not *no
axioms*. The claim at `PARITY-STATUS-AND-PATH.md:60`
...
"axiom-free through Lean's own inductive kernel" unless the relevant family is
explicitly routed through the real-inductive exporter. Positivity is checked by
Lean for those selected flat families and only by axeyum's `inductive.rs` for
the fallback representation.

### 6.1 CI validation — historical gap and the ADR-0165 mandatory slice
...
The original 2026-07-15 audit found:
...
That was a real gap: the broad optional harness skipped-and-passed in CI and
...
same test passes locally against the pinned binary.

The broad solver-family harness and several specialized reconstruction tests
remain locally optional and visibly skip without Lean; the new mandatory job is
a deliberately small inductive/recursor compatibility gate, not exhaustive
external validation of every emitted module. The in-tree check remains the
always-on load-bearing verifier, while widening mandatory external coverage is
still assurance work.
