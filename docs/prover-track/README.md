...
| Prelude assumptions form part of the trust boundary | Still material, and measurably smaller — and now confined to one prelude. The [30-row generated ledger](../plan/generated/lean-axiom-ledger.md) records real 30, integer 0, string 0 — down from 65 as ℤ was constructed over the proved ℕ development and `string.append` became a checked definition, with 35 rows filed as retired. It classifies those rows as 3 derivable-theorem, 19 external-assumption, and 8 primitive-interface. Its counts are generated from a measurement and `--check`-gated (ADR-0465); accepted TL3.2 classification/discharge work remains open. |
