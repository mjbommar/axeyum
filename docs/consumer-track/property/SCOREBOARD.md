...
| `sdk-explicit-nested-aggregate-replay` | P1 | caller-owned nested aggregate replay | disproved | nested `transfer.limits` initializer renders separately, plugs into `TransferInput`, and appears before the replay assertion in a generated `#[test]` | Rust verifier domain replay body / Kani nested harness struct |
