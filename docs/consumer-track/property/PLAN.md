...
- `tests/support/corpus_cases.rs` is the shared graduated SDK corpus. The
  ...
  `SCOREBOARD.md` plus `corpus.json`: 16 cases, 5 proved, 11 disproved, 0
  ...
  one actual fixed-seed proptest shrunk counterexample, one minimized struct
  counterexample, one generated replay counterexample test, one proved BV
  assertion, one proved assertion under an SDK assumption, and one Kani-style
  assume/assert counterexample baseline.
...
| PROP.6 | SDK measurement gate | WIP — committed graduated corpus, generated `SCOREBOARD.md`, and machine-readable `corpus.json` cover 16 cases with DISAGREE=0, 1/1 Lean-required coverage, a mixed Bool/BV/Int expression-builder alias proof, and deterministic executable baseline comparisons for scalar counterexamples, an actual fixed-seed proptest shrunk counterexample, struct/replay counterexamples, proved assertions, an assumption-backed proved assertion, and a Kani-style assume/assert counterexample baseline; richer proptest families and real Kani CLI-backed baseline comparison remain open |
