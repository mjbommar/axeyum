...
- `Property::symbolic_struct` gives macro-free named-field bundles for
  struct-shaped inputs before the derive macro exists;
...
| PROP.3 | `Symbolic` trait and derive | WIP — scalar Bool/uN/i128 plus 2-/3-tuples declare and lift deterministically; `symbolic_struct` covers macro-free named-field bundles; derive macro and signed fixed-width Rust integers remain |
