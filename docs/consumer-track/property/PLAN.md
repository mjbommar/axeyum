...
- `tests/support/corpus_cases.rs` is the shared graduated SDK corpus. The
  ...
  `SCOREBOARD.md` plus `corpus.json`: 14 cases, 5 proved, 9 disproved, 0
  ...
  one minimized struct counterexample, one generated replay counterexample
  test, one proved BV assertion, and one proved assertion under an SDK
  assumption.
...
| PROP.6 | SDK measurement gate | WIP — committed graduated corpus, generated `SCOREBOARD.md`, and machine-readable `corpus.json` cover 14 cases with DISAGREE=0, 1/1 Lean-required coverage, a mixed Bool/BV/Int expression-builder alias proof, and deterministic executable baseline comparisons for scalar counterexamples, struct/replay counterexamples, proved assertions, and an assumption-backed proved assertion; broader randomized/external proptest/Kani-style baseline comparison remains open |
