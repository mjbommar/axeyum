...
- `tests/support/corpus_cases.rs` is the shared graduated SDK corpus. The
  ...
  `SCOREBOARD.md` plus `corpus.json`: 15 cases, 5 proved, 10 disproved, 0
  ...
  assertion, and one proved assertion under an SDK assumption.
...
| PROP.6 | SDK measurement gate | WIP — committed graduated corpus, generated `SCOREBOARD.md`, and machine-readable `corpus.json` cover 15 cases with DISAGREE=0, 1/1 Lean-required coverage, a mixed Bool/BV/Int expression-builder alias proof, and deterministic executable baseline comparisons for scalar counterexamples, an actual fixed-seed proptest shrunk counterexample, struct/replay counterexamples, proved assertions, and an assumption-backed proved assertion; richer proptest families and external Kani-style baseline comparison remain open |
