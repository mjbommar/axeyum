...
- `tests/support/corpus_cases.rs` is the shared graduated SDK corpus. The
  ...
  `SCOREBOARD.md` plus `corpus.json`: 13 cases, 5 proved, 8 disproved, 0
  ...
  one minimized struct counterexample, one proved BV assertion, and one proved
  assertion under an SDK assumption.
...
| PROP.6 | SDK measurement gate | WIP — committed graduated corpus, generated `SCOREBOARD.md`, and machine-readable `corpus.json` cover 13 cases with DISAGREE=0, 1/1 Lean-required coverage, a mixed Bool/BV/Int expression-builder alias proof, and deterministic executable baseline comparisons for scalar counterexamples, struct counterexamples, proved assertions, and an assumption-backed proved assertion; broader external proptest/Kani-style baseline comparison remains open |
