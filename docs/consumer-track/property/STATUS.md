...
- **2026-06-27 — Kani-style assume/assert counterexample comparison landed.**
  The PROP.6 corpus now checks a bounded Kani-style counterexample workflow:
  an independent Rust scan of the `kani::assume(debit <= 10)` plus
  ...
  minimized SDK counterexample matches the same witness. This adds a
  disproved assume/assert baseline next to the existing assumption-backed proof
  row. Totals are now 16 cases, 5 proved, 11 disproved, 0 unknown,
...
- **2026-06-27 — Proptest baseline counterexample comparison landed.**
  The PROP.6 corpus now checks an actual proptest-backed baseline workflow:
  a fixed-seed `TestRunner` shrinks the `u8` wrapping-add monotonicity failure
  to `(x = 1, y = 255)`, matching both the exhaustive Rust baseline and
  Axeyum's minimized counterexample for the same symbolic property. This moves
  the gate beyond proptest-style scans into a real randomized/shrinking baseline
  slice. Totals are now 15 cases, 5 proved, 10 disproved, 0 unknown,
  DISAGREE=0, and 1/1 Lean-required coverage. Verification passed:
  `cargo fmt --all --check`;
  `git diff --check`;
  `CARGO_BUILD_JOBS=2 cargo run -p axeyum-property --example property_corpus_scoreboard -- json >/tmp/axeyum-property-corpus.json`;
...
- **2026-06-27 — Replay baseline counterexample comparison landed.**
  The PROP.6 corpus now checks a generated regression-test workflow: an
  independent bounded Rust scan finds the first failing guarded transfer input,
  Axeyum minimizes to the same
  `TransferInput { enabled: true, amount: 1, balance: 0 }`, and the generated
  replay test feeds that witness into a caller-owned failure predicate. This
  extends the baseline-backed gate from scalar, struct, proved, and
  assumption-backed assertions into an emitted counterexample test shape.
  Totals are now 14 cases, 5 proved, 9 disproved, 0 unknown, DISAGREE=0, and
...
- **2026-06-27 — Assumption baseline proof comparison landed.**
  The PROP.6 corpus now checks a bounded precondition/assertion workflow: an
  independent Rust scan finds no `x <= 10 && x + 1 > 11` failure for `u8`, and
  Axeyum proves the same postcondition through the SDK's `Property::assume`
  surface with checked evidence. This extends the baseline-backed gate from
  scalar and struct counterexamples into assumption-backed proved assertions.
  Totals are now 13 cases, 5 proved, 8 disproved, 0 unknown, DISAGREE=0, and
  1/1 Lean-required coverage. Verification passed:
  `cargo fmt --all --check`;
  `git diff --check`;
  `CARGO_BUILD_JOBS=2 cargo run -p axeyum-property --example property_corpus_scoreboard -- json >/tmp/axeyum-property-corpus.json`;
...
- **2026-06-27 — Expression builder aliases landed.**
  ...
  delegating to the existing typed-handle builders and returning `Result` so
  fallible term construction stays visible. The PROP.6 corpus adds a mixed
  Bool/BV/Int identity proof row that exercises those aliases with checked
  evidence. Totals are now 11 cases, 4 proved, 7 disproved, 0 unknown,
  DISAGREE=0, and 1/1 Lean-required coverage. Verification passed:
  `cargo fmt --all --check`;
  `git diff --check`;
  `CARGO_BUILD_JOBS=2 cargo run -p axeyum-property --example property_corpus_scoreboard -- json >/tmp/axeyum-property-corpus.json`;
...
- **2026-06-27 — Expression ergonomics first slice landed.**
  ...
  or hiding construction errors. The follow-up builder-alias slice adds
  property-owned Bool/BV/Int aliases; broader operator traits / richer chaining
  syntax remain open.
...
1. Broaden expression construction ergonomics toward operator-trait experiments
   ...
   visible.
2. Broaden the baseline runner across wider randomized and external property
   shapes, including richer proptest families and real Kani CLI-backed bounded
   assertions.
