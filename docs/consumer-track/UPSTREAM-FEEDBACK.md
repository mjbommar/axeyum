...
- **What:** the original consumer pain was that `SymbolicExecutor`'s warm path
  ...
  syntactic same-index hits, literal-distinct concrete-address store misses, and
  constant-array reads: the warm solver encodes the simplified BV term while
  retaining the original memory term for replay and assumption-core reporting.
...
- **Source:** `STATUS.md` entries "memory-aware incremental assumptions",
  ...
  "Warm constant-array read admission";
