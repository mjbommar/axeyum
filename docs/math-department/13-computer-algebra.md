...
- [ ] **1. Arbitrary-precision core.** `CasExpr` coefficients are `i128` and
      ...
      curve, not a design.
- [ ] **2. Validated numerics.** Replace the `f64` `evalf` with rational
...
- [ ] **5. Finite group computation.** Permutation groups with Schreier–Sims,
...
- [ ] **8. Simplicial homology over ℤ.** Boundary matrices into the existing
      `smith_normal_form`. The cheapest new subject on this list and the only
      thing 06 would use.
- [ ] **9. Symbolic probability.** Named distributions with exact mean,
...
- [ ] **10. A gated trust registry and a parity benchmark.** A script that
