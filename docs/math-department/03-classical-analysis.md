...
| 2026-09-06 | **The completion, begun** (roadmap topic 4, ADR-1678): carrier, embedding, isometry and the estimate; 16 axiom-free declarations. Also a correction to this file's own count — `Metric.Complete` has two witnesses, not three, because completeness on an interval is a different predicate. The path from here to a complete ℝⁿ and a complete L¹ is now named end to end with nothing new required. | `c175f529c` |
