...
- [~] **4. Hall's marriage theorem** — *five slices by 2026-09-05: necessity, the choice half, the counting half, the singleton shelf, and now `allBelow_congr` with both branches of the critical-subset split; the statement itself is assembly plus two measure inequalities away.*, over `Nat.Finset` with the existing
