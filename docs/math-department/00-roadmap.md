...
| W3-3 | Categories, functors, natural transformations | 09.4 | **landed** `0e4eeba47` | `CatS.*`, 61 declarations, footprint 0, setoid-enriched: hom-sets carry an explicit equivalence with `compCongr`, laws up to hom-equivalence, no `funext`. **Setoid cost zero by construction**: delooping an `AlgS.Monoid` supplies every enriched field from a selector of `M`, so the five fields the enrichment adds arrive pre-discharged. Instances: indiscrete (one morphism up to hom-equivalence — a quotient statement with no quotient), `ofMonoid`, `idFunctor`, `ofMonoidHom`; `IsFunctor`, `IsNat`, `IsInitial`/`IsTerminal` with `initial_unique`; a natural transformation between deloopings *is* an intertwiner. **The universe guard rejection is recorded verbatim** (`ConstructorFieldUniverseTooBig` at `Sort 1`; the same twelve fields admit at `Sort 2`), and **ADR-1609's "no record can hold another record" is corrected**: the level is fixed per `FieldKind`, and `CatS.Functor` holds two `CatS.Category` fields. **The category of groups is blocked on `Sigma`, not universes** — a morphism is a function plus two proofs — so it waits on W0-5, whose lane has since admitted `Sigma`. Landed unbundled: `IsGrpHom` with the identity and composition laws. [ADR-1620](../research/09-decisions/adr-1620-categories-are-setoid-enriched-and-the-universe-guard-is-not-what-blocks-the-category-of-groups.md). Shared-file touch: `CatS` added to `validate-facts.py`'s namespace allowlist. |
