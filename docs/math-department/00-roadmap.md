...
| W2-12 | Hall's marriage theorem | 07.4 | **necessity landed** `0a499a6d8`; choice half closed `bc3eb38a5`; **counting half closed** `4c46ddb92`; the statement itself still open | Necessity is one application of `card_le_of_injOn`. The choice half became a subset-search reflection primitive (ADR-1614). The counting half, 17 declarations: `anyBelow`'s elimination rule gives `unionOver` a two-sided characterisation (`memB_unionOver_elim`, `card_unionOver_congr`); **family modification is a quantifier commutation, not a counting argument** — the deleted set does not depend on the index, so deleting commutes with the union pointwise and the only counting is `Nat.Finset.card_le_card_sdiff_add`; ADR-1614 had priced it as a union whose bound changes at every step. Matching `glue` is stronger than the textbook (index sets need not be disjoint). **The obstruction moved to the empty set**: `Nat.Finset.singleton` has no lemmas at all (`--const` ABSENT at 3,017 declarations) and nothing turns a positive `card` into a member. Build the empty-set/singleton bridge, then the skeleton is unobstructed. Also found: the kernel-dependency projection has been stale on main since 2026-08-26 (1,644 indexed vs 3,000 live; `--check` takes ~35 min and hygiene does not cover it) — needs its own lane. |
