...
| 2026-09-06 | **The sum-of-squares fallback** (the finding this file raised, ADR-1673): the attested module's theorem name is now read from its axiom footprint rather than chosen, so an honest reconstruction's bytes are unchanged and an axiom-carrying one cannot wear the honest name. Measured, the defect was worse than described here: the fallback is a hand-inlined copy of the honest route with its banner removed, which is exactly why the two guards written to catch this could not fire, and it emits byte-identical modules for two different refutations. One mutant's prediction was wrong and that miss is the real finding — every context-built refutation reports a non-empty minted set, so the footprint guard is coarser than intended and the rename is scoped to this attestation. | `156e79dc7`; solver `--lib` 1,464 passed |
