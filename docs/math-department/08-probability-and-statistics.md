...
| 2026-09-06 | **The product space** (the blocker this file named, ADR-1677): the k-fold product index range with its weight, independence as a predicate with a witness rather than an assumption, and the product rule for expectations proved from it; 10 axiom-free declarations at ℚ. The absence this file recorded was real about names and wrong about structure — the index range existed already, built for a determinant identity, and only the port was missing, which is the third false absence found by searching for the step instead of the name today. Hoeffding now waits on one lemma, the marginal law, whose cost is a product split at a distinguished index. | `d9a3379dd`; `product_space` 7 passed |
