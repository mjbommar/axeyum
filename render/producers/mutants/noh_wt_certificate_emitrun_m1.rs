// DELIBERATELY BROKEN. DO NOT USE AS A CERTIFICATE.
//
// This is render/producers/noh_wt_certificate_emitrun.rs with mutant M1 applied:
//
//     -  i64::try_from((k - 1) / 3 + k % 2).expect("weight fits in i64")
//     +  i64::try_from((k - 1) / 3).expect("weight fits in i64")
//
// i.e. the weight a(k) loses its parity indicator and becomes KMU Remark 6.5's
// own weight floor((k-1)/3), which is NOT admissible: d(5) = 0 < 1.
//
// The mutation is not ours. It is
//   newton-over-hodge-char2/replication/certificate/mutants/M1-weight-loses-the-parity-term.patch
// applied verbatim with `patch -p0`, and the expected catcher is the one in the
// matching .expect file: `(A3) d(5) = 0 < 1`.
//
// WHY IT IS CHECKED IN. The render strand's fail-closed law needs a REAL run
// record with exit_status 1 to test against, and hand-writing one would be the
// exact fabrication the strand exists to prevent. Running this file produces
// render/examples-input/cert/run-mutant-M1.json, whose provenance hashes THIS
// file. Nothing else in the repository should build or run it.
//
// Produced by: render strand P0-A, agent CERT, 2026-08-21.
