...
pub fn emit_with_diagnostics(doc: &Value, opts: &HtmlOptions) -> (String, Vec<String>) {
    ...
    // ONE PROVENANCE LINE, NOT FIFTEEN. Every block a single producer emits
    // carries the identical `Provenance`, so a fact card printed the same
    // two-line "generated by ... exit 0" paragraph under all fifteen of its
    // blocks -- the most repeated element on the page, and the one carrying the
    // least information per repetition. When they are ALL equal the line is
    // hoisted to the header, said once, and the per-block copies are dropped.
    //
    // This is a dedup, never an omission: the moment two blocks disagree --
    // one table from a different run, one claim from a checker -- no hoisting
    // happens and every block prints its own again, because then the
    // difference is the information.
    let shared_provenance = blocks_share_provenance(doc);
    let mut body = String::new();
    let blocks = doc.get("blocks").and_then(|b| b.as_array());
    if let Some(bs) = blocks {
        for (i, b) in bs.iter().enumerate() {
            // Reader review, 2026-08-21, gripe 1: on the atlas and both pilot
            // pages the page header's `h1` and the first section's heading
            // were the same sentence, one line apart. Suppressed for the FIRST
            // block only, and only on an exact match -- a later section that
            // legitimately repeats the title still gets its heading, and
            // nothing about the block's content is touched.
            let first = if i == 0 {
                without_duplicate_heading(b, title)
            } else {
                None
            };
            block(
                &mut body,
                first.as_ref().unwrap_or(b),
                i,
                &mut d,
    ...
    header(&mut out, meta, opts, shared_provenance.as_ref());
...
fn link(label: &str, target: &str) -> String {
    ...
    } else if let Some(page) = crate::doc_link_target(target) {
        // A reference to ANOTHER Doc-IR document. This is the one relative
        // href the pages may carry, and it is live because the corpus is a
        // SITE: the atlas, the pilots and 324 fact cards are emitted together
        // and `render/tests/link_integrity.rs` fails if any one of these does
        // not resolve to a file that was emitted. Every page remains
        // self-contained -- it fetches nothing on load; this is a link the
        // reader clicks.
        format!(
            "<a class=\"ax-doclink\" href=\"{}\">{}</a>",
            esc_attr(&page),
            inline(label)
        )
    } else {
        // Any other repo-relative path is shown as text, not as a link: a
        // file:// or arbitrary relative href in a single-file document is a
        // dead link by construction, and a dead link that looks live is a
        // small lie.
...
/// The badge WITHOUT its word, for a place where the word has already been
/// said.
///
/// Reader review, 2026-08-21, gripe 2: a claim card printed `EVIDENCE` in its
/// header and then again on every evidence row inside it -- and the evidence
/// row's status is not independent information, it IS the claim's resolved
/// status (`normalize_evidence` copies `claim_status` into it). Repeating a
/// word teaches a reader that the second one carries no information, which is
/// the wrong lesson to teach about a status badge.
///
/// The SHAPE survives, because the shape is what makes the badge readable in
/// greyscale and to a colour-blind reader (`05-html-interactivity.md`), and
/// the word survives as the accessible name, so a screen reader still hears
/// it. Nothing is dropped; one of the two copies stops shouting.
pub fn badge_compact(status: &str) -> String {
    let (slug, path) = badge_shape(status);
    let upper = status.to_ascii_uppercase();
    format!(
        "<span class=\"badge badge-{slug} badge-compact\" role=\"img\" aria-label=\"{0}\" title=\"{0}\">\
<svg viewBox=\"0 0 16 16\" aria-hidden=\"true\" focusable=\"false\"><path d=\"{path}\"/></svg></span>",
        esc(&upper)
    )
}
...
/// The one `Provenance` every block shares, if they all share one.
///
/// `None` when the document has no blocks, when a block has none, or when two
/// differ -- and the last of those is the case that matters: a document mixing
/// runs must show which block came from which.
fn blocks_share_provenance(doc: &Value) -> Option<Value> {
    let blocks = doc.get("blocks")?.as_array()?;
    if blocks.len() < 2 {
        return None;
    }
    let first = blocks[0].get("provenance").filter(|p| p.is_object())?;
    blocks
        .iter()
        .all(|b| b.get("provenance") == Some(first))
        .then(|| first.clone())
}

fn header(out: &mut String, meta: &Value, opts: &HtmlOptions, shared: Option<&Value>) {
    ...
    if let Some(p) = shared {
        out.push_str(
            "<p class=\"ax-source ax-source-all\">Every block on this page comes \
from one run &#x2014; ",
        );
        let mut line = String::new();
        provenance_line(&mut line, p);
        // `provenance_line` writes a whole paragraph; unwrap it into this one.
        let inner = line
            .trim()
            .trim_start_matches("<p class=\"ax-source\">")
            .trim_end_matches("</p>");
        out.push_str(inner);
        out.push_str("</p>\n");
    }
...
///
/// `meta.nav` is data: the producer knows the site layout (which card is in
/// which component, where the atlas lives) and the emitter knows nothing about
/// it. An entry whose target is not a resolvable Doc-IR reference renders as
/// plain text rather than as a link, so a broken reference looks broken.
fn nav_bar(out: &mut String, meta: &Value) {
    let items = arr(meta, "nav");
    if items.is_empty() {
        return;
    }
    out.push_str("<nav class=\"ax-crumbs\" aria-label=\"document navigation\">\n");
    for (i, it) in items.iter().enumerate() {
        if i > 0 {
            out.push_str("<span class=\"ax-crumb-sep\" aria-hidden=\"true\">/</span>");
        }
        let label = s(it, "label").unwrap_or("document");
        match s(it, "href").and_then(crate::doc_link_target) {
            Some(page) => {
                let rel = s(it, "rel")
                    .map(|r| format!(" rel=\"{}\"", esc_attr(r)))
                    .unwrap_or_default();
                let _ = write!(
                    out,
                    "<a class=\"ax-doclink\" href=\"{}\"{rel}>{}</a>",
                    esc_attr(&page),
                    inline(label)
                );
            }
            None => {
                let _ = write!(out, "<span>{}</span>", inline(label));
            }
        }
    }
    out.push_str("\n</nav>\n");
...
/// The block with its heading removed, when that heading merely repeats the
/// document title. `None` means nothing was duplicated and the caller renders
/// the block untouched.
fn without_duplicate_heading(b: &Value, title: &str) -> Option<Value> {
    let (name, body) = kind_of(b)?;
    if s(&body, "heading")? != title {
        return None;
    }
    let mut body = body;
    body.as_object_mut()?.remove("heading");
    let mut out = b.clone();
    // `kind_of` accepts both the single-key form and a `{"kind": "name"}`
    // string; only the first can carry a heading, so writing it back under the
    // same key is total.
    out.as_object_mut()?.insert(
        "kind".into(),
        serde_json::json!({ name_key(b, &name): body }),
    );
    Some(out)
}

/// The exact key `kind_of` matched, preserving its original case.
fn name_key<'a>(b: &'a Value, lowered: &str) -> &'a str {
    b.get("kind")
        .and_then(Value::as_object)
        .and_then(|m| m.keys().find(|k| k.to_ascii_lowercase() == lowered))
        .map_or("Prose", std::string::String::as_str)
}
...
fn block(
    ...
    match kind.as_str() {
        ...
        "table" => table(out, &body, &id, tag, hoisted),
...
fn provenance_line(out: &mut String, p: &Value) {
    ...
    let generator = s(p, "generator");
    if let Some(g) = generator {
        bits.push(format!("generated by <code>{}</code>", esc(g)));
    }
    // A command that merely repeats the generator's name (`python3 x.py` beside
    // `x.py`) is not a second fact about the run.
    if let Some(c) = s(p, "command").filter(|c| {
        generator.is_none_or(|g| !c.split_whitespace().any(|w| w == g) || c.len() > g.len() + 16)
    }) {
...
fn evidence_row(out: &mut String, e: &Value, id: &str) {
    ...
    // Compact: the claim card's header already said this word, three lines up.
    out.push_str(&badge_compact(check));
...
fn table(out: &mut String, v: &Value, id: &str, tag: &str, hoisted: Option<&Value>) {
    let cols = arr(v, "columns");
    let rows = arr(v, "rows");
    // FIVE COLUMNS IS WHERE THE READING MEASURE STOPS BEING THE RIGHT FRAME.
    // The atlas index has twelve, and at 36rem its last four -- including the
    // link to each fact's own card -- were off the right edge behind a
    // scrollbar, so the page's whole navigation story was invisible. `ax-wide`
    // is the hook `style.css` already had for this; the table still scrolls
    // when it must.
    let wide = if cols.len() >= 4 { " ax-widetable" } else { "" };
    let _ = writeln!(
        out,
        "<section class=\"blk {tag}{wide}\" id=\"{}\">",
        esc_attr(id)
    );
    heading_of(v, out);
    ...
    for r in rows {
        ...
        if let Some(cells) = r.as_array() {
            for (i, cell) in cells.iter().enumerate() {
                ...
                // WHY WRAPPING IS DECIDED PER CELL. `style.css` sets
                // `white-space: nowrap` on non-first columns so a machine value
                // (`kernel-lean`, `QF_BV`) cannot be broken at its hyphen --
                // correct, and the reason the wrapper scrolls. But a fact
                // card's `supports` and `axiom_footprint` cells are SENTENCES,
                // and under that rule each one ran off the right edge with its
                // tail behind a scrollbar. A value containing a space is prose
                // and wraps at its spaces; one without a space is a token and
                // still cannot break.
                let td_class = if text.trim().contains(' ') {
                    format!("{a} wrap")
                } else {
                    a.to_string()
                };
                let _ = write!(out, "<td class=\"{td_class}\">{}</td>", cell_html(&text));
            }
        }
        out.push_str("</tr>\n");
    }
    out.push_str("</tbody>\n</table></div>\n");
    // The run the rows came from -- unless the page already said, once, that
    // every block comes from that same run.
    if let Some(src) = v
        .get("source")
        .filter(|p| p.is_object())
        .filter(|p| hoisted != Some(*p))
    {
...
/// One table cell.
///
/// `ir::Cell` is a SCALAR -- deliberately, so every emitter prints the
/// identical characters for a number -- which is why a card-to-card link
/// cannot be a structured value and rides in a cell as the path of the
/// document it names (`13-facts-diary.md`, item 5). A cell whose WHOLE
/// content is such a path is rendered as a link to that document's page,
/// labelled with its stem; everything else is inline markup as before.
///
/// The whole-cell condition matters: a path mentioned inside a sentence stays
/// text, so this convention cannot turn prose into a link by accident.
fn cell_html(text: &str) -> String {
    ...
    match crate::doc_link_target(trimmed) {
        Some(page) => {
            let stem = trimmed
                ...
                .unwrap_or(trimmed);
            format!(
                "<a class=\"ax-doclink\" href=\"{}\"><code>{}</code></a>",
                esc_attr(&page),
                esc(stem)
            )
        }
...
must either carry the status of a run or say why there is none"
    ...
    if let Some(x) = exit {
        let cls = if x == 0 { "ax-exit-ok" } else { "ax-exit-bad" };
        let _ = write!(out, "<span class=\"{cls}\">exit {x}</span>");
    } else if no_exit_reason.is_some() {
        // SHORT IN THE SUMMARY, FULL IN THE BODY. A summary row is one line
        // beside a badge; the ledger's reason is a sentence, and putting it
        // here made every one of 198 certificate folds a three-line block of
        // bold text that drowned the thing it was summarising. The reason is
        // not dropped -- it is the first thing inside the fold, which is where
        // a reader who wants it is looking.
        out.push_str("<span class=\"ax-exit-none\">no run recorded</span>");
    } else {
        out.push_str("<span class=\"ax-exit-bad\">exit status missing</span>");
    }
    out.push_str("</summary>\n<div class=\"ax-foldbody\">\n");
    if let Some(r) = no_exit_reason {
        let _ = writeln!(out, "<p class=\"ax-noexit\">{}</p>", inline(r));
    }
...
fn dep_graph(out: &mut String, spec: &Value, id: &str) {
    ...
    for p in &l.nodes {
        ...
        let d: Vec<String> = desc[p.index]
            ...
            .map(std::string::ToString::to_string)
            .collect();
        // TWO KINDS OF NODE LINK, and until 2026-08-21 only one existed and
        // it was the wrong one. A node whose `href` names another Doc-IR
        // document (`cards/F-x.doc.json`) used to be slugged into an in-page
        // anchor -- `data-href="cards-f-x-doc-json"` -- which no page contains,
        // so every node on the atlas was a clickable-looking box that did
        // nothing. It is now a real SVG anchor to that document's page, and the
        // JS cone handler leaves it alone (it returns early with no
        // `data-href`), so the browser navigates natively and the link works
        // with scripting off.
        //
        // An `href` that is NOT a document reference keeps the old behaviour:
        // it is an in-page scroll target, which is what a single-page document
        // with its own claim anchors wants.
        let page = s(n, "href").and_then(crate::doc_link_target);
        let href = match s(n, "href") {
            Some(raw) if page.is_none() => {
                format!(" data-href=\"{}\"", esc_attr(&slug(raw)))
            }
            _ => String::new(),
        };
        if let Some(page) = &page {
            let _ = writeln!(out, "<a class=\"gnode-link\" href=\"{}\">", esc_attr(page));
        }
        let _ = writeln!(
            out,
...
pub fn lint_self_contained(html: &str) -> Vec<LintFinding> {
    ...
    while let Some(rel) = lower[from..].find("href=") {
        ...
        if !is_local_ref(&val) && !is_sibling_page(&val) {
...
/// A relative link to another page of the SAME emitted site.
///
/// The one relative `href` a self-contained page may carry, and it is narrow
/// on purpose. Self-containment is about what the browser fetches WITHOUT
/// being asked: a page that links to `cards/F-x.html` still opens with zero
/// network requests, renders identically offline, and needs no server. What it
/// gains is that the 324-card corpus is a site rather than 324 orphans.
///
/// Deliberately NOT extended to the resource attributes (`src`, `poster`,
/// `data`, ...): those are fetched on load, and `src="x.html"` remains a
/// finding. The rule is about the reader clicking, never about the page
/// loading.
///
/// The guarantee that this stays honest is not in this function: it is
/// `render/tests/link_integrity.rs`, which renders the whole site and fails if
/// any one of these hrefs does not resolve to a file that was emitted. Without
/// that, this would be a lint hole that lets dead links through.
fn is_sibling_page(val: &str) -> bool {
    let v = val.trim();
    // A fragment on the end is fine; the path in front of it is what is fetched.
    let path = v.split('#').next().unwrap_or(v);
    // The emitter writes `.html` itself, in exactly one place
    // (`doc_link_target`), so a case-insensitive compare here would only widen
    // what the lint accepts without widening what the emitter can produce.
...
mod tests {
    ...
    fn certificate_may_declare_why_it_has_no_exit_status() {
        ...
        assert!(d.is_empty(), "{d:?}");
        // Short marker in the summary, full reason in the fold body: a summary
        // row is one line beside a badge, and the ledger's reason is a
        // sentence. Both must be present -- dropping either would be the
        // silence this field exists to forbid.
        assert!(h.contains(">no run recorded<"), "{h}");
        assert!(
            h.contains("<p class=\"ax-noexit\">a four-hour re-check cannot be a gate</p>"),
            "{h}"
        );
    ...
    #[test]
    fn lint_accepts_a_relative_link_to_a_sibling_page() {
        // The corpus is a SITE. A page that links to another page of it still
        // opens with zero network requests, which is what self-containment
        // means; `render/tests/link_integrity.rs` is what checks the other end.
        for ok in [
            "<a href=\"cards/F-nat-add-comm.html\">x</a>",
            "<a href=\"../facts-atlas.html\">x</a>",
            "<a href=\"../facts-atlas.html#dep-graph-c02\">x</a>",
        ] {
            assert!(lint_self_contained(ok).is_empty(), "{ok}");
        }
    }

    #[test]
    fn lint_still_rejects_what_the_sibling_page_rule_must_not_widen() {
        // The rule is about a link the READER CLICKS, never about a resource
        // the page FETCHES, and never about leaving the site. Each of these is
        // one character away from the accepted form above.
        for bad in [
            // fetched on load, not clicked
            "<img src=\"cards/F-a.html\">",
            // absolute path: not a sibling of anything
            "<a href=\"/cards/F-a.html\">x</a>",
            // another origin wearing the extension
            "<a href=\"https://evil.example/x.html\">x</a>",
            // protocol-relative
            "<a href=\"//evil.example/x.html\">x</a>",
            // not a page of this site
            "<a href=\"../../../etc/passwd\">x</a>",
            "<a href=\"notes.md\">x</a>",
        ] {
            assert!(!lint_self_contained(bad).is_empty(), "{bad}");
        }
    }
...
pub fn normalize_resolved(doc: &Value) -> Value {
    ...
    if let Some(n) = doc.get("nav").filter(|n| n.is_array()) {
        meta.insert("nav".into(), n.clone());
    }
    ...
    let inner: Value = match name.as_str() {
        ...
        "certificate" => {
            ...
            json!({ "Certificate": {
                ...
                "no_exit_reason": s(&body, "no_exit_reason"),
