...
| Public documentation and examples | `WIP`, user-directed lane | Beginner/user, WASM/browser, contributor, stable reference, internals, all 22 crate front doors, consumer applications, curriculum, reviews, plan, and research have substantive indexes; seven core crates have compile-tested rustdoc examples. All 34 Cargo examples are cataloged with prerequisites and mutation boundaries. The consumer front door routes current crate contracts and the 48-case aggregate instead of obsolete build-era journals; the public arithmetic snapshot is ledger-checked. Generated authorities remain canonical; continue only from concrete live-linked drift. |
