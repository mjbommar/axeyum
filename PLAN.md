...
> Commit this implementation unobserved, then repeat the exact fixed
> population from a clean detached source.
...
| **GQ5** | **Cheaper AIG construction and measured CNF encoding** | **Cold engineering lane active; broad warm construction lane closed by ADR-0219; artifact-v35 attribution ACCEPTED without optimization (ADR-0259), artifact-v36 origin profiler IMPLEMENTED but unobserved (ADR-0260).** The v35 run finds 119,260 exact primary duplicates and zero collision/repeated/complement work. V36 now preserves actual first and duplicate emission sites, same/cross owner, exact length/literal partitions, family/outcome participation, and the preregistered dominance/distribution gate while the disabled store stays zero-sized. Commit the tested implementation before the fixed 162-query observation. Select at most one later generator-elision experiment only if its cell passes ADR-0260; counts alone are not a speedup. |
...
|---:|---|---|
| 1 | Cold CNF/bit-blast micro-optimization | ADR-0259 rejects another collision-index change and isolates exact duplicate generation: 119,260 duplicates, all primary hits, concentrated in slice-partial/register-slice. Commit ADR-0260's tested artifact-v36 origin profiler unobserved, then execute its fixed detached observation before selecting one generator-elision candidate; accept that candidate only on repeated unprofiled real-corpus end-to-end gains with identical decisions/replay. |
