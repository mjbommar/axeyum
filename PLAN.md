...
**Status:** Exact official Lean 4.30 `Nat.fib_coprime_fib_succ` remains durably `proved` through dependency-bound receipt `34b9aad06fc8a640c81df0951b1af37a464f2d9305c048784e4f590b83ff0d0e`, and its sole newly ready child `F:ml430-nat-gcd-fib-add-self-5a92d5e3` remains open. Bottom-up Euclidean reconstruction retains the twice-reconstructed empty-footprint private quotient/remainder invariant and the local `Nat.sub_add_cancel` replacement. Public division, public gcd computation, and official xgcd coefficients remain declined under the zero-assumption policy. V2 stopped at its one main compilation with zero export/import/retry and exact baseline cleanup. Its direct `Nat.mod` and coefficient-scoped transport corrections worked, exposing only three narrower elaboration classes: the unfolded `Nat.modCore` positivity conditional, beta-redex forms on lifted multiplication equalities, and definitional notation drift in the induction hypothesis. V3 is preregistered with exactly those corrections: `dif_pos hm`, explicit normalized equality types, and a definitional `change` before contextual rewrite. The mathematical route, four-Nat carrier, explicit later-specialization parameters, forbidden quotient/xgcd dependencies, pinned environment, two-compilation/one-export/two-import ceiling, and zero target/ledger authority remain unchanged. No generic theorem, target specialization, cancellation, Fibonacci target, receipt, evaluation, fact, or ledger credit has yet followed.

**Next:** publish V2's immutable decline and the V3 preregistration together, then execute V3 once on pinned `s5`: verify the exact baseline and absent paths, compile support and V3 once each, export the two selected roots once, audit twice in fresh kernels, and clean exactly six paths. Accept only matching identities, empty footprints, and no forbidden dependency. On acceptance, freeze evidence before separately preregistering checked target specialization with existing empty-footprint `Nat.gcd_zero_left` and `Nat.gcd_succ`; on compilation or first-import failure, stop without retry. Do not modify V1/V2 or inspect proof-bearing streams/pre-existing files.
