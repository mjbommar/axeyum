...
**Status:** Exact official Lean 4.30 `Nat.fib_coprime_fib_succ` remains durably `proved` through dependency-bound receipt `34b9aad06fc8a640c81df0951b1af37a464f2d9305c048784e4f590b83ff0d0e`, and its sole newly ready child `F:ml430-nat-gcd-fib-add-self-5a92d5e3` remains open. Bottom-up Euclidean reconstruction retains the twice-reconstructed empty-footprint private quotient/remainder invariant, the local `Nat.sub_add_cancel` replacement, and the native empty-footprint `Nat.mod_lt`; public division and public gcd computation remain declined. The extended-gcd alternative is now measured end to end without rendering proof material. Official `Nat.gcd_eq_gcd_ab` carries `Quot` plus `propext`; all three direct xgcd coefficient roots are assumption-bearing; both `Nat.xgcd_val` and `Nat.xgcd.eq_1` are terminal `propext` carriers with no theorem dependency to descend through. Most decisively, an independently authored byte-identical `rfl` projection theorem compiled, exported, and imported twice with identical `[propext]` footprints and no theorem dependencies. The public `xgcd`/`gcdA`/`gcdB` definitional surface itself is therefore closed under the zero-assumption policy, not merely the official theorem proof. The useful retained seam is official `Nat.gcd.induction`, measured empty-footprint over `Nat.mod_lt` and `Nat.succ_pos`. The `s5` reference checkout's exact three-file pre-existing baseline was restored after the one rooted compilation; no pre-existing file changed. No projection, extended-gcd, balanced-Bézout, cancellation, Fibonacci target, receipt, evaluation, fact, or ledger credit followed.

**Next:** stop descending or wrapping official `xgcd`/`gcdA`/`gcdB`. Preregister the minimum target-owned coefficient carrier over the native gcd/Euclidean interface, with its computation equations and balanced integer linear-combination invariant fixed before implementation. Use the measured empty-footprint `Nat.gcd.induction` interface and existing native `Nat.mod_lt`; keep public official gcd/xgcd statements as statement oracles only. First establish the target-owned coefficient invariant twice with an empty kernel footprint, then preregister the smallest explicit adapter needed by balanced coprime-factor cancellation and only afterwards spend an exact Fibonacci target submission. Preserve the accepted support identities and remaining target budget. Do not inspect upstream proof bodies, import `WellFounded.Nat.fix_eq`, transport the official xgcd definitions, alter the three-file `s5` baseline, graft same-named native declarations, or publish a partial private kernel.
