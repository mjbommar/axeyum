...
| **GQ7** | **Cheaper warm entry and delta preprocessing** | **Axeyum API, opt-in Glaurung direct-delta wiring, and causal profile contract are DONE; real-stream acceptance is pending.** The first real run correctly rejects depth-only direct deltas combined with snapshot-era serial sibling leasing (497/2,551 disagreements). Glaurung `f4da0eb` now forces direct mode onto sound exclusive LIFO owner transfer/distinct siblings; the same stream is 2,551/2,551 agreed. Compare this topology against its snapshot control and the faster serial-snapshot production policy before any default change. |
...
| 2 | First-class incremental `Solver` trait | **Contracts, opt-in explorer wiring, and profiling done in Axeyum ADR-0201/0202 and Glaurung ADR-011/`00bd660`.** Both sides expose object-safe retained sessions; Glaurung translates only confirmed persistent suffix roots, keeps probes temporary, advances markers only after acknowledgement, and emits fail-closed v7 entry partitions. Run the ordered correctness/time/RSS gate next; `assert_configured` remains warm-only. |
...
| 5 | Sibling-prefix structural sharing | **Done for complete-snapshot serial DFS, open for first-class direct deltas.** ADR-0199's exact LCP/pop/push lease depends on translating complete snapshots. A depth-only direct marker cannot distinguish equal-depth opposite siblings; `f4da0eb` therefore disables that lease after a 497-disagreement real failure. Direct COW/source-identity prefix sharing requires a new proof/lifecycle contract. |
...
partitions. The remaining rank-2 work is the repeated ordered real-driver gate
...
1. **GQ7 warm end to end:** build on ADR-0164's measured snapshot-LCP bridge,
   ...
   depth-only/serial-sibling correctness conflict. Compare sound exclusive-
   transfer direct entry against both its topology-equivalent snapshot control
   and the current serial-snapshot production policy for decisions, findings,
   time, root traffic, and RSS before any default change;
