...
> rendering failures. ADR-0247 preregisters v3 at that revision with rebuilt
> authority hashes and the unchanged positive/tcpip work bounds. Run all five
> v3 cells from one clean detached Axeyum commit next; do not reuse v2 cells or
> start symbolic memory before the corrected sweep is analyzed.
...
|---|---|---|
| **GQ1** | **Capture and profile real queries first** | **Four-driver map, query/internal attribution, fresh/retained exact-CNF controls, neutral cold-reset plus source-owner-retained SMT breadth, independent four-oracle fuzz, bounded/canonical/four-schedule raw authority cells, process-isolated corrected-representative end-to-end faithfulness, deadline-aware generated proof widening, isolated behavior-preserving Glaurung A0, corrected taint/SystemBuffer baselines, versioned confidence partition, source-backed nonzero positive control, rejected complete-usbprint resource cell, v2 maximum-precision rejection, and model-independent stack-region repair DONE; corrected v3 sweep PREREGISTERED (ADR-0187/0188/0197/0213--0247 plus Glaurung `7f682e5`).** The independent/edge campaign reaches 12,000/12,000 four-way agreements and 4,471/4,471 SAT replays with all 14 edge families nonvacuous; cvc5 agrees on all 9,526 real checks in both external protocols; 302 raw sinks match under unrestricted sole authority on four drivers. Tcpip prefix 15 reaches an exact 128-row deterministic-setting raw union, but all existing settings have zero high-confidence rows and the former 33 AnyModel-only remainder is diagnostic `Arg0`/`Arg1` ancestry. Usbprint's apparent five-versus-four population is invalid, and ADR-0244 further shows its AnyModel-complete boundary is not policy invariant: all four minimum processes hit the 300-second safety wall. ADR-0243 validates 14/14 planted rows. Minimum retains that exact set at 60,064 solves. ADR-0245's maximum cell retained all 14 but added one source-rejected stack label; ADR-0246 repairs the classifier and the exact maximum control returns to 14/14 with zero unexpected rows. ADR-0247 fixes the clean five-policy rerun before observation. All 74 UNSAT rows in the corrected five-driver representative recheck both CNF DRAT and stronger end-to-end certificates in two clean process-isolated runs, and all 1,505 generated CNF proofs recheck. Run and analyze v3, separately bound usbprint, and add wider real proof cells. |
...
| **GQ7** | **Cheaper warm entry and delta preprocessing** | **Source identity, fair map, query/internal attribution, fresh/retained exact-CNF controls, four-driver neutral cold-reset and source-owner-retained SMT, bounded raw finding parity, canonical tcpip authority, four-schedule union, isolated configurable-policy A0, corrected taint/SystemBuffer baselines, versioned confidence partition, 14-row source-backed positive control, v1 resource rejection, v2 maximum-precision rejection, and detector repair DONE; corrected sweep PREREGISTERED (ADR-0201--0205/0213--0247 plus Glaurung `7f682e5`).** ADR-0232 shows 16.4x--57.0x within-cvc5 retained/full-reset reductions while preserving the external textual boundary. A0's public `ConcretizationPolicy` covers both concretization seams and preserves default value selection. ADR-0243 validates 14 exact planted rows. ADR-0244 shows minimum retains them but costs 60,064 solves, preserves exact tcpip 110/110 at 80,563 solves, and cannot complete usbprint under 300 seconds. ADR-0245 exposes the maximum-policy stack-label bug; ADR-0246 repairs it with expression-DAG ancestry and accepts the exact 14-row maximum control. ADR-0247 fixes the full corrected rerun; site-hash cells remain unobserved until it executes. Publish raw, confidence-gated, validated, work, and cost partitions; never use raw `>= AnyModel` or producer confidence alone as a recall target. Gate symbolic memory on residual validated coverage headroom after v3, not on the repaired classifier artifact. The companion cross-repository Pareto strategy owns the downstream/SOTA analysis and sequencing. |
...
3. **Neutral baselines and oracles — four-driver cold-reset and source-owner-
   ...
   WIP:** cvc5 1.3.4 agrees on all 9,526 accepted checks with exact model-output
...
4. **Authoritative finding parity — bounded four-driver, first canonical
   ...
   ancestry and restores the exact maximum-policy 14-row set. ADR-0247 now
   preregisters all five executable cells (AnyModel, least, greatest, site-
   hash-zero, and site-hash-one) at Glaurung `7f682e5` with the same hard
   regression stratum and separately reported unlabeled real-driver discovery
   output. Run and analyze that exact v3 campaign next. Add
   BoundarySet/DiverseEnum only as
   ...
   work bounds are configuration; deferred symbolic memory starts only if the
   corrected sweep leaves validated coverage headroom, not from a detector
