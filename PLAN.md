...
The current execution ledger for stabilizing the 156 current math packs,
...
it records all 156 current non-template packs as focused-lesson linked, with no
...
122 concept rows, 156 non-template packs, 1014 expected checks (515 `sat`,
380 `unsat`, 119 `not-run`), 382 checked rows, 513 replay-only rows, 119
Lean-horizon rows, and 156 promoted solver-reuse packs. The rules/law JSON
...
coverage audit records all 156 non-template packs as focused-lesson linked,
...
the foundational resource dashboards now report **156 promoted solver-reuse
