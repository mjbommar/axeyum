...
>    ADR commit `c9fa897e` is pushed. The thin v4 wrapper/registration pass six
>    focused plus one inherited parser test, including wrong/duplicate/non-proof
...
| **Lean certificate coverage** | **Substantial but uneven** | All 35 measured rows have complete dominance-audit artifacts: 327 baseline UNSAT decisions become 325 evidence-audit UNSAT outcomes, 271 certified + genuinely independently checked outcomes, and 261 Lean-checked outcomes. The v1 audit's 28 vacuous `bare-unsat` check booleans are normalized by the generated [proof-gap matrix](docs/plan/generated/proof-gap-matrix.md); audit schema v2 now gates checking on certification and records decision backend/check mode. The full conjunction is 259/327: 54 uncertified audit-row occurrences, eight reconstruction-only gaps, four declared `bit-blast` trust holes, and two QF_NIA `IntPow2` proof-production errors. The parser-backed [shape census](docs/plan/generated/proof-gap-shape-census.md) deduplicates the 54 to 52 paths / 47 exact contents (25 arithmetic, 22 string/sequence), and finds three contents discharged before the ordinary parsed-IR DAG. The [causal design](docs/plan/evidence-route-provenance-design-2026-07-21.md) makes route/reduction tracing the next prerequisite, not a syntax-selected proof rule. The trusted-reduction ledger is **not yet zero**, and the Lean tactic backend (P3.7) is unbuilt. |
