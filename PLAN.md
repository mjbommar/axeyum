...
The current execution ledger for stabilizing the 130 current math packs,
...
it records all 130 current non-template packs as focused-lesson linked, with no
...
122 concept rows, 130 non-template packs, 850 expected checks (429 `sat`,
328 `unsat`, 93 `not-run`), 356 checked rows, 401 replay-only rows, 93
Lean-horizon rows, and 130 promoted solver-reuse packs. The rules/law JSON
...
coverage audit records all 130 non-template packs as focused-lesson linked,
...
The generated solver-reuse disposition audit now reports 130 promoted math
...
the foundational resource dashboards now report **130 promoted solver-reuse
