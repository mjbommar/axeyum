...
**Next:** keep the direct official-gcd computation route declined. Preregister one importer-only audit of the proof-free official extended-gcd identity `Nat.gcd_eq_gcd_ab` and the minimum `Nat.gcdA`/`Nat.gcdB` coefficient interfaces needed to turn it into the target-side balanced identity. Prefer that exact official-statement route over a target-owned gcd that cannot itself rewrite the open official target. If the extended-gcd closure is assumption-bearing, measure its exact carriers before authoring anything; if it is empty, preregister a balanced-coefficient adapter and reconstruct it twice. Preserve both accepted native support identities and the remaining two exact-target submissions. Do not inspect upstream proof bodies, import `WellFounded.Nat.fix_eq`, graft same-named native declarations, or publish a partial private kernel.
