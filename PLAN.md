...
| Date | Commit | Result |
|---|---|---|
| 2026-08-21 | `5f0a25d6b` | Strengthened the unconditional low-order cutoff from square-root scale to the largest power `Q` with `3 ceil(log2 ell) Q<=ell`, leaving only `O(log log ell)` high-Witt order bands per conductor. |
...
Exact character order now supplies a second, independent representation of
that signed structure.  For characters killed by `2^s`, finite-group
orthogonality identifies the annihilator with the power subgroup `2^s E_j`.
Two inclusion-exclusions therefore reconstruct every exact-order,
exact-conductor trace from four ordinary Mangoldt populations, with closed
character count `2^(j-floor(j/2^s))`.  The bounded class-space calculation
agrees order by order with the independent cyclotomic/Galois calculation at
both endpoints through level 12.  This exposes each layer as a difference of
nested sparse-coefficient imbalances rather than leaving it as an opaque
Ramanujan sum (ADR-0591).
The first polynomial-coefficient experiment is deliberately stronger than
needed.  The exact endpoint ledger proves that saving only a factor `4ell`
