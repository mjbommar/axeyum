...
> CNF, 880 solver, and 42 bench tests plus strict targeted Clippy pass. Commit
> this unobserved implementation, then execute the fixed clean detached run.
> Select no optimization until those counts name a material causal category.
...
| **GQ5** | **Cheaper AIG construction and measured CNF encoding** | **Cold engineering lane active; broad warm construction lane closed by ADR-0219; artifact-v35 detailed clause-construction attribution IMPLEMENTED but unobserved (ADR-0259).** Cold still assigns most Axeyum time to bit blast+CNF, but retained four-driver profiles remove 98--99% of per-check structure and leave only 11--20% whole-driver warm CNF. Existing phase timing and clause totals do not distinguish literal canonicalization from fingerprint-index ownership, while ADR-0200's direct map replacement regressed. Commit the tested implementation, run the fixed 162-query raw representative from that clean detached commit, and select no deterministic change before exact causal counts are known. |
