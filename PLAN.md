...
The first deep-page audit corrected the stale curriculum depth/backlog and
documentation-build gates. Continue only when a live guide exposes a concrete
stale claim.
...
| Public documentation and examples | `WIP`, user-directed lane | Beginner/user, WASM/browser, contributor, stable reference, internals, all 22 crate front doors, curriculum, reviews, plan, and research have substantive indexes; seven core crates have compile-tested rustdoc examples. Generated authorities remain canonical. The first deep-page audit corrected the curriculum depth/backlog and documentation-build gates; continue only from concrete live-linked drift. |
