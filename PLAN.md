...
The current execution ledger for stabilizing the 146 current math packs,
...
it records all 146 current non-template packs as focused-lesson linked, with no
...
Adams-Bashforth rows, BDF2 rows, Schur complements, real-Schur rows, and
complex real-pair resources.
...
122 concept rows, 146 non-template packs, 955 expected checks (486 `sat`,
360 `unsat`, 109 `not-run`), 372 checked rows, 474 replay-only rows, 109
Lean-horizon rows, and 146 promoted solver-reuse packs. The rules/law JSON
...
coverage audit records all 146 non-template packs as focused-lesson linked,
...
The generated solver-reuse disposition audit now reports 146 promoted math
...
the foundational resource dashboards now report **146 promoted solver-reuse
