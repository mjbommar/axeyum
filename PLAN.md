...
> ten population authorities, all twelve axes, all terminal paired cells,
...
> [result](docs/plan/lean-execution-process-tl0.7.2-2026-07-22.md) now closes
...
> the pushed
> [M2 plan](docs/plan/lean-u2-official-execution-tl0.6.3-m2-shard-0001-plan-2026-07-22.md)
...
> evidence; do not invoke `run-r4`. The source-first
...
2. **Deepen the quantified sat direction** (Gap 2): MBQI model-finding for the
   ...
   adjudication found 105 Z3-SAT Axeyum declines, 96 at the ordinary
   satisfiable-instantiation boundary. The implemented ADR-0359 bounded
   ...
   39-case oracle-SAT remainder is now measured: the strict 16-value/256-tuple
   one/two-free-Int policy checks 28 additional models. The earlier 33-case
   ...
   one recursion-guarded inner MBQI pass for the first ordered candidate, the
   shared outer deadline, no transfer of fixed-query UNSAT, and exact replay
   without the fixing. A preimplementation test corrected the initial inert
   post-sweep ordering: run the one inner pass after older routes decline, then
   continue ADR-0361's unchanged evaluated sweep if it fails. Seed 111 succeeds
   on that first candidate (`-5`); the other eight seeds receive no credit.
   Implement only this one-level, one-candidate boundary next.
