...
The current execution ledger for stabilizing the 127 current math packs,
...
it records all 127 current non-template packs as focused-lesson linked, with no
...
Schur complements, singular-value shadows, dynamics/Euler rows, and complex
real-pair resources.
...
122 concept rows, 127 non-template packs, 826 expected checks (414 `sat`,
322 `unsat`, 90 `not-run`), 353 checked rows, 383 replay-only rows, 90
Lean-horizon rows, and 127 promoted solver-reuse packs. The rules/law JSON
...
coverage audit records all 127 non-template packs as focused-lesson linked,
...
The generated solver-reuse disposition audit now reports 127 promoted math
...
the foundational resource dashboards now report **127 promoted solver-reuse
