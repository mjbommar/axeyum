...
**Status:** Exact official Lean 4.30 `Nat.fib_coprime_fib_succ` remains durably `proved` through dependency-bound receipt `34b9aad06fc8a640c81df0951b1af37a464f2d9305c048784e4f590b83ff0d0e`, and its sole newly ready child `F:ml430-nat-gcd-fib-add-self-5a92d5e3` remains open. Bottom-up Euclidean reconstruction retains the twice-reconstructed empty-footprint private quotient/remainder invariant and the local `Nat.sub_add_cancel` replacement. Public division, public gcd computation, and official xgcd coefficients remain declined under the zero-assumption policy. The first generic official-gcd balanced-Bézout source execution stopped at three compiler diagnostics with zero export/import/retry and exact baseline cleanup. Corrected V2 is now preregistered as a new increment: direct `Nat.mod` applications replace mismatched `%` elaboration, and two `congrArg` equalities restrict quotient transport to the intended `n * mn` and `n * mp` factors without changing the gcd remainder. The four-Nat carrier, explicit later-specialization parameters, forbidden quotient/xgcd dependencies, absolute pinned Lean 4.30 `lake` path, exact three-file `s5` baseline, two-compilation/one-export/two-import ceiling, and zero target/ledger authority are fixed before execution. No generic theorem, target specialization, cancellation, Fibonacci target, receipt, evaluation, fact, or ledger credit has yet followed.

**Next:** after publishing the V2 preregistration, execute it once on pinned `s5`: verify the exact baseline and absent temporary paths, compile the accepted support and corrected main source once each through the bound absolute `lake`, export the two selected roots once, audit them in two fresh Axeyum kernels, and clean exactly six paths. Accept only matching identities, empty footprints, and absence of every forbidden dependency. On acceptance, freeze evidence before separately preregistering checked target specialization with the existing empty-footprint `Nat.gcd_zero_left` and `Nat.gcd_succ`. On compilation or first-import failure, stop without retry. Do not modify V1 or inspect proof-bearing streams/pre-existing files.
