...
| **GQ5** | **Cheaper AIG construction and measured CNF encoding** | **Cold engineering lane active; broad warm construction lane closed by ADR-0219; artifact-v35 construction attribution ACCEPTED with no optimization (ADR-0259), and origin attribution preregistered (ADR-0260).** The fixed 162-query run finds 119,260 exact duplicates (30.4817% of non-taut attempts), all primary exact hits; collision, repeated-literal, and complementary-literal work is zero. Slice-partial/register-slice own 73.4572%/26.0230% of duplicates. Implement profile-only first-origin/duplicate-origin pairs, commit before observation, and repeat the exact population. Select at most one later generator-elision experiment under ADR-0260's preregistered dominance/distribution gate; counts alone are not a speedup. |
...
|---:|---|---|
| 1 | Cold CNF/bit-blast micro-optimization | ADR-0259 rejects another collision-index change and isolates exact duplicate generation: 119,260 duplicates, all primary hits, concentrated in slice-partial/register-slice. Execute ADR-0260's first-origin/duplicate-origin diagnostic before selecting one generator-elision candidate; accept it only on repeated unprofiled real-corpus end-to-end gains with identical decisions/replay. |
