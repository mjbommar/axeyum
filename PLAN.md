...
| Public documentation and examples | `WIP`, user-directed lane | Public, crate, consumer, prover, curriculum, and contributor front doors are indexed; all 34 Cargo examples and the consumer 48-case aggregate are guarded. Current corrections cover route-specific beginner UNSAT assurance, built-vs-planned prover status, aspirational north-star claims, landed foundation phases, partial TL2.15, selected Lean 4.30 construct admission versus offline-only TL2.10 quotient work, resolved symbolic-shift/high-assurance-UNSAT research questions, the bounded-string sound-incomplete/default-12/retry-48/cap-512 support boundary, P2.7's landed-portfolio-vs-unbounded-target split, checked-verdict-vs-exported-proof plus native-inline-vs-fallback-rederivation boundaries, the concrete `i128` Int/rational reference and LRA/Farkas fail-closed ranges, the landed-native-CDCL/default-BatSat plus route-scoped RUP-only DRAT-to-LRAT boundaries, and full-feature/nonzero-test command coverage across the proof cookbook and learner corpus. Generated authorities remain canonical; continue from concrete live-linked drift. |
