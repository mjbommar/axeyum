...
| Public documentation and examples | `WIP`, user-directed lane | Beginner/user, WASM/browser, contributor, stable reference, internals, all 22 crate front doors, consumer applications, the prover design, curriculum, reviews, plan, and research have substantive indexes; seven core crates have compile-tested rustdoc examples. All 34 Cargo examples are cataloged with prerequisites and mutation boundaries. The consumer 48-case aggregate and the public arithmetic snapshot are artifact-checked. The prover front door distinguishes landed P6.0 kernel prerequisites from the absent P6.1/P6.2/P6.3 bridge/goal/tactic layers. Beginner UNSAT diagrams distinguish alternative checked routes from the default BatSat-backed `Unchecked` proof status. The north-star pages separate aspirational destinations from current status and reflect the existing first-order quantifier IR. The foundational DAG now labels Phases 2–7 as landed contracts and routes live coverage to generated authorities. Generated authorities remain canonical; continue only from concrete live-linked drift. |
