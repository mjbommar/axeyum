...
- **Strings — first slice (2026-06-14, ADR-0025).** Bounded-length string theory
  ...
  **`str.replace_all`** now added — a single left-to-right pass carrying a `skip`
  ...
  set is now complete. **Regex extended** with `Plus`/`Opt`/`AnyChar`/`Loop`
...
- **Floating point — non-arithmetic core (2026-06-14, ADR-0023).** New
  ...
  **Symbolic `fp.add` done for F16/F32/F64** (validated bit-blaster): bounded
  adder — place the larger operand with `sb+2` guard bits, shift the smaller
  right by the exponent difference with a **sticky** bit, add/subtract (magnitude
  compare for equal exponents), `pack_value`, then NaN/`∞+−∞`/`∞`/signed-zero
  muxing. Borrow-clean because the sticky is nonzero only when `exp_diff > sb+2`
  ...
  **Symbolic `fp.fma` (fused multiply-add) done for F16/F32** (`fma`): the exact
  `a·b` product (`2·sb`-bit significand, unrounded) is aligned with `c` and summed
  exactly via the `add` scheme, then `pack_value` rounds *once*; intermediate
  width `3·sb+5` fits F16/F32 (F64 → `InvalidWidth`). Specials: NaN on any-NaN /
  `0·∞` / `∞−∞`; the zero-sign rule (`−0` only if product and `c` are both `−0`).
...
- **Int↔Real coercions (2026-06-14).** Parser constant-folds `to_real`/`to_int`/
  ...
  **Symbolic `to_int`/`is_int`** (`Op::RealToInt`/`Op::RealIsInt`) now handled by
  ...
  also exact (tests: `to_int(3.5)=3` sat, `to_int(r)≥5 ∧ r<5` unsat).
  **`to_real(i) op to_real(j) ⟺ i op j`** (both coerced ints) is rewritten
  exactly, and **`to_real` sums fold** (`to_real(a)±to_real(b) → to_real(a±b)`,
  the embedding being a ring homomorphism) so linear combinations of coerced ints
  vs a constant decide exactly (test: `to_real(x)+to_real(y)≤5.5 ∧ x+y≥6` unsat).
  A complete Nelson-Oppen combination (coercions against non-constant *real*
  operands) remains future.
