...
| 2026-08-20 | (open) | **NEXT, and it is a defect not a feature:** find what regressed `str.replace`-with-ground-`indexof` and `str.to_code` bounds from `unsat` to `unknown`. Both were decided on 2026-07-21. `r0_QF_SLIA_replace-find-base` needs `str.indexof("ABCDEF","C") = 2` to fold so `(str.replace "ABCDEF" "C" x)` becomes `(str.++ "AB" x "DEF")`; `r1_QF_SLIA_str-code-unsat-2` needs `(= (str.len x) 1)` to bound `str.to_code x` into `[0, 0x2FFFF]`. Bisect range is the string-route work between 2026-07-21 and now. |
