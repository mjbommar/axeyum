...
The current execution ledger for stabilizing the 165 current math packs,
...
it records all 165 current non-template packs as focused-lesson linked, with no
...
129 concept rows, 165 non-template packs, 1075 expected checks (549 `sat`,
398 `unsat`, 128 `not-run`), 391 checked rows, 556 replay-only rows, 128
Lean-horizon rows, and 165 promoted solver-reuse packs. The latest math pack
adds finite precision-recall classifier-ranking replay with exact score order,
threshold precision/recall/F1 arithmetic, precision-recall curve points,
average precision `34/45`, a checked bad-average-precision QF_LRA/Farkas row,
and explicit threshold policy, calibration, tie/interpolation convention,
confidence-interval, statistical-generalization, continuous-score, and
...
coverage audit records all 165 non-template packs as focused-lesson linked,
...
the foundational resource dashboards now report **165 promoted solver-reuse
