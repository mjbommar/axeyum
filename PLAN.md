...
| **GQ7** | **Cheaper warm entry and delta preprocessing** | **Source identity, fair map, query/internal attribution, fresh/retained exact-CNF controls, four-driver neutral cold-reset and source-owner-retained SMT, bounded raw finding parity, canonical tcpip authority, four-schedule union, isolated configurable-policy A0, corrected taint/SystemBuffer baselines, versioned confidence partition, 14-row source-backed positive control, and five-policy sweep preregistration DONE; sweep execution WIP and wider default DEFERRED (ADR-0201--0205/0213--0244 plus Glaurung `b79f269`).** ADR-0232 shows 16.4x--57.0x within-cvc5 retained/full-reset reductions while preserving the external textual boundary. A0's public `ConcretizationPolicy` covers both concretization seams and preserves default value selection. ADR-0241 rebaselines every existing tcpip setting at zero high-confidence findings and classifies the old 33-row AnyModel-only remainder as producer diagnostics. ADR-0242 likewise retires usbprint's five apparent rows as consequences of a symbolic I/O-manager-owned pointer; the corrected complete control is 0/0 high-confidence with equal work. ADR-0243 validates 14 exact planted rows across nine fixtures independently of producer labeling; every swept policy must retain 14/14 with no unexpected high row. ADR-0244 pins five executable scalar cells, exact identities/work, and unlabeled tcpip/usbprint discovery before execution. Publish raw, confidence-gated, validated, work, and cost partitions; never use raw `>= AnyModel` or producer confidence alone as a recall target, and keep policy-dependent real-driver output unlabeled absent source validation. Gate symbolic memory on residual validated coverage headroom. The companion cross-repository Pareto strategy owns the downstream/SOTA analysis and sequencing. |
...
4. **Authoritative finding parity — bounded four-driver, first canonical
   ...
   preregistration DONE; execution WIP (ADR-0229/0236/0238/0239/0240/0241/0242/0243/0244):** sole-authority Z3 and Axeyum
