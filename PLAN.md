...
| **GQ7** | **Cheaper warm entry and delta preprocessing** | **Source identity, fair map, query/internal attribution, fresh/retained exact-CNF controls, four-driver neutral cold-reset and source-owner-retained SMT, bounded raw finding parity, canonical tcpip authority, four-schedule union, isolated configurable-policy A0, corrected taint/SystemBuffer baselines, versioned confidence partition, 14-row source-backed positive control, v1 resource rejection, v2 maximum-precision rejection, detector repair, and corrected sweep DONE (ADR-0201--0205/0213--0247 plus Glaurung `7f682e5`).** ADR-0232 shows 16.4x--57.0x within-cvc5 retained/full-reset reductions while preserving the external textual boundary. A0's public `ConcretizationPolicy` covers both concretization seams and preserves default value selection. ADR-0247 completes all five scalar cells: every policy preserves 14/14, deterministic tcpip populations vary 84--110 with exact authority parity, and every tcpip row remains diagnostic. Site-hash-one's Axeyum tcpip cell costs about 264 seconds / 235 MiB, so broader enumeration is not free. Publish raw, confidence-gated, validated, work, and cost partitions; never use raw `>= AnyModel` or producer confidence alone as a recall target. Keep symbolic memory deferred because v3 exposes no validated residual gap. Next label a bounded real-driver difference population before selecting broader policy mechanics. The companion cross-repository Pareto strategy owns the downstream/SOTA analysis and sequencing. |
...
4. **Authoritative finding parity — bounded four-driver, first canonical
   ...
   deterministic counts vary 84--110. Independently adjudicate a bounded
   policy-difference sample before claiming coverage or selecting memory work. Add
