...
> unprofiled end-to-end timing gates. That candidate is now implemented but
> unobserved: a deterministic post-collection pass normalizes parity inputs and
> retains only the first identical key while preserving every occurrence's
> helper nodes. The recorded red test and all 305 CNF, 880 solver, and 43 bench
> tests plus strict affected-crate Clippy pass. Commit this boundary before the
...
| **GQ5** | **Cheaper AIG construction and measured CNF encoding** | **Cold engineering lane active; broad warm construction lane closed by ADR-0219; artifact-v35 attribution ACCEPTED without optimization (ADR-0259), artifact-v36 origin observation ACCEPTED (ADR-0260), and the sole follow-on candidate IMPLEMENTED but unobserved (ADR-0261).** V36 attributes 107,000 (89.7199%) to one same-owner positive-root AND-tree forward-parity cell across 29 queries. ADR-0261's bounded normalization/elision pass preserves helper bookkeeping and passes all affected local suites. Commit it before the fixed structural observation; require exact `-107,000` attempts/duplicates and `-214,000` attempted literals with unchanged final CNF, then accept only on the repeated unprofiled timing criteria. Counts alone are not a speedup. |
...
| **GQ7** | **Cheaper warm entry and delta preprocessing** | **Source identity, fair map, query/internal attribution, fresh/retained exact-CNF controls, four-driver neutral cold-reset and source-owner-retained SMT, bounded raw finding parity, canonical tcpip authority, four-schedule union, isolated configurable-policy A0, corrected taint/SystemBuffer baselines, versioned confidence partition, 14-row source-backed positive control, v1 resource rejection, v2 maximum-precision rejection, detector repair, corrected sweep, and exhaustive source-backed difference adjudication DONE (ADR-0201--0205/0213--0248 plus Glaurung `7f682e5`).** ADR-0232 shows 16.4x--57.0x within-cvc5 retained/full-reset reductions while preserving the external textual boundary. A0's public `ConcretizationPolicy` covers both concretization seams and preserves default value selection. ADR-0247 completes all five scalar cells; ADR-0248 then proves their 54 varying planted-fixture rows contain zero independent primitives. Site-hash-one's Axeyum tcpip cell costs about 264 seconds / 235 MiB, so broader enumeration is not free. Publish raw, confidence-gated, validated, work, and cost partitions; never use raw `>= AnyModel` or producer confidence alone as a recall target. Keep symbolic-address memory gated off pending a genuinely broader labeled residual gap. The companion cross-repository Pareto strategy owns the downstream/SOTA analysis and sequencing. |
...
|---:|---|---|
| 1 | Cold CNF/bit-blast micro-optimization | ADR-0261's sole selected private parity-leaf elision is implemented and locally green but unobserved. Commit it first, then require the fixed structural run's exact `-107,000` attempt/duplicate and `-214,000` attempted-literal deltas with byte-identical final CNF and all 162 gates. Only after that may the six-pair unprofiled timing protocol accept or reject it. |
