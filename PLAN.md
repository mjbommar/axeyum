...
**Next:** preregister one importer-only audit of the exact direct dependency closures of `Nat.div_eq` and `Nat.mod_eq`, using the already sealed bounded-induction stream and rendering no proof terms or theorem values. Determine whether the assumption is confined to the private quotient fuel-congruence theorem and the public-remainder wrapper bridge before authoring any replacement. Then reconstruct only the measured carriers from the already audited empty-footprint worker equations and rerun the unchanged primitive bounded-induction proof twice. Preserve both accepted native support identities and the remaining two exact-target submissions. Do not inspect upstream proof bodies, graft `Nat.div_mod_exec` by name, import either public equation proof as durable support, or publish a partial private kernel.
