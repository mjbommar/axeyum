...
> unprofiled end-to-end timing gates.
...
| **GQ5** | **Cheaper AIG construction and measured CNF encoding** | **Cold engineering lane active; broad warm construction lane closed by ADR-0219; artifact-v35 attribution ACCEPTED without optimization (ADR-0259), artifact-v36 origin observation ACCEPTED (ADR-0260), and one follow-on candidate PREREGISTERED (ADR-0261).** V35 finds 119,260 exact primary duplicates and zero collision/repeated/complement work. V36 attributes 107,000 (89.7199%) to one same-owner positive-root AND-tree forward-parity cell across 29 queries, with a 9.9738% largest-query share; it is the only cell passing the fixed rule. Implement only ADR-0261's private parity-leaf elision, preserve byte-identical final CNF and all 162 gates, and accept it only on the preregistered repeated unprofiled end-to-end timing criteria. Counts alone are not a speedup. |
...
|---:|---|---|
| 1 | Cold CNF/bit-blast micro-optimization | ADR-0260's fixed detached artifact-v36 observation accepts all 162 correctness/identity gates and selects exactly one distributed cell: 107,000 same-owner positive-root AND-tree forward-parity duplicates across 29 queries. Implement only ADR-0261's preregistered private parity-leaf elision; require its exact `-107,000` attempt/duplicate and `-214,000` attempted-literal deltas with byte-identical final CNF, then accept only on repeated unprofiled real-corpus end-to-end gains with identical decisions/replay. |
