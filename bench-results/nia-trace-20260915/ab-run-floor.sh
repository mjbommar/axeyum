#!/usr/bin/env bash
# Interleaved per-file A/B of ONE binary under TWO env values, for ADR-2112.
#
...
# ADR-2112 is a LEVER, so both arms are the same binary by construction and the
...
# like agreement.
#
#   A = AXEYUM_INT_BLAST_WIDTH_FLOOR=$ENV_A   (`0`, the shipped arm (floor disarmed))
#   B = AXEYUM_INT_BLAST_WIDTH_FLOOR=$ENV_B   (`1`, the floor armed)
#
...
if [ "$ENV_A" = "$ENV_B" ]; then
  echo "ABORT $TAG: both arms set AXEYUM_INT_BLAST_WIDTH_FLOOR=$ENV_A."
  echo "  Two identical arms make every number in this run vacuous while looking"
  echo "  exactly like agreement. Name two different lever values."
  exit 2
fi

HASH=$(sha256sum "$AX" | cut -d' ' -f1)

run_arm() {  # $1 = AXEYUM_INT_BLAST_WIDTH_FLOOR value
  local t0 t1 raw rc v
  t0=$(date +%s%N)
  raw=$(AXEYUM_INT_BLAST_WIDTH_FLOOR="$1" timeout $((BUDGET + HEADROOM)) taskset -c "$PIN" \
