...
- Per-instance evidence/Lean coverage is measured by
  ...
  quantified `25% (1/4)`, QF_ABV/cvc5+bitwuzla `97% (164/169)`,
  ...
  coverage for `issue9519` and `proj-issue321`. The refreshed artifact leaves
  only three cvc5-specific ABV `bare-unsat` rows, plus the current ABV
  `BvAbstraction` rows.
