#!/usr/bin/env bash
# One shard of the fair head-to-head: one box, one problem at a time, three
# solvers, interleaved per file, rotating who goes first.
#
# FAIRNESS: all three solvers run file N before any runs file N+1, on the same
# two pinned cores of the same idle homogeneous box, so ambient drift cancels
# in the difference.  The solver that goes first rotates per file so nobody
# systematically benefits from a cold or warm page cache.
#
...
AX=/nas3/data/axeyum/harness/dt-divisions/bin/smtcomp_cli
Z3=/usr/bin/z3
...
while read -r f; do
  ...
  printf '%s\t%s\t%s\t%s\t%s\n' \
    "$(basename "$f")" "${R[axeyum]}" "${R[z3]}" "${R[cvc5]}" "${st:-none}" >> "$OUT"
done < "$LIST"
