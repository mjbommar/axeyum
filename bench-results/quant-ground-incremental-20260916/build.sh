#!/usr/bin/env bash
# QUANT-GROUND-INCREMENTAL (ADR-2124) -- build this lane's binary in its own
# target dir and verify it is NEWER than every crates/**/*.rs before it may be
# used.  Cargo decides freshness by MTIME, so a build exiting 0 can hand back a
# binary from an earlier branch.  The `find -newer` check licenses the binary;
# the exit status does not.
set -eu
LANE_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
TAG="${1:-lane}"
TARGET=/data0/axeyum/quant-ground-incremental-target
OUT=/nas3/data/axeyum/harness/quant-ground-incremental/bin
