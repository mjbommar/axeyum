#!/usr/bin/env bash
# Interleaved per-file A/B of ONE BINARY under TWO ENVIRONMENT VALUES (ADR-2144).
#
#   A = the variable UNSET  (the canonical cache OFF, the shipped default)
#   B = AXEYUM_CANONICAL_CACHE=<value>  (default `on`)
#
...
#   extra B env: further `VAR=value` words applied to arm B only.
set -u
TAG="$1"; LIST="$2"; OUT="$3"; PIN="$4"; AX="$5"; BUDGET="${6:-24}"; VALUE="${7:-on}"; B_EXTRA="${8:-}"
HEADROOM=16
VLIM=$((8 * 1024 * 1024))
...
[ -n "${AXEYUM_CANONICAL_CACHE:-}" ] && {
  echo "ABORT $TAG: AXEYUM_CANONICAL_CACHE is set in the launching shell; arm A would not be the default"
  exit 2
}
...
run_arm() {  # $1 = "A" or "B"
  ...
  if [ "$1" = "B" ]; then
    raw=$(timeout $((BUDGET + HEADROOM)) taskset -c "$PIN" \
            env AXEYUM_CANONICAL_CACHE="$VALUE" $B_EXTRA \
            bash -c "ulimit -v $VLIM; exec \"\$0\" \"\$1\" --timeout-ms $((BUDGET * 1000))" \
            "$AX" "$f" 2>/dev/null)
  else
    raw=$(timeout $((BUDGET + HEADROOM)) taskset -c "$PIN" \
            env -u AXEYUM_CANONICAL_CACHE \
            bash -c "ulimit -v $VLIM; exec \"\$0\" \"\$1\" --timeout-ms $((BUDGET * 1000))" \
            "$AX" "$f" 2>/dev/null)
  fi
  rc=$?
...
echo "AB-DONE $TAG $n files -> $OUT  bin=$(sha256sum "$AX" | cut -d' ' -f1) B=AXEYUM_CANONICAL_CACHE=$VALUE $B_EXTRA"
