#!/usr/bin/env bash
# Interleaved per-file A/B of two `AXEYUM_QUANT_VALID_UNIVERSAL_RESERVE` arms.
#
...
#   <arm-env-value> is what the ARM sets AXEYUM_QUANT_VALID_UNIVERSAL_RESERVE to; the
...
run_arm() {  # $1 = "base" | "arm"
  ...
  t0=$(date +%s%N)
  if [ "$1" = base ]; then
    raw=$(env -u AXEYUM_QUANT_VALID_UNIVERSAL_RESERVE timeout $((BUDGET + HEADROOM)) taskset -c "$PIN" \
            bash -c "ulimit -v $VLIM; exec \"\$0\" \"\$1\" --timeout-ms $((BUDGET * 1000))" \
            "$AX" "$f" 2>/dev/null)
  else
    raw=$(AXEYUM_QUANT_VALID_UNIVERSAL_RESERVE="$ARM" timeout $((BUDGET + HEADROOM)) taskset -c "$PIN" \
            bash -c "ulimit -v $VLIM; exec \"\$0\" \"\$1\" --timeout-ms $((BUDGET * 1000))" \
            "$AX" "$f" 2>/dev/null)
  fi
  rc=$?
