#!/usr/bin/env bash
# ADR-2125: re-run every MOVED row three times per arm, on ONE pinned core pair,
# at the same 24 s / 8 GiB envelope.
#
# Adapted from `bench-results/lra-propagation-20260915/recheck-movers.sh`, which
# was itself adapted from ADR-2100's, and the ADAPTATION is where the guard
# lives. ADR-2100's refuses unless the two BINARIES hash differently, because
# two identical arms give a perfect zero that looks exactly like agreement.
# ADR-2122's arms were one binary under two env values, so that risk moved to
# "the binary never reads the variable". THIS lane proved the risk is real: its
# first mechanism check read `warm_cube_checks=0` in BOTH arms, because the warm
# engine was refused by a dense-cell cap on a sparse tableau. `ab-run.sh
# --mechanism-check` is where that is refused, and it must have been run before
# this script's output means anything -- which is stated here rather than
# silently inherited from the older guard's reassurance.
#
