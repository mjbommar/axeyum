...
OUT=/nas3/data/axeyum/harness/six-divisions/out/loadframe.tsv
...
while true; do
  ...
  grep -l 'SHARD-DONE' /nas3/data/axeyum/harness/six-divisions/out/*.log 2>/dev/null \
    | wc -l | grep -q '^12$' && exit 0
  sleep 30
done
