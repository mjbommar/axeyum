#!/usr/bin/env bash
# One shard of the fair head-to-head for the seven Tier-1 divisions.
# Protocol copied verbatim from
# bench-results/six-divisions-headtohead-20260912/shard-run.sh (lane BOARD-SIX,
# ADR-1950); only the axeyum binary path differs.
#
# FAIRNESS: all three solvers run file N before any runs file N+1, on the same
# pinned physical core (both SMT threads) of the same box, so ambient drift
# cancels in the difference.  The solver that goes first rotates per file so
# nobody systematically benefits from a cold or warm page cache.
#
...
# lane keyed by basename, and basenames COLLIDE across directories in these
# corpora, so a basename key silently merges two different benchmarks when the
# TSV is joined back to a list.
