#!/usr/bin/env bash
# QUANT-ACTIVATION -- build this lane's binary in its own target dir and verify
...
# from an earlier branch. The `find -newer` check licenses the binary; the exit
# status does not.
set -eu
LANE_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
TAG="${1:-lever}"
TARGET=/data0/axeyum/quant-activation-target-$TAG
OUT=/nas3/data/axeyum/harness/quant-activation/bin
