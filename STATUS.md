...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: correctness/deployability evidence, neutral warm controls, exact cold attribution, proof denominators, authoritative-policy gates, selected-pair symbolic-CVE recall, and negative harder-driver/duplicate-clause/storage results are complete through ADR-0285; ADR-0300's representation-neutral BTree telemetry and fail-closed v39 analyzer are complete, and the clean frozen baseline is next.** ADR-0272 rejects performance leadership because warm Bitwuzla wins all four fair drivers. ADR-0273--0275 retain the harder-driver census as incomplete-work negative evidence; ADR-0277 removes its structurally exact candidate after the frozen variance/family gates fail. ADR-0285's flat CNF arena preserves all correctness and exact construction identities and reaches a favorable 0.540824 aggregate logical-storage ratio, but fails its frozen per-instance <=80% gate on 5/162 payload-dominated singleton-clause rows. ADR-0300 isolates dense `TermId` memo indexing from the absent reported `Rc` prototype; its committed BTree artifact must pass exact structural/storage validation before the dense code exists, and only then can balanced client timing run. Broader labeled recall and honest correctness/deployability/proof framing remain open; no warm-speed headline or concretization/symbolic-memory reopening is authorized. |
