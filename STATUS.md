...
- **2026-07-20 — ADR-0299 preregisters checked MIR relational calls before
  implementation.** The selected route extends only the located typed scalar
  MIR path with the optimized checksum operations and assigned direct-call
  terminator. The same contract must verify independently against checked MIR
  and LLVM `sum16` bodies; MIR construction must additionally prove the callee
  panic predicate false before discarding its body and ignoring the call's
  `unwind continue` edge. Separate MIR/LLVM havoc symbols, relations,
  weak-contract countermodels, cross-IR modular proofs, and 100,000 valid plus
  100,000 violating choices per route are frozen. No implementation, general
  panic contract, annotation, loop/memory call, effect, or performance claim is
  accepted yet.
...
- **2026-07-20 — ADR-0299 preregisters checked MIR checksum composition.** The
  zero-row design rejects the legacy panicking executor and an LLVM-only
  certificate. It freezes typed scalar MIR operations/call syntax, independent
  MIR-body relation and panic-freedom proof, body discard, separate MIR/LLVM
  havoc choices, cross-IR modular equivalence, weak-contract teeth, and exact
  no-drop evidence before code. No implementation is claimed.
