...
- **2026-07-21 — SMT-LIB/API distance is now measured by command behavior, not
  helper presence.** The new machine-readable 29-row conformance manifest and
  ...
  evidence, and residual scope. The audited snapshot has five absent command
  ...
  generated drift and now runs in the parity/docs gates. Next: design the
  session event/result IR and transcript invariants before implementing it;
  retain full `reset`, recursive definitions, and textual categorical commands
  as explicit negative controls until those contracts exist.
...
| P4.4 | SMT-LIB command/API conformance | WIP — the checked 29-row manifest records 27 rows with exact named tests: 5 command families are absent, 7 are accepted/no-op parser paths, and 0 produce an ordered interactive textual-session result stream. Existing direct helpers already cover single-query solving, incremental solving, model/value/assignment/assertion/info/option/core/proof retrieval, scoped push/pop, `check-sat-assuming`, and `reset-assertions`; the next architectural task is one command-point session event/result contract and runner. Remaining language gaps include parametric sorts, recursive definitions, full-reset/option semantics, textual categorical adapters, and separately scoped general SyGuS. |
