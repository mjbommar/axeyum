...
- **2026-07-18 — ADR-0244 preregisters the corrected five-policy A0 sweep;
  results remain unobserved.** The machine protocol fixes Glaurung `b79f269`,
  both authority binary hashes, the five executable scalar policy cells in
  order, all 11 driver inputs, and three N=2 fixed-work strata. Every policy
  must retain ADR-0243's independently validated 14/14 positive control with
  no unexpected high row. Tcpip-prefix-15 and complete usbprint are separate
  unlabeled discovery populations; no direction or magnitude of variation is
  an outcome gate. The runner uses the preferred A0 selector for nondefault
  cells, preserves default AnyModel, refuses overwrite, and records exact
  execution identities. The analyzer rejects missing cells, identity/work/
  policy/coverage drift, unstable or non-disjoint confidence partitions,
  corrupt population/report hashes, positive misses, and missing cost data.
  Focused runner/analyzer/harness/validator tests pass 39/39 and all 11 input
  hashes resolve exactly. Next commit this unchanged protocol, execute it from
  that clean detached Axeyum revision against the clean isolated Glaurung
  branch, and preserve partial failure without adapting the matrix. The live
  dirty Glaurung checkout remains untouched; integration is separate owner
  coordination. BoundarySet/DiverseEnum and symbolic-memory gates are unchanged.
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Map, query/internal attribution, fresh/retained exact-CNF controls, four-driver neutral cold-reset and source-owner-retained SMT breadth, bounded/canonical/four-schedule raw authority cells, process-isolated corrected-representative end-to-end faithfulness, deadline-aware generated proof widening, isolated behavior-preserving Glaurung A0, taint/SystemBuffer baseline corrections, confidence partition, source-backed nonzero positive control, and corrected five-policy sweep preregistration DONE (ADR-0187/0188/0213--0244 plus Glaurung `b79f269`).** cvc5 agrees on all 9,526 checks in both external protocols; 302 raw sinks match under unrestricted sole authority on four drivers. Tcpip's five existing settings all have zero high-confidence rows, and the former 33 AnyModel-only remainder is diagnostic `Arg0`/`Arg1` ancestry. Usbprint's five apparent rows are also producer-model false positives; its corrected complete N=2 cells have 0/0 accepted rows, 214 diagnostics, and 16,537 solves per process. ADR-0243 separately accepts 14/14 source-backed planted rows across nine fixtures with zero misses/unexpected high rows; this is a positive control, not real-driver recall. ADR-0244 fixes all five executable cells and three strata before results. All 74 UNSAT rows in the corrected five-driver representative recheck both CNF DRAT and stronger end-to-end certificates in two clean process-isolated runs, and a 1 ms kill control retains all 74 rows; all 1,505 generated CNF proofs recheck, with 1,487/1,505 stronger generated certificates under policy; BatSat beats Z3 Boolean on retained Axeyum CNF despite native warm Z3 winning end-to-end | Run ADR-0244 unchanged from a clean detached commit; coordinate corrected A0 integration separately; add wider real proof cells |
  ...
  | **GQ7 warm delta entry** | **Source identity, fair map, query/internal attribution, fresh/retained CNF controls, four-driver neutral cold-reset plus source-owner-retained SMT, bounded raw finding parity, canonical tcpip authority, four-schedule union, isolated configurable-policy A0, taint/SystemBuffer baseline corrections, confidence partition, 14-row source-backed positive control, and five-policy sweep preregistration DONE; execution WIP, wider default DEFERRED (ADR-0201--0244 plus Glaurung `b79f269`).** Neutral same-protocol retention is 16.4x--57.0x below full reset; A0 preserves default value selection. Existing tcpip settings and corrected usbprint have zero accepted rows; the separate planted control is 14/14 under both authorities. ADR-0244 pins the exact scalar-policy matrix and claim limits | Run ADR-0244 unchanged; coordinate corrected A0 integration separately; gate symbolic memory on residual validated headroom |
  ...
  **Next actions:** (1) coordinate with the active Glaurung explorer owner and
  integrate isolated branch `axeyum-concretization-policy-a0` at `b79f269`
  ...
  and producer confidence is not ground truth; (3) run ADR-0244's preregistered
  initial sweep across the five executable AnyModel, least/greatest, and two
  site-hash configurations unchanged from its clean detached commit; extend the same sweep with BoundarySet/DiverseEnum
  only after bounded multi-successor execution is implemented, keeping
  ...
  unlabeled discovery output until separately validated, and begin symbolic memory only if this sweep
  leaves validated coverage headroom; (4) after
  ADR-0235's completed process-isolated representative real-query denominator,
  add wider corrected real manifests; (5) widen
  ADR-0237's accepted four-oracle/edge-frequency gate as a standing bounded
  correctness control; (6) keep each subprocess/FFI boundary named; (7)
  preserve ADR-0236's rejected any-model
  divergence beside every canonical result and never treat canonical output as
  the union of reachable findings; (8) regenerate a wider timeout-sensitive driver and keep each timeout
  cell separate; (9) advance GQ5 through fresh causal CNF attribution while
  preserving explicit controls, exact counter partitions, and honest residual
  `Unknown`s; (10) stage solver namespace/module, duplicate-removal, and
  typed-config refactors only as bounded behavior-preserving tranches; (11) keep
  GQ4 explicit/off.
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver fair map, internal/fresh/retained controls, neutral cold/retained SMT, complete three-solver timeout frontier, independent four-oracle fuzz, proof denominators, executable WebAssembly, warm time/RSS/hit-rate deployment, bounded/canonical/four-schedule authority cells, representative whole-certificate process isolation, isolated behavior-preserving Glaurung A0, corrected provenance/SystemBuffer environment, versioned confidence partition, source-backed nonzero positive control, and corrected five-policy sweep preregistration DONE; sweep execution open (ADR-0213--0244 plus Glaurung `b79f269`).** The 12,000-row independent/edge campaign has 12,000 four-way agreements, 4,471/4,471 SAT replays, 14/14 semantic-corner families, and zero nondecision/failure/disagreement under its correctness-only resource bound. The 52-formula 50--1000 ms sweep has complete four-population accounting, zero three-solver contradiction, and an all-decided cold Axeyum/Z3 geomean of 0.21095 at 1000 ms; it remains separate from the workload-dependent retained-warm map. All 1,505 width<=8 generated UNSAT rows recheck CNF DRAT and 1,487 recheck end to end; all 74 UNSAT rows in the corrected 162-query real representative recheck both CNF DRAT and stronger end-to-end certificates in two clean process-isolated runs, while a 1 ms kill control retains all 74 rows; 302 raw sinks match under unrestricted sole authority on four drivers. Tcpip's existing settings and corrected usbprint remain zero-positive after producer-model corrections. ADR-0243 separately accepts all 14 source-backed planted rows under both authorities as a regression control, not real-world recall; ADR-0244 fixes the exact five-policy/three-stratum campaign before observation. Browser and high-reuse memory costs are measured. Remaining: run ADR-0244 unchanged from a clean detached commit, coordinate clean A0 integration separately, and add wider real manifests. |
