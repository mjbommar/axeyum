...
- **2026-07-18 — ADR-0242 retires usbprint's apparent nonzero target after an
  ...
  not a recall denominator; select and independently validate a new nonzero
  population before preregistering the A0 configuration sweep. The exact
  artifact is
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ7 warm delta entry** | **Source identity, fair map, query/internal attribution, fresh/retained CNF controls, four-driver neutral cold-reset plus source-owner-retained SMT, bounded raw finding parity, canonical tcpip authority, four-schedule union, isolated configurable-policy A0, taint/SystemBuffer baseline corrections, and confidence partition DONE; validated-target selection WIP, labeled policy sweep gated, wider default DEFERRED (ADR-0201--0242 plus Glaurung `b79f269`).** Neutral same-protocol retention is 16.4x--57.0x below full reset; A0 preserves default value selection. Every existing tcpip setting has zero accepted findings, the old 33-row remainder is producer-diagnostic, and corrected usbprint also has zero accepted rows | Integrate the corrected A0 branch; select an independently validated nonzero target before sweeping deterministic policy configs; gate symbolic memory on residual validated headroom |
  ...
  **Next actions:** (1) coordinate with the active Glaurung explorer owner and
  ...
  select a new candidate population, correct any WDF/KMDF environment-model
  assumptions it exercises, and independently validate a nonzero finding set;
  tcpip and corrected usbprint both have zero accepted rows, and producer
  confidence is not ground truth; (3) only then preregister the policy sweep across AnyModel,
  ...
  **Validation (2026-07-18, ADR-0242):** the reduced guarded-SystemBuffer
  ...
  documentation links and diff hygiene are the remaining Axeyum commit gates.
