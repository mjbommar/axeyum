...
- **2026-07-19 — ADR-0263 accepts symbolic-CVE corpus qualification before any
  detector execution.** The exact 22-row Linux IOCTL census and
  ...
  hash, changed file, and parent handler. Seven tests began red and pass. The
  committed run accepts all 22 rows and the exact 6/16 partition; only two of
  the six candidates have `handler_found=true` in the separate census result.
  Candidate is not executable or detected; no recall claim exists yet. Next,
  bind vulnerable/fixed artifacts and prove frontend executability row by row.
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **Next actions:** first materialize hash-bound vulnerable/fixed artifacts for
  ADR-0263's six candidates and preregister the frontend-executability gate;
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver fair map, internal/fresh/retained controls, neutral cold/retained SMT, complete three-solver timeout frontier, independent four-oracle fuzz, proof denominators including the accepted 1,024-query holdout, executable WebAssembly, warm time/RSS/hit-rate deployment, bounded/canonical/four-schedule plus wider timeout-sensitive authority cells, representative whole-certificate process isolation, isolated behavior-preserving Glaurung A0, corrected provenance/SystemBuffer environment, versioned confidence partition, source-backed nonzero positive control, two rejected usbprint resource protocols, v2 maximum-precision rejection, model-independent detector correction, corrected v3 scalar sweep, exhaustive source-backed difference adjudication, v6 hidden-work rejection and execution, one neutral standard DRAT consumer plus checker-sanity control, capped nontrivial-proof no-selection, the accepted 22-row symbolic-CVE qualification, and the closed artifact-v35/v36 cold CNF origin lane DONE without optimization (ADR-0213--0263 plus Glaurung `7f682e5`; campaign revision `ff3c0a7`).** ADR-0233 closes neutral timeout-sensitive formula breadth. ADR-0253 adds 1,024/1,024 manifest/Z3 agreements, 515 SAT replays, 509 CNF DRAT rechecks, and stable 508/509 stronger coverage. ADR-0256 accepts one pinned real external verification and the preregistered satisfiable-CNF control while ADR-0258 closes further holdout mining. ADR-0259/0260 attribute 119,260 duplicates; ADR-0261's sole selected leaf-elision candidate changes the structural counters by exactly zero and is rejected before timing. ADR-0262 accepts all six first-20 v6 authority cells: timeout is a no-op, AnyModel remains raw-divergent, and LeastUnsigned reaches exact parity at substantial cost. ADR-0263 qualifies six current-fragment candidates from 22 CVEs but makes no executability or detection claim. Remaining: materialize and execute paired artifacts for those candidates, add an in-process warm Bitwuzla baseline, and defer A1 work bounds to the harder-driver tier. |
...
- **2026-07-19 — ADR-0263 accepts symbolic-CVE corpus qualification.** The
  committed validator accepts all 22 source-hash-bound census rows, unique
  ...
  detector scoring, and the eventual recall artifact remain open.
