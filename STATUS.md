...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Repeated native policy timing DONE; phase partition WIP (ADR-0171).** Native lineage runs 20,958 repeated checks at weighted 0.746x Z3 with 0.36% ratio CV; snapshot is 2.093x | Capture exact native hashes and use `check_profiled` to partition translation, first-prefix construction, delta assertion, model, SAT, and caller time against the pre-parsed bench |
  ...
  **Next actions:** (1) bound native lineage live sessions, inherited-prefix
  construction, memory, and deterministic fallback/eviction while retaining
  snapshot and one-shot fixed controls; (2) add exact native hashes plus
  `check_profiled` phase attribution for translation, assertion, model, SAT,
  and caller cost, paired to raw `axeyum-bench`; (3) widen ordered/native driver
  families and add per-commit full-tier variance; (4) only then specify GQ8's
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the cold GQ10 gate and exact v2→v3→v4 transitions; native/warm widening WIP.** ADR-0155's full cold result is 0.730x Z3. ADR-0160--0163 expose and improve native phases/CNF. ADR-0166--0170 establish ordered controls; ADR-0171 adds repeated live comparison: lineage 0.746x Z3 versus snapshot 2.093x, 0.36% ratio CV, all 41,916 combined checks green. Remaining: exact native phase/hash reconciliation, bounded memory lifecycle, wider drivers/full variance, and per-commit user-visible ratios. |
