...
- **2026-07-14 — Glaurung's `slice-partial` affine-add reduction is accepted;
  ...
  These values validate the schema but do not select the next optimization.
  Next: commit the v28 boundary, run one clean full process, and aggregate the
  exact post-word inventory with family/stage cost before opening another ADR.
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **DONE for the current cold timing boundary; operator refinement active.** Artifact v27 representative raw/canonical p50 ratios are 1.65x/1.37x; full single trials are 3.17x/2.71x. Artifact v28 adds complete original/post-word operator inventories and passes the representative gate | Run one clean full v28 process for the post-v3 family/operator residual; repeat full trials when accepting an optimization |
  | **GQ2 cheap cold tier** | **WIP with two accepted tranches.** Canonical v2 cuts corrected full total 13.3%; ADR-0153's v3 add-chain fold then cuts full total 9.80%, clauses 17.23%, and `slice-partial` time 24.4% | Use the full v28 post-word inventory before adding another word rule; it must again reduce downstream construction and total time |
  | **GQ3 coercion/affine peepholes** | **WIP with two production wins.** ADR-0142 lands exact slice/coercion rules; ADR-0153 lands exact scalar/wide modular add-chain folding with identity projection | Use the full v28 residual before considering subtraction or a broader affine normal form |
  ...
  | **GQ5 AIG/CNF construction** | **ACTIVE with four direct wins plus ADR-0153's upstream reduction.** Current canonical is 14.11 s / 1.85x Z3 with 40.72M clauses, down 17.23% from v2 | Correlate full v28 post-word operators with family construction cost before another cold experiment |
  ...
  **Next actions:** (1) hand the ordered worker/path/scope trace v1 and
  ...
  residual with one clean full artifact-v28 process before another cold
  experiment, keeping broad GQ4, SAT, and further memo work gated pending
  measured opportunity; (3) fix Glaurung's explicit width
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the generic/cold GQ10 gates and first rewrite-transition comparison; WIP for producer repair and post-v3 attribution.** Artifact v27 provides fixed identities/resources, in-process Z3, decided-rate/original-replay gates, and regular representative plus scheduled full tiers; artifact v28 adds complete original/post-word scalar QF_BV operator inventories. Five baseline full trials establish 0.51% total/ratio CV and 3%/3%/2% alarms; ADR-0153's exact v2→v3 additive comparator passes them and reaches 14.11 s / 1.85x. Remaining: one clean full v28 residual profile, malformed-dump/dedup repair, and an ordered trace gate. Synthetic tiers cannot substitute for the real lifter |
