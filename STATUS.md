...
- **Session 2026-06-26 — post-candidate unary-Int UF sibling scheduling.**
  Lazy function-consistency CEGAR now tracks `sibling_lemmas` and, after a
  candidate exposes a real violated equal-argument UF pair, schedules at most
  one additional valid Ackermann lemma between the same unary-Int dynamic
  application and a sibling constant application from that function group. The
  key constraint is timing: this is post-candidate scheduling, not pre-seeding,
  so it does not perturb the first arithmetic solve that must produce the first
  useful UF candidate.

  Wider caps were measured and rejected before commit: cap **16** dropped the
  10 s hard row to **3** UF rounds / **2** candidates, and cap **4** to **4**
  UF rounds / **3** candidates. The committed cap **1** preserves the useful
  frontier. On `cli__regress2__uflia-error0.smt2`, the 1 s run remains
  `unknown` with **2** UF rounds, **1** candidate, **282** pair checks,
  **6** equal-argument pairs, **5** violations, **sibling_lemmas=1**, and
  **lemmas_added=7**. At 10 s the row remains `unknown` but preserves **6** UF
  rounds and **5** candidates, with **1352** pair checks,
  **22** equal-argument pairs, **15** violations, **sibling_lemmas=5**,
  **lemmas_added=27**, **total_rounds=280**, **blocking_lemmas=295**,
  **core_src_affine=45**, **core_src_lp=204**, and **core_len_avg=6.4**. Direct
