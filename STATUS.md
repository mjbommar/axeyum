...
- **2026-07-19 — ADR-0282 freezes typed LLVM CFG syntax and validation before
  implementation.** The next T5.1.2 slice gives `phi`, `br`, `switch`, `ret`,
  and `unreachable` explicit span-carrying types; preserves true/false/default/
  case roles; and validates terminator placement, label resolution, entry
  predecessor exclusion, switch constants, and exact PHI predecessor sets.
  Real clang/rustc diamonds, every control shape, malformed graph classes,
  legacy CFG fixtures, and deterministic noise are frozen gates. This slice
  deliberately does not execute graphs or inherit the compatibility executor's
  unreachable-arm dropping. Checked acyclic value+definedness execution is the
  following increment; cycles, memory, and Glaurung lowering remain deferred.
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: fair six-cell in-process regime map, external neutral controls, timeout frontier, independent four-oracle fuzz, proof denominators, executable WebAssembly, warm time/RSS/hit-rate deployment, authoritative policy controls, process-isolated certificates, behavior-preserving Glaurung A0, corrected provenance/SystemBuffer findings, source-backed positive controls, external DRAT consumption, cold-CNF attribution, symbolic-CVE artifact attrition/frontend routing, and bounded paired AArch64 recall DONE without overclaiming (ADR-0213--0272 plus Glaurung `2961d7c`; campaign revision `ff3c0a7`).** ADR-0272 closes the in-process neutral blocker with 64,510 repeated occurrences, complete six-way parity, zero fallback, and four accepted gates. Warm Axeyum beats warm Z3 on three drivers and loses on Dptf; warm Bitwuzla beats both on all four, so performance leadership is explicitly rejected. Remaining: preregister deterministic harder-driver work bounds, broaden independently labeled recall only as artifacts become admissible, and keep the paper centered on correctness, deployability, proofs, and reproducibility. |

### Track 5 — Verified Systems (IR reflection) — ADR-0056, adopted 2026-07-06
| Phase | Title | Status |
|---|---|---|
| P5.1 | Reflection front end (crate-ify the MIR+LLVM reflectors, full `.ll` parser, MIR extraction pipeline, loops→`TransitionSystem`, memory beyond byte arrays) | WIP — **T5.1.1 DONE (`cc695925`, ADR-0057)**: the reflectors are now the real library module `axeyum_verify::reflect` (`src/reflect/{mod,mir,llvm}.rs`, submodules `reflect::mir`/`reflect::llvm`), no longer per-test scaffolding — 8 test binaries (62 tests) rewired to `use axeyum_verify::reflect::…` and green, `missing_docs`+`implicit_hasher` API-hardened, clippy/rustdoc `-D warnings` clean; the crate split is deferred (one consumer today). The prototyped *capability* (rounds Q–U, design log `docs/consumer-track/verify/reflect-common-abstraction.md`): CFG symbolic executors for both IRs over one shared op vocabulary; 16 cross-IR equivalence proofs (MIR≡LLVM per function, LLVM O0≡O2, if-conversion/strength-reduction/umin-idiom validated, hypothesis-gated `unreachable`); 5-shape wrong-transform refutation corpus with replay-checked countermodels; exact panic specs from rustc's own checks (overflow, division `b==0` / signed `∨ (a==MIN ∧ b==-1)`, bounds over all 2^64 indices) with `catch_unwind` witness replay; checksum micro-module end-to-end on both platforms. **T5.1.2 WIP (ADR-0279--0282):** the first accepted slice replaces parameter line splitting with a non-panicking structured function boundary; the second accepts typed scalar instructions and fail-closed value+definedness reflection; the third now freezes typed PHIs/terminators and exact graph validation before checked acyclic execution. Memory and module-wide semantics remain later slices; no crate split yet. Remaining T5.1.3–6: build-time MIR extraction, automatic loop bridging, `gep`/`load`/`store` + array writes, and the semantics gate. Individual proofs are milliseconds — the suites already run as ordinary per-commit tests |
