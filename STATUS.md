...
- **2026-07-15 — ADR-0167 accepts opt-in ordered per-lineage T3 replay; T4
  controls are next.** `glaurung-ordered-trace --warm` builds one deterministic
  shared arena, retains a distinct `IncrementalBvSolver` per live path, and
  maps ordered push/assert/pop/check events to that owner. A fork must match its
  live parent's exact constraint sequence, then replays the validated prefix
  into a fresh child solver; mutable SAT state is never shared or cloned. A
  check fails closed on scope/depth mismatch or any active assertion whose bytes
  never entered the exact query store.

  The bounded development trace is 784/784 warm-agreed (473 SAT, 311 UNSAT),
  with original-assertion replay on every SAT candidate. All 243 model reads
  evaluate: 242 match the recorded value and one exposes a legitimate
  alternative model. Thirteen never-checked terminal assertions are explicitly
  unmaterialized; none reaches a fork prefix or check. The initial strategy
  creates 232 child solvers and replays 7,378 inherited roots, costing about
  813 ms of the 1.249 s warm pass; check latency is about 0.322/0.672 ms
  p50/p95. This accepts functionality, not a speed/default claim. Next compare
  cold occurrence replay, ADR-0164 snapshot inference, and explicit lineage on
  identical bytes; add complete assertion capture, peak RSS/resource identity,
  and clean multi-driver break-even before GQ8/GQ9.
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Native instrumentation DONE; clean multi-driver publication WIP (ADR-0160).** The first 13,126-query ordered driver run is fully decided/agreed and separates all native/client phases | Repeat on clean multi-driver processes; pair same-revision exact hashes with `axeyum-bench`, retain order/duplicates and fixed experiment identity |
  ...
  | **GQ7 warm delta entry** | **WIP; ADR-0164 bridge, ADR-0166 T1/T2, and ADR-0167 opt-in T3 accepted.** Explicit lineage replay owns one solver per live path, agrees on all 784 checks, and evaluates all 243 choices; naive fork replay spends ~813 ms on 7,378 inherited roots | Run cold-occurrence/snapshot/lineage controls on identical bytes; add complete assertion capture and RSS/resource identity; then repeat cleanly across drivers with p50/p95/break-even before default admission |
  | **GQ8 verdict/CNF cache** | **TODO; ordered opportunity now measured, implementation still GQ7-gated.** The bounded trace has 276/784 exact duplicate occurrences (35.2%), 156 same-lineage repeats, and 271 prefix extensions | Complete retained per-lineage warm replay first; then evaluate bounded versioned exact-query reuse with mandatory model/proof replay rather than inferring cache value from frequency alone |
  ...
  | **GQ10 real-lifter regression tier** | **WIP; cold gate plus bounded ordered T1/T2 landed.** The ordered sample validates exact occurrences, lineage, scopes, and choices, but was captured from a development worktree on one driver | Publish clean ordered multi-driver traces with deterministic resource/memory identity, then track cold and warm per-commit family/stage/Axeyum÷Z3 trends separately |

  **Next actions:** (1) compare ADR-0167 explicit lineage, ADR-0164 snapshot
  inference, and a cold occurrence-by-occurrence control on identical trace
  bytes, with p50/p95/RSS/resource identity and fork replay attribution; (2)
  capture every asserted term and publish clean ordered traces, then repeat
  ADR-0160's native
  cold and ADR-0164 warm profiles cleanly across
  the multi-driver set and pair same-revision exact hashes with raw
  `axeyum-bench`; (3) attribute the remaining real-client/bench entry gap and
  AIG construction cost per node; (4) expand the cold and ordered captures
  separately and retain strict coercion,
...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | **WIP — highest-leverage Glaurung item; ADR-0164 bridge through ADR-0168 T4 controls accepted.** Separate identical-occurrence processes remain 784/784 agreed: cold 2.737 s, snapshot 0.545 s, and naive lineage 1.371 s. Snapshot uses 38.4 MB high-water RSS versus lineage's 83.9 MB and avoids 7,378 fork-root replays. Repeat/harden snapshot next; complete assertion capture, per-backend timing, and clean multi-driver p50/p95/RSS/break-even still precede GQ8/GQ9 decisions. |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the cold GQ10 gate and exact v2→v3→v4 transitions; native/warm publication WIP.** ADR-0155's full cold result is 5.625/7.709 s (0.730x). ADR-0160 exposes native phases/duplicates; ADR-0162/0163 pass paired cold-native gates. ADR-0164 cuts one native snapshot ratio 2.648x→1.462x; ADR-0166/0167 validate ordered lineage and explicit replay; ADR-0168's identical-byte controls select snapshot over cold and naive lineage. The trace timer includes both shadow backends, so it is not a Z3 bar. Remaining: producer-separated backend time, clean multi-driver p50/p95/RSS, wider drivers/full variance, and per-commit user-visible ratio. |
