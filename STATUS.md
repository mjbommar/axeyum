...
- **2026-07-19 — ADR-0259's preregistered cold GQ5 attribution is implemented
  but deliberately unobserved.** Artifact v35 adds a zero-sized ordinary CNF
  profiler and a separately selected counter-carrying monomorph, threads the
  opt-in through `SolverConfig` and typed layer stats, binds it into benchmark
  identity, and publishes exact per-instance/corpus partitions. A separate
  analyzer re-sums every row, fails on incomplete decision/oracle/replay
  populations or any of six invariants, and emits exact family counters. The
  302 CNF, 880 solver, and 42 bench tests, four analyzer tests, and strict
  all-target/all-feature targeted Clippy pass. Next: commit this unobserved
  boundary, then run one clean detached release process over the fixed 162-query
  corrected-wide-v3 representative and select no optimization in ADR-0259.
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ5 AIG/CNF construction** | **Cold lane active; broad warm lane closed by ADR-0219; artifact-v35 detailed clause-construction profile IMPLEMENTED but unobserved in ADR-0259.** Retention removes 98--99% of cold structure and leaves 11--20% warm CNF; ADR-0200 rejected an unattributed map replacement | Commit the tested production-zero-overhead profile, execute the fixed 162-query raw representative from that detached commit, and select no change before exact canonicalization/index ownership is known |
...
- **2026-07-19 — ADR-0259 implements cold CNF construction attribution before
  observation.** Artifact v35 keeps ordinary profiling storage zero-sized,
  selects a separate detailed monomorph, exposes typed/per-instance/corpus
  counters, and adds an independently re-summing family analyzer. All focused
  suites and strict targeted Clippy pass. Commit this boundary before the fixed
  162-query run; no GQ5 optimization is selected yet.
