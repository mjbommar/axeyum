...
- **2026-07-17 — The consolidated code/publication review changes the success
  ...
  persistent clause-stream control, and ADR-0222 supplies the first neutral
  Dptf cold-reset external SMT point. Publication blockers are now: (1) wider
  neutral replay plus a topology-equivalent warm representation/integration
  control; (2) a standing well-typed
  multi-oracle fuzzer seeded by empty-model, extension-width, concat-width, and
  W128 truncation failures; (3) broader cvc5/Bitwuzla correctness and performance
...
- **2026-07-17 — ADR-0213 resets the Glaurung paper-evidence boundary without
  ...
  mechanisms; ADR-0222 adds the first neutral cold-reset point, while
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Map, query/internal attribution, fresh/retained exact-CNF controls, and first neutral cold-reset SMT point DONE (ADR-0187/0188/0213--0222).** cvc5 agrees on all 561 Dptf checks; BatSat beats Z3 Boolean on retained Axeyum CNF despite native warm Z3 winning end-to-end | Widen neutral replay, then add warm-neutral, timeout-sensitive, and finding-authoritative cells |
  ...
  | **GQ7 warm delta entry** | **Source identity, fair map, query/internal attribution, fresh/retained CNF controls, and neutral cold-reset SMT DONE; wider default DEFERRED (ADR-0201--0222).** The remaining reversal boundary is warm word-level representation/integration | Keep direct opt-in; widen neutral replay, then add a topology-equivalent neutral cell before harder/finding gates |
  ...
  **Next actions:** (1) widen ADR-0222's neutral cold-reset SMT replay across
  vwififlt, IntcSST, and SurfacePen, then add a neutral in-process or
  topology-equivalent warm cell for the representation/integration boundary;
  (2) measure
  ...
  keep each subprocess/FFI boundary named; (5) run
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver map, query/internal attribution, fresh/retained exact-CNF controls, and first neutral cold-reset SMT point DONE; correctness-led evidence open (ADR-0213--0222).** Five fixed-work repetitions establish two wins, parity, and a Z3 win; 9,526 rows reject size alone; 431 ordered core calls show retained BatSat beating Z3 Boolean on Axeyum CNF; cvc5 agrees on all 561 Dptf checks. Remaining: wider/warm-neutral SMT; multi-oracle fuzzing; finding parity; timeout-sensitive driver; measured qfbv/WASM/RSS/DRAT deployment. |
