...
- **2026-07-17 — The consolidated code/publication review changes the success
  ...
  (1) matched persistent clause-stream/learned-state control after ADR-0220's
  fresh exact-CNF reversal, reporting only a supported boundary; (2)
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Map, query/internal attribution, and fresh exact-CNF parity DONE; retained-state control WIP (ADR-0187/0188/0213--0220).** Fresh Z3 loses on the same CNF despite winning warm end-to-end | Replay the ordered clause stream through matched persistent cores, then add neutral end-to-end, timeout-sensitive, and finding-authoritative cells |
  ...
  | **GQ6 cold SAT/CDCL** | **Fresh exact-CNF control DONE; retained-core control leading (ADR-0220).** Proof core beats BatSat before checking; fresh Z3 loses | Replay the ordered persistent clause stream with proof and deterministic limits; do not swap cores from the fresh slice |
  | **GQ7 warm delta entry** | **Source identity, fair map, query/internal attribution, and fresh CNF parity DONE; wider default DEFERRED (ADR-0201--0220).** Fresh/warm reversal selects learned state/topology | Keep direct opt-in; run matched persistent control, then neutral/harder/finding gates |
  ...
  **Next actions:** (1) replay the ordered Dptf clause stream through matched
  persistent cores and isolate learned-state/topology effects;
  (2) measure
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver map, query/internal attribution, and fresh exact-CNF parity DONE; retained-state and correctness-led evidence open (ADR-0213--0220).** Five fixed-work repetitions establish two wins, parity, and a Z3 win; 9,526 rows reject size alone; 244 exact CNFs agree across four cores and expose a fresh/warm Z3 reversal. Remaining: persistent clause-stream control; multi-oracle fuzzing; finding parity; timeout-sensitive driver; measured qfbv/WASM/RSS/DRAT deployment. |
