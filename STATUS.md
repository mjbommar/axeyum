...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Clean fixed-revision three-driver dual baseline DONE; phase partition WIP (ADR-0170).** All 3,769 occurrences decide/agree; weighted native Axeyum/Z3 is 1.255x but ranges 0.426x--2.679x by driver | Repeat processes for variance; pair same-revision exact hashes with `axeyum-bench`/`check_profiled` and partition translation, construction, model, and caller time |
  ...
  | **GQ7 warm delta entry** | **WIP; ADR-0170 multi-driver controls accepted.** Weighted snapshot/lineage are 1.049x/0.698x Z3, but each regresses one driver class; all 1,081 assertions materialize and lineage replays 26,930 fork roots | Integrate native per-lineage/delta ownership with snapshot as a fixed comparator; measure repeated variance, online topology/cost, and bounded memory before default admission |
  | **GQ8 verdict/CNF cache** | **TODO; multi-driver opportunity measured, implementation still GQ7-gated.** The tier has 957/3,769 exact duplicate occurrences (25.4%), 439 same-lineage repeats, and 2,192 prefix extensions adding 3,070 roots | Complete native retained per-lineage warm replay first; then evaluate bounded versioned exact-query reuse with mandatory model/proof replay rather than inferring cache value from frequency alone |
  | **GQ9 auto cost model/docs** | **TODO; ADR-0170 rejects snapshot-only and depth-only admission.** P1.8 shape/resource probes are only the general foundation | Telemetry-visible choice using reuse topology, fork reconstruction, formula cost, latency, and memory that beats or matches every fixed policy and documents embedder guidance |
  | **GQ10 real-lifter regression tier** | **WIP; clean fixed-revision three-driver ordered tier landed.** It validates 3,769 occurrences, every assertion, lineage/scopes/choices, memory, and separate native Z3/Axeyum timing | Repeat processes/full-tier variance, widen driver families, and track cold/warm per-commit family/stage/Axeyum÷Z3 trends separately |

  **Next actions:** (1) carry retained per-lineage/delta ownership through
  Glaurung's native worker/path boundary with snapshot as the fixed comparator;
  (2) repeat both policies across clean processes and measure online topology,
  fork reconstruction, formula cost, latency, and memory, then repeat
  ADR-0160's native cold and ADR-0164 warm profiles cleanly across
...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | **WIP — highest-leverage downstream client item; ADR-0170 multi-driver controls accepted.** All 3,769 checks and 1,081 assertions replay. Weighted snapshot/lineage ratios are 1.049x/0.698x Z3, but snapshot wins only `vwififlt` and lineage wins Dptf/IntcSST. Integrate native per-lineage/delta ownership with snapshot comparison and repeated admission/memory gates before GQ8/GQ9. |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the cold GQ10 gate and exact v2→v3→v4 transitions; native/warm publication WIP.** ADR-0155's full cold result is 5.625/7.709 s (0.730x). ADR-0160 exposes native phases/duplicates; ADR-0162/0163 pass paired cold-native gates. ADR-0166--0169 establish ordered controls and complete timing; ADR-0170 adds the clean three-driver gate. Weighted native/cold/snapshot/lineage are 1.255x/1.591x/1.049x/0.698x Z3, with policy reversal by driver. Remaining: native lineage plus snapshot comparison, repeated p50/p95/RSS variance, wider drivers/full variance, and per-commit user-visible ratio. |
