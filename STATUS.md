...
- **2026-07-19 — ADR-0260 accepts the fixed duplicate-origin observation and
  ADR-0261's sole selected experiment is implemented but unobserved.** Clean detached
  ...
  speedup. ADR-0261 now adds only a deterministic positive-root private-parity
  post-collection pass; it normalizes inputs, retains the first identical key,
  and leaves every helper node in the skip plan. Its red-first focused test and
  all 305 CNF, 880 solver, and 43 bench tests plus strict affected-crate Clippy
  pass. Next: commit the candidate unobserved, then require its exact fixed-
  population structural deltas before the repeated unprofiled timing gate.
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ5 AIG/CNF construction** | **Cold lane active; broad warm lane closed by ADR-0219; artifact-v35 attribution ACCEPTED without optimization (ADR-0259), artifact-v36 origin observation ACCEPTED (ADR-0260), and the sole candidate IMPLEMENTED but unobserved (ADR-0261).** The bounded parity-key pass is locally green and retains all helper bookkeeping | Commit the candidate, then require exact `-107,000` attempts/duplicates, `-214,000` attempted literals, byte-identical final CNF, all 162 gates, and the fixed repeated unprofiled timing win |
  ...
  **Next actions:** (1) coordinate with the active Glaurung explorer owner and
  ...
  no-optimization result and ADR-0260's accepted duplicate-origin evidence;
  commit ADR-0261's implemented same-owner positive-root private-parity-leaf
  elision unobserved, then retain it only if its exact structural and repeated
  unprofiled timing gates pass; (11) stage solver
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver fair map, internal/fresh/retained controls, neutral cold/retained SMT, complete three-solver timeout frontier, independent four-oracle fuzz, proof denominators including the accepted 1,024-query holdout, executable WebAssembly, warm time/RSS/hit-rate deployment, bounded/canonical/four-schedule authority cells, representative whole-certificate process isolation, isolated behavior-preserving Glaurung A0, corrected provenance/SystemBuffer environment, versioned confidence partition, source-backed nonzero positive control, two rejected usbprint resource protocols, v2 maximum-precision rejection, model-independent detector correction, corrected v3 scalar sweep, exhaustive source-backed difference adjudication, v6 hidden-work rejection, one neutral standard DRAT consumer plus checker-sanity control, capped nontrivial-proof no-selection, accepted artifact-v35 cold CNF attribution, accepted artifact-v36 origin observation, and ADR-0261's sole selected candidate IMPLEMENTED but unobserved (ADR-0213--0261 plus Glaurung `7f682e5`; stop-class instrumentation candidate `ff3c0a7`).** ADR-0253 adds 1,024/1,024 manifest/Z3 agreements, 515 SAT replays, 509 CNF DRAT rechecks, and stable 508/509 stronger coverage. ADR-0256 accepts one pinned real external verification and the preregistered satisfiable-CNF control while retaining ADR-0254's no-op mutation rejection; ADR-0258 shows all 32 fixed follow-on DRATs are also trivial two-byte proofs and closes further holdout mining. ADR-0259 accepts 162/162 decisions/manifest/Z3 agreements and all 88 SAT replays; ADR-0260 attributes 107,000 of 119,260 exact duplicates to the sole passing cell and authorizes only ADR-0261's separately gated experiment, not an optimization claim. Tcpip variation remains zero-high/unlabeled. Remaining: commit, structurally verify, and time ADR-0261; coordinate the Glaurung footer; add genuinely broader labeled evidence and timeout-sensitive neutral breadth; and coordinate clean integration separately. |
