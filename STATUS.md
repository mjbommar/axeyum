...
- **2026-07-10 — canonical arrays admit every finite Bool/BitVec component
  ...
  Next: low-load remeasure, then structural store/ITE/default/ROW merge events,
...
- **2026-07-10 — base parent-select scheduling follows live e-classes.** Base
  ...
  replay failures=0. ADR-0078. Next: structural store/ITE/default/ROW merge
  events with dynamic in-search insertion, non-symbol class models, warm reuse,
  and ROW/diff-witness/equality-chain proof logging.
...
| P2.2 | Arrays: lazy ROW axioms + extensionality + func_interp models | WIP — eager elimination/certifying fallback remains (ADR-0010). Canonical QF_ABV/QF_AUFBV has replay-guided base-select congruence (ADR-0071), lazy ROW (ADR-0072), bounded equality/diff observations (ADR-0073), majority-default models (ADR-0074), live e-graph equality plus direct-symbol class models (ADR-0077), explanation-guarded base-parent scheduling (ADR-0078), and Bool/BitVec finite-scalar component coverage (ADR-0079). Function-then-array projection and replay gate SAT; cloned probes preserve fallbacks. All 1,152 comparisons are clean, including 384 Bool/mixed analytic/front-door/Z3 checks; ADR-0078's low-load 1 s aggregate remains QF_ABV 187/193 and QF_AUFBV 49/53 pending comparable remeasurement. ADR-0075 checks direct select congruence externally. Remaining: structural store/ITE/default/ROW scheduling with dynamic insertion, non-symbol/warm class models, nested/extended arrays, and ROW/diff-witness/equality-chain/online proof integration |
