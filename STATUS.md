...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ2 cheap cold tier** | **WIP with two accepted tranches.** Canonical v2 cuts corrected full total 13.3%; ADR-0153's v3 add-chain fold then cuts full total 9.80%, clauses 17.23%, and `slice-partial` time 24.4% | Re-attribute v3 before adding another word rule; it must again reduce downstream construction and total time |
  | **GQ3 coercion/affine peepholes** | **WIP with two production wins.** ADR-0142 lands exact slice/coercion rules; ADR-0153 lands exact scalar/wide modular add-chain folding with identity projection | Measure the residual operator shape before considering subtraction or a broader affine normal form |
  ...
  | **GQ5 AIG/CNF construction** | **ACTIVE with four direct wins plus ADR-0153's upstream reduction.** Current canonical is 14.11 s / 1.85x Z3 with 40.72M clauses, down 17.23% from v2 | Re-attribute v3 construction by family before another cold experiment |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the generic/cold GQ10 gates and first rewrite-transition comparison; WIP for producer repair.** Artifact v27 provides fixed identities/resources, in-process Z3, decided-rate/original-replay gates, and regular representative plus scheduled full tiers. Five baseline full trials establish 0.51% total/ratio CV and 3%/3%/2% alarms; ADR-0153's exact v2→v3 additive comparator passes them and reaches 14.11 s / 1.85x. Remaining: malformed-dump/dedup repair and an ordered trace gate. Synthetic tiers cannot substitute for the real lifter |
