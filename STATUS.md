...
| Phase | Title | Status |
|---|---|---|
| P4.1 | Warm lazy arrays / symbolic memory (ADR-0030 deferred half) | WIP — committed assertions over arrays/UFs are now scoped as deferred theory assertions and decided by `check_with_memory` through the full pure-Rust dispatcher; one-shot branch assumptions over arrays/UFs are supported by `check_assuming_with_memory` / `check_assuming_core_with_memory`, with a coarse-but-sound full-assumption core on UNSAT. `IncrementalBvSolver` exposes deferred-theory introspection, `SymbolicExecutor` exposes memory-aware assume/branch/status/model/enumerate calls plus auto route-selection helpers, and `SymbolicMemory` provides typed load/store helpers plus compact write-log read helpers over array-backed memory states. This is a consumer-facing one-shot fallback and encoding mitigation, not the final warm lazy-array/UF incremental engine: deferred theory checks rebuild through `check_auto`, while the warm BV path still refuses active deferred theories rather than silently ignoring them. Remaining: true warm lazy arrays/UF with learned theory clauses, path-condition CFG/import frontends, and deeper memory model helpers |
