...
- **2026-07-17 — ADR-0228 exposes the bounded-warm time/RSS Pareto and real
  ...
  and authoritative finding parity now outrank optional four-driver RSS
  widening.
...
- **2026-07-17 — The consolidated code/publication review changes the success
  ...
  runtime trap.
  Publication blockers are now: (1) a topology-equivalent warm representation/integration
  control; (2) independent multi-oracle rounds plus deadline-aware and
  real-query proof coverage; (3) another neutral implementation/API boundary;
  and (4) authoritative finding parity with checked canonical model
  ...
  real-query DRAT deployment remains in the first artifact tranche. Solver
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Map, query/internal attribution, fresh/retained exact-CNF controls, and four-driver neutral cold-reset SMT breadth DONE (ADR-0187/0188/0213--0223).** cvc5 agrees on all 9,526 checks; BatSat beats Z3 Boolean on retained Axeyum CNF despite native warm Z3 winning end-to-end | Add topology-equivalent neutral, timeout-sensitive, and finding-authoritative cells |
  ...
  | **GQ7 warm delta entry** | **Source identity, fair map, query/internal attribution, fresh/retained CNF controls, and four-driver neutral cold-reset SMT DONE; wider default DEFERRED (ADR-0201--0223).** The remaining reversal boundary is warm word-level representation/integration | Keep direct opt-in; add a topology-equivalent neutral cell before harder/finding gates |
  ...
  **Next actions:** (1) add a neutral in-process or topology-equivalent warm
  ...
  ADR-0228's two-driver cold/warm RSS and hit-rate control, measure the
  real-query DRAT deployment denominator; (3) widen
  ADR-0225's multi-oracle evidence with independent seeds/edge-case frequencies
  and another neutral implementation, then make ADR-0226 certification
  deadline-aware and measure a real-query proof denominator; (4) keep each
  subprocess/FFI boundary named; (5) run
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver map, query/internal attribution, fresh/retained exact-CNF controls, neutral cold-reset SMT breadth, full-cvc5 4,000-row fuzz, first generated proof denominator, executable WebAssembly, and two-driver warm time/RSS/hit-rate deployment DONE; correctness-led evidence open (ADR-0213--0228).** Five fixed-work repetitions establish two wins, parity, and a Z3 win; 9,526 rows reject size alone; cvc5 agrees on all real checks and generated formulas; 169/169 selected UNSAT rows recheck end to end (6.725030% of generated UNSAT); stable browser size/latency and explicit high-reuse memory cost are measured. Remaining: warm-neutral SMT; independent/real fuzz and proof coverage with deadlines; finding parity; timeout-sensitive driver; real-query DRAT deployment. |
