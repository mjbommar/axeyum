...
- **2026-07-10 — structural store parents join explanation-guarded select
  ...
  Next: array-valued ITE/default/UF and merge-triggered ROW events with dynamic
  insertion, non-symbol/warm models, proof logging, and a low-load remeasure.
- **2026-07-10 — canonical arrays admit every finite Bool/BitVec component
  ...
  ADR-0080 subsequently adds store-parent select scheduling. Next: low-load
  remeasure, then array-valued ITE/default/UF and merge-triggered ROW events,
  dynamic insertion, non-symbol/warm models, and proof logging.
- **2026-07-10 — base parent-select scheduling follows live e-classes.** Base
  ...
  replay failures=0. ADR-0078; ADR-0080 subsequently adds store parents. Next:
  array-valued ITE/default/UF and merge-triggered ROW events with dynamic
  in-search insertion, non-symbol class models, warm reuse, and
...
| Phase | Title | Status |
|---|---|---|
| P1.6l | Explanation-guarded store-parent select scheduling | **DONE, outer-round slice (ADR-0080)** — original store terms join final e-class grouping; only candidate-violated equal-index/unequal-result pairs materialize, distinct-parent lemmas retain merge guards, and lazy ROW stays independent. Same/congruent/alternate/unrelated/UF-index/80-parent gates pass; 802 solver tests + 1,536 comparisons are clean. Remaining: ITE/default/UF and merge-triggered ROW events, dynamic insertion, non-symbol/warm models, and proofs |
...
| P1.6j | Explanation-guarded parent-select scheduling | **DONE, base-symbol outer-round slice (ADR-0078)** — base reads group by final live e-class; only candidate-violated equal-index/unequal-result pairs materialize, and cross-parent lemmas carry the merge explanation as a branch-safe guard. Direct-symbol equality drops query-index preconstruction; direct/transitive/UF/alternate-path/backtracking/80-array gates pass. 794 solver tests + 768 comparisons are clean; public coverage is unchanged. ADR-0080 subsequently adds store parents; ITE/default/UF and merge-triggered ROW events, dynamic insertion, non-symbol/warm models, and proofs remain |
...
| P2.2a | Deterministic majority-default array projection | **DONE, direct-symbol-class slice (ADR-0074/0077)** — votes count distinct observed indices, ties use stable smallest-value order, and only non-default overrides remain. ADR-0077 unions candidate-true direct-symbol classes before one shared model is built; transitive disjoint-read SAT replays. Remaining: store/ITE/array-valued-UF class ownership, nested/extended arrays, and warm reuse |
...
| P2.2c | Canonical e-graph equality + class-owned symbol models | **DONE, bounded slice (ADR-0077/0078)** — live backtrackable EUF handles array reflexivity/transitivity/congruence; direct symbol classes share one model and base reads follow explanation-guarded final class merges. Conflict/stress/transitive/alternate-path/backtracking/80-array gates, 794 solver tests, and 768 comparisons pass. ADR-0080 subsequently adds store-parent scheduling; ITE/default/UF and merge-triggered ROW events, dynamic insertion, and non-symbol/warm class ownership remain |
