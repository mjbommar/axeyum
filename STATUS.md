...
| P3.6 / TL0.6.5 | U2 matched official/Axeyum execution | **WIP — comparison contract and terminal registry schema corrected; execution blocked on complete accepted TL0.6.3/TL0.6.4 parents.** The preregistered plan separates 3,723 case rows, 408,374 candidate execution slots, and the future layer-expanded comparison-obligation denominator. Terminal cells contain independent official/native execution records plus a third normalization/comparison record; typed `not-run`/`invalid` sides preserve missing and inconsistent observations. R1 recomputes domain-separated execution, comparison, whole-cell, and population cell-seal digests; comparisons cite both exact side seals, while per-U0-U9 count/sorted-ID/sorted-cell-seal authorities make G3 reject nonempty agreeing subsets, omitted rows, and ID-preserving content substitution. R2 now requires a typed result class on each complete side, binds distinct normalized-observable identities, and derives every comparison outcome; fully resealed semantic mutations cannot retain stale agreement. No comparison authority, layer normalizer, Axeyum process, native outcome, pair, performance row, U2 promotion, axis/gate completion, or parity credit exists. Next: complete the two parents, derive the exact comparison-obligation authority, implement layer-specific normalizers and their semantic/ignored-field mutation controls, and only then authorize the first native shard. |
