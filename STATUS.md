...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the generic and cold GQ10 regression gates; WIP for producer repair and future scheduled candidates.** Artifact v27 gives the real access-controlled 128-query representative and 13,462-query full tiers a corrected production boundary, explicit profiling provenance, fixed identities/resources, in-process Z3, decided-rate gates, and original replay. `just check` runs availability-aware raw + canonical semantics; five clean canonical full trials establish 0.51% total/ratio CV and guarded 3% ratio / 3% Axeyum / 2% Z3-drift alarms. Remaining: producer-side malformed-dump/dedup repair and applying the guarded comparison to future accepted candidates. Synthetic tiers still cannot substitute for the real-lifter gate |
