...
- **2026-07-10 — structural array reads now retain warm definitions.** ADR-0086
  ...
  Next: candidate-triggered warm ROW activation, then structural equality/
  extensionality, array-valued UF parents, memory BMC/k-induction, and proofs.
...
| Phase | Title | Status |
|---|---|---|
| P4.1a | Retained warm structural array reads | **DONE, bounded observation-triggered slice (ADR-0086)** — store/constant/ITE reads receive exact permanent definitions in the persistent incremental CNF; roots stay selector-scoped, direct leaves alone project models, and original replay gates SAT. Exact 512-node/256-depth limits, push/pop/reuse, 192 warm/check-auto/Z3 comparisons, 816 solver units, 77 symexec tests, complete EVM gates, and DISAGREE=0 measurement pass. Performance exit remains open: depth-32 retained definitions are 30.933 ms vs 0.368 ms ITE-fold, so candidate-triggered activation is next |
