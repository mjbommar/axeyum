...
| P2.2c | Canonical e-graph equality + class-owned symbol models | **DONE, bounded slice (ADR-0077)** — live backtrackable EUF handles array reflexivity/transitivity/congruence; direct symbol classes share one model. One-round conflict/stress gates, transitive/backtracking SAT model replay, 790 solver tests, and 768 comparisons pass. Remaining: parent-select merge scheduling and non-symbol/warm class ownership |
| P2.2 | Arrays: lazy ROW axioms + extensionality + func_interp models | WIP — eager elimination/certifying fallback remains (ADR-0010). Canonical QF_ABV/QF_AUFBV has replay-guided base-select congruence (ADR-0071), lazy ROW (ADR-0072), bounded equality/diff observations (ADR-0073), majority-default models (ADR-0074), and live e-graph equality plus direct-symbol class models (ADR-0077). Function-then-array projection and replay gate SAT; cloned probes preserve fallbacks. The 768 comparisons are clean, 456 equality-bearing; public 1 s coverage is QF_ABV 187/193 and QF_AUFBV 49/53, DISAGREE=0. ADR-0075 checks direct select congruence externally. Remaining: parent-select scheduling, non-symbol/warm class models, nested/extended arrays, and ROW/diff-witness/equality-chain/online proof integration |
