...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ7 warm delta entry** | **WIP; snapshot bridge accepted (ADR-0164), ordered T1/T2 accepted (ADR-0166).** Structural LCP reuse retains 679,870 roots and cuts the native ratio 2.648x→1.462x; the bounded trace now independently validates 235 lineages, 784 checks, explicit scopes, and 243 choices | Implement T3 explicit per-lineage retained state and fork replay; compare it to snapshot inference, then repeat cleanly across drivers with p50/p95/memory/break-even before default admission |
  ...
  **Next actions:** (1) map the accepted ordered worker/path/scope/model trace
  to explicit per-lineage `IncrementalBvSolver` state, replay a validated prefix
  at forks, and compare ADR-0164 snapshot inference on identical occurrences
  with p50/p95/memory/break-even; (2) publish clean ordered traces and repeat
  ADR-0160's native
...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | **WIP — highest-leverage Glaurung item; ADR-0164 bridge and ADR-0166 T1/T2 accepted opt-in.** The snapshot adapter retains one arena/AIG/CNF/SAT session per thread and cuts the single-driver median ratio 2.648x→1.462x. Independent ordered replay now validates 235 paths, 784 checks, 508 unique exact queries, and 243 controlled choices; 35.2% duplicate occurrences plus 271 prefix extensions establish reuse opportunity. Next: explicit per-lineage retained state/fork replay, clean multi-driver p50/p95/memory/break-even, then GQ8/GQ9 decisions. |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the cold GQ10 gate and exact v2→v3→v4 transitions; native/warm publication WIP.** ADR-0155's full cold result is 5.625/7.709 s (0.730x). ADR-0160 exposes native phases/duplicates; ADR-0162/0163 pass paired cold-native gates. ADR-0164's first real warm bridge cuts the median native ratio 2.648x→1.462x; ADR-0166 independently validates bounded ordered lineage/scopes/choices but makes no timing claim. Active measurement reconciles bench vs real-client by exact hash/phase and reports pre-parsed Z3 separately from Glaurung's AST/context backend. Remaining: explicit-lineage warm solving, clean multi-driver cold/warm pairing, p50/p95/memory, wider drivers/full variance, and per-commit user-visible ratio. |
