...
| P5.5 | External target, measured | WIP — **Dedicated cache preparation accepted; target build still gated (ADR-0323--0332):** ADR-0332 v5 retains an ignored 3,077-row / 41.18 MB distinct-byte cache with hard-link inventory `fd6ee33d`, active graph `da6971e4` (162 nodes/814 edges authenticated against 169 lock entries), independent replay, and zero OOM deltas. No build/capture/admission/query or scoreboard row exists. Next preregister capture v2 pinning the exact cache read-only, then run or freeze negative. Phase exit still requires authenticated capture plus a separately preregistered measured scoreboard result, DISAGREE=0, replay, and wall-times. |
