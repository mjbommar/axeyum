...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **Next actions:** preregister and test Glaurung's existing AArch64 ELF→LLIR
  path on ADR-0267's four exact ordinary objects, including handler/CFG,
  relocation, unsupported-intrinsic, AAPCS64, and environment accounting; do not
  score recall until both sides of a pair clear it;
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver fair map, internal/fresh/retained controls, neutral cold/retained SMT, complete three-solver timeout frontier, independent four-oracle fuzz, proof denominators including the accepted 1,024-query holdout, executable WebAssembly, warm time/RSS/hit-rate deployment, bounded/canonical/four-schedule plus wider timeout-sensitive authority cells, representative whole-certificate process isolation, isolated behavior-preserving Glaurung A0, corrected provenance/SystemBuffer environment, versioned confidence partition, source-backed nonzero positive control, two rejected usbprint resource protocols, v2 maximum-precision rejection, model-independent detector correction, corrected v3 scalar sweep, exhaustive difference adjudication, v6 hidden-work rejection and execution, one neutral standard DRAT consumer plus checker-sanity control, capped nontrivial-proof no-selection, closed artifact-v35/v36 cold CNF origin lane, corrected symbolic-CVE execution preflight, immutable twelve-side artifact campaign admission, complete 4/12 artifact attrition, and deterministic four-side frontend-surface routing DONE without overclaiming (ADR-0213--0268 plus Glaurung `7f682e5`; campaign revision `ff3c0a7`).** ADR-0233 closes neutral timeout-sensitive formula breadth. ADR-0253 adds 1,024/1,024 manifest/Z3 agreements, 515 SAT replays, 509 CNF DRAT rechecks, and stable 508/509 stronger coverage. ADR-0256 accepts one pinned real external verification and the preregistered satisfiable-CNF control while ADR-0258 closes further holdout mining. ADR-0259/0260 attribute 119,260 duplicates; ADR-0261's sole selected leaf-elision candidate changes the structural counters by exactly zero and is rejected before timing. ADR-0262 accepts all six first-20 v6 authority cells: timeout is a no-op, AnyModel remains raw-divergent, and LeastUnsigned reaches exact parity at substantial cost. ADR-0263's first 22-row qualification is invalidated before build; ADR-0264 accepts the corrected 22-row declared/effective-handler join; ADR-0265 accepts the exact six-row zero-execution artifact/frontend preflight; ADR-0266 accepts the exact tool-hash-pinned builder campaign; ADR-0267 retains two complete frontend-eligible pairs and eight explicit artifact failures; ADR-0268 measures their real LLVM surface and selects the existing AArch64 ELF seam over a general LLVM parser. Remaining: execute the minimum Linux ELF frontend on four admitted sides, add an in-process warm Bitwuzla baseline, and defer A1 work bounds to the harder-driver tier. |
