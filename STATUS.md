...
| P2.5 | NRA: incremental linearization → nlsat/CAD | WIP — linear-abstraction + sign/zero lemmas + McCormick + spatial B&B + point-lemma refinement already shipped. **Added threshold-1 multiplication monotonicity lemmas** (`a≥1 ∧ b≥0 ⇒ r≥b`, …; two-operand only) — decides `x≥1 ∧ y≥1 ∧ x·y<1` the sign/zero rules miss — **plus a refinement overflow safety net** (`too_large_to_refine`: stop refining past a 2³¹ magnitude bound, → `unknown` not a panic; hardens the exact-rational simplex against escalating witnesses). 20 NRA tests. Remaining: nlsat/CAD for completeness |
