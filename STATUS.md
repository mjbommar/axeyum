...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ7 warm delta entry** | **WIP; ADR-0164 bridge through ADR-0169 clean T4 sample accepted.** Snapshot plus arena build is 0.476 s (0.590x same-stream Z3); naive lineage is 1.291 s and spends ~827 ms on 7,378 inherited roots. All 180 assertions materialize; 45/46 depth buckets beat Z3 | Repeat cleanly across drivers, then integrate snapshot through the native Glaurung boundary before default admission |
  ...
  **Next actions:** (1) repeat ADR-0169's complete cold/snapshot/lineage control
  and depth buckets across drivers; (2) carry the winning
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the cold GQ10 gate and exact v2→v3→v4 transitions; native/warm publication WIP.** ADR-0155's full cold result is 5.625/7.709 s (0.730x). ADR-0160 exposes native phases/duplicates; ADR-0162/0163 pass paired cold-native gates. ADR-0164 cuts one native snapshot ratio 2.648x→1.462x; ADR-0166--0168 validate ordered policies; ADR-0169 adds clean per-backend timing, complete assertions, and depth buckets. One bounded stream is native 2.593x versus snapshot-control 0.590x Z3. Remaining: native snapshot integration, clean multi-driver p50/p95/RSS/depth repetition, wider drivers/full variance, and per-commit user-visible ratio. |
