...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Native lineage timing and phase partition DONE for the bounded tier (ADR-0171/0172).** Unprofiled lineage is 0.746x Z3; 6,986 exact records attribute CNF/bit/SAT at 43.78%/22.86%/17.45% with 100% decisions | Extend exact records with causal gate/root and AIG-per-node attribution; never substitute diagnostic timing for the unprofiled bar |
  ...
  | **GQ5 AIG/CNF construction** | **Leading measured lane (ADR-0172).** Live lineage spends 43.78% adding 11.73M clauses and 22.86% adding 8.76M AIG nodes | Add causal warm gate/root deltas and target the dominant CNF pattern first; attribute AIG cost per node second; require lower unprofiled native time |
  ...
  | **GQ10 real-lifter regression tier** | **WIP; ordered, repeated native, and exact warm-profile three-driver tiers landed.** Diagnostic 6,986-record identity/phase evidence complements the repeated 0.746x-Z3 bar | Widen families, add full-tier/per-commit variance, and keep pre-parsed, actual-client, unprofiled lineage, and diagnostic bars separate |

  **Next actions:** (1) extend ADR-0172 with causal incremental CNF gate/root
  deltas and select the dominant 11.73M-clause pattern; (2) measure the bounded
  encoding change with profiling off against the repeated native gate; (3)
  attribute AIG construction per node, then SAT; (4) calibrate lineage caps
...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | **WIP — ADR-0171/0172 accept native path-owned timing and phase gates.** Repeated lineage is 0.746x Z3; 6,986 exact records attribute CNF/bit/SAT at 43.78%/22.86%/17.45%. Capacity fallback is live. Calibrate memory and widen before GQ8/GQ9. |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for cold transitions and bounded native timing/phase gates; widening WIP.** ADR-0171 repeats lineage at 0.746x Z3 versus snapshot 2.093x. ADR-0172 validates 6,986 exact warm records and attributes CNF/bit/SAT at 43.78%/22.86%/17.45%; diagnostic overhead is explicitly excluded from performance claims. Remaining: causal gate/root attribution, bounded memory, wider/full variance, and per-commit ratios. |
