...
- **Session 2026-06-27 — Property SDK generated corpus artifacts.**
  The property SDK corpus gate is now regenerated from one shared source instead
  of maintained by hand. `tests/support/corpus_cases.rs` owns the five SDK
  workflows and expected outcomes; `tests/corpus.rs` executes them and checks
  the results against the committed snapshots; and
  `examples/property_corpus_scoreboard.rs` regenerates
  `docs/consumer-track/property/corpus.json` plus the auto-generated
  `SCOREBOARD.md`. The first PROP.6 gate remains 5 cases, 2 proved, 3
  disproved, 0 unknown, DISAGREE=0, and 1/1 Lean-required coverage. External
  proptest/Kani-style baselines and broader corpus coverage remain open.
  Verification passed:
  `cargo fmt --all --check`;
  `git diff --check`;
...
- **Session 2026-06-27 — Property SDK corpus scoreboard first slice.**
  ...
  `docs/consumer-track/property/SCOREBOARD.md`. The corpus covers five SDK
  workflows: BV proof with checked evidence and a required Lean module,
  ...
  counterexample rendering. Current scoreboard totals are 5 cases, 2 proved, 3
  disproved, 0 unknown, 0 mismatches / DISAGREE, and 1/1 Lean-required case
  available. This advances PROP.6 from TODO to WIP; external proptest/Kani-style
  comparison and generated scoreboard artifacts remain open. Verification
  passed:
  `cargo fmt --all --check`;
  `git diff --check`;
  `CARGO_BUILD_JOBS=2 cargo test -p axeyum-property --test corpus -j1 -- --nocapture`;
