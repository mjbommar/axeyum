...
- **2026-07-10 — cross-array equality observations are candidate-triggered.** A
  ...
  QF_AUFBV 49/53 at 206 ms, DISAGREE=0 and replay failures=0. ADR-0076. Next:
  live backtrackable e-graph merge hooks, class-parent/multiple-path scheduling,
  class-owned/warm models, and proof logging.
...
- **2026-07-09 — projected arrays use deterministic majority-default models.**
  ...
  four overrides and full replay. The 768 equality-bearing comparisons remain
  clean. Public 1 s decisions hold at QF_ABV 187/193 and QF_AUFBV 49/53 with zero
  disagreements/replay failures; one AUFBV PAR-2 sample moves 221→206 ms, which
  is not a performance claim. ADR-0074. ADR-0076 adds candidate-triggered
  cross-equality queue state; live merge hooks, class defaults, warm reuse, and
  proof integration remain.
- **2026-07-09 — array equality/disequality is live on canonical CDCL(T).** The
  ...
  clean; ADR-0076 expands it to 456 equality-bearing comparisons. Public
  ...
  claimed. ADR-0073. ADR-0074 adds majority-default projection and ADR-0076 adds
  candidate-triggered cross-equality paths; live merge hooks, warm class
  ownership, and proofs remain.
- **2026-07-09 — lazy read-over-write is live on the canonical array bus.** The
  ...
  this is breadth evidence rather than a broad performance claim. ADR-0072. Next:
  ADR-0076 subsequently adds candidate-triggered cross-atom path state; live
  merge hooks and class models remain.
...
| Phase | Title | Status |
|---|---|---|
| P1.6h | Candidate-triggered cross-array equality queue | **DONE, outer-round slice (ADR-0076)** — false equalities transition new/delayed/applied and schedule their diff index only along one deterministic candidate-true BFS path. Transitive equality closes with two observations; disconnected SAT, store/UF+ROW, state transition, and exact 512-cap gates pass. The 768 comparisons remain clean, 456 equality-bearing; public coverage/PAR-2 is unchanged. Remaining: live backtrackable e-graph merge hooks, class-parent/multiple-path scheduling, class models, and proof logging |
| P1.6g | Candidate-guided array extensionality | **DONE (ADR-0073)** — shared equality flags receive one diff witness and paired reads at existing query/store indices; only violated congruence/witness instances materialize. UF-bearing observations share canonical refinement, cloned probes preserve pristine fallbacks, and replay gates SAT. Five mechanism + two isolation gates pass; the equality-bearing differential expansion remains DISAGREE=0. Public coverage holds at QF_ABV 187/193 and QF_AUFBV 49/53; no speed claim. ADR-0074 adds original-symbol majority models, ADR-0075 externally checks direct select congruence, and ADR-0076 adds cross-equality path scheduling; live merge hooks, class models, ROW/diff-witness/online proofs, and opaque-heavy arithmetic lifting remain |
...
| P1.5 | CDCL(T) loop (theory-as-extension, final-check, theory propagation) **[keystone]** | WIP — EUF on the e-graph: `prove_unsat_by_congruence` (conjunctive), `prove_unsat_lazy` (offline DPLL(T)), and `check_qf_uf` (full decision with **replay-checked sat models** from e-graph classes + function interps). Conflicts independently checked; **differentially validated vs Ackermann**. T1.5.5 met for the equality/UF fragment. **Online `TheorySolver` trait + `EufTheory` landed** (one backtrackable e-graph, explained conflict cores, lockstep push/pop) — the online theory side of the loop. **Slices a+b LANDED 2026-07-03** (`a3460101`,`c9d332c1`): the generic online `CdclT<T: TheorySolver>` driver (1-UIP over the mixed implication graph, lockstep theory push/pop, deadline in the loop; EufTheory parity 2500/2500 vs offline, z3 QF_UF fuzz unchanged) + the StringTheory adapter (per-assert certified refutations, premise-index→trail-literal explanations, replay-gated sat; census disjunctive shapes decide; 1500-case fuzz DISAGREE=0; found+fixed a real 1-UIP underflow on non-current-level theory cores). Front-door QF_S wiring landed same day (`c924fcb0`). **2026-07-09 DFS slices:** the generic LIA/LRA adapters now forward checked theory propagations through `CdclT`, with propagation-count telemetry and LIA/LRA gates proving the path fires; `StringTheory::propagate` now emits conservative whole-atom variable-equality closure and disequality transport with asserted-literal reasons; `solve_qf_uf_online` / `prove_unsat_qf_uf_online` now route through the generic replay-checked `CdclT` driver, retiring the embedded EUF DPLL from production (test-only diagnostics remain); `check_auto`'s `euf-online` route now invokes that generic `CdclT` path with caller `SolverConfig`, so ADR-0055 criterion (2) fired and QF_UF online is default-on with offline EUF fallback retained; pure QF_LIA/QF_LRA now lead with generic `CdclT`, with caller-owned deadlines through LRA construction/FM/model work and a measured 1,024-atom admission cap; Boolean QF_UFLIA/QF_UFLRA now drive their live combined theories through canonical `CdclT`, retiring the arithmetic-local driver from production combination; canonical search now has deterministic VSIDS, phase saving, Luby restarts, and LBD-based learned-clause reduction with stable tombstones plus glue/active-reason protection; bounded scalar QF_UFBV now combines `EufTheory` with a warm exact BV solver over explicit interface equalities on that same driver. Remaining: broader corpus timing, within-level BV core precision when measurement justifies it, merge-triggered array queue/model/proof integration, and opaque-heavy arithmetic participation |
| P1.6 | Theory combination (th_eq bus, interface equalities) | WIP — EUF+LIA/LRA dispatch and canonical QF_UFBV/QF_ABV/QF_AUFBV combination are live. ADR-0066–0070 establish exact BV/EUF interfaces and replay-guided UF refinement; ADR-0071/72/73 add candidate-guided select congruence, lazy ROW, and bounded array equality/diff observations on the same bus; ADR-0074 adds majority-default original-symbol models; ADR-0076 adds deterministic candidate-path cross observations. Function-then-array projection and original replay gate SAT; eager routes remain fallbacks/proof producers. ADR-0052 already closes the recorded bounded-string `str.len` marker. Remaining: partitioned opaque-heavy arithmetic admission, arithmetic-UF model lifting, live backtrackable array merge hooks/class-parent scheduling, e-graph-class/warm models, and proof integration |
...
| P2.2a | Deterministic majority-default array projection | **DONE, original-symbol slice (ADR-0074)** — votes count distinct observed indices, ties use stable smallest-value order, and only non-default overrides remain for compact BV and generic arrays. An end-to-end 16-read canonical model compresses to four overrides in one round with replay. The 768 differential/public gates remain clean. Remaining model depth: e-graph-class ownership, explicit class defaults, nested/extended arrays, and warm reuse |
| P2.2b | Candidate-triggered cross-equality observations | **DONE, outer-round slice (ADR-0076)** — new/delayed/applied state and deterministic true-path BFS add only missing path-edge reads at a false equality's diff index. Transitive UNSAT, disconnected replayed SAT, store/UF+ROW, state transition, and exact cap gates pass. Remaining: in-search merge trails, parent-select propagation, alternate paths, and class ownership |
| P2.2 | Arrays: lazy ROW axioms + extensionality + func_interp models | WIP — eager elimination/certifying fallback remains (ADR-0010). Canonical QF_ABV/QF_AUFBV now has replay-guided base-select congruence (ADR-0071), candidate-guided lazy ROW (ADR-0072), bounded equality/diff observations (ADR-0073), majority-default finite-map models (ADR-0074), and candidate-triggered transitive path observations (ADR-0076). Function-then-array projection and original replay gate SAT; cloned probes leave fallbacks pristine. The 768 eager/front-door/Z3 comparisons are clean, 456 equality-bearing; public 1 s coverage is QF_ABV 187/193 and QF_AUFBV 49/53, DISAGREE=0. ADR-0075 externally checks direct select congruence. Remaining: live backtrackable merge hooks and class-parent/multiple-path scheduling, e-graph-class model ownership, warm reuse, nested/extended arrays, and ROW/diff-witness/online proof integration |
...
- **2026-07-10 — candidate-triggered cross-array equality queue landed.** False
  ...
  retained live-merge/class-model/proof boundary.
