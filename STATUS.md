...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **Next actions:** (1) coordinate with the active Glaurung explorer owner and
  ...
  coordinate isolated Glaurung instrumentation candidate `ff3c0a7`; any future
  fixed-work harness must require explicit exploration-stop telemetry and zero
  deadline/timeout stops rather than accepting outer function count alone; (5) after
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver fair map, internal/fresh/retained controls, neutral cold/retained SMT, complete three-solver timeout frontier, independent four-oracle fuzz, proof denominators, executable WebAssembly, warm time/RSS/hit-rate deployment, bounded/canonical/four-schedule authority cells, representative whole-certificate process isolation, isolated behavior-preserving Glaurung A0, corrected provenance/SystemBuffer environment, versioned confidence partition, source-backed nonzero positive control, two rejected usbprint resource protocols, v2 maximum-precision rejection, model-independent detector correction, corrected v3 scalar sweep, and exhaustive source-backed difference adjudication DONE (ADR-0213--0249 plus Glaurung `7f682e5`; stop-class instrumentation candidate `ff3c0a7`).** The 12,000-row independent/edge campaign has 12,000 four-way agreements and all 14 corner families. All accepted proof/deployability results remain unchanged. ADR-0247 accepts all five policies at exact 14/14; ADR-0248 finds zero independent primitives among all 54 varying planted-fixture rows. ADR-0249 establishes a common usbprint prefix of 10 but rejects at prefix-15/site-hash-one because a concealed inner wall deadline breaks fixed-work reproduction; it establishes no resource bracket. Tcpip variation remains zero-high/unlabeled. Browser and high-reuse memory costs are measured. Remaining: require explicit worklist-stop telemetry in future fixed-work evidence, add genuinely broader labeled evidence, coordinate clean integration separately, and add wider real manifests. |
