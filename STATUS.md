...
- **2026-07-21 — ADR-0333 accepts the exact Tock capture-v2 replay negative.**
  ...
  Next: commit/push the zero-result ADR, then implement/push the thin wrapper,
  tests, and registration before any invocation; no query.
...
| P5.5 | External target, measured | WIP — **Authenticated cache accepted; capture v2 closed; v3 preregistered (ADR-0323--0334):** inventory `fd6ee33d` replays twice, but pushed v2 passes a merged capture registration lacking cache-only `expected_lock_packages` to the structural validator and closes on `KeyError` before Cargo/build. ADR-0334 freezes a narrow v3 that supplies the pinned full cache registration only to structural replay and inherits every other gate. Commit/push ADR before implementation; no target module/capture/admission/query or scoreboard row exists. Phase exit still requires authenticated capture plus a separately preregistered measured scoreboard result, DISAGREE=0, replay, and wall-times. |
