...
- **2026-07-17 — ADR-0217 establishes a fair Axeyum-winning regime but keeps
  ...
  formula/FFI-bound” remains a hypothesis. Next join per-query shape,
  AIG/CNF/SAT, and reuse features to the paired rows, then add a neutral solver
  and timeout-sensitive marked workload.
...
- **2026-07-17 — ADR-0216 makes the minimal QF_BV solver profile real in
  ...
  and latency/parser footprint while the higher-priority causal attribution of
  the completed fair map proceeds.
...
- **2026-07-17 — The consolidated code/publication review changes the success
  ...
  (1) joined query-feature attribution of ADR-0217's completed four-driver fair
  map, reporting only a causally supported boundary; (2) a standing well-typed
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Engineering capture, paired analyzer, and four-driver fair regime map DONE; causal attribution WIP (ADR-0187/0188/0213--0215/0217).** N=5 fixed-work controls give IntcSST/SurfacePen Axeyum wins, vwififlt parity, and a Dptf Z3 win, all without nondecisions or fallback | Join query features to paired rows, then add a neutral cell and timeout-sensitive marked driver; keep native/profile/cold bars distinct and report every win/tie/loss |
  ...
  | **GQ7 warm delta entry** | **Source-identity/product comparison and four-driver fair map DONE; wider default DEFERRED (ADR-0201--0217).** Warm Axeyum wins IntcSST/SurfacePen, ties vwififlt, and loses Dptf; dxgkrnl remains functionally green but fails the product timing-variance gate | Keep direct opt-in; explain the measured split, then widen through neutral/harder/finding-parity gates and rerun product admission only in a quieter predeclared environment or another valid no-timeout driver |
  ...
  **Next actions:** (1) join per-query formula/operator/AIG/CNF/SAT/reuse
  features to the completed four-driver fair map and test a causal boundary;
  (2) measure
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: paired analysis and four-driver topology-equivalent regime map DONE; causal and correctness-led publication evidence open (ADR-0213--0215/0217).** Five fixed-work repetitions per driver establish warm Axeyum wins on IntcSST/SurfacePen, parity on vwififlt, and a Z3 win on Dptf without nondecisions or fallback. Remaining: query-feature causal attribution; seeded multi-oracle typed fuzzing; neutral solver; authoritative finding parity/canonical model selection; timeout-sensitive marked driver; measured qfbv/WASM/RSS/DRAT deployment. |
...
- **2026-07-17 — ADR-0217 accepts the small-driver fair regime map.** Five
  ...
  committed. Because cold outcomes split too, PLAN now requires joined
  query-feature attribution before naming the causal regime.
