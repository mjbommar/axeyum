...
| P5.2 | Contracts & modular verification (`#[requires]`/`#[ensures]`, calls as composition) | TODO — ADR-0295 accepts the checked direct-body/inlined baseline that T5.2.4's modular-vs-inlined differential needs; it is not contract composition. Next, define and preregister the smallest explicit `leaf` contract composition against that baseline. Phase exit remains: the checksum module re-proves modularly (without the MIR inliner), with the differential at DISAGREE=0 |
