...
- **2026-07-09 — P1.5 QF_UF online production path migrated to generic `CdclT`.**
  `solve_qf_uf_online` / `prove_unsat_qf_uf_online` are now compatibility
  wrappers over the replay-checked `check_qf_uf_online_cdclt` route. The old
  embedded EUF DPLL is compiled only for in-source diagnostics, so production
  QF_UF online solving has one `CdclT` spine. The QF_UF front-door default still
  stays offline pending the broad re-measure/ADR flip, and the stale #52 note was
  corrected: regex-emptiness evidence is already wired through
  `Evidence::UnsatRegexEmptiness`. Gates: `cargo fmt --all --check`,
  `cargo test -p axeyum-solver --test cdclt_online`,
  `cargo test -p axeyum-solver --lib euf_egraph::tests`,
  `cargo test -p axeyum-solver --test qf_uf_differential_fuzz`,
  `cargo check -p axeyum-solver --tests`, and
...
- **2026-07-09 — QF_UF online wrapper migrated to the generic `CdclT` spine.**
  `solve_qf_uf_online` now delegates to `check_qf_uf_online_cdclt`, so the
  exported online QF_UF route uses the same replay-checked generic CDCL(T) loop
  as the shared P1.5 driver. The previous embedded EUF DPLL is now test-only and
  survives solely for learned-clause diagnostics. Also corrected the stale #52
  tracker note: regex-emptiness evidence is already wired into the live
  dispatcher/checker. Focused gates passed: fmt check, `cdclt_online`,
  `euf_egraph::tests`, the `qf_uf_differential_fuzz` target, `cargo check -p
  axeyum-solver --tests`, and clippy all-targets/all-features.
