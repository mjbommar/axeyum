...
- **2026-07-17 — ADR-0221 closes the ordered retained-core control and moves
  ...
  searches Axeyum CNF faster. Next add a neutral end-to-end SMT cell; the
  timeout-sensitive, finding-authoritative, multi-oracle-fuzz, and deployment
  gates remain open.
...
- **2026-07-17 — The consolidated code/publication review changes the success
  ...
  and an honestly delimited performance regime. ADR-0221 now closes the
  matched persistent clause-stream control and moves the open performance
  boundary to a neutral end-to-end SMT cell. Publication blockers are now:
  (1) that neutral representation/integration control; (2) a standing well-typed
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Map, query/internal attribution, fresh exact-CNF parity, and ordered retained-core control DONE (ADR-0187/0188/0213--0221).** BatSat beats Z3 Boolean on the same retained Axeyum CNF despite native warm Z3 winning end-to-end | Add neutral end-to-end SMT, then timeout-sensitive and finding-authoritative cells |
  ...
  | **GQ7 warm delta entry** | **Source identity, fair map, query/internal attribution, and fresh/retained CNF controls DONE; wider default DEFERRED (ADR-0201--0221).** The remaining reversal boundary is word-level representation/integration | Keep direct opt-in; run neutral end-to-end, then harder/finding gates |
  ...
  **Next actions:** (1) add a neutral end-to-end SMT cell to isolate the
  word-level representation/integration boundary selected by ADR-0221;
  (2) measure
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver map, query/internal attribution, and fresh/retained exact-CNF controls DONE; correctness-led evidence open (ADR-0213--0221).** Five fixed-work repetitions establish two wins, parity, and a Z3 win; 9,526 rows reject size alone; 431 ordered core calls show retained BatSat beating Z3 Boolean on Axeyum CNF. Remaining: neutral end-to-end SMT; multi-oracle fuzzing; finding parity; timeout-sensitive driver; measured qfbv/WASM/RSS/DRAT deployment. |
