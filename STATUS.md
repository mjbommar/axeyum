...
- **Session 2026-06-27 — Tiny BV assembly import.**
  ...
  and `#`/`;` comments. Malformed text returns line-numbered
  ...
  targets cannot bypass the existing checks.
...
- **2026-06-27** — **Tiny BV assembly import.**
  ...
  constants and branch targets, comments, and the existing instruction set,
  then validates through `TinyBvProgram::new`. Focused tests cover an imported
  memory program through checked reachability/concrete replay and verify
  line-numbered parse errors plus shared register-validation errors.
