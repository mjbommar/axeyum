...
- **2026-07-17 — ADR-0230 closes representative real-query CNF DRAT
  ...
  separate; add deadline-aware faithfulness on real rows and wider manifests.
...
- **2026-07-17 — The consolidated code/publication review changes the success
  ...
  real-query end-to-end faithfulness coverage; and (3) another neutral implementation/API boundary.
  ...
  ADR-0230 closes representative real-query CNF DRAT deployment; stronger
  faithfulness and wider proof populations remain. Solver
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Map, query/internal attribution, fresh/retained exact-CNF controls, four-driver neutral cold-reset SMT breadth, bounded authoritative finding parity, and representative real-query DRAT DONE (ADR-0187/0188/0213--0230).** cvc5 agrees on all 9,526 checks; 302 raw sinks match under sole Z3/Axeyum authority; all 64 representative UNSAT proofs recheck; BatSat beats Z3 Boolean on retained Axeyum CNF despite native warm Z3 winning end-to-end | Add topology-equivalent neutral and timeout-sensitive/wider authority plus faithfulness cells |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver map, query/internal attribution, fresh/retained exact-CNF controls, neutral cold-reset SMT breadth, full-cvc5 4,000-row fuzz, first generated and real-query proof denominators, executable WebAssembly, two-driver warm time/RSS/hit-rate deployment, and bounded four-driver authoritative finding parity DONE; correctness-led evidence open (ADR-0213--0230).** Five fixed-work repetitions establish two wins, parity, and a Z3 win; 9,526 rows reject size alone; cvc5 agrees on all real checks and generated formulas; 169/169 selected generated UNSAT rows recheck end to end (6.725030% of generated UNSAT); all 64 UNSAT rows in the 128-query real manifest carry rechecked CNF DRAT; 302 raw sinks match under sole backend authority; stable browser size/latency and explicit high-reuse memory cost are measured. Remaining: warm-neutral SMT; independent fuzz and proof coverage with deadlines; real-query faithfulness; timeout-sensitive/wider authority. |
