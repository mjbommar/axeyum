...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Native lineage timing, phase, and CNF gate/root attribution DONE for the bounded tier (ADR-0171--0173).** Unprofiled lineage is 0.746x Z3; 6,986 exact v2 records partition all 11.73M clauses with 100% decisions | Run the selected bounded CNF experiment, then attribute AIG per node; never substitute diagnostic timing for the unprofiled bar |
  ...
  | **GQ5 AIG/CNF construction** | **Leading measured lane (ADR-0172/0173).** Definitions own 71.75% of 11.73M clauses; AND-tree shapes own 53.89% of halves; root fusion is saturated and duplicates/tautologies are zero | Try future-reuse-safe positive internal AND-tree half flattening; require lower repeated unprofiled native time, then attribute AIG cost per node |
  | **GQ6 cold SAT/CDCL** | **WIP foundation; third behind measured construction.** Native lineage SAT is 17.45% weighted, 16.29%--34.51% by driver | After CNF/AIG attribution, compare identical CNF across cores and tune only with proof replay and deterministic limits |
  | **GQ7 warm delta entry** | **Native timing/phase gates accepted; memory policy WIP (ADR-0171/0172, `49f1fe2`/`13f4bbe`).** Lineage is 0.746x Z3; session creation is only 0.21%; atomic fallback is live | Calibrate caps against RSS/inherited-prefix cost and widen drivers before default admission |
  ...
  | **GQ10 real-lifter regression tier** | **WIP; ordered, repeated native, and exact warm-profile v2 three-driver tiers landed.** Diagnostic 6,986-record identity/phase/gate evidence complements the repeated 0.746x-Z3 bar | Widen families, add full-tier/per-commit variance, and keep pre-parsed, actual-client, unprofiled lineage, and diagnostic bars separate |

  **Next actions:** (1) implement ADR-0173's future-reuse-safe positive internal
  AND-tree half flattening with eligible/applied counters; (2) measure the
  bounded encoding change with profiling off against the repeated native gate;
  (3) attribute AIG construction per node, then SAT; (4) calibrate lineage caps
  against RSS/prefix cost and widen ordered/native driver families; (5) only
  then specify GQ8's
...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | **WIP — ADR-0171--0173 accept native path-owned timing, phase, and CNF gate/root attribution.** Repeated lineage is 0.746x Z3; 6,986 exact v2 records partition all 11.73M clauses. Capacity fallback is live. Run the selected bounded CNF experiment, then calibrate memory and widen before GQ8/GQ9. |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for cold transitions and bounded native timing/phase/gate attribution; widening WIP.** ADR-0171 repeats lineage at 0.746x Z3 versus snapshot 2.093x. ADR-0172/0173 validate 6,986 exact warm records, attribute CNF/bit/SAT, and partition all 11.73M clauses; diagnostic overhead is explicitly excluded from performance claims. Remaining: bounded encoding acceptance, memory admission, wider/full variance, and per-commit ratios. |
