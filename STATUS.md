...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ10 real-lifter regression tier** | **WIP; regular real-capture automation landed.** `just check` now runs raw + canonical over all 128 representative rows when the access-controlled pack is available; both first runs pass every manifest/Z3/decided/error/replay gate. Full-tier accepted confirmations exist; 2,225 malformed dumps are isolated | Establish repeated full-tier variance thresholds and fix producer validation/dedup before calling the raw capture authoritative |

  **Next actions:** (1) establish independent-process full-tier variance at the
  accepted ADR-0151/canonical boundary and define provisional ratio/raw-Axeyum/
  Z3-drift thresholds; (2) keep broad GQ4, SAT, and further memo work gated
  pending larger measured opportunity; (3) fix Glaurung's explicit width
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the generic and regular representative gates; WIP for the scheduled GQ10 performance lane.** Artifact v27 gives the real access-controlled 128-query representative and 13,462-query full tiers a corrected production boundary, explicit profiling provenance, fixed identities/resources, in-process Z3, decided-rate gates, and original replay. `just check` now runs availability-aware raw + canonical real-capture semantics; ADR-0144/0145/0150/0151 accepted full confirmations reach 15.60 s / 1.99x Z3. Remaining: repeated full-tier variance thresholds and producer-side malformed-dump/dedup repair. Synthetic tiers still cannot substitute for the real-lifter gate |
