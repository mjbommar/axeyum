...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | **WIP — highest-leverage Glaurung item; ADR-0164 bridge, ADR-0166 T1/T2, and ADR-0167 opt-in T3 accepted.** Explicit per-lineage replay agrees on all 784 checks and evaluates all 243 choices, but naive fork construction replays 7,378 roots and consumes ~813 ms. Next: identical-byte cold/snapshot/lineage controls, complete assertion capture, and clean multi-driver p50/p95/RSS/break-even; then GQ8/GQ9 decisions. |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the cold GQ10 gate and exact v2→v3→v4 transitions; native/warm publication WIP.** ADR-0155's full cold result is 5.625/7.709 s (0.730x). ADR-0160 exposes native phases/duplicates; ADR-0162/0163 pass paired cold-native gates. ADR-0164 cuts one native snapshot ratio 2.648x→1.462x; ADR-0166 validates ordered lineage/scopes/choices; ADR-0167 adds sound explicit-lineage replay and exposes fork replay as its dominant cost, but is not yet a comparable baseline. Remaining: identical-byte cold/snapshot/lineage pairing, clean multi-driver p50/p95/RSS, wider drivers/full variance, and per-commit user-visible ratio. |
