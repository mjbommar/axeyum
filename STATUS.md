...
| P5.5 | External target, measured | WIP — **Prior external/capture/preparation routes close negative; v4 validates hard-link inventory then rejects a bad metadata-count invariant (ADR-0323--0331):** DNS/fetch/inventory pass, but 162 active metadata packages do not equal 169 lock entries. No cache/target/partial bytes, capture credit, proof, or scoreboard row exists. Next preregister structural resolved-package-to-lock authentication for v5 while retaining every other gate. Phase exit still requires authenticated capture plus a separately preregistered measured scoreboard result, DISAGREE=0, replay, and wall-times. |
