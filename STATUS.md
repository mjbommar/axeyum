...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Engineering capture DONE; publication schema WIP (ADR-0187/0188/0213).** Five drivers, 30,628 scripts, 7,953 wide-root scripts, zero exclusions, exact clean sharded profiles and guarded variance | Add both backend outcomes and explicit warm/fallback class per check; keep native/profile/cold bars distinct |
  ...
  **Next actions:** (1) implement ADR-0213's per-check paired fixed-work schema
  and N>=5 analyzer before accepting another headline ratio; (2) add the fair
  warm Z3 and neutral-solver cells; (3) run authoritative finding parity and
  specify checked canonical model selection if needed; (4) preserve explicit
  continuation on/off controls, exact counter partitions, and honest residual
  `Unknown`s; (5) rerun the exact `dxgkrnl` no-op comparison only in a quieter
  predeclared environment or on another valid no-timeout IOCTL driver; (6)
  route `win32k` through a future system-service/callout frontend; (7) obtain
  fresh native canonical-stage attribution and select GQ5/GQ6 work only through
  the paired harness; (8) keep GQ4 explicit/off.
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: engineering admission gates are mature; publication evidence is not yet closed (ADR-0213).** Cold transitions and bounded native phase/CNF/AIG attribution support local product decisions, but their aggregate ratios are not paper speedups. Remaining critical work: per-query both-outcome paired fixed-work analysis with geomean/bootstrap CI/quantiles/CDF and timeout/fallback partitions; topology-equivalent warm Z3 plus a neutral solver; authoritative finding parity and canonical model selection; multi-oracle correctness support. |
