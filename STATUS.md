...
- **2026-07-15 — ADR-0169 completes one clean ordered T4 capture and
  ...
  headroom but is not native integration. Repeat across drivers, add scope-depth
  break-even buckets, and carry snapshot reuse through Glaurung's client
  boundary before GQ8/GQ9 or default admission.
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **One clean separated dual baseline DONE; multi-driver publication WIP (ADR-0169).** The 776-check stream is fully decided/agreed and measures native Axeyum/Z3 at 2.095/0.808 s (2.593x) | Repeat on clean multi-driver processes; pair same-revision exact hashes with `axeyum-bench`/`check_profiled`, retain order/duplicates and fixed experiment identity |
  ...
  | **GQ7 warm delta entry** | **WIP; ADR-0164 bridge through ADR-0169 clean T4 sample accepted.** Snapshot plus arena build is 0.476 s (0.590x same-stream Z3); naive lineage is 1.291 s and spends ~827 ms on 7,378 inherited roots. All 180 assertions materialize | Repeat cleanly across drivers with scope-depth break-even, then integrate snapshot through the native Glaurung boundary before default admission |
  ...
  | **GQ10 real-lifter regression tier** | **WIP; cold gate plus one clean complete ordered driver landed.** The sample validates exact occurrences, all assertions, lineage/scopes/choices, memory, and separate native Z3/Axeyum timing | Publish the same clean artifact across drivers, then track cold and warm per-commit family/stage/Axeyum÷Z3 trends separately |

  **Next actions:** (1) repeat ADR-0169's complete cold/snapshot/lineage control
  across drivers and add scope-depth break-even buckets; (2) carry the winning
  snapshot policy through Glaurung's native translation boundary, then repeat
...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | **WIP — highest-leverage Glaurung item; ADR-0164 bridge through ADR-0169 clean complete T4 sample accepted.** All 776 checks and 180 assertions replay. Native Axeyum/Z3 is 2.095/0.808 s (2.593x); snapshot plus build is 0.476 s (0.590x Z3) and 38.1 MB, versus naive lineage 1.291 s and 88.7 MB. Repeat across drivers, add depth break-even, and integrate snapshot natively before GQ8/GQ9. |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the cold GQ10 gate and exact v2→v3→v4 transitions; native/warm publication WIP.** ADR-0155's full cold result is 5.625/7.709 s (0.730x). ADR-0160 exposes native phases/duplicates; ADR-0162/0163 pass paired cold-native gates. ADR-0164 cuts one native snapshot ratio 2.648x→1.462x; ADR-0166--0168 validate ordered policies; ADR-0169 adds clean per-backend timing and complete assertions. One bounded stream is native 2.593x versus snapshot-control 0.590x Z3. Remaining: native snapshot integration, clean multi-driver p50/p95/RSS/depth break-even, wider drivers/full variance, and per-commit user-visible ratio. |
...
- **2026-07-15 — ADR-0169 accepts complete assertions and separated backend
  ...
  selects snapshot for multi-driver repetition and native integration; it does
  not authorize a default, cache, or product claim.
