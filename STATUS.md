...
- **2026-07-21 — ADR-0322 preregisters the P5.3 obligation catalog.**
  The documentation-only closeout requires one independent page for
  control-flow constant-time, bounded memory/page-table math, and FSM
  refinement, plus a compact comparison index linked from Track 5 and the
  primary Verify scoreboard. Each page must expose its exact goal, fragment,
  evidence route, worked example, reproduction command, and residuals. In
  particular, T5.3.1's committed MIR-text evidence must not be presented as
  authenticated owning-build capture. No new semantics or research work is
  authorized.
