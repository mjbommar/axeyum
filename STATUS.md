...
- **2026-07-17 — The consolidated code/publication review changes the success
  ...
  control; (2) independent multi-oracle rounds plus a measured proof-coverage
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver map, query/internal attribution, fresh/retained exact-CNF controls, neutral cold-reset SMT breadth, and full-cvc5 4,000-row fuzz round DONE; correctness-led evidence open (ADR-0213--0225).** Five fixed-work repetitions establish two wins, parity, and a Z3 win; 9,526 rows reject size alone; 431 ordered core calls show retained BatSat beating Z3 Boolean on Axeyum CNF; cvc5 agrees on all real checks and all 4,000 generated formulas. Remaining: warm-neutral SMT; independent fuzz/proof coverage; finding parity; timeout-sensitive driver; measured qfbv/WASM/RSS/DRAT deployment. |
