...
| P5.5 | External target, measured | WIP — **Authenticated Tock capture DONE; v3 completes dual-DRAT work but closes on first-row parsing (ADR-0323--0337):** v1 closes pre-query on stale lock; v2 correctly rejects plain DRAT's uncertified bit-blast. V3 selects the existing end-to-end route, and its Rust test exits zero after eight certificate/recheck calls and six replayed controls. The outer parser sees 7 rows because the harness prefixes the first marker; atomic cleanup grants no result credit. Never rerun v3. A successor may change only prefix-aware extraction and retain failure source/logs; all semantic/evidence gates stay frozen. |
