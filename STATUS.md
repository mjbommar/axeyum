...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ7 warm delta entry** | **WIP; snapshot bridge ACCEPTED opt-in (ADR-0164), completion lineage-gated.** Structural LCP reuse retains 679,870 roots, adds 8,027, and cuts median native Axeyum 47.0%, 2.648x→1.462x Z3, with 13,126/13,126 agreement and zero resets | Obtain ordered worker/path/scope/model trace v1; compare explicit per-lineage state to snapshot inference, repeat multi-driver, and publish p50/p95/memory/break-even before default admission |
  ...
  **Next actions:** (1) ingest and validate the ordered worker/path/scope/model
  trace, then compare ADR-0164 snapshot inference with explicit per-lineage warm
  state and publish p50/p95/memory/break-even; (2) repeat ADR-0160's native
...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | **WIP — highest-leverage Glaurung item; ADR-0164 bridge accepted opt-in.** The snapshot adapter retains one structural arena/AIG/CNF/SAT session per thread, maps complete snapshots by longest common assertion prefix, and cuts the single-driver median Axeyum time 47.0% / paired ratio 2.648x→1.462x with 13,126/13,126 agreement, zero unknown splits/resets, and unchanged findings. It retains 679,870 roots and adds 8,027. Ordered warm-trace v1 remains required for explicit worker/path lineage, scopes, non-consecutive forks, controlled model choices, p50/p95/memory/break-even, multi-driver validation, and GQ9 admission. GQ8 remains separately replay/cache gated. |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the cold GQ10 gate and exact v2→v3→v4 transitions; native/warm publication WIP.** ADR-0155's full cold result is 5.625/7.709 s (0.730x). ADR-0160 exposes native phases/duplicates; ADR-0162/0163 pass paired cold-native gates. ADR-0164's first real warm bridge passes three alternating pairs and cuts the median native ratio 2.648x→1.462x with complete agreement, but covers one driver and inferred consecutive prefixes. Active measurement reconciles bench vs real-client by exact hash/phase and reports pre-parsed Z3 separately from Glaurung's AST/context backend. Remaining: ordered lineage, clean multi-driver cold/warm pairing, p50/p95/memory, wider drivers/full variance, and per-commit user-visible ratio. |
