...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ5 AIG/CNF construction** | **WIP; first fusion ACCEPTED (ADR-0162).** Positive-root AND/XOR direct encoding cuts incremental clauses 21.36% and mean native Axeyum time 4.52%, with unchanged AIG/scopes/replay and all real verdict gates green | Attribute the remaining 69,632-clause (+12.75%) one-shot gap before selecting negative-root/not-AND, guarded-root deduplication, or another bounded slice |
  ...
  **Next actions:** (1) attribute ADR-0162's residual 12.75% incremental/one-shot
  clause gap by negative roots, inverted-AND definitions, repeated guarded-root
  clauses, and assertion families before selecting another bounded GQ5 slice;
  (2) repeat ADR-0160's ordered native profile cleanly across
...
| P1.2 | Preprocessing (word-level rewrite, solve_eqs, bv_slice/bounds/max-sharing, AIG 2-level rewrite) | WIP — the general opt-in preprocessing foundation and model trail are landed; exact Glaurung canonical v4 reaches 0.730x Z3 on the standalone full capture. ADR-0157/0158 demand slicing are semantically green but deferred after real performance failures. ADR-0159 closes current structural extract attribution. ADR-0160/0161 attribute the same-query native path and incremental gate mix; ADR-0162/0163 accept selector-safe direct-root fusion and exact context dedup, cumulatively cutting pinned incremental clauses 28.60%, with native wins of 4.52% then 2.10%. The incremental/one-shot residual is only 2.36%; keep GQ3/GQ4 off and move to GQ1/GQ7 rather than adding another unmeasured CNF structure. |
...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | WIP — retained CNF, learned state, and push/pop are foundations; ordered warm-trace v1 specifies event order, worker/path lineage, scopes, hashes, unknown/errors, and controlled model choices. ADR-0160's first ordered driver stream measures 6,061/13,126 duplicate occurrences, establishing opportunity but not sound lineage. Next is a validated producer trace, persistent per-worker/path integration, affected-term-only preprocessing, and warm break-even. Exact caching follows only from scoped prefix/duplicate measurement |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the cold GQ10 gate and exact v2→v3→v4 rewrite transitions; native ordered instrumentation landed, publication WIP.** ADR-0155's full cold result is 5.625/7.709 s (0.730x). ADR-0160 adds exact-query process-ordered native profiles and a fail-closed summarizer; its first exploratory driver run is 100% decided/agreed with 42.81% bit blast, 37.58% incremental CNF, 7.23% SAT, and 46.18% duplicate occurrences. ADR-0162/0163 are native optimizations accepted by paired gates: direct-root fusion improves Axeyum 4.52%, then root-context dedup improves it another 2.10%; a structurally stronger clause-index candidate is rejected for a 2.16% regression. Remaining: clean multi-driver/same-revision pairing, producer malformed-dump/dedup repair, and worker/path/scope trace. |
