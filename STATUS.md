...
- **2026-07-20 — ADR-0285's pre-observation flat-arena gate is complete.**
  Commit `725858b1` replaces fragmented `Vec<CnfClause>` retention with one
  ordered literal arena plus monotone `u32` clause ends, migrates every
  consumer to borrowed slices, and reuses the Tseitin canonicalization
  scratch. Commit `a57d5ace` freezes artifact v38 and a fail-closed independent
  validator: every instance must prove clause-count/byte accounting, monotone
  and in-bounds ends, terminal-end equality, aggregate re-summing, and the
  <=80% logical-storage gate. All CNF and benchmark suites, the focused solver
  profile gate, 13 analyzer tests, strict workspace Clippy/rustdoc/format/links,
  QF_BV/no-default, and ordinary plus SIMD wasm32 builds pass. The prior full
  workspace functional suite also passes; its final EVM doctest was rerun
  successfully on `/var/tmp` after shared `/tmp` quota exhaustion. No
  corrected-wide-v3 query has been observed through artifact v38. Next: run
  exactly one clean detached 162-query structural profile and reject before
  timing unless every frozen correctness, structure, and storage gate passes.
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: correctness/deployability evidence, neutral warm controls, exact cold attribution, proof denominators, authoritative-policy gates, selected-pair symbolic-CVE recall, and negative harder-driver/duplicate-clause results are complete through ADR-0278.** ADR-0272 rejects performance leadership because warm Bitwuzla wins all four fair drivers. ADR-0273--0275 retain the harder-driver census as incomplete-work negative evidence; ADR-0277 removes its structurally exact candidate after the frozen variance/family gates fail. ADR-0285's flat CNF formula arena, reusable Tseitin scratch, artifact-v38 storage telemetry, and fail-closed analyzer have now passed every pre-observation gate; exactly one fixed structural run must pass before the conditional >=3% paired total-CNF timing protocol. Broader labeled recall and honest correctness/deployability/proof framing remain open; no warm-speed headline or concretization/symbolic-memory reopening is authorized. |

### Track 5 — Verified Systems (IR reflection) — ADR-0056, adopted 2026-07-06
| Phase | Title | Status |
|---|---|---|
| P5.1 | Reflection front end (crate-ify the MIR+LLVM reflectors, full `.ll` parser, MIR extraction pipeline, loops→`TransitionSystem`, memory beyond byte arrays) | WIP — **T5.1.1 DONE (`cc695925`, ADR-0057)**: the reflectors are now the real library module `axeyum_verify::reflect` (`src/reflect/{mod,mir,llvm}.rs`, submodules `reflect::mir`/`reflect::llvm`), no longer per-test scaffolding — 8 test binaries (62 tests) rewired to `use axeyum_verify::reflect::…` and green, `missing_docs`+`implicit_hasher` API-hardened, clippy/rustdoc `-D warnings` clean; the crate split is deferred (one consumer today). The prototyped *capability* (rounds Q–U, design log `docs/consumer-track/verify/reflect-common-abstraction.md`): CFG symbolic executors for both IRs over one shared op vocabulary; 16 cross-IR equivalence proofs (MIR≡LLVM per function, LLVM O0≡O2, if-conversion/strength-reduction/umin-idiom validated, hypothesis-gated `unreachable`); 5-shape wrong-transform refutation corpus with replay-checked countermodels; exact panic specs from rustc's own checks (overflow, division `b==0` / signed `∨ (a==MIN ∧ b==-1)`, bounds over all 2^64 indices) with `catch_unwind` witness replay; checksum micro-module end-to-end on both platforms. **T5.1.2 WIP (ADR-0279--0284):** accepted slices now provide a non-panicking function boundary, typed scalar instructions with explicit value+definedness, typed PHIs/terminators with exact CFG validation, bounded checked acyclic execution, and a canonical scalar render/reparse fixpoint with exact LLVM identifier escapes. Memory and module-wide semantics remain later slices; no crate split yet. Remaining T5.1.3–6: build-time MIR extraction, automatic loop bridging, `gep`/`load`/`store` + array writes, and the semantics gate. Individual proofs are milliseconds — the suites already run as ordinary per-commit tests |
...
- **2026-07-20 — ADR-0285 pre-observation implementation and validator gate
  complete (`725858b1`, `a57d5ace`).** The production CNF formula is now flat,
  all consumers use borrowed clause slices, and Tseitin canonicalization reuses
  one scratch allocation. Artifact v38 makes offset/storage evidence explicit;
  ...
  formatting, Clippy, rustdoc, and link gates pass. No fixed-corpus observation
  has occurred; the next action remains exactly one structural process.
