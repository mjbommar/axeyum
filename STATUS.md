...
- **2026-07-21 — ADR-0334 accepts authenticated Tock capture v3.** Pushed
  ...
  pushed-HEAD archive isolation, and an atomic scoreboard. Next: commit/push the
  zero-row ADR before implementation; no query.
...
| P5.5 | External target, measured | WIP — **Authenticated Tock capture/parser admission DONE; proof scoreboard preregistered (ADR-0323--0335):** v3's two raw-identical modules admit both helpers with exact time/RSS and zero path/OOM drift. ADR-0335 freezes eight certified property proofs plus six reflected/native-replayed controls over the authenticated canonicals, exact limits, pushed-HEAD isolation, and atomic scoreboard. No query/proof/replay/row exists. Commit/push ADR before implementation; phase exit still requires the measured result with DISAGREE=0. |
