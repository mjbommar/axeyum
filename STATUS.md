...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | **WIP — highest-leverage Glaurung item.** Retained CNF, learned state, push/pop, ADR-0156 batch assertion, and ADR-0162/0163 incremental encoding are foundations; ordered warm-trace v1 specifies event order, worker/path lineage, scopes, hashes, unknown/errors, and controlled model choices. The native stream measures 6,061/13,126 duplicate occurrences but lacks sound lineage. Next: ingest the offered ordered/prefix capture, validate a small lineage/scope/model sample, wire persistent per-worker/path state with delta-only preprocessing, and measure end-to-end break-even. GQ8 cache design follows measured exact/prefix frequencies and preserves replay. |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the cold GQ10 gate and exact v2→v3→v4 transitions; native reconciliation/publication WIP.** ADR-0155's full cold result is 5.625/7.709 s (0.730x). ADR-0160's native profile is 100% decided/agreed and exposes phase shares plus 46.18% duplicates. ADR-0162/0163 pass paired native gates (4.52%, then 2.10% Axeyum wins); a stronger clause index is rejected for a 2.16% regression. Active measurement now reconciles the reported roughly 1.34x bench versus 2.5x real-client ratio by exact hash and phase, and reports two Z3 bars separately: pre-parsed in-process oracle and Glaurung's actual AST/context backend. Remaining: clean multi-driver pairing, ordered lineage capture, wider drivers/full variance, and per-commit user-visible ratio. |
