...
- **2026-07-09 — array equality/disequality is live on canonical CDCL(T).** The
  ...
  claimed. ADR-0073. Next: merge-triggered/cross-atom queue state, scalable
  majority-default models, warm reuse, and proof integration.
...
| P1.6 | Theory combination (th_eq bus, interface equalities) | WIP — EUF+LIA/LRA dispatch and canonical QF_UFBV/QF_ABV/QF_AUFBV combination are live. ADR-0066–0070 establish exact BV/EUF interfaces and replay-guided UF refinement; ADR-0071/72/73 add candidate-guided select congruence, lazy ROW, and bounded array equality/diff observations on the same bus. Function-then-array projection and original replay gate SAT; eager routes remain fallbacks/proof producers. ADR-0052 already closes the recorded bounded-string `str.len` marker. Remaining: partitioned opaque-heavy arithmetic admission, arithmetic-UF model lifting, merge-triggered/cross-atom array queue states, scalable models, and proof integration |
...
| P2.2 | Arrays: lazy ROW axioms + extensionality + func_interp models | WIP — eager elimination/certifying fallback remains (ADR-0010). Canonical QF_ABV/QF_AUFBV now has replay-guided base-select congruence (ADR-0071), candidate-guided lazy ROW (ADR-0072), and bounded equality flags with observed-index congruence plus one diff witness per atom (ADR-0073). Function-then-array projection and original replay gate SAT; online probes use cloned arenas so declines leave fallbacks pristine. The 768 eager/front-door/Z3 comparisons are clean and now half equality-bearing; public 1 s coverage is QF_ABV 187/193 and QF_AUFBV 49/53, DISAGREE=0. Existing direct congruence and structural array certificates remain ahead of the generic route. Remaining: merge-triggered new/delayed/applied queue state, cross-atom observations, majority-default models, warm reuse, and proof integration |
