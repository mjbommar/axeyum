...
| P5.5 | External target, measured | WIP — **Authenticated Tock capture/parser admission DONE; proof v1 accepted negative and v2 frozen/preflighted (ADR-0323--0336):** v3's two raw-identical modules admit both helpers. V1 closes before compilation on the stale committed lock. ADR-0336 changes only to the corrected committed lock plus versioned schema/output; pushed producer `07b22549` passes its fresh archived locked/offline non-authenticated build preflight with no target query/output. Commit/push that gate, then invoke once for eight certified proofs, six reflected/native controls, and required DISAGREE=0. |
