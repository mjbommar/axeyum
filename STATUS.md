...
- **2026-07-20 — ADR-0292 preregisters the single-latch LLVM natural-loop
  continuation.** Auditing the existing replay-checked BMC route shows that a
  second textual-CFG unroller would duplicate semantics before the frontend can
  derive a trustworthy one-step relation. The zero row therefore freezes an
  exact clang-21 `capdiv` module (SHA-256
  `1592057004b6db95281a96f8a24835cd8fbeade5a297a70ec2a76988fc2bc8e7`):
  implicit entry `%2`, header `%6`, division block `%11`, latch `%15`, exit
  `%4`, and paths `%6 -> %15` / `%6 -> %11 -> %15`. The accepted profile must
  preserve path-conditioned `udiv` UB, latch-PHI semantics, deterministic
  state/path order, and precise rejection boundaries, then reuse solver BMC for
  bounded unrolling. No multi-block implementation or result exists yet; the
  ADR and exact `llvm-as-21`-clean fixture are committed before implementation.
  Next: implement the frozen constructor and all twelve acceptance gates.
...
- **2026-07-20 — ADR-0291 accepts the first typed canonical LLVM loop bridge.**
  ...
  `DISAGREE = 0`; negative shape/error tests pass. The standing gate remains
  62/62 semantic variants and grows to eight binaries / 70 tests. Complete
...
- **2026-07-20 — ADR-0292 freezes the next T5.1.4 loop input before
  implementation.** The exact clang-21 `capdiv` module and provenance are now
  registered with a twelve-gate profile for one single-latch natural loop. The
  selected route derives a path-sensitive checked transition, preserves the
  even-`d=0` path that skips division, and delegates k-unrolling to the existing
  replay-checked BMC engine. This is a zero row: no new loop semantics or
  performance/correctness result is claimed yet.
