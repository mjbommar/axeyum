...
- **2026-07-21 — ADR-0332 preregisters structural metadata authentication.**
  V5 keeps the exact 169-entry lock check but replaces active-count equality
  with closed unique package/node/edge IDs, exact external lock identities,
  in-tree path manifests, one workspace kernel, and a canonical result-only
  active digest/count. The thin wrapper, five v5 plus 18 inherited tests, and
  compact overlay validate nine producer files without an expected active
  count. Next: commit/push, then invoke v5 once under the registered cgroup.
...
| P5.5 | External target, measured | WIP — **Prior routes close negative; structural metadata preparation v5 frozen pre-fetch (ADR-0323--0332):** v4 proves DNS/fetch/hard-link inventory then rejects invalid active==locked count. ADR-0332's thin wrapper, 23 combined tests, and compact overlay now freeze closed graph IDs, exact lock sources, in-tree manifests, and result-only active count/digest while retaining every other gate. Commit/push before invocation. No cache/target/partial bytes, capture credit, proof, or scoreboard row exists. Phase exit still requires authenticated capture plus a separately preregistered measured scoreboard result, DISAGREE=0, replay, and wall-times. |
