...
| P5.5 | External target, measured | WIP — **Authenticated Tock capture and T5.5.3 measured proof scoreboard DONE (ADR-0323--0338):** accepted v4 proves definedness/zero/floor-log/MSB for both helpers with eight rechecked dual-DRAT rows and refutes/replays six controls, UNKNOWN=0, DISAGREE=0. Query time 12.700 s, peak RSS 1,256,496 KiB, zero OOM deltas; committed summary authenticates every certificate hash/size. This is a concrete checked-proof use case, not a speed or whole-kernel claim. Remaining P5.5.4: honest comparison/write-up and any responsible upstream links. |
