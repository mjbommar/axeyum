...
| P3.6 / TL0.6.4 | U2 native-surface classification | **WIP — M0/M1/M2.0 accepted; M2.1 pre-execution ready; M2.2 plus R1 and M2.3-M2.4 semantics preregistered.** M0 supplies the 3,723-case harness floor; M1 supplies the complete 7,004-file content census; M2.0 freezes the empty typed dependency/provider graph across 111 official variants and 408,374 factored case/variant occurrences. M2.1 freezes 4,092 exact Lean inputs, 32 batches, 14 controls, and a 39-process no-retry fast/full header pass, but observed processes and edges remain zero pending explicit authorization. M2.2 freezes first-prefix resolution, separate existence/content, raw/effective closure, ordered `.olean` parts, IR reachability, cycle declines, and controls. M2.3 freezes the 3,670 wrapper / 52 inline Lake / one lint dispatch partition, exact configured CMake/CTest provenance, 41 runner paths, shell-state/source semantics, hooks/markers, static-versus-observed edges, and downstream ownership. M2.4 freezes Lake configuration, manifest/materialization, target/facet/job, query/setup, artifact/cache/network, and downstream ownership semantics over a 52-case/70-root direct floor plus 28 wrapper candidates pending M2.3. M2.2-M2.4 have no input authority, process budget, observation, or credit. Next: explicitly authorize and validate M2.1, then separately bind and execute M2.2-M2.7 before M3 complete-row review. |
