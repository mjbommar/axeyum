...
- **Session 2026-06-27 — Property SDK setup-aware replay tests.**
  `Counterexample::render_rust_test_with_setup` now emits replay-checked scalar
  bindings first, caller-owned setup snippets next, and the replay/assertion
  body last. This lets frontends render nested/domain aggregate initializers
  with `render_rust_named_struct_let` /
  ...
  the nested aggregate row checking that the setup snippets appear before the
  replay assertion; totals remain 2 proved, 6 disproved, 0 unknown,
  DISAGREE=0, and 1/1 Lean-required coverage. External proptest/Kani-style
  baselines remain open. Verification passed:
  `cargo fmt --all --check`;
  `git diff --check`;
...
- **2026-06-27** — **Property SDK setup-aware replay tests.**
  Added `Counterexample::render_rust_test_with_setup` so generated Rust
  counterexample tests can insert caller-owned aggregate/domain setup snippets
  between replay-checked scalar bindings and the replay assertion; refreshed the
