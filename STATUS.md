...
- **2026-07-09 — projected arrays use deterministic majority-default models.**
  ...
  is not a performance claim. ADR-0074. Next: e-graph-class defaults, warm reuse,
  merge-triggered/cross-atom queue state, and proof integration.
- **2026-07-09 — array equality/disequality is live on canonical CDCL(T).** The
  ...
  768-comparison matrix is now half equality-bearing and remains clean. Public
  ...
  claimed. ADR-0073. ADR-0074 adds majority-default projection; next remains
  merge-triggered/cross-atom queue state, warm class ownership, and proofs.
- **2026-07-09 — lazy read-over-write is live on the canonical array bus.** The
  ...
  merge-triggered/cross-atom queue states and scalable models.
...
| P1.6 | Theory combination (th_eq bus, interface equalities) | WIP — EUF+LIA/LRA dispatch and canonical QF_UFBV/QF_ABV/QF_AUFBV combination are live. ADR-0066–0070 establish exact BV/EUF interfaces and replay-guided UF refinement; ADR-0071/72/73 add candidate-guided select congruence, lazy ROW, and bounded array equality/diff observations on the same bus; ADR-0074 adds majority-default original-symbol models. Function-then-array projection and original replay gate SAT; eager routes remain fallbacks/proof producers. ADR-0052 already closes the recorded bounded-string `str.len` marker. Remaining: partitioned opaque-heavy arithmetic admission, arithmetic-UF model lifting, merge-triggered/cross-atom array queue states, e-graph-class/warm models, and proof integration |
...
| P2.2 | Arrays: lazy ROW axioms + extensionality + func_interp models | WIP — eager elimination/certifying fallback remains (ADR-0010). Canonical QF_ABV/QF_AUFBV now has replay-guided base-select congruence (ADR-0071), candidate-guided lazy ROW (ADR-0072), bounded equality/diff observations (ADR-0073), and deterministic distinct-index majority-default finite-map models (ADR-0074). Function-then-array projection and original replay gate SAT; cloned probes leave fallbacks pristine. The 768 eager/front-door/Z3 comparisons are clean and half equality-bearing; public 1 s coverage is QF_ABV 187/193 and QF_AUFBV 49/53, DISAGREE=0. ADR-0075 externally checks direct select congruence. Remaining: merge-triggered new/delayed/applied queue state, cross-atom observations, e-graph-class model ownership, warm reuse, nested/extended arrays, and ROW/diff-witness/online proof integration |
...
- **2026-07-09 — candidate-guided array extensionality joined canonical CDCL(T).**
  ...
  eager/front-door/Z3 comparisons pass; 384 comparisons are equality-bearing.
