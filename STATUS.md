...
| P3.6 / TL0.6.5 | U2 matched official/Axeyum execution | **WIP — comparison contract and terminal registry schema corrected; execution blocked on complete accepted TL0.6.3/TL0.6.4 parents.** The preregistered plan separates 3,723 case rows, 408,374 candidate execution slots, and the future layer-expanded comparison-obligation denominator. Terminal cells now contain independent official/native execution records plus a third normalization/comparison record; typed `not-run`/`invalid` sides preserve missing and inconsistent observations. Exact per-U0-U9 expected-count and sorted-ID authorities make G3 reject nonempty agreeing subsets and omitted rows. No comparison authority, Axeyum process, native outcome, pair, performance row, U2 promotion, axis/gate completion, or parity credit exists. Next: complete the two parents, derive the exact comparison-obligation authority, implement normalization mutation controls, and only then authorize the first native shard. |
...
- **2026-07-23 — Preregistered TL0.6.5 matched execution and corrected the
  ...
  per-population expected-count/sorted-ID authorities make G3 reject a
  nonempty all-agree subset or omitted rows. Focused mutation tests and
