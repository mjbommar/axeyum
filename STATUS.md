...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Native lineage timing, phase, gate/root, and bounded candidate attribution DONE (ADR-0171--0174).** Unprofiled lineage is 0.746x Z3; v2 partitions all 11.73M clauses and v3 rejects immediate-only flattening on Dptf | Attribute AIG construction per added node; never substitute diagnostic timing for the unprofiled bar |
  ...
  | **GQ5 AIG/CNF construction** | **AIG per-node attribution is leading (ADR-0172--0174).** Root fusion/dedup are closed; internal AND flattening is deferred after +17.62% clauses / +3.65% native Dptf time | Separate structural-hash lookup/reuse, allocation/copy, and lowering bookkeeping per added node; reopen fusion only with future-use evidence/replacement |
  ...
  | **GQ10 real-lifter regression tier** | **WIP; ordered, repeated native, exact warm-profile v2 three-driver, and bounded v3 candidate tiers landed.** The diagnostic bars complement repeated 0.746x-Z3 lineage | Widen families, add full-tier/per-commit variance, and keep pre-parsed, actual-client, unprofiled lineage, and diagnostic bars separate |

  **Next actions:** (1) attribute AIG construction per added node, separating
  hash lookup/reuse, allocation/copy, and lowering bookkeeping; (2) select one
  bounded AIG change under the repeated unprofiled native gate; (3) keep SAT
  third; (4) calibrate lineage caps
...
| Phase | Title | Status |
|---|---|---|
| P4.1j | Glaurung warm delta and duplicate/prefix reuse (GQ7/GQ8) | **WIP — ADR-0171--0174 accept native timing/attribution and reject the first internal-CNF candidate.** Repeated lineage is 0.746x Z3; v2 partitions 11.73M clauses and v3 defers flattening after a Dptf structural/time regression. Capacity fallback is live. Attribute AIG per node, then calibrate memory and widen before GQ8/GQ9. |
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for cold transitions and bounded native timing/phase/gate/candidate attribution; widening WIP.** ADR-0171 repeats lineage at 0.746x Z3; ADR-0172--0174 partition the warm path and defer internal AND flattening on cumulative clauses plus unprofiled time. Diagnostic overhead is excluded from claims. Remaining: AIG per-node attribution/acceptance, memory admission, wider/full variance, and per-commit ratios. |
