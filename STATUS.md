...
- **2026-07-19 — ADR-0259 accepts the fixed cold GQ5 attribution and ADR-0260's
  artifact-v36 origin diagnostic is implemented but unobserved.** Clean
  detached `d29470cf`
  decides all 162 corrected-wide-v3 representative queries (88 SAT / 74
  UNSAT), agrees 162/162 with the manifest and in-process Z3, replays all 88 SAT
  models, and passes all six independent profile identities. Of 391,251
  non-tautological clause attempts, 119,260 (30.4817%) are exact duplicates;
  every one is a primary hit. Collision work, repeated literals, and
  complementary-literal tautologies are zero. Slice-partial owns 87,605
  duplicates (73.4572%) and register-slice 31,035 (26.0230%). This rejects
  another index redesign but does not convert counts into a wall-time claim.
  Artifact v36 now carries stable first/duplicate emission sites, actual
  collision-entry provenance, same/cross-owner cells, and exact length/literal
  partitions only in the profiled monomorph; the disabled store stays
  zero-sized. Its analyzer independently re-sums cells, family/outcome and
  participation distributions, and the fixed 50% / 10-query / 50% selection
  rule. All 304 CNF, 880 solver, 43 bench, and five analyzer tests plus strict
  targeted Clippy pass. Next: commit this unobserved implementation, then
  repeat the exact fixed population from that clean detached commit before
  selecting at most one separately gated generator-elision experiment.
...
- **2026-07-19 — ADR-0258 retains the capped nontrivial-DRAT no-selection and
  ...
  breadth; keep symbolic memory gated.
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Map, query/internal attribution, fresh/retained exact-CNF controls, four-driver neutral cold-reset and source-owner-retained SMT breadth, bounded/canonical/four-schedule raw authority cells, process-isolated corrected-representative plus wider-holdout end-to-end faithfulness, deadline-aware generated proof widening, isolated behavior-preserving Glaurung A0, taint/SystemBuffer baseline corrections, confidence partition, source-backed nonzero positive control, two rejected usbprint resource protocols, v2 maximum-precision rejection, stack-region correction, corrected v3 sweep, exhaustive difference adjudication, v6 hidden-work rejection, one neutral standard DRAT consumer plus checker-sanity control, and the capped nontrivial-proof no-selection DONE (ADR-0187/0188/0213--0258 plus Glaurung `7f682e5`; instrumentation candidate `ff3c0a7`).** cvc5 agrees on all 9,526 checks in both external protocols; ADR-0253 accepts 515 SAT replays, all 509 CNF DRAT rechecks, and 508/509 stronger certificates on the holdout. ADR-0256 accepts the real `drat-trim` verification and fixed satisfiable-CNF control; ADR-0258 retains 32/32 follow-on two-byte proofs and closes further holdout mining. Tcpip remains zero-high/unlabeled | Add broader labeled findings and timeout-sensitive neutral breadth; keep symbolic memory gated; coordinate the Glaurung footer |
  ...
  | **GQ5 AIG/CNF construction** | **Cold lane active; broad warm lane closed by ADR-0219; artifact-v35 attribution ACCEPTED without optimization (ADR-0259), artifact-v36 origin profiler IMPLEMENTED but unobserved (ADR-0260).** V35 finds 119,260 exact primary duplicates and zero collision/repeated/complement work; v36 adds stable first/duplicate sites, same/cross owner, lengths/literals, and independent selection-gate analysis with a zero-sized disabled store | Commit the tested v36 implementation before observation, then repeat the fixed 162-query run; select at most one later generator-elision candidate only under the preregistered dominance/distribution gate |
  ...
  **Next actions:** (1) coordinate with the active Glaurung explorer owner and
  ...
  coordinate isolated Glaurung instrumentation candidate `ff3c0a7`; ADR-0250's
  ...
  driver and keep each timeout cell separate; (10) preserve ADR-0259's accepted
  no-optimization result and execute ADR-0260's preregistered duplicate-origin
  attribution from a committed clean detached source before considering one
  generator-elision candidate; (11) stage solver
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver fair map, internal/fresh/retained controls, neutral cold/retained SMT, complete three-solver timeout frontier, independent four-oracle fuzz, proof denominators including the accepted 1,024-query holdout, executable WebAssembly, warm time/RSS/hit-rate deployment, bounded/canonical/four-schedule authority cells, representative whole-certificate process isolation, isolated behavior-preserving Glaurung A0, corrected provenance/SystemBuffer environment, versioned confidence partition, source-backed nonzero positive control, two rejected usbprint resource protocols, v2 maximum-precision rejection, model-independent detector correction, corrected v3 scalar sweep, exhaustive source-backed difference adjudication, v6 hidden-work rejection, one neutral standard DRAT consumer plus checker-sanity control, capped nontrivial-proof no-selection, accepted artifact-v35 cold CNF attribution, and the unobserved artifact-v36 origin implementation DONE (ADR-0213--0260 plus Glaurung `7f682e5`; stop-class instrumentation candidate `ff3c0a7`).** ADR-0253 adds 1,024/1,024 manifest/Z3 agreements, 515 SAT replays, 509 CNF DRAT rechecks, and stable 508/509 stronger coverage. ADR-0256 accepts one pinned real external verification and the preregistered satisfiable-CNF control while retaining ADR-0254's no-op mutation rejection; ADR-0258 shows all 32 fixed follow-on DRATs are also trivial two-byte proofs and closes further holdout mining. ADR-0259 accepts 162/162 decisions/manifest/Z3 agreements and all 88 SAT replays; 119,260 exact primary duplicates select only ADR-0260's committed-before-observation origin diagnostic, not an optimization. Tcpip variation remains zero-high/unlabeled. Remaining: execute the fixed v36 observation, coordinate the Glaurung footer, add genuinely broader labeled evidence, add timeout-sensitive neutral breadth, and coordinate clean integration separately. |
