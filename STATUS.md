...
- **Session 2026-06-27 — Warm same-index ROW admission.**
  `IncrementalBvSolver::assert_simplifying_memory` now folds the narrow
  syntactic read-over-write identity `select(store(a, i, v), i) -> v` before
  warm BV encoding while retaining the original memory assertion for model
  replay and memory-aware checks. `SymbolicExecutor::assume_auto` uses that
  path, so simple store/read-back constraints can continue through ordinary
  warm `status` / `model` instead of forcing the one-shot memory dispatcher.
  The boundary is explicit: distinct-index ROW, extensionality, and UF lemmas
  remain deferred to `check_with_memory` until the full retained-lemma lazy
  array/UF engine lands. Verification passed:
  `cargo fmt --all --check`;
  `git diff --check`;
...
- **Session 2026-06-27 — SymbolicMemory write-log normalization.**
  ...
  same-index writes shadowed by later writes, `load_with_write_log` emits one
  deterministic read-over-write `ite` guard per visible write with true
  last-writer-wins ordering for symbolic aliases, and the load-equality
  branch/assume helpers route those compact reads through the existing
  memory-aware dispatcher. This mitigates U7's frontend encoding pain while
  U6's true warm lazy-array/UF engine remains open. Verification passed:
  `cargo fmt --all --check`;
  `git diff --check`;
...
- **2026-06-27** — **Warm same-index ROW admission.**
  Added `IncrementalBvSolver::assert_simplifying_memory`, a narrow warm-safe
  memory simplifier that folds syntactic `select(store(a, i, v), i)` terms to
  `v` for BV encoding while retaining the original assertion for replay.
  `SymbolicExecutor::assume_auto` now uses it, so simple store/read-back path
  constraints can stay on ordinary warm `check`/`model`; general array/UF
...
- **2026-06-27** — **SymbolicMemory write-log normalization.**
  ...
  same-index shadowed writes, emit one deterministic `ite` guard per visible
  write, and branch/assume on the resulting load equality through the
