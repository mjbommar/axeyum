...
| P1.6 | Theory combination (th_eq bus, interface equalities) | WIP — **EUF+LIA/LRA combination landed & dispatched (QF_UFLIA/UFLRA), complete for conjunctive UNSAT**: `declare_fun` admits Int/Real UF sorts, and `check_with_uf_arithmetic` decides the core squeeze/nested congruence UNSAT cases; SAT model lifting for arith UF remains conservatively `Unknown`. The QF_UFLIA overbound lazy CEGAR path now avoids duplicate generic LIA timeouts, collapses strict/non-strict order complements, folds simple integer-bound theory tautologies/contradictions before atom allocation, batches same-candidate UF lemmas, batches simple bound conflicts, seeds bounded complement-bound monotonicity, checks deterministic Boolean-justified arithmetic supports, reports support-path counters and dynamic arithmetic core-source counters, carries reusable arithmetic clauses, and now keeps an `IncrementalArithDpll` warm while UF lemmas are appended. The `uf_pair_profile` diagnostic shows the hard row has **214/282** constant-vs-constant pairs, capped unary-Int preseed was rejected (**673 atoms**, **0** candidates at 10 s), violated-pair-only refinement was rejected because it lost the 1 s UF candidate, and implication flattening was rejected for the same reason; the retained all-equal-argument batching, implication-preserving Boolean shape, and narrow Int-bound fold are regression-guarded/documented. The generated rows stay `unknown`, but 1 s diagnostics now reach actual UF refinement (**2** UF rounds, **1** candidate, **6** UF lemmas), and at 10 s the first row reaches **6** UF CEGAR rounds, **5** candidates, **1357** pair checks, **24** equal-argument pairs, **15** violations, and **24** learned UF lemmas; the warm arithmetic state reports **solve_calls=6**, **total_rounds=305**, **bound_lemmas=659**, **blocking_lemmas=319**, and a late dynamic-core source mix dominated by **core_src_lp=276** with **core_src_minimized=0** and **core_src_large=0** before timeout. Plus the combination primitives `theory_combination` (shared/propose/classify/arrangement) + `th_eq` bus (`theory_var_classes`/`interface_th_eqs`) and the earlier lazy/on-demand Ackermann for QF_UFBV. Remaining: UF CEGAR convergence/relevance after several candidate models, then full online interface-equality (Nelson-Oppen) combination of the e-graph + BV to drop Ackermann reduction entirely |
