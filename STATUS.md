...
- **Session 2026-06-27 — First-class counterexample minimization helper.**
  ...
  optimization surface. Remaining follow-up: wire this as an optional mode on
  higher-level `prove` / evidence APIs and extend the objective surface when
  wider BV or richer theory values have a native minimization order.
  Verification passed:
  `cargo fmt --all`;
  `cargo fmt --all --check`;
