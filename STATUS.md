...
- **2026-07-18 — ADR-0246 accepts model-independent stack-region semantics;
  ADR-0247 preregisters the corrected v3 sweep.** V2 remains preserved and
  ...
  tests; only the same two baseline WinAPI rendering assertions fail. V3 fixes
  that revision, rebuilt authority hashes, all five scalar policies, and the
  unchanged positive/tcpip work bounds before observing any cell. Run v3 next;
  keep complete usbprint separate and keep symbolic memory conditional. The
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  |---|---|---|
  | **GQ1 real-query profile** | **Map, query/internal attribution, fresh/retained exact-CNF controls, four-driver neutral cold-reset and source-owner-retained SMT breadth, bounded/canonical/four-schedule raw authority cells, process-isolated corrected-representative end-to-end faithfulness, deadline-aware generated proof widening, isolated behavior-preserving Glaurung A0, taint/SystemBuffer baseline corrections, confidence partition, source-backed nonzero positive control, rejected usbprint resource cell, v2 maximum-precision rejection, and stack-region correction DONE; v3 PREREGISTERED (ADR-0187/0188/0213--0247 plus Glaurung `7f682e5`).** cvc5 agrees on all 9,526 checks in both external protocols; 302 raw sinks match under unrestricted sole authority on four drivers. ADR-0243 accepts 14/14 planted rows. Minimum retains the exact set at 60,064 solves and preserves tcpip 110/110 at 80,563 solves. ADR-0244 rejects complete usbprint as policy-invariant after four deadline hits. ADR-0245 records the rejected maximum stack label; ADR-0246 restores its exact 14-row set. All accepted proof results remain unchanged | Run/analyze the exact corrected v3 sweep; separately bound usbprint; add wider real proof cells |
  ...
  | **GQ7 warm delta entry** | **Source identity, fair map, query/internal attribution, fresh/retained CNF controls, four-driver neutral cold-reset plus source-owner-retained SMT, bounded raw finding parity, canonical tcpip authority, four-schedule union, isolated configurable-policy A0, taint/SystemBuffer baseline corrections, confidence partition, 14-row source-backed positive control, v1 resource rejection, v2 precision rejection, and detector correction DONE; v3 PREREGISTERED (ADR-0201--0247 plus Glaurung `7f682e5`).** Minimum retains 14/14 but costs 60,064 solves; ADR-0246 restores maximum to the exact 14-row set; complete usbprint remains a separate resource failure | Run all five v3 scalar settings; gate symbolic memory on validated headroom and keep usbprint separately bounded |
  ...
  **Next actions:** (1) coordinate with the active Glaurung explorer owner and
  ...
  prefix and ADR-0246's accepted correction trail; run ADR-0247's exact
  preregistered five-policy v3 campaign from a clean detached Axeyum commit,
  preserve/analyze every cell fail-closed, and update the result state before
  selecting further work; extend it with BoundarySet/DiverseEnum only after bounded
  ...
  memory only if the corrected sweep leaves validated coverage headroom; (4)
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver fair map, internal/fresh/retained controls, neutral cold/retained SMT, complete three-solver timeout frontier, independent four-oracle fuzz, proof denominators, executable WebAssembly, warm time/RSS/hit-rate deployment, bounded/canonical/four-schedule authority cells, representative whole-certificate process isolation, isolated behavior-preserving Glaurung A0, corrected provenance/SystemBuffer environment, versioned confidence partition, source-backed nonzero positive control, rejected usbprint resource cell, v2 maximum-precision rejection, and model-independent detector correction DONE; v3 PREREGISTERED (ADR-0213--0247 plus Glaurung `7f682e5`).** The 12,000-row independent/edge campaign has 12,000 four-way agreements and all 14 corner families. All accepted proof/deployability results remain unchanged. ADR-0243 accepts 14/14 planted rows. ADR-0244 shows minimum retains 14/14 but raises work to 60,064 solves and fails all four complete-usbprint processes at the fixed wall. ADR-0245 preserves the rejected model-dependent stack classification; ADR-0246 restores maximum to 14/14 exact. Browser and high-reuse memory costs are measured. Remaining: run/analyze v3, preregister a bounded usbprint frontier, coordinate clean integration separately, and add wider real manifests. |
...
- **2026-07-18 — ADR-0246 repairs stack-region semantics and ADR-0247 freezes
  ...
  findings with no unexpected high row and exact authority parity. Glaurung
  `7f682e5` and both rebuilt authority hashes are now preregistered with all five
  scalar policies and the unchanged positive/tcpip work bounds before result
  observation. Symbolic memory remains gated on the analyzed v3 result.
