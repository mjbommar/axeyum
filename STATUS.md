...
- **2026-07-18 — ADR-0241 makes finding confidence machine-readable and selects
  the first nonzero corrected corpus.** Glaurung `931d8a8` adds opt-in,
  ...
  supplies the next target: five Z3 versus four Axeyum stable high-confidence
  rows, including one Z3-only `SystemBuffer` null dereference, so the harness
  correctly rejects parity. Next integrate the isolated Glaurung branch, then
  independently label and root-cause those five rows before preregistering a
  policy-configuration sweep. Boundary/diverse selection remains configuration;
  symbolic memory remains conditional on validated residual headroom.
...
- **2026-07-18 — ADR-0240 corrects the concretization finding baseline before
  ...
  selects nonzero `usbprint.sys`, and advances the isolated branch to `9692f3c`.
...
- **2026-07-18 — behavior-preserving Glaurung A0 concretization policy accepted
  ...
  the existing settings and selects the nonzero corpus. Next coordinate and
  integrate the corrected clean branch, label usbprint, then treat
  BoundarySet/DiverseEnum as settings of this mechanism. Symbolic memory
  remains conditional on validated sweep headroom.
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ7 warm delta entry** | **Source identity, fair map, query/internal attribution, fresh/retained CNF controls, four-driver neutral cold-reset plus source-owner-retained SMT, bounded raw finding parity, canonical tcpip authority, four-schedule union, isolated configurable-policy A0, taint-baseline correction, and confidence partition DONE; labeled policy sweep WIP; wider default DEFERRED (ADR-0201--0241 plus Glaurung `9692f3c`).** Neutral same-protocol retention is 16.4x--57.0x below full reset; A0 preserves default value selection. Every existing tcpip setting has zero accepted findings, the old 33-row remainder is producer-diagnostic, and usbprint supplies the selected nonzero labeling target with one authority-only row | Integrate the corrected A0 branch; label/root-cause usbprint before sweeping deterministic policy configs; gate symbolic memory on residual validated headroom |
  ...
  **Next actions:** (1) coordinate with the active Glaurung explorer owner and
  integrate isolated branch `axeyum-concretization-policy-a0` at `9692f3c`
  ...
  selector aliases, and exact-source taint correction are complete, so do not
  reimplement them or edit the live dirty checkout; (2) independently label the
  five usbprint high-confidence rows and root-cause its one stable Z3-only
  `SystemBuffer` row; the existing settings are rebaselined and the tcpip 33-row
  remainder is producer-diagnostic, but producer confidence is not ground
  truth; (3)
  preregister the resulting policy sweep across AnyModel,
  least/greatest/site-hash, BoundarySet, and eventually DiverseEnum, keeping
  ...
  `>= AnyModel` as a recall gate, and begin symbolic memory only if this sweep
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver fair map, internal/fresh/retained controls, neutral cold/retained SMT, complete three-solver timeout frontier, independent four-oracle fuzz, proof denominators, executable WebAssembly, warm time/RSS/hit-rate deployment, bounded/canonical/four-schedule authority cells, representative whole-certificate process isolation, isolated behavior-preserving Glaurung A0, corrected provenance, and a versioned confidence partition DONE; configurable-policy sweep and correctness-led evidence open (ADR-0213--0241 plus Glaurung `9692f3c`).** The 12,000-row independent/edge campaign has 12,000 four-way agreements, 4,471/4,471 SAT replays, 14/14 semantic-corner families, and zero nondecision/failure/disagreement under its correctness-only resource bound. The 52-formula 50--1000 ms sweep has complete four-population accounting, zero three-solver contradiction, and an all-decided cold Axeyum/Z3 geomean of 0.21095 at 1000 ms; it remains separate from the workload-dependent retained-warm map. All 1,505 width<=8 generated UNSAT rows recheck CNF DRAT and 1,487 recheck end to end; all 74 UNSAT rows in the corrected 162-query real representative recheck both CNF DRAT and stronger end-to-end certificates in two clean process-isolated runs, while a 1 ms kill control retains all 74 rows; 302 raw sinks match under unrestricted sole authority on four drivers. Tcpip's existing settings all have zero accepted rows and its former 33-row remainder is producer-diagnostic; usbprint supplies a five-versus-four nonzero labeling target. Browser and high-reuse memory costs are measured. Remaining: integrate the clean A0 branch, label/root-cause usbprint, preregister and run the wider fixed-work policy sweep, and add wider real manifests. |
