...
- **2026-07-14 — Glaurung's `slice-partial` affine-add reduction is accepted;
  ...
  Proposed ADR-0155 therefore selects exact modular constant cancellation
  across equality: `sum(symbolic) + c = k` becomes
  `sum(symbolic) = k - c`. It removes one wide adder and exposes hash sharing
  across offset variants without changing symbols or model projection. The
  rule remains unaccepted until exhaustive/wide/Z3 evidence, five
...
- **Historical Glaurung build-up through 2026-07-14 (superseded by the measured
  ...
  **GQ1/GQ10 readiness increment:** artifact v25 retains v23's word-policy
  ...
  | **GQ2 cheap cold tier** | **WIP with two accepted tranches and one proposed.** Canonical v2 cuts corrected full total 13.3%; ADR-0153's v3 add-chain fold cuts another 9.80%. V28 selects proposed ADR-0155 exact equality cancellation | Implement and semantically gate ADR-0155, then require representative structural and timing wins before a guarded full run |
  | **GQ3 coercion/affine peepholes** | **WIP with two production wins and one proposed exact rule.** ADR-0142 lands slice/coercion rules; ADR-0153 lands scalar/wide modular add-chain folding; ADR-0155 proposes canceling its remaining constant across equality | Keep ADR-0155 exact/wide/bounded; broader affine normalization remains deferred pending its measured result |
  ...
  **Next actions:** (1) hand the ordered worker/path/scope trace v1 and
  ...
  proposed ADR-0155 through exhaustive/wide/Z3 tests and representative
  structural/timing gates before any guarded full experiment, keeping broad
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **DONE for the generic/cold GQ10 gates, first rewrite-transition comparison, and post-v3 attribution; WIP for producer repair.** Artifact v27 provides fixed identities/resources and scheduled tiers; artifact v28 adds complete original/post-word scalar QF_BV inventories, with a clean 13,462-query residual profile selecting proposed ADR-0155. Five baseline full trials establish 0.51% total/ratio CV and 3%/3%/2% alarms; ADR-0153's exact v2→v3 comparator passes them and reaches 14.11 s / 1.85x. Remaining: ADR-0155 experiment, malformed-dump/dedup repair, and an ordered trace gate. Synthetic tiers cannot substitute for the real lifter |
