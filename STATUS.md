...
- **2026-07-21 — ADR-0332 preregisters structural metadata authentication.**
  ...
  active digest/count. Next: implement/push the thin validator before DNS/fetch.
...
| P5.5 | External target, measured | WIP — **Prior routes close negative; structural metadata preparation v5 preregistered (ADR-0323--0332):** v4 proves DNS/fetch/hard-link inventory then rejects an invalid 162-active-versus-169-locked equality. ADR-0332 now freezes closed package/node/edge IDs, exact lock source identities, in-tree path manifests, and result-only active count/digest while retaining every other gate. Implement/push before DNS/fetch. No cache/target/partial bytes, capture credit, proof, or scoreboard row exists. Phase exit still requires authenticated capture plus a separately preregistered measured scoreboard result, DISAGREE=0, replay, and wall-times. |
