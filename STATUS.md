...
- **2026-07-17 — ADR-0223 completes neutral cvc5 breadth across the accepted
  ...
  done; topology-equivalent neutral performance, the typed multi-oracle fuzzer,
...
- **2026-07-17 — The consolidated code/publication review changes the success
  ...
  checks. Publication blockers are now: (1) a topology-equivalent warm representation/integration
  control; (2) a standing well-typed
  multi-oracle fuzzer seeded by empty-model, extension-width, concat-width, and
  W128 truncation failures; (3) another neutral implementation/API boundary;
...
| P4.5 | Benchmarking & the performance gate (measured Z3 head-to-head) | **WIP: four-driver map, query/internal attribution, fresh/retained exact-CNF controls, and neutral cold-reset SMT breadth DONE; correctness-led evidence open (ADR-0213--0223).** Five fixed-work repetitions establish two wins, parity, and a Z3 win; 9,526 rows reject size alone; 431 ordered core calls show retained BatSat beating Z3 Boolean on Axeyum CNF; cvc5 agrees on all 9,526 checks. Remaining: warm-neutral SMT; multi-oracle fuzzing; finding parity; timeout-sensitive driver; measured qfbv/WASM/RSS/DRAT deployment. |
