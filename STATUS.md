...
- **Session 2026-06-26 — AUFLIA scalar equality projection repair.**
  Replay projection now has a bounded scalar equality repair pass for
  non-branch generated equalities. For a false direct scalar equality it tries
  both assignment directions when possible, counts positive replay conjuncts on
  each trial, and keeps only a direction that strictly reduces replay failures.
  The pass is capped, full-replay gated, and reports separate
  `scalar_repair_*` counters so select and branch repair telemetry remains
  interpretable. A final post-scalar stabilization also reruns select repair if
  scalar-triggered branch repair mutated arrays.
  ...
  **diff_skolems=146**, and **working_assertions=7127**. Projection repair now
  reports **select_repair_candidates=1694**, **select_repair_array_changes=20**,
  **select_repair_symbol_changes=133**, **branch_repair_candidates=60**,
  **branch_repair_symbol_changes=60**, **scalar_repair_candidates=5**,
  **scalar_repair_symbol_changes=5**, and **projection_repair_changes=218**.
  The first false conjunct is now direct equality **ordinal 190**, term
  **3017**, `x_366 = x_92`, with values **1** vs **0**. Next useful work is
  support-aware scalar/readback propagation for the remaining generated equality
  chain, or a direct explanation of why that equality should stay unconstrained
  and the array route must refine instead.
  Verification passed:
  `cargo fmt --all --check`;
