...
- **2026-07-20 — ADR-0298 preregisters relational scalar call results before
  implementation.** The selected checksum-module rule gives each opt-in
  straight-line call a fresh internal result and exposes its verified
  `ensures` relation separately from LLVM value definedness. The frozen gate
  verifies `sum16`, discards its body, re-proves modular `cksum_pair` against
  inlined MIR/LLVM, classifies valid and deliberately invalid havoc choices for
  100,000 inputs with no dropped work, and requires a replayed weak-contract
  countermodel proving no hidden exact substitution. Strict `Result`/equality/
  `ite` sorts, namespace isolation, mutations, explicit `Unknown`, and a 4 GiB
  cap are mandatory. This proposes no code, annotations, MIR calls, loop
  havoc, effects, or performance claim.
...
| P5.2 | Contracts & modular verification (`#[requires]`/`#[ensures]`, calls as composition) | WIP — ADR-0295 accepts the checked direct-body/inlined baseline. **ADR-0296 accepts the first actual composition rule:** one exact scalar `leaf` contract is checked against its body once and the body is discarded. **ADR-0297 accepts nontrivial requirements without silent pruning:** `trans` assumes the requirement only after its exact reached complement becomes a replayable, source-attributed `bad` state. **ADR-0298 now preregisters the checksum continuation:** a fresh straight-line scalar result plus a separate verified relation, weak-contract havoc teeth, and 100,000 valid/violating result choices. Phase exit still requires accepted implementation plus MIR-side modular composition and DISAGREE=0 |
...
- **2026-07-20 — ADR-0298 preregisters relational checksum-call havoc.** The
  frozen design separates a fresh internal return, its LLVM definedness, and
  its asserted postcondition. It requires exact `sum16` body verification,
  body discard, modular/inlined MIR+LLVM proofs, 100,000 dual-choice rows,
  strict sort/namespace/mutation gates, and a replayed weak-contract
  countermodel. No implementation or acceptance is claimed yet.
