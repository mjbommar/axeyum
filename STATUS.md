...
- **2026-07-21 — ADR-0329 preregisters dedicated Tock cache preparation.**
  ...
  inventory. It may not compile or capture. Next: implement and push the
  producer/tests/registration before any fetch or cache observation.
...
| P5.5 | External target, measured | WIP — **Maestro and Tock capture v1 close negative; dedicated-cache preparation preregistered (ADR-0323--0329):** ADR-0327 accepts typed `range`/`ctlz`; ADR-0328 v1 stops pre-build on uncached `ghash 0.4.4`. ADR-0329 now freezes a non-compiling locked fetch plus offline read-only validation and whole-cache inventory before capture v2. Implement/push its producer before network. No target/partial bytes, capture credit, proof, or scoreboard row exists. Phase exit still requires authenticated capture plus a separately preregistered measured scoreboard result, DISAGREE=0, replay, and wall-times. |
