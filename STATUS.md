...
| P5.5 | External target, measured | WIP — **Authenticated Tock capture/parser admission DONE; proof v1 accepted as a pre-query stale-lock negative (ADR-0323--0335):** v3's two raw-identical modules admit both helpers. The runner freezes eight certified proofs plus six reflected/native-replayed controls, limits, pushed-HEAD isolation, and atomic output. Its sole v1 invocation passes archive validation but Cargo rejects the committed stale lock under `--locked --offline` before compilation; zero query/proof/control/row exists and no output survives. V1 is closed. Preregister v2 with only a matching committed lock snapshot and new output; phase exit still requires the measured result with DISAGREE=0. |
