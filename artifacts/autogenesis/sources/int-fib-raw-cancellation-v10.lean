  ...
  have cancel : ∀ x z : Nat,
    ...
    | succ x =>
        ...
        | succ z =>
            ...
            exact Int.subNatNat_of_sub_eq_succ (Nat.add_sub_cancel (x + 1) (z + 1))
...
theorem oddAdd (a b : Nat) :
  ...
  have cancel : ∀ x z : Nat,
    ...
    | succ z =>
        ...
        have hcomm : x + (z + 1) = (z + 1) + x := Nat.add_comm x (z + 1)
        have hzero : (z + 1) - (x + (z + 1)) = 0 :=
          (congrArg (fun q => (z + 1) - q) hcomm).trans
            (Nat.sub_self_add (z + 1) x)
        exact
          (Int.subNatNat_of_sub_eq_zero hzero).trans
            (congrArg Int.ofNat (Nat.add_sub_cancel x (z + 1)))
  exact (cancel a (b + a)).symm
