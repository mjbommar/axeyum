...
- Evidence rows: 268 — `checked` 264, `not-checked` 1, `replay-only` 3
- Evidence kinds: `cube-cover` 6, `cube-tree-cover` 1, `instance-pin` 52, `unsat-certificate` 100, `witness-replay` 109
...
| [`rado-r5-a3-b2-frontier`](rado/rado-r5-a3-b2-frontier/claim.json) | R_5(3(x-y)=2z) > 357: checked five-colour frontier bound | `open` | `witness-replay`: checked<br>`witness-replay`: checked<br>`witness-replay`: checked | 5 |
...
### `rado-r5-a3-b2-frontier` — R_5(3(x-y)=2z) > 357: checked five-colour frontier bound
...
- R_5(3(x-y)=2z) > 357, witnessed and verified (evidence row witness-357); the earlier 350 and 319 witnesses are retained as reproducibility and historical frontier points
...
- Canonical exact instances 351 through 357 are SAT and replayed; instance 358 is actively solving, and its incomplete proof stream has no evidentiary status
...
**Attack notes:** The satisfiable side remained tractable through 357, although solve time rose to 24:46.01 at 357. The refutation side at five colours is still unclosed and is the real exact-value question.
