...
Ledger: 2318 facts, 2136 `proved`. Every proved fact appears
...
|---|---:|---:|
| `kernel_theorem` | 1482 / 2136 | 69.4% |
| `per_theorem_footprint` | 59 / 2136 | 2.8% |
| `env_footprint` | 1878 / 2136 | 87.9% |
| `circularity` | 14 / 2136 | 0.7% |
| `semantic_falsification` | 96 / 2136 | 4.5% | **UPPER BOUND — 8 demonstrated.** Counts facts whose evidence names a semantic control, not facts whose control was shown to discriminate.
| `mutation_control` | 15 / 2136 | 0.7% |
| `independent_replay` | 7 / 2136 | 0.3% |
| `coverage_bearing_checker` | 1458 / 2136 | 68.3% |
...
|---|---:|---:|---|
| `exact_statement` | 2136 / 2136 | 100.0% | `artifacts/ontology/settled-fact-statement-pins.json` `pins[].fact_id` (S1, ADR-0763) |
...
2316 distinct `checker_command`s cover 2136 proved facts.
...
| 331 | `cargo run -q --release -p axeyum-lean-kernel --example nat_axiom_inventory -- --require-axiom-free nat` |
...
- 680 / 2136 proved facts hold two protections or fewer.
- 678 / 2136 have no discriminating checker naming their own subject, where the subject is taken only from an EXPLICIT
...
- 0 / 2136 have no `formal.statement` drift pin.
- 2129 / 2136 have no independent Lean replay.
- 2040 / 2136 carry no semantic-falsification evidence row.
- 2121 / 2136 name no mutation or negative control.
- 2077 / 2136 have no PER-THEOREM axiom footprint check; their footprint evidence is the prelude-wide sweep.
- 50 / 2136 have NO checker of their own: every command they cite is shared with another fact.
- 17 / 2136 cite no `checker_command` at all.
- 2003 / 2136 carry an evidence row listing two or more named `checkers`, and 1358 of those name the PRODUCING run as one of
...
- 1920 evidence rows across 1920 facts declare a semantic `kind` (`exhaustive-enumeration` / `instance-pin`) while their
  ...
  1920 rows, so this census reads `supports`, not `kind`.

## By curation

| curation | facts | median protections | own-subject checker | statement pin |
|---|---:|---:|---:|---:|
| (unset) | 1084 | 1 | 406 | 1084 |
| curated | 14 | 3 | 14 | 14 |
