...
- checked QF_LRA/Farkas rejection of a malformed subset-measure claim;
...
The promoted bad row keeps the measure-theory computation outside the solver.
Finite replay computes `mu({a}) = 1/6`; the source SMT-LIB artifact checks only
the final equality conflict against the false claim `mu({a}) = 2/3` through
