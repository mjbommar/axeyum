...
that evidence independently. The same row now also carries a source-level
SMT-LIB artifact that the route regression parses before checking Farkas
evidence. The feasible witness rows remain exact replay-only until they route
...
cargo test -p axeyum-solver --test math_resource_lra_routes linear_optimization_objective_threshold
```
