...
- replay rejection of malformed survival-mass and expected-time tables;
...
The bad survival-mass and expected-time replay rows stay replay-only: they
recompute the finite survival mass and expected-time equation from the finite
tables. Separate `qf-lra-*` rows point at source-linked QF_LRA/Farkas
regressions so the isolated linear contradictions are checked independently of
