...
use crate::ntheory::{factorize, gcd, is_prime, lcm};
...
/// All **primitive Pythagorean triples** `(a, b, c)` with `a < b < c ≤ limit` and
/// `gcd(a, b, c) = 1`, generated by **Euclid's formula** — `a = m²−n²`, `b = 2mn`,
/// `c = m²+n²` for `m > n > 0`, `gcd(m, n) = 1`, `m − n` odd — which enumerates
/// each primitive triple exactly once. Sorted by `c` then `a`. Each satisfies
/// `a² + b² = c²` (the re-check). `None` on overflow; empty for `limit < 5`.
///
/// ```
/// use axeyum_cas::ntheory_more::primitive_pythagorean_triples;
/// // The only primitive triples with hypotenuse ≤ 20: (3,4,5), (5,12,13), (8,15,17).
/// assert_eq!(
///     primitive_pythagorean_triples(20),
///     Some(vec![(3, 4, 5), (5, 12, 13), (8, 15, 17)]),
/// );
/// ```
#[must_use]
pub fn primitive_pythagorean_triples(limit: i128) -> Option<Vec<(i128, i128, i128)>> {
    let mut triples: Vec<(i128, i128, i128)> = Vec::new();
    let mut outer = 2i128;
    while outer.checked_mul(outer)? <= limit {
        let mut inner = 1i128;
        while inner < outer {
    ...
    triples.sort_unstable_by(|x, y| x.2.cmp(&y.2).then(x.0.cmp(&y.0)));
    Some(triples)
}
...
mod tests {
    ...
    #[test]
    fn primitive_pythagorean_triples_generation() {
        // Exact list up to 20.
        assert_eq!(
            primitive_pythagorean_triples(20),
            Some(vec![(3, 4, 5), (5, 12, 13), (8, 15, 17)])
        );
        // For any limit: every triple is valid, primitive, ordered, distinct, and
        // the count matches a brute-force enumeration.
        for limit in [5i128, 25, 50, 100] {
            let generated = primitive_pythagorean_triples(limit).unwrap();
            let mut brute: Vec<(i128, i128, i128)> = Vec::new();
            for a in 1..=limit {
                for b in a + 1..=limit {
                    let c2 = a * a + b * b;
                    let c = crate::ntheory_more::integer_nth_root(c2, 2).unwrap();
                    if c * c == c2 && c <= limit && gcd(gcd(a, b), c) == 1 {
                        brute.push((a, b, c));
                    }
                }
            }
            brute.sort_unstable_by(|x, y| x.2.cmp(&y.2).then(x.0.cmp(&y.0)));
            assert_eq!(generated, brute, "limit={limit}");
            for &(a, b, c) in &generated {
                assert_eq!(a * a + b * b, c * c);
            }
        }
        assert_eq!(primitive_pythagorean_triples(4), Some(vec![]));
    }
