...
use axeyum_cas::{
    ...
    discriminant, dsolve_homogeneous, dsolve_inhomogeneous, eigenvectors, equal, evaluate_trig,
    expand, factor, factor_expr, gosper_sum, gradient, integrate, limit, minimal_polynomial,
    ntheory, ntheory_advanced, real_root_intervals, resultant, series, series_at,
    simplify_radicals, solve, solve_polynomial_inequality, standard_deviation, stats,
    sum_polynomial,
};
...
fn main() {
    ...
    println!(
        "cancel((x^2-1)/(x-1)) = {}",
        cancel(&((x().pow(2) - i(1)) / (x() - i(1)))).unwrap()
    );
    println!(
        "apart(1/(x^2-1)) = {}",
        apart(&(i(1) / (x().pow(2) - i(1))), "x").unwrap()
    );
    println!(
        "sum_(k=0)^(n-1) k = {}",
        sum_polynomial(&CasExpr::var("n"), "n").unwrap()
    );
    ...
    println!(
        "solve(x^2 - 5x + 6 > 0): {} interval(s) (-inf,2) U (3,inf)",
        solution.len()
    );
    // Exact trig.
    ...
    for (lambda, basis) in eigenvectors(&m, "L").unwrap() {
        let vecs: Vec<String> = basis
            .iter()
            .map(|v| v.to_string().replace('\n', ""))
            .collect();
        println!(
            "eigenvalue {lambda} → eigenvector(s) {}   [A·v=λ·v]",
            vecs.join(", ")
        );
    }
    println!(
        "minimal_polynomial([[2,0],[0,3]]) = {}   [m(A)=0]",
        minimal_polynomial(&m, "L").unwrap()
    );

    println!("\n-- more calculus --");
    println!(
        "∫_0^1 3x^2 dx = {}   [CERTIFIED by FTC]",
        definite_integrate(&(i(3) * x().pow(2)), "x", &i(0), &i(1))
            .unwrap()
            .value
    );
    println!(
        "Taylor(ln(x), about x=1, order 3) = {}",
        series_at(&x().ln(), "x", &i(1), 3).unwrap()
    );
    ...
    println!(
        "factor_expr(x^4 - 1) = {}   [CERTIFIED]",
        factor_expr(&(x().pow(4) - i(1)), "x").unwrap()
    );
    println!(
        "discriminant(x^2 - 5x + 6) = {}",
        discriminant(&(x().pow(2) - i(5) * x() + i(6)), "x").unwrap()
    );
    println!(
        "resultant(x^2-1, x-1) = {}   [0 ⇒ common root]",
        resultant(&(x().pow(2) - i(1)), &(x() - i(1)), "x").unwrap()
    );
    println!(
        "simplify_radicals(√12) = {}",
        simplify_radicals(&i(12).sqrt())
    );
    let radical_id = equal(&(i(2).sqrt() * i(2).sqrt()), &i(2));
    println!(
        "√2·√2 = 2 ? {}",
        matches!(radical_id, ZeroTest::Certified { equal: true, .. })
    );
    ...
    println!(
        "mean = {}, median = {}",
        stats::mean(&data).unwrap(),
        stats::median(&data).unwrap()
    );
    println!(
        "population variance = {}, stddev = {}",
        stats::variance(&data).unwrap(),
    ...
    println!(
        "legendre(3, 7) = {}   (3 is a non-residue mod 7)",
        ntheory_advanced::legendre_symbol(3, 7)
    );
    println!(
        "primitive_root(7) = {:?}",
        ntheory_advanced::primitive_root(7)
    );
    println!(
        "discrete_log(2, 3, 5) = {:?}   (2^3 = 8 ≡ 3 mod 5)",
        ntheory_advanced::discrete_log(2, 3, 5)
    );
    println!(
        "Pell x²-61y²=1 fundamental solution = {:?}",
        ntheory_advanced::pell_fundamental_solution(61)
    );
