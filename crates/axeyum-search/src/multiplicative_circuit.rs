...
impl MultiplicativeEncoding {
    ...
    pub fn lift_model(
        ...
        let inputs: Vec<String> = (0..self.problem.input_bits)
            .map(|index| format!("x{index}"))
            .collect();
        let mut nonlinear = Vec::<String>::new();
        for gate_index in 0..self.problem.and_gates {
            let left = emit_affine(
                &format!("a{gate_index}_left"),
                &inputs,
                &nonlinear,
                &self.layout.left[gate_index],
                model.values(),
                &mut gates,
            );
        ...
        let artifact = BooleanCircuitArtifact {
            ...
            gates,
            outputs,
            truth_table: self.problem.truth_table.clone(),
        };
...
pub fn synthesize_multiplicative_circuit(
    ...
    let encoding = encode_multiplicative_circuit(problem, limits)?;
