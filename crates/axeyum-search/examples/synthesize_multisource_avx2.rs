//! Synthesize and certify bounded multi-source AVX2 byte permutations.
...
use axeyum_search::simd_program_synthesis::encode_multisource_avx2_sequence;
use axeyum_search::simd_synthesis::UnaryAvx2SynthesisLimits;
...
fn arguments() -> Arguments {
    ...
    assert!(
        args.len() >= 2,
        "usage: synthesize_multisource_avx2 TARGET STEPS [SECONDS] [--dimacs PATH] [--check-drat PATH]"
    );
...
fn main() {
    let args = arguments();
    let encoding = encode_multisource_avx2_sequence(
        &args.target,
        args.steps,
        UnaryAvx2SynthesisLimits::default(),
    )
    .expect("encoding must fit explicit defaults");
    println!("schema=axeyum.multisource-avx2-synthesis-run.v1");
