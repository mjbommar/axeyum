//! Emit every declaration the `FO.Qle` package (`fo_order.rs`, ADR-1669) adds
//! on top of the Robinson-arithmetic package, with its kind, axiom footprint,
//! and canonical type.
//!
//! This is the `checker_command` for the `F:fo-qle-*` facts. Like
//! `fo_robinson_inventory`, which it is modelled on, it is built to **fail on
//! absence** rather than report an empty set: a named filter matching nothing
//! exits non-zero, so deleting `FO.Qle.consistency` from the kernel turns the
//! fact's evidence red instead of leaving it green with nothing to say.
//!
//! The baseline here is `build_fo_robinson_prelude`, NOT `build_nat_prelude`,
//! so the default report is exactly this slice's own additions -- the six
//! order axioms, the context, its N model and its consistency. Everything Q
//! already had is in the baseline and does not appear.
//!
//! ```sh
//! # everything this slice declares
...
//! `FO.Qle` also matches every one of `FO.Qle.axLtZero`, `FO.Qle.natModels`
//! and the rest, which is the right behaviour when checking a family and the
...
use axeyum_lean_kernel::{Declaration, Kernel, build_fo_order_prelude, build_fo_robinson_prelude};
...
fn main() -> ExitCode {
    ...
    eprintln!("{} FO.Qle declarations", rows.len());

    let mut failed = false;
    if !filter.is_empty() && rows.is_empty() {
        eprintln!(
            "error: no FO.Qle declaration matches {filter:?} -- an absent \
