//! **The `Real` axiom package has a model that is actually ℝ** — ADR-0468
...
//!    verbatim — ADR-0468 Measurement 2, read out of the kernel; and
...
//! equality slot (ADR-0468 phase R3), which is the interface this model
...
//!
//! And the `Real` package's 30 axioms are **unchanged** by this: ADR-0468
...
fn main() {
    ...
    // (3): ADR-0468 Measurement 2, read out of the kernel.
    if restated != 9 {
        eprintln!(
            "FAIL: {restated} laws were restated over CReal.Equiv, not 9. The Real \
             package's Eq-fragment changed shape and ADR-0468's Measurement 2 no \
             longer describes it."
        );
        failed = true;
    }
    ...
    if failed {
        eprintln!(
            "FAIL: the constructed reals are NOT a model of the Real axiom package as \
             claimed — see above. ADR-0468 phase R4 does not hold as stated."
        );
        std::process::exit(1);
    }
    eprintln!(
