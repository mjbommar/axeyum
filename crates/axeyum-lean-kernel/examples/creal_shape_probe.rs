...
//! logic prelude. ADR-0468 takes the third route — a Bishop-style *setoid*: a
...
//! four structural questions, all of which had to hold before ADR-0468 could
//! name a plan:
//!
    ...
    if failed {
        eprintln!(
            "FAIL: the setoid carrier is NOT free — see the footprints above. \
             ADR-0468's cost claim does not hold."
        );
        std::process::exit(1);
    }
    eprintln!(
