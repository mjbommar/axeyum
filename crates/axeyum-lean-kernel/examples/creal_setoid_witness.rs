...
//! depending on it (ADR-0512 phase R1).
//!
...
//!    obstruction rather than a scoping note (ADR-0510).
//!
//! # How far the ordered-field structure gets (ADR-0512 phase R2, partial)
//!
//! `zero`, `one`, `neg` and `add` are built, with `neg` and `add` each carrying
//! its `Equiv`-congruence — two of the five congruence obligations ADR-0512
...
//! off an *existential* modulus and has to extract it. With ADR-0512's **fixed**
...
//!
//! # The order (ADR-0512 phase R2, continued)
//!
...
//! equality-slot binders the setoid ring telescope takes (ADR-0512 phase R3).
//!
...
//! That is the substitution ADR-0512 phase R4 has to make good, by
...
//! this: ADR-0512 retires them by *deletion* in phase R4, once consumers are
    ...
    if failed {
        eprintln!(
            "FAIL: the constructed reals are NOT free, or not inhabited, or not \
             discriminating — see above. ADR-0512's cost claim does not hold as stated."
        );
        std::process::exit(1);
    }
    eprintln!(
        "reflexivity, symmetry and transitivity of CReal.Equiv all hold at ZERO \
         ...
         interface ADR-0512 phase R4 instantiates is complete. CReal.Apart — \
         ...
         explicit Nat (ADR-0510). CReal.inv IS BUILT: the modulus is an \
