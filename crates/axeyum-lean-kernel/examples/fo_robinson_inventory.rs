//! Emit every declaration the Robinson-arithmetic package (`fo_robinson.rs`,
//! ADR-1651) adds on top of the `Nat` prelude, with its kind, axiom footprint,
//! and canonical type.
//!
//! This is the `checker_command` for the `F:fo-q-*` facts. Like
//! `fo_soundness_inventory`, which it is modelled on, it is built to **fail on
//! absence** rather than report an empty set: a named filter matching nothing
//! exits non-zero, so deleting `FO.Q.consistency` from the kernel turns the
//! fact's evidence red instead of leaving it green with nothing to say.
//!
//! It is a DIFFERENT report from `fo_soundness_inventory`, not a superset by
//! accident: `build_fo_robinson_prelude` is the first builder that puts BOTH
//! `fo_*` chains in one kernel — the arithmetization (`fo_code` →
//! `fo_roundtrip`, which owns `FO.Term.numeral`) and the calculus
//! (`fo_semantics` → `fo_soundness`). So `FO.Code.diagAux_code` and
//! `FO.soundness` appear in the same row set here and in neither of the two
//! older inventories.
//!
//! ```sh
...
//! cargo run -q --release -p axeyum-lean-kernel --example fo_robinson_inventory \
//!   -- FO.Q.consistency --exact --require-axiom-free
//! ```
//!
//! `--exact` matches the whole name rather than a substring. Without it,
//! `FO.Q` also matches every one of `FO.Q.axAddZero`, `FO.Q.natModels`,
//! `FO.Q.add_numeral` and the rest, which is the right behaviour when checking
//! a family and the wrong one when checking a single declaration.
//!
...
use axeyum_lean_kernel::{Declaration, Kernel, build_fo_robinson_prelude, build_nat_prelude};
