...
fn fixture() -> Fixture {
    ...
    let p = build_arith_prelude(&mut k).expect("AxReal prelude must build");
    let anon = k.anon();
    // a : R.
...
fn build_is_deterministic() {
    ...
    let p1 = build_arith_prelude(&mut k1).expect("AxReal prelude must build");
    let mut k2 = Kernel::new();
    let p2 = build_arith_prelude(&mut k2).expect("AxReal prelude must build");
