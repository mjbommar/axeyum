//! Inventory shard for `crates/axeyum-lean-kernel/src/creal/congruence.rs`.
//!
...
//! declaration to `crates/axeyum-lean-kernel/src/creal/congruence.rs` adds its
//! entry HERE and nowhere else — this file is the only one that needs
//! touching for a change confined to that module.
//!
//! `congruence.rs` itself declares exactly one PERMANENT prelude declaration
//! (`CReal.mulPowCongr`, the power-series term congruence) through its own
//! dispatch entry point, `declare_congruence_extras`. Everything else that
//! module builds (`congruenceDemo_absCongr`, `congruenceDemo_clampLike`, …) is
//! `#[cfg(test)]`-only scaffolding, built against a throwaway kernel clone in
//! each test and never entering `build_creal_prelude`'s own environment — so
//! it never appears in `kernel.environment()` for the `built()` template this
//! coverage check reads from, and does not belong here.
//!
...
/// declaration built by `crates/axeyum-lean-kernel/src/creal/congruence.rs`.
///
...
pub(crate) fn entries(p: crate::CRealPrelude) -> Vec<(&'static str, crate::NameId, &'static str)> {
    vec![("CReal.mulPowCongr", p.mul_pow_congr, "theorem")]
}
