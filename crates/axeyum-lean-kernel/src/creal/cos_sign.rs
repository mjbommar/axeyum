...
///
/// `qj_rat` is the NATURAL, multi-layer `Rat.mul`/`Rat.pow`/`Rat.normalize`
/// tower this construction produces -- correct, but expensive for the
/// KERNEL to reduce once several such towers are nested inside further
/// `Rat.add` calls (see [`declare_cos_wide_nonpositive`]'s doc comment).
/// Callers needing the numeric VALUE should collapse this via
/// [`collapse_to_flat`] before combining.
...
/// `Rat.normalize numerator denominator positivity` for a LITERAL small
/// value known in Rust (`numerator`, `denominator > 0`) -- the flat,
/// single-layer representative [`collapse_to_flat`] targets, as opposed to
/// the multi-layer tower [`cos_wide_term_as_rat`] naturally produces.
...
/// `Equiv t_j (ofRat q_flat)`, given `proof_tower : Equiv t_j (ofRat
/// q_tower)` and a LITERAL `q_flat` Rust already knows is the value
/// `q_tower` reduces to. The closing step is `Equiv.refl` at `ofRat
/// q_tower` itself, accepted against the stated `Equiv (ofRat q_tower)
/// (ofRat q_flat)` by the kernel's OWN definitional-equality check between
/// `q_tower` and `q_flat` -- one bounded `Rat` reduction, done ONCE, here,
/// rather than deferred to a later combination where it would be forced
/// again inside a larger unreduced expression.
///
/// This is the fix for the reverted `pi-r2c` attempt
/// (`worktree-agent-a42605c8b2b81444f`, `965165995`): that lane fed THREE
/// un-collapsed towers straight into two more `Rat.add` calls, so the FINAL
/// `Rat.ble` computation had to reduce a nested `Rat.add(Rat.add(Rat.mul(…),
/// Rat.mul(…)), Rat.mul(…))` nine-odd layers deep -- `creal_prelude_builds`
/// went from 97 s to 616 s and climbing before it was killed. Collapsing
/// each term to its flat, single-`normalize`-layer form FIRST keeps every
/// later `Rat.add` shallow: the biggest single `Nat` value any later step
/// touches is the cross-multiplied denominator `25 * 1875 = 46,875`,
/// comparable to the documented feasible point (`32 * 1875 = 60,000`, ~0.5s,
/// `on_a_deep_stack`).
fn collapse_to_flat(
    d: &mut IntDev<'_>,
    p: CRealPrelude,
    ...
    let of_rat_tower = d.const_app(p.of_rat, &[q_tower]);
    let of_rat_flat = d.const_app(p.of_rat, &[q_flat]);
    let collapse = erefl(d, p, of_rat_tower);
    echain(
        d,
        p,
        t_j,
        &[(of_rat_tower, proof_tower), (of_rat_flat, collapse)],
    )
}
...
/// `Bool.true`), on the three terms' FLAT, already-reduced representatives
/// ([`collapse_to_flat`]) rather than their natural multi-layer towers --
/// see that function's doc comment for why the distinction is what keeps
/// this declaration inside the measured build-time band.
/// [`CRealPrelude::le_trans`] composes the two.
///
/// # Errors
///
/// Returns the trusted gate's rejection. An `Err` here means the kernel
...
pub(super) fn declare_cos_wide_nonpositive(
    ...
    let (t0, q0_tower, p0_tower) =
        cos_wide_term_as_rat(d, p, r, r_rat, zero_nat, zero_nat, sign0, one_cc, rone_val);
    let (t1, q1_tower, p1_tower) = cos_wide_term_as_rat(
        d,
        p,
        r,
        r_rat,
    ...
    let (t2, q2_tower, p2_tower) =
        cos_wide_term_as_rat(d, p, r, r_rat, two_nat, four_nat, sign2, one_cc, rone_val);

    // Collapse each term's natural tower to a flat representative BEFORE any
    // combination -- see `collapse_to_flat`'s doc comment for why this is
    // the fix for the reverted attempt, not merely a style choice.
    let q0_flat = crate::rat_prelude::ops::rone(d, rat);
    let q1_flat = small_rat(d, -32, 25);
    let q2_flat = small_rat(d, 512, 1875);

    let p0 = collapse_to_flat(d, p, t0, q0_tower, p0_tower, q0_flat);
    let p1 = collapse_to_flat(d, p, t1, q1_tower, p1_tower, q1_flat);
    let p2 = collapse_to_flat(d, p, t2, q2_tower, p2_tower, q2_flat);

    // w := add (add (add zero_c t0) t1) t2 -- defeq to `sumRange t_lam 3`.
    let s0 = cadd(d, p, zero_c, t0);
    let s0_eq = zero_add_c(d, p, t0); // Equiv s0 t0
    let of_rat_q0 = d.const_app(p.of_rat, &[q0_flat]);
    let s0_rat_eq = echain(d, p, s0, &[(t0, s0_eq), (of_rat_q0, p0)]);

    let s1 = cadd(d, p, s0, t1);
    let of_rat_q1 = d.const_app(p.of_rat, &[q1_flat]);
    let add_cg1 = d.lemma(p.add_congr, &[s0, of_rat_q0, t1, of_rat_q1, s0_rat_eq, p1]);
    let sum01 = cadd(d, p, of_rat_q0, of_rat_q1);
    let of_rat_add1 = d.lemma(p.of_rat_add, &[q0_flat, q1_flat]);
    let q01 = crate::rat_prelude::ops::radd(d, q0_flat, q1_flat);
    ...
    let of_rat_q2 = d.const_app(p.of_rat, &[q2_flat]);
    let add_cg2 = d.lemma(p.add_congr, &[s1, of_rat_q01, t2, of_rat_q2, s1_rat_eq, p2]);
    let sum012 = cadd(d, p, of_rat_q01, of_rat_q2);
    let of_rat_add2 = d.lemma(p.of_rat_add, &[q01, q2_flat]);
    let q_final = crate::rat_prelude::ops::radd(d, q01, q2_flat);
    ...
    // le (ofRat q_final) zero, via `Rat.ble`'s own computation. `q_final`
    // is now SHALLOW -- `Rat.add(Rat.add(q0_flat,q1_flat),q2_flat)` on flat
    // operands -- so the largest single `Nat` value this touches is the
    // cross-multiplied denominator `25 * 1875 = 46,875`, inside the
    // measured feasible band.
