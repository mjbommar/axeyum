...
/// Admit `CReal.le_of_forall_le_add_small` and `CReal.equiv_zero_of_small`.
///
/// # Errors
///
...
fn declare_le_of_forall_le_add_small(
    ...
    let hypothesis_over_j = {
        ...
        let b1 = div_succ(d, p, 1, j);
        let c1 = div_succ(d, p, 3, j);

        // --- Term B: xj − ys ≤ 3/(j+1), from the hypothesis at e=j, m=j. ---
        ...
        // Bound: ba+(c1+bc_weak) = goal_bound + 5/(j+1).
        let five_j = div_succ(d, p, 5, j);
        ...
        // sorted = a1+(a1+(b1+(b1+c1))). Innermost pair (b1+c1) -> four.
        let four = div_succ(d, p, 4, j);
        let fuse_inner = d.lemma(rat.nat_div_succ_add, &[one_nat, three_nat, j]);
        let bc_pair = radd(d, b1, c1);
        let after_inner = rcongr(d, bc_pair, four, fuse_inner, &|d, t| {
            let level1 = radd(d, b1, t);
            let level2 = radd(d, a1, level1);
            radd(d, a1, level2)
        });
        let sorted_1 = {
            let level1 = radd(d, b1, four);
            let level2 = radd(d, a1, level1);
            radd(d, a1, level2)
        };
        ...
        let fuse_mid = d.lemma(rat.nat_div_succ_add, &[one_nat, four_nat, j]);
        let b_four = radd(d, b1, four);
        let after_mid = rcongr(d, b_four, five_j, fuse_mid, &|d, t| {
            let level2 = radd(d, a1, t);
            radd(d, a1, level2)
        });
        let sorted_2 = {
            let level2 = radd(d, a1, five_j);
            radd(d, a1, level2)
        };

        // Regroup a1+(a1+five) -> (a1+a1)+five.
        let forward = d.lemma(rat.add_assoc, &[a1, a1, five_j]);
        let aa = radd(d, a1, a1);
        let flat_pair = radd(d, aa, five_j);
        let regroup = rsymm(d, flat_pair, sorted_2, forward);

        // (a1+a1) -> goal_bound.
        let fuse_head = d.lemma(rat.nat_div_succ_add, &[one_nat, one_nat, n]);
        let after_head = rcongr(d, aa, goal_bound, fuse_head, &|d, t| radd(d, t, five_j));
        let final_target = radd(d, goal_bound, five_j);
    ...
    let at_index = d.lemma(
        ...
        &[target, goal_bound, five_nat, hypothesis_over_j],
    );
    let value = {
...
fn declare_equiv_zero_of_small(d: &mut IntDev<'_>, p: CRealPrelude) -> Result<(), KernelError> {
    ...
    // le v zero: ∀ e, le v (add zero (ofRat (1/(e+1)))).
    let hyp_v_zero = {
        let e_fv = d.fresh_fvar();
        let e = d.kernel().fvar(e_fv);
        let qe_rat = div_succ(d, p, 1, e);
    ...
    let le_v_zero = d.lemma(p.le_of_forall_le_add_small, &[v, zero_real, hyp_v_zero]);

    // le zero v: ∀ e, le zero (add v (ofRat (1/(e+1)))).
    let hyp_zero_v = {
        let e_fv = d.fresh_fvar();
        let e = d.kernel().fvar(e_fv);
        let qe_rat = div_succ(d, p, 1, e);
    ...
    let le_zero_v = d.lemma(p.le_of_forall_le_add_small, &[zero_real, v, hyp_zero_v]);
