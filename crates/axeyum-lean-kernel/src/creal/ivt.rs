...
//! have equal width by construction. Iterating `N` times (not built in this
//! slice; see below) shrinks the width geometrically as `(1/2)^N`, and once
//! it drops under the modulus
...
//! of the two final endpoints. That combination step, the outer `∀ e : Nat`
//! statement, and the `Nat.pow`/Archimedean bookkeeping to choose `N`, are
//! **not** built in this slice — this file lands exactly the one-step lemma,
//! which is "the entire constructive content" per this theorem's brief.
//!
//! ## Why not trisection
//!
...
//! step above, fully general in `F`, `P`, `Q`, `eps`. **Not landed**: the
//! outer `∀ e : Nat, ∃ x, …` statement (`CReal.ivt_approx`), which needs a
//! primitive-recursive iteration of this step `N` times with `N` chosen via
//! `Nat.pow_lt_pow_of_lt` plus the Archimedean property, and the final
//! combination with
//! [`CReal.UniformlyContinuousOn.spec`](super::CRealPrelude::uniformly_continuous_on)
//! — real additional engineering (a primitive recursion carrying the
//! six-part invariant through `N` steps), not a same-slice extension of this
//! lemma.
