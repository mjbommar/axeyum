...
// ============================================================================
// `Int.prodRangeIf_const_eq_pow_count` -- the FIRST bullet of this file's own
// "final assembly" gap (module doc, item 3): a predicate-restricted product
// where every contributing factor is the SAME constant `a` collapses to `a`
// raised to the COUNT of indices where the predicate holds. Reusable by both
// Euler's theorem (this file's own headline target) and Gauss's lemma's
// sign-product step (`nat_prelude/gauss_lemma.rs`'s module doc names exactly
// this shape, at `a := Int.neg Int.one`, as its own remaining "product of
// signs equals `(-1)^count`" piece) -- built here, generically, rather than
// twice.
// ============================================================================

/// `fun (_ : Nat) => a` -- local copy of `prod.rs`'s private `const_int_fn`
/// (not `pub(super)` there; same per-file-local convention as the helpers
/// below).
...
/// local `IntDev`-typed copy of `nat_prelude/ops.rs`'s `bool_true_or_false`
/// (that helper is hardcoded to `&mut NatDev<'_>`, so it cannot be called
/// from here directly; per this development's own convention of small
/// per-file local copies rather than exposing another file's private
/// helper across a `NatDev`/`IntDev` boundary, see
...
fn bool_true_or_false_int(d: &mut IntDev<'_>, b: ExprId) -> ExprId {
    ...
    let is_true_true = d.bool_eq(true_, true_);
    let is_true_false = d.bool_eq(true_, false_);
    let case_true = {
        let refl_true = d.bool_refl(true_);
        d.or_inl(is_true_true, is_true_false, refl_true)
    };
    let is_false_true = d.bool_eq(false_, true_);
    let is_false_false = d.bool_eq(false_, false_);
    let case_false = {
        let refl_false = d.bool_refl(false_);
        d.or_inr(is_false_true, is_false_false, refl_false)
    };
    let motive = {
    ...
    d.apply(rec, &[motive, case_false, case_true, b])
}

/// `heq : Eq Bool cond true ⊢ Eq Nat (bool_select_nat cond x y) x` -- local
/// `Nat`-target copy of `prod.rs`'s private `select_nat_true` (that copy is
/// also `IntDev`-typed but not `pub(super)`; same per-file-local convention).
fn select_nat_true_local(d: &mut IntDev<'_>, cond: ExprId, x: ExprId, y: ExprId, h: ExprId) -> ExprId {
    let true_v = d.bool_true();
    let motive = d.bool_eq_motive(true_v, &|d, v| {
        let sel = d.bool_select_nat(v, x, y);
        d.eq(sel, x)
    });
    let refl_case = d.refl(x);
    let symm_h = d.bool_symm(cond, true_v, h);
    d.bool_transport(true_v, motive, refl_case, cond, symm_h)
}

/// `heq : Eq Bool cond false ⊢ Eq Nat (bool_select_nat cond x y) y` -- local
/// `Nat`-target copy of `prod.rs`'s private `select_nat_false`.
fn select_nat_false_local(d: &mut IntDev<'_>, cond: ExprId, x: ExprId, y: ExprId, h: ExprId) -> ExprId {
    let false_v = d.bool_false();
    let motive = d.bool_eq_motive(false_v, &|d, v| {
        let sel = d.bool_select_nat(v, x, y);
        d.eq(sel, y)
    });
    let refl_case = d.refl(y);
    let symm_h = d.bool_symm(cond, false_v, h);
    d.bool_transport(false_v, motive, refl_case, cond, symm_h)
}

/// `Int.prodRangeIf_const_eq_pow_count :
///   ∀ pred a n, Eq Int (prodRangeIf pred (fun _ => a) n) (pow a (Nat.countRange pred n))`
/// (statement in the unfolded form this file's whole convention uses, see
/// the module doc on [`declare_prod_range_if`]).
///
/// Induction on `n`. Base case: both sides reduce to `Int.one` by δι alone
/// (`prodRangeIf _ _ zero` and `pow a zero`, and `Nat.countRange pred zero`
/// reduces to `zero` first), so `Eq.refl one` closes it regardless of
/// `pred`/`a`. Successor step: `prodRangeIf pred (const a) (succ j)` reduces
/// (`prodRangeIf_succ`) to `mul (prodRangeIf pred (const a) j) (bool_select_int
/// (pred j) a one)`, and `Nat.countRange pred (succ j)` reduces
/// (`Nat.count_range_succ`) to `add (countRange pred j) (bool_select_nat
/// (pred j) 1 0)` -- both stuck on the SYMBOLIC `pred j`, so
/// [`bool_true_or_false_int`] case-splits it:
///
/// - **`pred j = true`**: [`select_int_true`] collapses the selector to `a`;
///   [`select_nat_true_local`] collapses the count increment to `1`, so the
///   exponent becomes `add C 1`, defeq `succ C` (`Nat.add` recurses on its
///   RIGHT argument). `pow a (succ C) = mul (pow a C) a` by `pow_succ`
///   (refl) -- exactly the shape the LHS reaches via the induction
///   hypothesis and one `icongr` on the selector-collapse.
/// - **`pred j = false`**: [`select_int_false`] collapses the selector to
///   `Int.one`; [`select_nat_false_local`] collapses the increment to `0`,
///   so the exponent stays `add C 0`, defeq `C` (base case of `Nat.add`'s
///   recursion, no lemma needed). `mul_one` collapses the LHS's trailing
///   `mul _ one`, landing both sides at the induction hypothesis's own
///   `pow a C` with no `pow_succ` step at all.
///
/// Each branch chains its LHS-side and exponent-side reasoning through a
/// shared endpoint (`mul (pow a C) a` in the true branch, `pow a C` in the
/// false branch) via `itrans`/`isymm`; the two branches combine with
/// `or_elim` into one proof of the successor case, and defeq handles every
/// unfold this doc names as "reduces to" without an explicit rewrite step.
///
/// # Errors
///
/// Returns the trusted kernel gate's typed rejection.
#[allow(clippy::too_many_lines)]
pub(super) fn declare_prod_range_if_const_eq_pow_count(d: &mut IntDev<'_>) -> Result<(), KernelError> {
    let p = d.int();
    let nat = d.nat_ty();
    let bool_ty = d.bool_ty();
    let int_ty = d.int_ty();
    ...
    let count = |d: &mut IntDev<'_>, x: ExprId| -> ExprId {
        let p = d.int();
        d.const_app(p.nat.count_range, &[pred, x])
    };

    let motive = |d: &mut IntDev<'_>, x: ExprId| -> ExprId {
        let ca = const_int_fn(d, a);
        let lhs = prod_range_if(d, pred, ca, x);
        let cx = count(d, x);
        let rhs = d.ipow(a, cx);
        d.ieq(lhs, rhs)
    };
    ...
    let proof = d.induct(
        ...
        &|d, j, ih| {
            // ih : Eq Int (prodRangeIf pred (const a) j) (pow a (count j)).
            let ca = const_int_fn(d, a);
            let pr_j = prod_range_if(d, pred, ca, j);
            let cj = count(d, j);
            let pow_a_cj = d.ipow(a, cj);
            let pred_j = d.apply(pred, &[j]);

            let one_i = d.ione();
            let one_n = d.num(1);
            let zero_n = d.zero();
            let sel_j = bool_select_int(d, pred_j, a, one_i);
            let incr = d.bool_select_nat(pred_j, one_n, zero_n);
            let add_cj_incr = d.add(cj, incr);
            let mul_pj_selj = d.imul(pr_j, sel_j);
            let pow_a_addincr = d.ipow(a, add_cj_incr);

            let sj = d.succ(j);
            let target = motive(d, sj);
            let true_v = d.bool_true();
            let false_v = d.bool_false();
            let left_ty = d.bool_eq(pred_j, true_v);
            let right_ty = d.bool_eq(pred_j, false_v);
            let disj = bool_true_or_false_int(d, pred_j);

            let on_true = |d: &mut IntDev<'_>, h: ExprId| -> ExprId {
                // LHS side: sel_j = a (given h), so
                // mul pr_j sel_j -> mul pr_j a -> mul (pow a cj) a (via ih).
                let sel_eq_a = select_int_true(d, pred_j, a, one_i, h);
                let step_lhs1 = d.icongr(sel_j, a, sel_eq_a, &|d, t| d.imul(pr_j, t));
                let mul_pj_a = d.imul(pr_j, a);
                let step_lhs2 = d.icongr(pr_j, pow_a_cj, ih, &|d, t| d.imul(t, a));
                let mul_powc_a = d.imul(pow_a_cj, a);
                let (_e, lhs_chain) =
                    d.ichain(mul_pj_selj, &[(mul_pj_a, step_lhs1), (mul_powc_a, step_lhs2)]);
                // lhs_chain : Eq Int mul_pj_selj mul_powc_a.

                // Exponent side: incr = 1 (given h), so add cj incr -> add
                // cj 1, defeq succ cj; pow a (succ cj) = mul (pow a cj) a by
                // `pow_succ` (refl) -- so `pow a add_cj_1` is defeq
                // `mul_powc_a` with no further proof needed.
                let incr_eq_1 = select_nat_true_local(d, pred_j, one_n, zero_n, h);
                let add_cj_1 = d.add(cj, one_n);
                let step_exp = d.congr(incr, one_n, incr_eq_1, &|d, t| d.add(cj, t));
                let exp_lift = d.nat_eq_to_int(add_cj_incr, add_cj_1, step_exp, &|d, t| {
                    d.ipow(a, t)
                });
                // exp_lift : Eq Int pow_a_addincr (pow a add_cj_1), and
                // `pow a add_cj_1` is defeq `mul_powc_a`.
                let exp_backward = d.isymm(pow_a_addincr, mul_powc_a, exp_lift);
                // exp_backward : Eq Int mul_powc_a pow_a_addincr.

                d.itrans(mul_pj_selj, mul_powc_a, pow_a_addincr, lhs_chain, exp_backward)
            };

            let on_false = |d: &mut IntDev<'_>, h: ExprId| -> ExprId {
                let p = d.int();
                // LHS side: sel_j = one (given h), so
                // mul pr_j sel_j -> mul pr_j one -> pr_j (mul_one).
                let sel_eq_one = select_int_false(d, pred_j, a, one_i, h);
                let step_lhs1 = d.icongr(sel_j, one_i, sel_eq_one, &|d, t| d.imul(pr_j, t));
                let mul_pj_one = d.imul(pr_j, one_i);
                let mul_one_pf = d.lemma(p.mul_one, &[pr_j]);
                let (_e, lhs_chain) =
                    d.ichain(mul_pj_selj, &[(mul_pj_one, step_lhs1), (pr_j, mul_one_pf)]);
                // lhs_chain : Eq Int mul_pj_selj pr_j.
                let full_lhs = d.itrans(mul_pj_selj, pr_j, pow_a_cj, lhs_chain, ih);
                // full_lhs : Eq Int mul_pj_selj pow_a_cj.

                // Exponent side: incr = 0 (given h), so add cj incr -> add
                // cj 0, defeq cj -- the BASE case of `Nat.add`'s own
                // recursion, no lemma beyond the congr on `incr` itself.
                let incr_eq_0 = select_nat_false_local(d, pred_j, one_n, zero_n, h);
                let add_cj_0 = d.add(cj, zero_n);
                let step_exp = d.congr(incr, zero_n, incr_eq_0, &|d, t| d.add(cj, t));
                let exp_lift = d.nat_eq_to_int(add_cj_incr, add_cj_0, step_exp, &|d, t| {
                    d.ipow(a, t)
                });
                // exp_lift : Eq Int pow_a_addincr (pow a add_cj_0), and
                // `pow a add_cj_0` is defeq `pow_a_cj`.
                let exp_backward = d.isymm(pow_a_addincr, pow_a_cj, exp_lift);
                // exp_backward : Eq Int pow_a_cj pow_a_addincr.

                d.itrans(mul_pj_selj, pow_a_cj, pow_a_addincr, full_lhs, exp_backward)
            };

            d.or_elim(left_ty, right_ty, target, disj, &on_true, &on_false)
        },
        n,
    );
...
pub(super) fn declare_prod_range_if_all(d: &mut IntDev<'_>) -> Result<(), KernelError> {
    ...
    declare_prod_range_if_const_eq_pow_count(d)?;
    Ok(())
}
