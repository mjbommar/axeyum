...
fn not_and_iff_or_not(d: &mut IntDev<'_>, q1: ExprId, q2: ExprId, decision: ExprId) -> ExprId {
    ...
    let mp = {
        ...
        let body = {
            ...
            let on_right =
                &|d: &mut IntDev<'_>, hnq1: ExprId| -> ExprId { d.or_inl(not_q1, not_q2, hnq1) };
...
fn nat_valuation_step(
    ...
    dvd_elim_nat(d, pk, x, goal, h_x, &|d, xprime, eq_x| {
        dvd_elim_nat(d, pl, y, goal, h_y, &|d, yprime, eq_y| {
            ...
            let cancelled =
                dvd_cancel_left_of_pos_nat(d, pkl, p_var, xy_prime, pkl_pos, h_xy_transported);
            // cancelled : dvd p_var xy_prime

            let euclid_or = d.lemma(
                p.nat.euclid_lemma,
                &[p_var, xprime, yprime, prime_hyp, cancelled],
            );
...
pub(super) fn declare_succ_dvd_or_succ_dvd_of_succ_sum_dvd_mul(
    ...
    let core_or = nat_valuation_step(
        d, p_var, big_m, big_n, k, l, prime_hyp, hx_nat, hy_nat, hxy_nat,
    );
