//! `Nat.sumRange_permute` — **summing over `[0,n)` is invariant under any
//! injective self-map of `[0,n)`**, the additive half of the bijection Gauss's
//! lemma already uses multiplicatively, and the piece ADR-1260's residue 3
//! (Eisenstein's lemma) needs and did not have.
//!
//! ## Statement
//!
//! ```text
//! Nat.sumRange_permute :
//!   ∀ (f : Nat → Nat) (σ : Nat → Nat) (n : Nat),
//!     Nat.InjectiveOn σ n → Nat.MapsInto σ n →
//!     Eq Nat (sumRange f n) (sumRange (fun k => f (σ k)) n)
//! ```
//!
//! Same hypotheses, same argument order and same `f`-outside/`σ`-generalized
//! induction as [`super::count_range_permute`]'s `Nat.countRange_permute` and
//! `Int.prodRange_permute` (`int_prelude/prod.rs`), deliberately, so the three
//! can be read against each other.
//!
//! ## Why it is not a corollary of `countRange_permute`, and vice versa
//!
//! `Nat.countRange_eq_sumRange` (`lattice_count.rs`, ADR-1260) is `Eq.refl`:
//! `countRange f n` IS `sumRange (fun k => bool_select_nat (f k) 1 0) n`. So
//! `countRange_permute` is the special case of this theorem at a `{0,1}`-valued
//! summand — but only that case, and a permutation argument over a general
//! `Nat`-valued family cannot be recovered from it, because the counting
//! statement never sees a summand bigger than `1`. The transport runs the other
//! way and it is a genuine generalization; nothing here is derived from the
//! counting version.
//!
//! What that observation *does* buy is confidence in the route: the counting
//! proof is exactly this proof with the summand specialized, so every step is
//! known to close, and the only question is whether dropping the selector loses
//! anything. It does not — no step in `countRange_permute` uses the summand's
//! `Bool`-ness. Each `Eq Bool` hypothesis becomes an `Eq Nat` one, each
//! `bool_congr_nat`/`nat_congr_bool` becomes the ordinary [`NatOps::congr`],
//! and the `bool_select_nat` wrapper disappears from every term.
//!
//! ## What is declared
//!
//! - [`declare_sum_range_point_change`] — `Nat.sumRange_point_change`, the
//!   one-index-differs summing law: two families agreeing on `[0,n)` except
//!   possibly at `i0` have sums that differ exactly as their values at `i0` do.
//!   Stated ADDITIVELY (`Σa + b i0 = Σb + a i0`) rather than subtractively,
//!   because `Nat.sub` is truncated and the subtractive form would need a side
//!   condition this one does not.
//! - [`declare_sum_range_permute`] — the headline.
//!
//! `Nat.sumRange_congr_lt` (the bounded pointwise congruence) already exists,
//! in `binomial.rs`, so unlike the counting file this one does not have to
//! build its own.
//!
//! ## What replaces `Int.prodRange_swap`
//!
//! The same thing that replaced it for counting: the accumulator is `Nat.add`,
//! so moving a value from slot `i0` up to slot `n` is one point-change lemma
//! rather than the adjacent-transposition induction `wilson.rs` records as
//! taking three drafts.
//!
...
// ============================================================================
// `Nat.sumRange_point_change`.
// ============================================================================

/// `∀ k, Lt k bound → Eq Nat (a k) (b k)`.
fn agree_below(d: &mut NatDev<'_>, a: ExprId, b: ExprId, bound: ExprId) -> ExprId {
    let nat = d.nat_ty();
...
/// `∀ k, Lt i0 k → Lt k bound → Eq Nat (a k) (b k)`.
...
///   (∀ k, Lt k i0 → Eq Nat (a k) (b k)) →
///   (∀ k, Lt i0 k → Lt k n → Eq Nat (a k) (b k)) →
///   Eq Nat (add (sumRange a n) (b i0)) (add (sumRange b n) (a i0))`
///
/// Induction on `n`, with `Lt i0 n` and both agreement hypotheses carried in
/// the motive. `n = 0` is vacuous (`not_lt_zero`). At `succ j`,
/// `lt_or_eq_of_le` splits `Le i0 j`:
///
/// - `Lt i0 j` — the induction hypothesis applies at `j`, the top index `j` is
///   off `i0` so `a j = b j`, and the two sides differ by one
///   [`add_swap_right`] on each side.
/// - `Eq i0 j` — the top index IS `i0`; below it the two families agree
///   everywhere, so `Nat.sumRange_congr_lt` equates the two prefix sums and one
///   [`add_swap_right`] finishes.
///
/// # Errors
///
...
pub(super) fn declare_sum_range_point_change(
    d: &mut NatDev<'_>,
    p: &NatPrelude,
) -> Result<(), KernelError> {
    let p = *p;
    ...
    let conclusion = |d: &mut NatDev<'_>, x: ExprId| {
        let sa = sum_range(d, a, x);
        let sb = sum_range(d, b, x);
        let ai0 = d.apply(a, &[i0]);
        let bi0 = d.apply(b, &[i0]);
        let lhs = d.add(sa, bi0);
        let rhs = d.add(sb, ai0);
        d.eq(lhs, rhs)
    };
    ...
    let motive = |d: &mut NatDev<'_>, x: ExprId| {
        ...
        let below = agree_below(d, a, b, i0);
    ...
    let proof = d.induct(
        ...
        &|d| {
            ...
            let below_ty = agree_below(d, a, b, i0);
        ...
        &|d, j, ih| {
            ...
            let sa_prior = sum_range(d, a, j);
            let sb_prior = sum_range(d, b, j);
            let aj = d.apply(a, &[j]);
            let bj = d.apply(b, &[j]);
            let ai0 = d.apply(a, &[i0]);
            let bi0 = d.apply(b, &[i0]);
            ...
            let on_lt = {
                ...
                // (Sa j + a j) + b i0
                //   = (Sa j + b i0) + a j     [add_swap_right]
                //   = (Sb j + a i0) + a j     [ih]
                //   = (Sb j + a j)  + a i0    [add_swap_right]
                //   = (Sb j + b j)  + a i0    [a j = b j]
                let lhs0 = {
                    let inner = d.add(sa_prior, aj);
                    d.add(inner, bi0)
                };
                let s1_rhs = {
                    let inner = d.add(sa_prior, bi0);
                    d.add(inner, aj)
                };
                let step1 = add_swap_right(d, &p, sa_prior, aj, bi0);

                let s2_rhs = {
                    let inner = d.add(sb_prior, ai0);
                    d.add(inner, aj)
                };
                let lhs_ih = d.add(sa_prior, bi0);
                let rhs_ih = d.add(sb_prior, ai0);
                let step2 = d.congr(lhs_ih, rhs_ih, ih_result, &|d, t| d.add(t, aj));

                let s3_rhs = {
                    let inner = d.add(sb_prior, aj);
                    d.add(inner, ai0)
                };
                let step3 = add_swap_right(d, &p, sb_prior, ai0, aj);

                let s4_rhs = {
                    let inner = d.add(sb_prior, bj);
                    d.add(inner, ai0)
                };
                let step4 = d.congr(aj, bj, at_j, &|d, x| {
                    let inner = d.add(sb_prior, x);
                    d.add(inner, ai0)
                });
            ...
            let on_eq = {
                ...
                let prefix_eq = d.lemma(p.sum_range_congr_lt, &[a, b, j, below_j]);

                // `a i0 = a j` and `b i0 = b j`, from `i0 = j`.
                let a_at = d.congr(i0, j, h, &|d, x| d.apply(a, &[x]));
                let b_at = d.congr(i0, j, h, &|d, x| d.apply(b, &[x]));

                // (Sa j + a j) + b i0
                //   = (Sb j + a j)  + b i0    [prefix_eq]
                //   = (Sb j + a i0) + b i0    [a i0 = a j, backwards]
                //   = (Sb j + b i0) + a i0    [add_swap_right]
                //   = (Sb j + b j)  + a i0    [b i0 = b j]
                let lhs0 = {
                    let inner = d.add(sa_prior, aj);
                    d.add(inner, bi0)
                };
                ...
                let step1 = d.congr(sa_prior, sb_prior, prefix_eq, &|d, t| {
                    let inner = d.add(t, aj);
                    d.add(inner, bi0)
                });
                ...
                let a_at_rev = d.symm(ai0, aj, a_at);
                let step2 = d.congr(aj, ai0, a_at_rev, &|d, x| {
                    let inner = d.add(sb_prior, x);
                    d.add(inner, bi0)
                });

                let s3_rhs = {
                    let inner = d.add(sb_prior, bi0);
                    d.add(inner, ai0)
                };
                let step3 = add_swap_right(d, &p, sb_prior, ai0, bi0);

                let s4_rhs = {
                    let inner = d.add(sb_prior, bj);
                    d.add(inner, ai0)
                };
                let step4 = d.congr(bi0, bj, b_at, &|d, x| {
                    let inner = d.add(sb_prior, x);
                    d.add(inner, ai0)
                });
    ...
    let ty = {
        ...
        let over_b = d.pi_fv(b_fv, fn_ty, over_i0);
        d.pi_fv(a_fv, fn_ty, over_b)
    };
    let value = {
        let over_n = d.lam_fv(n_fv, nat, proof);
        let over_i0 = d.lam_fv(i0_fv, nat, over_n);
        let over_b = d.lam_fv(b_fv, fn_ty, over_i0);
        d.lam_fv(a_fv, fn_ty, over_b)
    };
    d.declare_theorem(p.sum_range_point_change, ty, value)?;
    Ok(())
}
...
///   Eq Nat (sumRange f x) (sumRange (fun k => f (σ k)) x)` — the induction
/// motive, generalized over `σ` but NOT over `f`.
fn permute_motive(d: &mut NatDev<'_>, p: &NatPrelude, f: ExprId, x: ExprId) -> ExprId {
    ...
    let lhs = sum_range(d, f, x);
    let rhs = sum_range(d, composed, x);
...
fn permute_branch_fixed(
    ...
    let composed = compose(d, f, sigma);
    let f_prior = sum_range(d, f, n);
    let g_prior = sum_range(d, composed, n);
    let f_n = d.apply(f, &[n]);
    let f_sigma_n = d.apply(f, &[sigma_n]);

    let start = d.add(f_prior, f_n);
    let mid = d.add(g_prior, f_n);
    let step1 = d.congr(f_prior, g_prior, ih_result, &|d, t| d.add(t, f_n));
    let end_ = d.add(g_prior, f_sigma_n);
    let fn_eq = d.congr(n, sigma_n, n_eq_sigma_n, &|d, x| d.apply(f, &[x]));
    let step2 = d.congr(f_n, f_sigma_n, fn_eq, &|d, x| d.add(g_prior, x));
...
/// [`declare_sum_range_point_change`] — `f ∘ τ` and `f ∘ σ` agree on `[0,n)`
...
fn permute_branch_override(
    ...
    let below = {
        ...
        let body = d.congr(ov, sk, ov_eq, &|d, x| d.apply(f, &[x]));
    ...
    // `change : S(f∘τ) n + f (σ i0) = S(f∘σ) n + f (τ i0)`,
    // with `σ i0 = n` and `τ i0 = σ n`.
    let s_tau = sum_range(d, f_tau, n);
    let s_sigma = sum_range(d, f_sigma, n);
    ...
    let change_lhs = d.add(s_tau, f_sigma_i0);
    let change_rhs = d.add(s_sigma, f_tau_i0);

    let f_si0_eq_fn = d.congr(sigma_i0, n, sigma_i0_eq_n, &|d, x| d.apply(f, &[x]));
    let rewrite_lhs = d.congr(f_sigma_i0, f_n, f_si0_eq_fn, &|d, x| d.add(s_tau, x));
    let tau_i0_eq_v = override_eq_at(d, &p, sigma, i0, v);
    let f_ti0_eq_fv = d.congr(tau_i0, v, tau_i0_eq_v, &|d, x| d.apply(f, &[x]));
    let rewrite_rhs = d.congr(f_tau_i0, f_v, f_ti0_eq_fv, &|d, x| d.add(s_sigma, x));

    let reduced_lhs = d.add(s_tau, f_n);
    let reduced_rhs = d.add(s_sigma, f_v);
    ...
    // `sumRange f (succ n) ≡ S(f) n + f n`
    //   = S(f∘τ) n + f n        [ih]
    //   = S(f∘σ) n + f (σ n)    [reduced]
    //   ≡ sumRange (f∘σ) (succ n)
    let s_f = sum_range(d, f, n);
    let start = d.add(s_f, f_n);
    let step1 = d.congr(s_f, s_tau, ih_result, &|d, t| d.add(t, f_n));
...
fn permute_step(d: &mut NatDev<'_>, p: &NatPrelude, f: ExprId, n: ExprId, ih: ExprId) -> ExprId {
    ...
    let composed = compose(d, f, sigma);
    let lhs = sum_range(d, f, sn);
    let rhs = sum_range(d, composed, sn);
...
/// Returns the trusted gate's rejection if the constructed term does not
/// type-check.
pub(super) fn declare_sum_range_permute(
    d: &mut NatDev<'_>,
    p: &NatPrelude,
) -> Result<(), KernelError> {
    let p = *p;
    ...
    let proof = d.induct(
        ...
        &|d| {
            ...
            let fn_ty_inner = d.arrow(nat_inner, nat_inner);
            let zero = d.zero();
            let sigma_fv = d.fresh_fvar();
            let sigma = d.kernel().fvar(sigma_fv);
            ...
            let inj_fv = d.fresh_fvar();
            let with_inj = d.lam_fv(inj_fv, inj_ty, with_maps);
            d.lam_fv(sigma_fv, fn_ty_inner, with_inj)
        },
    ...
    let composed = compose(d, f, sigma);
    let lhs = sum_range(d, f, n);
    let rhs = sum_range(d, composed, n);
    let concl = d.eq(lhs, rhs);
    ...
    let value = {
        ...
        d.lam_fv(f_fv, fn_ty, over_sigma)
    };
    d.declare_theorem(p.sum_range_permute, ty, value)?;
    Ok(())
}

/// Declares the whole family, in dependency order.
///
/// # Errors
///
/// Returns the trusted kernel gate's typed rejection.
pub(super) fn declare_sum_range_permute_all(
    d: &mut NatDev<'_>,
    p: &NatPrelude,
) -> Result<(), KernelError> {
    declare_sum_range_point_change(d, p)?;
    declare_sum_range_permute(d, p)?;
    Ok(())
}
