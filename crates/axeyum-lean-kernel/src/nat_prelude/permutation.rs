//! `Nat.inverseIndex` — an explicit, computable inverse for an injective
...
//! [`Nat.inverseIndex`](declare_inverse_index) is that missing explicit
...
//! `Nat.comp`, `Nat.id`, `Nat.comp_assoc`, `Nat.inverseIndex` with its two
...
//!   f (inverseIndex f n k) = k` — `f` composed with `inverseIndex f n` is
...
//!   ∀ i, i < n → inverseIndex f n (f i) = i` — `inverseIndex f n` composed
...
//!   and `f`'s injectivity (plus `Nat.inverseIndex`'s own boundedness, see
...
//! `inverseIndex f n k < n` for `0 < n`, and the raw existence-hypothesis
...
/// Delta height for `Nat.inverseIndex`: it calls `Nat.beq` (height 1) inside
...
/// `Nat.inverseIndex f m k`, built directly (no lambda redex), for a
/// caller-supplied bound `m` (usually the induction variable, not always the
/// final `n`).
fn inverse_index_at(d: &mut NatDev<'_>, p: &NatPrelude, f: ExprId, m: ExprId, k: ExprId) -> ExprId {
    d.const_app(p.inverse_index, &[f, m, k])
}

/// The predicate `fun i => And (Lt i m) (Eq Nat (f i) k)`, the existential
/// body `inverseIndex`'s correctness lemma both destructures (as a
...
/// `Or (Eq Nat m 0) (Lt (inverseIndex f m k) m)`, by induction on `m`
/// (`f`, `k` fixed throughout, captured from the enclosing scope). Base
/// `m = 0` takes the left disjunct (`Eq.refl`). Step `m = succ j`: reduce
/// the prior bound `Or (Eq j 0) (Lt (inverseIndex f j k) j)` to
/// `Lt (inverseIndex f j k) (succ j)` either way (via `zero_lt_succ`
...
fn inverse_index_bounded(
    ...
    d.induct(
        &|d, x| {
            let idx = inverse_index_at(d, &p, f, x, k);
            let zero = d.zero();
            let eq0 = d.eq(x, zero);
            let ltx = d.lt(idx, x);
        ...
        &|d| {
            let zero = d.zero();
            let idx0 = inverse_index_at(d, &p, f, zero, k);
        ...
        &|d, j, ih| {
            ...
            let idx_j = inverse_index_at(d, &p, f, j, k);
            let idx_sj = inverse_index_at(d, &p, f, sj, k);
            ...
            let on_eq = {
                ...
                let motive = d.eq_motive(zero, &|d, x| {
                    let idxx = inverse_index_at(d, &p, f, x, k);
                    let sx = d.succ(x);
                    d.lt(idxx, sx)
                });
...
/// `pos_n : Lt 0 n ⊢ Lt (inverseIndex f n k) n`, by eliminating
/// [`inverse_index_bounded`]'s left disjunct (`n = 0` contradicts `pos_n`
/// via `lt_irrefl`, transported).
fn inverse_index_lt_proof(
    d: &mut NatDev<'_>,
    p: &NatPrelude,
    f: ExprId,
    k: ExprId,
    ...
    let p = *p;
    let bounded = inverse_index_bounded(d, &p, f, k, n);
    let zero = d.zero();
    let eq_n0 = d.eq(n, zero);
    let idx = inverse_index_at(d, &p, f, n, k);
    let lt_n = d.lt(idx, n);
...
/// `Exists (fun i => And (Lt i m) (Eq Nat (f i) k)) → Eq Nat (f (inverseIndex
...
/// so `inverseIndex f (succ j) k` selects `j` on the nose
/// (`finite.rs::select_nat_true`).
fn inverse_index_correct(
    d: &mut NatDev<'_>,
    p: &NatPrelude,
    f: ExprId,
    k: ExprId,
    ...
    d.induct(
        &|d, m| {
            ...
            let idx = inverse_index_at(d, &p, f, m, k);
...
/// Admit `Nat.inverseIndex : (Nat → Nat) → Nat → Nat → Nat`.
///
/// # Errors
///
/// Returns the kernel's rejection if the generated definition does not
/// type-check or the name is already taken.
fn declare_inverse_index(d: &mut NatDev<'_>, p: &NatPrelude) -> Result<(), KernelError> {
    let p = *p;
    let nat = d.nat_ty();
    ...
    // `inverse_index_at` calling convention `[f, m, k]`), so `k` is the
...
/// Admit `Nat.inverseIndex_right : ∀ f n, SurjectiveOn f n → ∀ k, k < n →
/// f (inverseIndex f n k) = k`.
///
/// # Errors
///
/// Returns the trusted gate's rejection if the constructed term does not
/// type-check.
fn declare_inverse_index_right(d: &mut NatDev<'_>, p: &NatPrelude) -> Result<(), KernelError> {
    let p = *p;
    let nat = d.nat_ty();
    let fn_ty = d.arrow(nat, nat);
    ...
    let correct = inverse_index_correct(d, &p, f, k, n);
    ...
    d.declare_theorem(p.inverse_index_right, ty, value)
}

/// Admit `Nat.inverseIndex_left : ∀ f n, MapsInto f n → InjectiveOn f n →
/// ∀ i, i < n → inverseIndex f n (f i) = i`.
///
/// # Errors
///
/// Returns the trusted gate's rejection if the constructed term does not
/// type-check.
fn declare_inverse_index_left(d: &mut NatDev<'_>, p: &NatPrelude) -> Result<(), KernelError> {
    let p = *p;
    let nat = d.nat_ty();
    let fn_ty = d.arrow(nat, nat);
    let logic = p.logic;
    ...
    // Existence witness for `inverse_index_correct` at k := f i: i itself,
    ...
    let correct = inverse_index_correct(d, &p, f, fi, n);
    let correct_at_i = d.apply(correct, &[ex_witness]); // Eq Nat (f (inverseIndex f n fi)) fi
    ...
    let idx = inverse_index_at(d, &p, f, n, fi);
    let idx_lt_n = inverse_index_lt_proof(d, &p, f, fi, n, pos_n);
    ...
    d.declare_theorem(p.inverse_index_left, ty, value)
}
...
/// conjunct; the inverse conjunct needs `Nat.inverseIndex_left`/`_right`
...
/// Admit `Nat.inverseIndex`, its two two-sided-inverse theorems, `Nat.id`,
...
pub(super) fn declare_permutation_all(
    ...
    declare_inverse_index(d, p)?;
    declare_inverse_index_right(d, p)?;
    declare_inverse_index_left(d, p)?;
    declare_id(d, p)?;
