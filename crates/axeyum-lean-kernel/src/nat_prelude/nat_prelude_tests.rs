...
fn theorem_names(p: &NatPrelude) -> Vec<NameId> {
    vec![
        ...
        p.group_left_cancel,
        p.mod_add_is_group,
        p.inverse_index_right,
        p.inverse_index_left,
        p.comp_assoc,
    ]
}
...
/// `Nat.inverseIndex` COMPUTES the right inverse table for a concrete
...
#[test]
fn inverse_index_computes_a_concrete_permutation_table_with_a_transposed_negative_control() {
    let mut f = Fixture::new();
    let p = f.p;
    let nat = f.nat_ty();
    ...
    // The genuine inverse table: inverseIndex sigma 4 k, for k = 0,1,2,3,
    ...
    for (k_val, expected_val) in expected {
        let k = f.num(k_val);
        let idx = f.const_app(p.inverse_index, &[sigma, four, k]);
        let want = f.num(expected_val);
        assert!(
            f.k.def_eq(idx, want),
            "inverseIndex sigma 4 {k_val} must COMPUTE to {expected_val}"
        );
        ...
        assert!(
            ...
            "inverseIndex sigma 4 {k_val} must NOT also compute to {}",
            (expected_val + 1) % 4
        );
        if k_val == 1 {
    ...
    // `sigma (inverseIndex sigma 1 4) = 4` instead of
    // `sigma (inverseIndex sigma 4 1) = 1`. This is a genuinely different
    // computed value (confirmed below before trusting the rejection), not
    // an accidental tautology: `inverseIndex sigma 1 4` computes to `0`
    ...
    let idx_transposed = f.const_app(p.inverse_index, &[sigma, one_bound, four_target]);
    let zero = f.zero();
    assert!(
        f.k.def_eq(idx_transposed, zero),
        "inverseIndex sigma 1 4 must compute to 0 (only index 0 is searched)"
    );
    let sigma_idx_transposed = f.apply(sigma, &[idx_transposed]);
    assert!(
        f.k.def_eq(sigma_idx_transposed, one),
        "sigma (inverseIndex sigma 1 4) computes to 1, genuinely != the claimed 4"
    );
    let transposed_stmt = f.eq(sigma_idx_transposed, four_target);

    let bad_name = f.name("nc_inverse_index_transposed_n_and_k");
    let err = f
    ...
    println!(
        "NC (inverseIndex with n and k transposed) rejected:\n  {}",
        f.explain(&err)
    );
