...
pub struct CRealPrelude {
    ...
    /// (add j j)))`, `R := 8/5` -- π rung 2 item 3, the `Converges` witness
    ...
    /// `CReal.cosWideNonpositive : le (cosFnWide R) zero`, `R := 8/5` -- π
    ...
    /// (sumRange t 3)`; the numeric leaf `sumRange t 3 = -13/1875 <= 0`
    /// closes by `Rat.ble`'s own computation on FLAT, already-reduced
    /// representatives of the three terms (never their natural multi-layer
    /// towers -- see `creal/cos_sign.rs::collapse_to_flat`'s doc comment for
    /// why that distinction is load-bearing for build time). See
    /// `creal/cos_sign.rs`.
