...
//! independently proof-isolated imported target, then run
//! **conclusion-directed** application
//! (`axeyum_lean_import::producers::conclusion_directed_application`).
//!
//! Identical inputs, identical audit and identical trust anchor as
//! `imported_candidate_transport_probe`; only the producer differs. The
//! forward product closure that probe uses cannot reach a five-argument
//! application inside its 128-term budget, which is exactly the shape every
//! `Nat.ModEq` law carrying a congruence hypothesis has. Exact declaration
//! reuse or checked closure composition still has to succeed, each transported
//! theorem must have an empty axiom footprint, and the target theorem is
...
use axeyum_lean_import::{
    ...
    producers::propose_conclusion_directed_application, transport_checked_theorem_candidate,
};
use axeyum_lean_kernel::Declaration;

const USAGE: &str = "usage: conclusion_directed_transport_probe <target-stream.ndjson> <target-definition> <source-stream.ndjson> <source-theorem>...";
...
fn main() {
    if let Err(error) = run() {
        eprintln!("CONCLUSION_DIRECTED_TRANSPORT|result=declined|reason={error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let candidate =
        propose_conclusion_directed_application(&mut kernel, goal, &candidates).map_err(
            |error| {
                format!(
                    "conclusion-directed-application:{error:?};transported={};added={added};reused={reused};transport_declines={}:{}",
                    candidates.len(),
                    declined.len(),
                    declined.join(";")
                )
    ...
    let transport = kernel.name_str(axeyum, "ConclusionDirectedTransport");
    ...
    println!(
        "CONCLUSION_DIRECTED_TRANSPORT|result=accepted|target={target}|roots={}|transported={}|added={added}|reused={reused}|transport_declines={}|goal_binders={}|holes={}|holes_matched={}|declarations_tried={}|declarations={}|axioms={}|theorem_dependencies={}|theorem_dependency_names={theorem_dependency_names}|target_dependency={target_dependency}|goal_sha256={goal_sha256}|proof_sha256={proof_sha256}|target_content_sha256={target_content_sha256}",
        roots.len(),
        candidates.len(),
        declined.len(),
        candidate.goal_binders,
        candidate.holes,
        candidate.holes_matched,
        candidate.declarations_tried,
