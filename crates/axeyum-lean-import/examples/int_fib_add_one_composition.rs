//! Compose and specialize the exact integer Fibonacci add-one corollary.
...
const ALGEBRA_SHA256: &str = "42683af8480ea4d7bef42b32ecab3f3d43a2228c8a30f8b73417bbda6eca4fa2";
const RESIDUAL_SHA256: &str = "1705049d25be1b04447d7cd02b08f02c18d7f11e62dd500ad5f980be352bd65b";
...
const RESIDUAL: &str = "Axeyum.Autogenesis.intFibAddOneResidualV3";
const TARGET: &str = "Int.fib_add_one";
const USAGE: &str = "usage: int_fib_add_one_composition <recurrence.ndjson> <algebra.ndjson> <residual.ndjson> <output.ndjson>";

fn main() {
    if let Err(error) = run() {
        eprintln!("int-fib-add-one-composition: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let algebra_path = path(&mut arguments)?;
    ...
    let algebra = import_bound(&algebra_path, ALGEBRA_SHA256, "algebra")?;
    let residual = import_bound(&residual_path, RESIDUAL_SHA256, "residual")?;
    for (label, imported, root) in [
        ("recurrence", &recurrence, RECURRENCE),
        ("commutativity", &algebra, ADD_COMM),
        ("right cancellation", &algebra, CANCEL_RIGHT),
    ...
    let algebra_composition =
        compose_checked_theorem_slice(algebra.kernel(), &kernel, &[ADD_COMM, CANCEL_RIGHT])
            .map_err(|error| format!("algebra composition declined: {error:?}"))?;
    verify_checked_theorem_composition(
        algebra.kernel(),
        &kernel,
        algebra_composition.kernel(),
        algebra_composition.receipt(),
    )
    .map_err(|error| format!("algebra composition replay failed: {error:?}"))?;
    kernel = algebra_composition.kernel().clone();
    ...
    let specialization_arguments = [
        ...
        find_name(&kernel, ADD_COMM)?,
        find_name(&kernel, CANCEL_RIGHT)?,
    ];
    let target_name = nested_name(&mut kernel, &["Int", "fib_add_one"]);
    let specialized =
    ...
    let expected_dependencies = [RESIDUAL, ADD_COMM, CANCEL_RIGHT, RECURRENCE];
    ...
    println!(
        ...
        serde_json::to_string_pretty(&json!({
            "schema_version": 1,
            "kind": "axeyum-int-fib-add-one-exact-composition",
            ...
            "composition_receipts": {
                ...
                "algebra": algebra_composition.receipt().receipt_sha256
            },
