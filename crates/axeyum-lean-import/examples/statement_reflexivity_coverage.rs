...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for mapped in mapping_rows {
        ...
        let completed = match import_statement_ndjson(
            ...
            Err(error) => {
                ...
                row.insert("outcome".into(), json!("adapter-rejection"));
                row.insert("reason".into(), json!(reason));
                row.insert("detail".into(), json!(error.to_string()));
                *counts
                    .entry(format!("adapter-rejection:{reason}"))
                    .or_default() += 1;
        ...
        let candidate = match propose_reflexivity(&mut kernel, goal) {
            ...
            Err(detail) => {
                ...
                row.insert("outcome".into(), json!("producer-decline"));
                row.insert("reason".into(), json!(reason));
                row.insert("detail".into(), json!(detail));
                *counts
                    .entry(format!("producer-decline:{reason}"))
                    .or_default() += 1;
                rows.push(Value::Object(row));
                continue;
            }
        ...
        if let Err(error) = kernel.add_declaration(Declaration::Theorem {
            ...
            row.insert("outcome".into(), json!("kernel-rejection"));
            row.insert("reason".into(), json!("candidate-typecheck-failed"));
            row.insert("detail".into(), json!(format!("{error:?}")));
            *counts
                .entry("kernel-rejection:candidate-typecheck-failed".into())
                .or_default() += 1;
            rows.push(Value::Object(row));
            continue;
        }
        ...
        if target_dependency || axioms != 0 || theorem_dependencies != 0 {
            ...
            *counts
                .entry("assurance-rejection:dependency-audit-failed".into())
                .or_default() += 1;
        } else {
            row.insert("outcome".into(), json!("admissible-proof"));
            row.insert("reason".into(), Value::Null);
            *counts.entry("admissible-proof".into()).or_default() += 1;
        }
        rows.push(Value::Object(row));
    }
