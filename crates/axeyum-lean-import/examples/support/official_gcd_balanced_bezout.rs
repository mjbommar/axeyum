// SHARED BODY, `include!`d by two examples — deliberately NOT a Cargo target.
//
// It sat directly in `examples/` until 2026-08-21, which auto-registers it as a
// Cargo target. That target then failed `missing_docs`, and it could not be given
// `//!` crate docs either, because those land mid-file in whichever example
// `include!`s it (`error[E0753]: expected outer doc comment`). Two requirements,
// one file, contradictory — so it moved here, where this repo already keeps
// `include!`d bodies, and each caller carries its own docs.
