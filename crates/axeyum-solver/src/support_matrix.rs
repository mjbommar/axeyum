...
pub const SUPPORT_MATRIX: &[SupportRow] = &[
    ...
    SupportRow {
        ...
        note: "complete over finite (Bool/BV) domains, guarded-finite Int expansion, \
               ...
               and guarded unit-gap original-assertion checkers. One leading block of at most 16 \
               distinct almost-uninterpreted Int/Real universals may instead carry an \
               ...
               require source-bound QF_BV DRAT/LRAT implications (ADR-0129). A complete free-BV \
               model can certify direct positive universals through exact affine-LSB invariants and \
               direct negated universals through evaluator-replayed complete binder witnesses under \
               explicit source caps (ADR-0130). A directly negated existential implication may carry \
               a complete free-BV model only when exact ground replay proves every binder-free \
               antecedent and falsifies the untouched conclusion, while a separate nonempty signed \
               interval check proves lower <= upper <= cap for its sole binder-dependent clause \
               (ADR-0131). A separate directly negated existential class requires one exact \
               ...
               (ADR-0133). A query-scoped UNSAT certificate may carry 1 through 256 unique complete \
               source bindings for admitted positive Bool/BV universals; an independent checker \
               rebuilds the exact ground weakening and every instance before replaying its \
               QF_BV DRAT/LRAT proof, and heuristic candidate blocks cannot enter the artifact \
               ...
               against the original negated source axiom. ADR-0139 likewise applies a concrete \
               Bool/BV assignment to an untouched closed universal and refutes the exact body \
               with an evaluated-AIG proof. Positive \
