...
impl StringGate {
    ...
    fn confirm(
        ...
        if !has_quantifier && !self.map.is_empty() {
            ...
            // Step 1a — the LIA length **projection**, tried FIRST. The bounded
            // encoding emits per-string-variable well-formedness constraints
            // (padding above the length field is zero) that pass through the
            // abstraction as pure bit-vectors; mixed with the length facts' `Int`
            // atoms they defeat the exact refuters (a free `BitVec` forces the
            // sat-only bounded-integer path, which returns `unknown` rather than
            // the true `unsat` — e.g. `xx = xx ++ yy ∧ len(yy) > len(xx)`).
            // Dropping every abstracted assertion that carries no `Int` subterm
            // is a **sound weakening**: removing constraints only *adds* models,
            // so an `unsat` of the subset still implies the full abstraction
            // (hence the real theory) is `unsat`. The kept subset is pure
            // Bool+LIA, which the length refuters decide — cheaply, which is why
            // it runs before the full abstraction: the full solve can burn the
            // whole budget in the bounded-integer path on large scripts
            // (full_str_int) whose refutation lives entirely in the projection.
            ...
            // Step 1b — the **bit-vector-free** projection: keep every abstracted
            // assertion with no `BitVec`-sorted subterm. Same sound-weakening
            // argument (dropping assertions only adds models), but a different
            // subset from step 1a: generated Boolean-glue scripts (kaluza
            // `(= T_5 (not T_4)) ∧ T_5 ∧ T_4'`) tie string atoms together
            // through *pure-Bool* assertions that carry no `Int` subterm, so the
            // Int-mentions projection drops exactly the glue the refutation
            // needs. The BV-free subset keeps the glue and stays pure Bool+LIA
            // — decidable by the exact refuters.
            let bv_free: Vec<TermId> = abstracted_assertions
                .iter()
                .copied()
                .filter(|&a| !mentions_bv_sort(arena, a))
                .collect();
            if bv_free.len() < abstracted_assertions.len() {
                let mut bv_free = bv_free;
                bv_free.extend(self.facts.iter().copied());
                if matches!(solve(arena, &bv_free, config)?, CheckResult::Unsat) {
                    return Ok(CheckResult::Unsat);
                }
            }
...
/// Whether `t` has any `BitVec`-sorted subterm — used by the step-1b BV-free
/// projection to keep only pure Bool+LIA abstracted assertions (packed-string
/// well-formedness constraints carry `BitVec` content and are dropped; see the
/// projection comment for the soundness argument).
fn mentions_bv_sort(arena: &TermArena, t: TermId) -> bool {
    fn go(arena: &TermArena, t: TermId, memo: &mut HashMap<TermId, bool>) -> bool {
        if let Some(&b) = memo.get(&t) {
            return b;
        }
        let b = matches!(arena.sort_of(t), Sort::BitVec(_))
            || match arena.node(t) {
                TermNode::App { args, .. } => {
                    let args = args.clone();
