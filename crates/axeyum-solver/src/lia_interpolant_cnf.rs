...
impl Relax {
    ...
    fn translate_uncached(
        ...
        let out = match node {
            ...
            // No real analogue for a bit-vector, and no `Rational::integer`
            // image for an integer outside `i128` (ADR-1702 keeps promotion
            // opt-in): both decline rather than narrow.
