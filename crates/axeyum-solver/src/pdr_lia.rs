//! IC3/PDR (Spacer-style) inductive-invariant discovery over **linear integer
//! arithmetic** (`LIA`) transition systems — the integer mirror of the
//! [`pdr_lra`](crate::pdr_lra) engine over the reals.
//!
//! Where [`prove_safety_pdr_lra`](crate::prove_safety_pdr_lra) discovers an
//! inductive invariant for `Real`-sorted systems, [`prove_safety_pdr_lia`] does
//! the same for systems whose state variables are **`Int`**-sorted and whose
//! `init`/`trans`/`bad` are linear-integer-arithmetic formulas. The structural
...
//! 3-check gate) is exactly [`pdr_lra`](crate::pdr_lra); only the *cube* literal
//! recognizer and the *predecessor generalizer* change for the real→integer
//! switch:
//!
//! * A **cube** here is a conjunction of atomic `LIA` literals (a region of the
//!   integer state space) — an integer comparison (`IntLt`/`IntLe`/`IntGt`/
//!   `IntGe`), an `Eq` over `Int`-sorted operands, or a single `BoolNot` of one.
//!   A **lemma** is the negation of a cube (a blocking clause). Frames
//!   `F[0] = init`, `F[1..=k]` are sets of lemma clauses, exactly as in
//!   [`pdr_lra`](crate::pdr_lra).
//! * The **predecessor cube** (the pre-states that can step into an obligation
//!   `c`) is generalized by **integer model-based projection**
//!   ([`mbp_lia`](crate::mbp_lia)): given the satisfying model `M` of
//!   `trans(s, s') ∧ c'`, project the next-state variables `s'` out of the
//!   conjunction over ℤ, leaving a cube of `LIA` literals over `s` that `M`
//!   satisfies. `mbp_lia` self-verifies every projection with an **exact integer
//!   decision**, so a bug there can only shrink coverage, never produce an
//!   unsound predecessor. At the **divisibility boundary** (a non-unit
//!   coefficient whose Cooper `δ | …` output cannot be verified) `mbp_lia`
//!   returns `None`; the obligation is then routed to the trusted
//!   [`confirm_cex`](PdrLiaEngine::confirm_cex) unrolling (a real `Reachable` or a
//!   sound `Unknown`, never an unsound predecessor cube). Integer non-convexity
//!   is thus never papered over: when projection cannot soundly generalize, the
//!   engine falls back to the trusted gate rather than fabricating a cube.
//!
//! # The soundness contract (the whole point)
//!
//! The [`IC3`](crate::pdr) search in this module is **untrusted**. Its frames,
//! obligations, cube extraction, `mbp_lia`-driven predecessor generalization, and
//! lemma generalization are all best-effort: a bug in any of them can only ever
//! cause an over-eager [`PdrLiaOutcome::Unknown`], never a wrong
//! [`PdrLiaOutcome::Safe`]. That guarantee is enforced by a *single* trusted gate,
...
//! all three classical inductive-invariant checks over ℤ, each decided
//! independently by the trusted decider [`check_auto`](crate::check_auto)
//! (which routes `LIA` to [`check_with_lia_simplex`](crate::check_with_lia_simplex)
//! / [`check_with_lia_dpll`](crate::check_with_lia_dpll)) returning `Unsat`:
//!
...
//! [`PdrLiaOutcome::Unknown`]. A [`PdrLiaOutcome::Reachable`] is likewise gated: the
//! search may *believe* it found a counterexample, but the result is confirmed
//! only by [`confirm_cex`](PdrLiaEngine::confirm_cex), an inline `LIA` k-unrolling
//! `init(s0) ∧ trans(s0,s1) ∧ … ∧ bad(s_i)` decided `Sat` by
//! [`check_auto`](crate::check_auto) (the model is replay-checked). A non-`LIA`
//! system (e.g. `BV` state variables) degrades gracefully: the inner queries and
//! `mbp_lia` decline and the engine reports `Unknown`, never a panic.
//!
...
//! # References
//!
//! A. R. Bradley, *SAT-Based Model Checking without Unrolling* (`IC3`, 2011);
//! K. Hoder & N. Bjørner, *Generalized Property Directed Reachability* (`PDR`,
//! 2012); A. Komuravelli, A. Gurfinkel & S. Chaki, *`SMT`-Based Model Checking
//! for Recursive Programs* (Spacer, 2014). The integer predecessor generalizer is
//! Cooper-style model-based projection (see [`mbp_lia`](crate::mbp_lia)).
...
use crate::mbp::mbp_lia;
use crate::model::Model;

/// Resource caps for the `LIA` `IC3`/`PDR` search. All degrade to
/// [`PdrLiaOutcome::Unknown`]; none can cause a wrong verdict.
#[derive(Debug, Clone)]
...
/// The result of [`prove_safety_pdr_lia`].
#[derive(Debug, Clone)]
pub enum PdrLiaOutcome {
    ...
    /// consecution, safety) over ℤ under the trusted
    ...
    /// `steps` transitions, confirmed by an inline `LIA` k-unrolling decided `Sat`
    ...
    /// Undecided: a resource cap, an unsupported construct, the `mbp_lia`
    /// divisibility boundary the unrolling could not close, or a candidate
...
/// A cube: a conjunction of atomic `LIA` literals (each a single inequality /
...
#[derive(Debug, Clone)]
struct Cube {
    /// Atomic `LIA` literal terms, over the canonical step-0 vocabulary `s`.
    lits: Vec<TermId>,
}
...
/// discovery** — the integer mirror of
/// [`prove_safety_pdr_lra`](crate::prove_safety_pdr_lra). Model-based projection
/// over ℤ ([`mbp_lia`](crate::mbp_lia)) generalizes predecessors during blocking.
///
...
/// [`PdrLiaOutcome::Unknown`]. A [`PdrLiaOutcome::Reachable`] is confirmed by an
/// inline `LIA` k-unrolling decided `Sat` by [`check_auto`](crate::check_auto)
/// (the model is replay-checked).
///
/// Coverage is partial by design (the cube/predecessor machinery only handles the
/// `LIA` fragment `mbp_lia` and `check_auto` decide; the integer divisibility
/// boundary declines); a non-`LIA` system or a shape the search cannot close
/// degrades to `Unknown`. Soundness is total: a search bug can only cause an
/// over-eager `Unknown`.
///
/// # Errors
///
...
/// failed invariant verification is reported as [`PdrLiaOutcome::Unknown`], never
/// an error.
pub fn prove_safety_pdr_lia<S: TransitionSystem>(
    arena: &mut TermArena,
    system: &S,
    config: &SolverConfig,
) -> Result<PdrLiaOutcome, SolverError> {
    let limits = PdrLiaLimits::default();
    let mut engine = match PdrLiaEngine::new(arena, system, config, limits) {
        Ok(engine) => engine,
        Err(EngineSetup::Unsupported(reason)) => return Ok(PdrLiaOutcome::Unknown { reason }),
        Err(EngineSetup::Error(error)) => return Err(error),
    };
    engine.run(arena)
}

/// Setup failures for [`PdrLiaEngine::new`]: an unsupported construct degrades to
...
/// The `LIA` `IC3`/`PDR` engine over a fixed transition system. Frames are sets of
/// lemmas (blocking clauses, stored as the cubes they negate); `F[0]` is `init`.
struct PdrLiaEngine<'sys, S: TransitionSystem> {
    system: &'sys S,
    config: SolverConfig,
    limits: PdrLiaLimits,
...
impl<'sys, S: TransitionSystem> PdrLiaEngine<'sys, S> {
    fn new(
        arena: &mut TermArena,
        system: &'sys S,
        config: &SolverConfig,
    ...
    fn run(&mut self, arena: &mut TermArena) -> Result<PdrLiaOutcome, SolverError> {
        ...
        match self.init_is_bad(arena)? {
            ...
            Decision::Unknown(detail) => {
                return Ok(unknown(&format!(
                    "LIA PDR init-safety check undecided: {detail}"
                )));
            }
            Decision::Unsat => {}
        }
        ...
        for _iteration in 0..self.limits.max_iterations {
            if self.timed_out() {
                return Ok(unknown("LIA PDR timed out"));
            }
            if self.k() > self.limits.max_frames {
                return Ok(unknown("LIA PDR exceeded the maximum frame count"));
            }
        ...
        Ok(unknown("LIA PDR exceeded the maximum iteration count"))
    }
        ...
        loop {
            if self.timed_out() {
                return Ok(BlockResult::Decline(
                    "LIA PDR timed out in block".to_owned(),
                ));
            }
            let k = self.k();
    ...
    /// recursing on `mbp_lia`-generalized predecessors.
    fn block_obligation(
        &mut self,
        arena: &mut TermArena,
        cube: Cube,
        ...
        while let Some((cube, level)) = stack.pop() {
            *processed += 1;
            if *processed > self.limits.max_obligations {
                return Ok(BlockResult::Decline(
                    "LIA PDR exceeded the proof-obligation budget".to_owned(),
                ));
            }
            if self.timed_out() {
                return Ok(BlockResult::Decline(
    ...
    /// atomic `LIA` literals true at the witnessing model, or `None` if
    ...
    fn bad_cube_in_frame(
        ...
        match self.solve(arena, &[frame_term, bad])? {
            Decision::Sat => {
                ...
                // A bad cube must be non-empty to make progress; if no atomic LIA
    ...
    ///   witnessing model `M` (via [`mbp_lia`]), yielding a cube over `s`.
    fn relative_inductive_predecessor(
        ...
        match self.solve(arena, &[frame_prev, not_cube, trans, cube_primed])? {
            ...
            Decision::Sat => {
                let Some(model) =
                    ...
                    return Ok(RelInd::Decline(
                        "LIA PDR could not recover a predecessor model".to_owned(),
                    ));
                };
                match self.predecessor_cube(arena, cube, &model)? {
                    Some(pred) => Ok(RelInd::Predecessor(pred)),
                    // `mbp_lia` declined to generalize the predecessor (e.g. the
                    // divisibility boundary / a non-unit coefficient), but a
            ...
            Decision::Unknown(reason) => Ok(RelInd::Decline(format!(
                "LIA PDR relative-inductiveness query undecided: {reason}"
            ))),
        }
    }

    /// The **`mbp_lia` predecessor generalizer**. Builds the conjunction
    /// `trans(s, s') ∧ cube'` as a flat list of atomic `LIA` literals (all true at
    /// `model`), then projects out **every** next-state variable `sp[i]` one at a
    /// time via [`mbp_lia`]. The result is a cube of `LIA` literals over `s` (the
    /// remaining vocabulary) that `model` satisfies — the pre-states that can step
    /// into `cube`. Returns `None` (decline) if the conjunction is not flattenable
    /// to `LIA` literals or any projection step declines (the integer divisibility
    /// boundary); the caller then routes the obligation to `confirm_cex`.
    fn predecessor_cube(
        &mut self,
        arena: &mut TermArena,
        cube: &Cube,
        ...
        // Flatten both into atomic LIA literals. A non-LIA shape declines.
        ...
        // toward s only. A `None` (divisibility boundary) declines the whole
        // generalization — sound, never an unsound predecessor.
        let mut current = literals;
        for &var in &self.sp.clone() {
            match mbp_lia(arena, &current, model, var) {
                Some(projected) => current = projected,
                None => return Ok(None),
            }
        }

        // Defensive: drop any literal still mentioning a primed var (mbp_lia
        ...
        for i in 1..top {
            if self.timed_out() {
                return Ok(PropagateResult::Decline(
                    "LIA PDR timed out in propagate".to_owned(),
                ));
            }
            ...
            for lemma in lemmas {
                ...
                match self.clause_pushes_forward(arena, &lemma, i)? {
                    ...
                    None => {
                        return Ok(PropagateResult::Decline(
                            "LIA PDR propagation query undecided".to_owned(),
                        ));
                    }
                }
            }
    ...
    /// Confirms a believed counterexample via an inline `LIA` k-unrolling — the
    ...
    fn confirm_cex(&mut self, arena: &mut TermArena) -> Result<PdrLiaOutcome, SolverError> {
        ...
        for i in 0..=depth {
            if self.timed_out() {
                return Ok(unknown(
                    "LIA PDR timed out confirming a believed counterexample",
                ));
            }
            if i > 0 {
            ...
            match check_auto(arena, &assertions, &self.config) {
                Ok(CheckResult::Sat(model)) => {
                    return Ok(PdrLiaOutcome::Reachable { steps: i, model });
                }
        ...
        Ok(unknown(
            "LIA PDR believed a counterexample exists, but no inline LIA unrolling up to the cap \
...
/// safety only if it passes all three classical inductive-invariant checks over
/// ℤ, each decided independently by [`check_auto`](crate::check_auto) returning
/// `Unsat`.
///
...
fn verify_invariant(
    ...
    if !is_unsat(arena, &[init, not_inv], config)? {
        return Ok(unknown(
            "LIA PDR candidate failed the initiation check (init ⇒ Inv is not valid); declining",
        ));
    }
    ...
    if !is_unsat(arena, &[invariant, trans, not_inv_primed], config)? {
        return Ok(unknown(
            "LIA PDR candidate failed the consecution check (Inv is not transition-closed); \
             declining",
        ));
    }
    ...
    if !is_unsat(arena, &[invariant, bad], config)? {
        return Ok(unknown(
            "LIA PDR candidate failed the safety check (Inv does not exclude bad); declining",
        ));
    }

    Ok(PdrLiaOutcome::Safe { invariant })
}
...
/// The atomic `LIA` literals of `formula` that are true at `model`, as a cube over
...
/// Flattens a `BoolAnd`-tree `term` into its atomic `LIA` literal leaves, pushing
/// each leaf onto `out`. Returns `false` if any leaf is **not** an atomic `LIA`
/// literal (an integer inequality, an `Eq` over integers, or a single `BoolNot` of
/// one) — so the caller declines a non-`LIA`/non-conjunctive shape rather than
/// feeding `mbp_lia` something it cannot parse. `true` (a `BoolConst(true)`)
/// flattens to no leaves.
...
/// Whether `term` is an atomic `LIA` literal that [`mbp_lia`] can parse: an
/// integer comparison, an `Eq` between `Int`-sorted operands, or a single
/// `BoolNot` of such a literal.
fn is_lia_atom(arena: &TermArena, term: TermId) -> bool {
    match arena.node(term) {
        TermNode::App {
            op: Op::BoolNot,
            args,
        } => is_lia_atom(arena, args[0]),
        TermNode::App {
            op: Op::IntLt | Op::IntLe | Op::IntGt | Op::IntGe,
            args,
        } => args
            .iter()
            .all(|&a| arena.sort_of(a) == axeyum_ir::Sort::Int),
...
/// Collects the atomic `LIA`-literal leaves of `formula` (descending through
/// `BoolAnd`/`BoolOr`/`BoolNot`), used to seed a bad cube. Non-atomic, non-`LIA`
/// leaves are skipped.
fn collect_atoms(arena: &TermArena, formula: TermId, out: &mut Vec<TermId>) {
    match arena.node(formula) {
        ...
        TermNode::App {
            ...
            // A negated atom is itself an LIA atom mbp_lia handles; keep the whole
            // negation as the literal.
            let inner = args[0];
            if is_lia_atom(arena, formula) {
                out.push(formula);
            } else {
...
fn unknown(reason: &str) -> PdrLiaOutcome {
...
enum RelInd {
    Blockable,
    Predecessor(Cube),
    /// A predecessor genuinely exists but `mbp_lia` could not generalize it (the
    /// integer divisibility boundary); the caller routes to the trusted
    /// `confirm_cex` unrolling.
