...
pub fn check_qf_uflra_online(
    ...
    // 2. Boolean-structured QF_UFLRA: drive the combination from a DPLL(T) layer over
    //    the Tseitin skeleton (one proposition per distinct UFLRA atom). Early
    //    theory-conflict detection (partial-assignment pruning) is on by default.
    Ok(check_qf_uflra_boolean(
