...
//!
//! - The sound *conjunctive* MBTC (`decide_conjunction`): flatten the assertions to
//!   a conjunction of literals, assert each theory's atoms, and DFS-split on the
//!   shared pairs (`s = t` vs `s ≠ t`, the disequality branch realized as
//!   `s < t ∨ s > t` on the `LRA` side, recorded as a disequality on the `EUF` side).
//!   Every undetermined pair the `EUF` congruence already pins (entailed / refuted)
//!   prunes a branch — the equality-sharing exchange. A leaf consistent in **both**
//!   theories yields a combined model that is **replayed against the original
...
/// Decides an **arbitrary Boolean combination** of `QF_UFLRA` literals (`EUF` +
/// linear real arithmetic) by the **online** model-based `Nelson–Oppen` combination,
/// returning a **replay-checked** model on `sat`. The warm, equality-sharing
...
/// skeleton uses a connective the encoder does not cover, when the interface split or
/// the propositional enumeration exceeds an internal cap (depth, model count, atom
/// count, clauses, or `config.timeout`), or when arithmetic overflow made a
...
pub fn check_qf_uflra_online(
    ...
    // fast-path applies only when every flattened literal is a genuine theory atom
    // (an equality or an order atom); otherwise the Boolean layer handles it.
...
#[doc(hidden)]
#[must_use]
pub fn check_qf_uflra_boolean_with_metrics(
    arena: &mut TermArena,
    assertions: &[TermId],
    config: &SolverConfig,
    ...
    let result = check_qf_uflra_boolean_enumerative(
        arena,
        assertions,
        config,
        enable_early_prune,
...
/// (the original conjunctive core, factored out so both the fast-path and the
/// Boolean DPLL(T) layer call it). Returns a replay-checked model on `sat`, `Unsat`
...
/// **prepared** `LraTheory` whose atom indices are already the original `LRA` literals
/// (positions `0..part.lra.len()`) plus the per-pair interface atoms named by
/// `pair_atoms`. Factored out of [`decide_conjunction`] so the warm
/// [`crate::combined_theory::CombinedTheory`] runs the *exact same* DFS over its
/// persistent `LraTheory`, guaranteeing the warm oracle returns the identical verdict
/// to the from-scratch core (the parallel-run equivalence gate).
...
/// Whether `term` is a genuine `QF_UFLRA` theory atom — an equality (`(= s t)`) or a
/// real order atom (`<`, `<=`, `>`, `>=`). Used to tell a conjunction of real atoms
/// (the fast-path) from a flattened literal that is itself Boolean structure (a
...
pub(crate) fn shared_real_terms(arena: &TermArena, part: &Partition) -> Vec<TermId> {
...
pub(crate) fn unordered_pairs(shared: &[TermId]) -> Vec<(TermId, TermId)> {
...
/// model builder omitted — a symbol living purely in the propositional skeleton (never
/// as a theory-atom side), so it never entered the e-graph / LRA witness. Its truth
/// value is read from the Boolean layer via `value_of` (the SAT variable the encoder
/// assigned it, in `term_var`). Symbols are visited in `TermId`-sorted order so the
/// injection is independent of `HashMap` iteration order (the determinism hard rule).
/// Additive and **replay-gated by the caller**, so it can never manufacture a wrong
/// `sat`.
...
//
// Full QF_UFLRA with arbitrary Boolean structure: Tseitin-encode the skeleton over
// one proposition per distinct theory atom, enumerate propositional models with a
// self-contained DPLL search, and decide each total model's conjunction with the
// conjunctive combination [`decide_conjunction`] above (the theory oracle). The
// search prunes with theory-conflict blocking clauses (the negation of an UNSAT
// model's theory-literal assignment). It is the sound enumerative DPLL(T): SAT if
// some model's combination is consistent (replay-checked), UNSAT if every model is
// blocked, and a conservative Unknown on any cap or an Unknown combination.
...
/// interface **structural clauses** (`eq ∨ lt ∨ gt` totality + pairwise exclusion). The
/// `Dpll`'s joint unit + theory propagation and `1-UIP` conflict analysis now span the
/// Boolean, theory, **and** interface-equality literals: the interface case-split that the
/// retired [`BoolSearch`] enumeration ran as a private DFS is now ordinary `SAT` branching
/// on the registered interface variables.
///
...
/// combined model — the soundness gate. The verdict is translated to the
/// [`check_qf_uflra_online`] contract: `Sat` (replay-checked) on a replaying leaf model,
...
///
/// Falls back to the retained enumerative layer ([`check_qf_uflra_boolean_enumerative`])
...
fn cdclt_combined(
    ...
    // interface variables `CombinedIncremental` registered at `atom_count..combined_count`.
    ...
    match decide_conjunction(arena, &literals) {
        CheckResult::Sat(mut model) => {
            ...
            // the EUF/LRA witness, so it is omitted and the original assertion fails to
            // replay on the unassigned symbol. Inject each such skeleton-only Bool symbol
...
/// **terms** (the slice-3c soundness-gate harness shape): the clause literals as
...
/// The diagnostics the real-`CDCL(T)` combined driver reports (slice-3c gate harness):
/// the verdict code (`0` `Unsat` / `1` `Sat` / `2` `Unknown`), the number of registered
/// shared-interface pairs (`> 0` proves the interface case-split machinery is wired), the
/// number of 1-UIP analyses that fired, and every learned 1-UIP clause over terms.
#[cfg(test)]
...
/// **Slice-3c soundness-gate harness** (test-only): drive a Boolean-structured
/// `QF_UFLRA` query through the real-`CDCL(T)` combined layer
/// (`Dpll<CombinedIncremental>`) and report its diagnostics ([`CombinedCdcltDiag`]) — the
/// verdict, the registered interface-pair count, the 1-UIP fire count, and every learned
/// 1-UIP clause **over terms**. The slice-3c gate uses it to assert (a) the new loop fires
/// (interface pairs registered, 1-UIP analyses run) and (b) every learned theory lemma is
/// genuinely entailed (its term-level negation conjoined with the level-0 facts is `UNSAT`
/// under the trusted conjunctive decider — the 1-UIP-over-combination soundness check).
///
...
pub(crate) fn combined_cdclt_diag(
    ...
    // eq/lt/gt interface terms (in the same order `CombinedIncremental::new` registered).
    ...
    for p in combined.interface_pairs() {
        let (s, t) = p.terms;
        let (Ok(eq), Ok(lt), Ok(gt)) = (arena.eq(s, t), arena.real_lt(s, t), arena.real_gt(s, t))
        else {
            return None;
        };
        var_term.push(eq);
    ...
    // clause references only combined or Tseitin-aux variables; an aux var has no atom
    // term and is dropped from the term-level report, which only weakens the re-validation
    // claim conservatively, never strengthens it).
...
/// Decides a Boolean-structured `QF_UFLRA` query by the **enumerative** `DPLL(T)` layer
/// (the retained slice-1/2 components: warm [`crate::combined_theory::CombinedTheory`]
/// oracle, early theory-conflict pruning, and combined theory propagation, behind a
/// chronological [`BoolSearch`] enumeration). The production path
/// ([`check_qf_uflra_boolean`]) now uses the real `CDCL(T)` driver
/// (`Dpll<CombinedIncremental>`); this enumerative layer remains as the fallback for the
/// rare unbuildable-incremental-state case and as the harness the slice-1/2 metric gates
/// exercise.
///
/// The distinct theory atoms (`EUF` equalities, `LRA` order atoms) become the first
...
/// model and the search continues; an `unknown` combination degrades the whole query
/// to a conservative [`CheckResult::Unknown`]. `UNSAT` is reported only when every
/// propositional model is blocked.
fn check_qf_uflra_boolean_enumerative(
    arena: &mut TermArena,
    assertions: &[TermId],
    config: &SolverConfig,
    enable_early_prune: bool,
...
/// backtracking over the Tseitin skeleton; on each total propositional model the
/// theory atoms' conjunction is decided by [`decide_conjunction`], and an `unsat`
/// model is blocked by a theory-conflict clause.
struct BoolSearch<'a> {
    var_count: usize,
    ...
    /// `LraTheory` and reuses it when the `LRA` atom layout repeats across the
    /// enumeration. The enumeration above is unchanged — only the theory oracle is warmed.
    combined: crate::combined_theory::CombinedTheory,
}
...
impl BoolSearch<'_> {
    ...
    /// Boolean unit propagation to fixpoint. `Err` carries the (Boolean) conflict
    /// clause `¬clause` on an all-false clause; `Ok(())` at a consistent fixpoint.
    ...
    /// Undoes the trail back to (and excluding) the most recent decision, returning
    /// that decision's `(var, value)`; `None` when no decision remains.
    ...
    /// decision, and flips it as an implied assignment. `false` when no decision
    /// remains (the propositional search is exhausted).
    ...
    /// The theory-literal conjunction of the current total propositional model: each
    /// atom variable's term at its assigned polarity.
            ...
            match self.pick_unassigned() {
                None => {
                    ...
                    match self.combined.check(arena, &literals) {
                        ...
                        // The combination could not certify this model either way: a
                        // sound decline (the offline decider may still settle it). We
                        // cannot soundly call the whole query UNSAT, so degrade.
...
/// Collects the distinct `QF_UFLRA` theory atoms in `term` — `EUF` equalities
/// (`(= s t)`) and `LRA` order atoms (`<`, `<=`, `>`, `>=`) — in a stable
/// left-to-right scan (so the proposition indexing is deterministic). An atom is not
/// descended into (its sides are theory terms, not Boolean structure).
pub(crate) fn collect_uflra_atoms(
    arena: &TermArena,
    term: TermId,
    out: &mut Vec<TermId>,
    ...
    match arena.node(term) {
        ...
        TermNode::App { args, .. } => {
            let args = args.clone();
            for a in args {
                collect_uflra_atoms(arena, a, out, seen);
            }
        }
        _ => {}
    }
...
/// Tseitin encoder from the typed Boolean IR into the propositional skeleton, with
/// the first `atom_terms.len()` variables reserved for the theory atoms (numbered to
/// match the [`BoolSearch`] atom variables). Mirrors the encoder in
/// [`crate::lra_online`], retargeted to the combination's atom set.
struct BoolEncoder {
    term_var: HashMap<TermId, usize>,
    var_count: usize,
}
...
impl BoolEncoder {
    ...
    /// fresh-variable counter started at `reserved` (≥ `atom_terms.len()`), so the
    /// Tseitin auxiliaries it allocates begin **after** a reserved block — the interface
    /// `eq`/`lt`/`gt` variables the real-`CDCL(T)` layer registers between the atoms and
    /// the auxiliaries. With `reserved == atom_terms.len()` this is exactly [`Self::new`].
    ...
    /// `None` for structure outside the supported connectives (a sound give-up).
    fn encode(
        &mut self,
        arena: &TermArena,
        t: TermId,
...
mod tests {
    ...
    /// A Boolean-structured `QF_UFLRA` query with shared interface terms (a unary real
    /// `f` applied to the same vars that occur in `LRA` atoms), so the combined layer
    /// registers interface pairs and the `Dpll` branches them — the case the slice-3c
    /// loop must drive.
    fn build_bool_case(arena: &mut TermArena, state: &mut u64) -> Vec<TermId> {
        ...
        for _ in 0..4 {
            ...
            let atom = match next_rand(state) % 4 {
                0 => arena.real_lt(lhs, rhs).expect("lt"),
                1 => arena.real_le(lhs, rhs).expect("le"),
                2 => arena.real_ge(lhs, rhs).expect("ge"),
    ...
    /// **Slice-3c gate (a): the new loop fires.** Over a deterministic batch of
    /// Boolean-structured `QF_UFLRA` queries with shared interface terms, the real-`CDCL(T)`
    /// combined driver (`Dpll<CombinedIncremental>`) must engage: at least one case
    /// registers shared-interface pairs *and* runs a 1-UIP conflict analysis (so the
    /// Boolean + theory + interface-equality search genuinely drives the combination, not a
    ...
    /// **Slice-3c gate (b): 1-UIP-over-combination soundness.** Every learned *theory
    ...
    /// `UNSAT` under the trusted offline conjunctive decider (eager Ackermann). Mirrors the
    /// `LRA` driver's 1-UIP soundness gate, extended over the EUF + LRA + interface-equality
    /// combination. A learned lemma that is NOT entailed would be an unsound resolvent — a
    /// hard failure.
    #[test]
