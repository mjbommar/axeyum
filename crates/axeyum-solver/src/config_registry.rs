...
pub static REGISTRY: &[ConfigEntry] = &[
    ...
    ConfigEntry {
        name: "ADDITIVE_NO_OVERFLOW",
        module: "crates/axeyum-rewrite/src/int_blast.rs",
        value: "1",
        unit: "off (0) / on (non-zero)",
        protects: Protects::Completeness,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: Some("AXEYUM_INT_BLAST_ADDITIVE_NO_OVERFLOW"),
        justification: undated("doc comment"),
        note: "Whether the int blaster also constrains `int_add`/`int_sub`/`int_neg` against wraparound, as it already does for `int_mul`. SHIPS ON (1) since ADR-1937. It closes the ADR-1921 standing hypothesis -- that the surviving width-32 replay failures are ADDITIVE, not multiplicative, wraparound -- which measured RIGHT: interleaved per-file A/B over the whole 200-file QF_NIA division, 39 decided -> 78, 40 gains, 0 real losses, 0 verdict flips, wall -10.2%; cost on 298 already-decided files across nine integer-bearing divisions 0 real losses, 0 flips, +1.9% wall; 78 verdicts, 0 disagreements against :status, z3 and cvc5. Set the variable to 0 to re-run the A/B against the pre-ADR-1937 encoding. Arming it STRENGTHENS the bit-vector query, so it can only lose models, never gain a wrong one: `sat` stays anchored by the exact-integer replay and an in-range `unsat` is already `unknown`. Guarded by the exact-integer replay in `combined.rs`: a bit-vector `Unsat` with integers present is reported `unknown`, never `unsat`, and the decline names the bounded width, so the crossing IS visible to the caller. Soundness-negative: crates/axeyum-solver/tests/additive_no_overflow_never_becomes_unsat.rs.",
    },
    ConfigEntry {
    ...
    ConfigEntry {
        ...
        signal: Signal::NotApplicable,
        guarded_by: "",
        env_override: Some("AXEYUM_CANONICAL_CACHE"),
        justification: undated("ADR-2144"),
        note: "Whether a warm `IncrementalBvSolver` keeps ADR-2144's canonical constraint cache: decided queries keyed by the sorted, duplicate-elided set of live assertion identities (ADR-0303's key), a cached `sat` served only after its model replays against the live set, a cached `unsat` only for a superset of the cached set, `unknown` never cached, 4,096-entry deterministic LRU. Not a cap: a bool default, registered because it carries the ONE-BINARY A/B LEVER `AXEYUM_CANONICAL_CACHE` (`on`/`off`), read once per process in `SolverConfig::default()`. It cannot move a verdict (`tests/canonical_constraint_cache_2144.rs` pins both invariants, and the corpus identity compares cache on and off on every flat QF_BV fixture); what it changes is WHICH model a repeated `sat` returns and how much work a repeated check does. OFF as shipped: the one-shot backends never read it, and a consumer with a retained session (Glaurung's warm path) turns it on per solver or per process. `the_lever_ships_off_and_the_explicit_config_turns_it_on` dies if the default moves.",
    },
    ConfigEntry {
        name: "DEFAULT_MODEL_PREFERENCE",
        module: "crates/axeyum-solver/src/backend.rs",
        value: "ModelPreference::Any",
        unit: "policy",
        protects: Protects::Time,
        on_exceed: OnExceed::SearchEvent,
        signal: Signal::NotApplicable,
        guarded_by: "",
        env_override: Some("AXEYUM_MODEL_PREFERENCE"),
        justification: undated("ADR-2140"),
        note: "Which model a `sat` prefers (ADR-2140): `any` (the shipped search, byte for byte), `zero` (every SAT decision is `false`, z3's `phase=always_false`), or `least-unsigned` (`zero` plus the front door's bounded re-solve ladder over magnitudes 1, 16, 256, 4096). Not a cap: an enum default, registered because it carries the ONE-BINARY A/B LEVER `AXEYUM_MODEL_PREFERENCE`, read once per process in `SolverConfig::default()`. A preference cannot move a verdict -- every `sat` still replays and every `unsat` still carries its proof (`tests/model_preference_2140.rs`) -- and what it costs is the changed search trajectory; ADR-2140's QF_BV pinned-list A/B records that price. `Any` is the default and `tests/model_preference_2140.rs::any_is_the_shipped_search_on_every_qf_bv_fixture` dies if it moves.",
    },
    ConfigEntry {
        name: "DEFAULT_MODEL_PREFERENCE_PHASE",
        module: "crates/axeyum-solver/src/backend.rs",
        value: "false",
        unit: "policy",
        protects: Protects::Time,
        ...
        signal: Signal::NotApplicable,
        guarded_by: "",
        env_override: Some("AXEYUM_MODEL_PREFERENCE_PHASE"),
        justification: dated(
            "ADR-2140",
            "2026-09-17",
            None,
    ...
    ConfigEntry {
        ...
        module: "crates/axeyum-solver/src/datatype_native.rs",
        value: "20_000",
        unit: "Ackermann congruence PAIRS",
        protects: Protects::Memory,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: None,
        justification: undated(
            "crates/axeyum-solver/src/datatype_native.rs (MAX_ACK_PAIRS doc comment)",
        ),
        note: "REGISTERED BY ADR-2128 RATHER THAN BY THE BOUND'S OWN LANE, and that is worth naming: `datatype_native.rs` is not in `GOVERNED_FILES`, so `every_governing_constant_is_registered` could never have asked for it and this bound stood unregistered from ADR-1935 until 2026-09-16. It is here because ADR-2128 added a sibling bound next to it and registering one of two identical-looking caps in one file would be worse than registering neither. Bounded on PAIRS rather than applications because that is what costs: Ackermann is quadratic in the applications of one function and the SPARK/Ada divisions apply one accessor to hundreds of records.",
    },
    ConfigEntry {
        name: "MAX_NESTED_CHILDREN",
        module: "crates/axeyum-solver/src/datatype_native.rs",
        value: "20_000",
        unit: "materialised child datatype variables",
        protects: Protects::Memory,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: None,
        justification: undated(
            "crates/axeyum-solver/src/datatype_native.rs (MAX_NESTED_CHILDREN doc comment)",
        ),
        note: "UNDATED, and the honest reason is that nothing has measured it: it is `MAX_ACK_PAIRS`'s value copied, on the same argument (refusing at a bound is a decline the harness reads; not refusing is a term explosion that spends the whole budget). It must stay a REFUSAL and never a truncation -- `datatype_expansion_is_exact_to_depth` has already been consulted by the time the materialiser runs, so an encoding that quietly stopped short of the budget would be claiming an exactness it did not build, which is the ADR-1930 wrong-`unsat` shape. Sizing that makes this datable: `AUFDTLIRA`'s undecided files declare 1717 datatypes across 82 files and the materialiser is seeded from EQUALITY SITES only (both references do the same -- cvc5 `theory_datatypes.cpp:1932-1948`, z3 `theory_datatype.cpp:432-436`), so the live population is far smaller than the declared one and nobody has measured how much.",
    },
    ConfigEntry {
        name: "MAX_NESTED_FIELD_DEPTH",
        module: "crates/axeyum-solver/src/datatype_native.rs",
        value: "8",
        unit: "levels of datatype-typed field nesting",
        protects: Protects::Time,
        on_exceed: OnExceed::Truncate,
        signal: Signal::None,
        guarded_by: "the value is a CLAMP on the parsed lever, not a runtime crossing: a request above it silently becomes this depth, and the encoding built at this depth is exact-or-relaxed exactly as `datatype_expansion_is_exact_to_depth` reports at this depth, so no verdict rests on the clamp",
        env_override: Some("AXEYUM_DT_NESTED_FIELD_DEPTH"),
        justification: dated(
            "docs/research/09-decisions/adr-2128-nested-datatype-field-expansion.md",
            "2026-09-16",
            None,
            &[sym(
                "crates/axeyum-solver/src/datatype_native.rs",
            ...
            &[doc(
                "bench-results/dt-field-expansion-20260916/census/reach-file-rollup.tsv",
            )],
        ),
        note: "THE DEFAULT IS 0 -- THE LEVER IS OFF -- and this is the ceiling on what turning it on may ask for. 8 is above the deepest datatype-field nest measured anywhere in ADR-2128's seven populations (5, in the SPARK `AUFDTLIRA` corpus; `bench-results/dt-field-expansion-20260916`), so every larger request would be budget spent on a shape no corpus file has. The clamp is on the PARSED value rather than on the walk so that `AXEYUM_DT_NESTED_FIELD_DEPTH=1000` degrades to a bounded run instead of a term explosion. Note what the cap does NOT do: it does not make the walk terminate on a recursive datatype -- `datatype_expansion_is_exact_to_depth` terminates on its own decreasing budget, and answers `false` for a cyclic closure at every depth, so no clamp value can make a cyclic expansion claim exactness.",
    },
    ConfigEntry {
        name: "MAX_CYCLE_WALK",
    ...
    ConfigEntry {
        ...
        value: "off",
        unit: "lever: off | on | screened -- decides the offline lazy-SMT loop's cubes on a persistent tableau and basis",
        protects: Protects::Time,
        on_exceed: OnExceed::SearchEvent,
        signal: Signal::None,
        ...
        justification: dated(
            ...
            None,
            &[
                sym("crates/axeyum-solver/src/dpll_t.rs", "warm_cube_mode"),
                sym(
                    "crates/axeyum-solver/src/dpll_t.rs",
                    "MIN_WARM_CUBE_SCREEN_BUILDS",
                ),
            ],
            &[adr("ADR-2125"), adr("ADR-2132")],
        ),
    ...
    ConfigEntry {
        name: "MIN_WARM_CUBE_SCREEN_BUILDS",
        module: "crates/axeyum-solver/src/dpll_t.rs",
        value: "64",
        unit: "from-scratch tableaux a file must build before `AXEYUM_LRA_WARM_CUBE=screened` keeps a basis",
        protects: Protects::Time,
        on_exceed: OnExceed::SearchEvent,
        signal: Signal::None,
        ...
        justification: dated(
            ...
            &[
                ...
                sym("crates/axeyum-solver/src/simplex.rs", "cold_builds_so_far"),
            ],
            &[adr("ADR-2132"), adr("ADR-2125")],
        ),
        note: "ADR-2132, and the number was chosen from a WINDOW rather than fitted to a point. [ADR-2125] shipped the warm basis `off` on 1 STABLE-GAIN against 2 STABLE-LOSS and named `simplex_cold_builds` as the axis between them -- it wins where the cubes are many and small and loses where they are few and large. Over the pinned QF_LRA 200 the two shapes do not overlap: the few-enormous rows are at 1, 28 and 42 builds (23,908 / 283.5 / 548.1 ms per build, and the 28-build row IS the pinned stable loss) while the winning family runs 841-1,543 builds at 0.4-6.4 ms each. Every row at or above 51 builds costs at most 12.2 ms per build and every row above 20 ms per build has at most 42, so any value in [43, 51] separates them; 64 is the next power of two above that window, 1.52x the largest few-enormous row and 13.1x below the smallest winning one. `cold_builds / lra_rounds` is 1.00 above 100 builds, so the screen trips after about 64 refinement rounds -- 4.1-7.6 % of the winning family's rounds run cold, which is what a screen costs instead of a clock. A CLOCK is not available here even though it would separate more: determinism is a public API promise and a wall-clock screen would make the verdict depend on the machine. WHAT THIS VALUE DOES NOT DO, measured after it was chosen and stated rather than buried: ADR-2125's SECOND stable loss, `QF_LRA/uart/uart-8.induction.cvc.smt2`, was not in its sizing table and it offered the builds reading for that row as an explicit HYPOTHESIS. Measured here it is 1,598 builds at 1.04 ms each -- the many-small shape, on the winning side of every threshold in the window -- so this screen cannot remove that loss and the hypothesis is REFUTED rather than confirmed. The threshold was NOT then moved between 1,598 and the gain's 1,815: those two rows are the held-out evaluation population, and a boundary drawn between two of its members would be fitted to the set it is scored on. CONFIRMED BY THE A/B rather than left as a prediction: over the complete 400 rows, `uart-8.induction.cvc` is a STABLE-LOSS under `screened` at 3/3 in both directions, and the screened arm answered 1,872 cubes warm on it. What the value DOES do, on the same run: it removes ADR-2125's PINNED stable loss (28 builds, refused, 0 cubes answered warm, the row comes back BOTH-DECIDE), 0 of 244 sub-threshold rows opened the screen, and the admitted set matched the sizing's prediction at 51 of 51 -- same set, 0 missing, 0 extra.",
    },
    ConfigEntry {
    ...
    ConfigEntry {
        name: "MODEL_SHRINK_EVALUATIONS",
        module: "crates/axeyum-solver/src/incremental.rs",
        value: "4_096",
        unit: "evaluator calls",
        protects: Protects::Time,
        on_exceed: OnExceed::Truncate,
        signal: Signal::None,
        guarded_by: "the pass only ever REPLACES a model with one `check_model` accepted; stopping early leaves the last accepted model, which is the search's own model when nothing was accepted, so the verdict decided before the pass began is untouched by construction",
        env_override: None,
        justification: undated("ADR-2140"),
        note: "Evaluator calls the replay-checked model shrink of `PreferZero`/`LeastUnsigned` (ADR-2140) may spend per query, each call re-evaluating every active assertion (and assumption, on the warm engine) against a model with one more bit cleared; the text front door re-exports it as `smtlib::MODEL_SHRINK_EVALUATIONS`. Bounds the pass on a query with thousands of symbols; also cut short by the query's own deadline. Not measured: the value is the first one written down, and the QF_BV A/B in ADR-2140 records what the pass cost at it.",
    },
    ConfigEntry {
        name: "MODEL_SHRINK_MIN_BUDGET",
        module: "crates/axeyum-solver/src/incremental.rs",
        ...
        on_exceed: OnExceed::Truncate,
        signal: Signal::None,
        guarded_by: "the shrink only ever REPLACES a model with one the replay accepted; running out of time leaves the last accepted model, so the verdict decided before the pass began is untouched by construction",
        env_override: None,
        justification: dated(
            "ADR-2140",
            "2026-09-17",
        ...
        note: "The FLOOR of the replay-checked model shrink's wall budget (ADR-2140); the budget itself is what the solve that produced the model spent, capped by the query's deadline (`shrink_deadline`). MEASURED 2026-09-17: with only the evaluation budget (`MODEL_SHRINK_EVALUATIONS`) the pass spent 25 s finishing a 1.7 s `sat` on `QF_BV/Sage2/bench_12354.smt2` (549 symbols), which the 24 s harness reported as a STABLE-LOSS; bounding the pass by the solve's own time keeps a preference from more than doubling a query. The floor is so a millisecond solve on a small query still gets a pass that can clear anything.",
    },
    ConfigEntry {
    ...
    ConfigEntry {
        name: "AXEYUM_LRA_ADMIT_NONZEROS",
        module: "crates/axeyum-solver/src/lra_online.rs",
        value: "off",
        unit: "lever: 1 | on arms it; anything else is off -- admits the online CDCL(T) engine's warm tableau on stored nonzeros instead of dense cells",
        protects: Protects::Memory,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: Some("AXEYUM_LRA_ADMIT_NONZEROS"),
        justification: dated(
            "docs/research/09-decisions/adr-2146-the-online-tableau-is-admitted-on-the-currency-it-is-stored-in.md",
            "2026-09-17",
            None,
            &[
                sym(
                    "crates/axeyum-solver/src/lra_online.rs",
                    "admit_nonzeros_lever",
                ),
                sym(
                    "crates/axeyum-solver/src/simplex.rs",
                    "MAX_ONLINE_TABLEAU_NONZEROS",
                ),
                sym(
                    "crates/axeyum-solver/src/simplex.rs",
                    "MAX_TABLEAU_FILL_NONZEROS",
                ),
            ],
            &[adr("ADR-2146")],
        ),
        note: "Pinned by: lra_online::tests::the_nonzero_admission_builds_the_tableau_the_dense_cap_refuses (the mechanism: 2k^2 cells over the cap, k nonzeros under it, read from `uses_simplex` per arm) and lra_online::tests::the_two_admissions_agree_on_a_satisfiable_and_an_infeasible_system (both arms, both verdict sides, the witness replayed and the core naming both crossing atoms); lra_theory::tests::the_admission_arms_agree_through_the_route_on_a_query_the_dense_cap_refuses (through the route); simplex::tests::the_fill_cap_declines_before_the_pivot_that_would_cross_it (the run-time half); qf_lra_differential_fuzz::boundary_tableaux_agree_with_z3 under both env arms. ADR-2146. The LRA-MODEL-REPLAY census (bench-results/lra-model-replay-20260916) found 27 of the 47 QF_LRA rows that die at \"model did not replay\" had NO warm tableau: `LraTheory::try_new_with_budget` admits on `TableauAdmission::DenseCells`, `m x (nvars+m)` against `MAX_TABLEAU_CELLS`, a count over storage ADR-2111 made sparse, so the witness fell to `solve_values` -- a full Fourier-Motzkin elimination keeping a clone of the whole system per variable -- which declined with the clock still available. The cube decider already asks the nonzero question (ADR-2125); this lever makes the online route ask it too, through `Incremental::with_online_admission`, which adds what the entry count cannot bound: a ROW ceiling (a row is a slot in every per-row scan whatever its nonzeros) and a RUN-TIME fill-in cap, because ADR-2132 measured fill-in growing a warm tableau 21x. THE POPULATION EXISTS ONLY ABOVE THE ATOM SCREEN: every one of the 27 has more than the 1,024 atoms `AXEYUM_LRA_ATOM_SCREEN=1` admits (the smallest is sc-27 at 1,036), so at the shipped screen this lever touches none of them and its own A/B measures the files at or under the screen whose dense count crosses the cap. The composition with the screen is measured separately and is the ADR's second table. Ships off until the A/B is read.",
    },
    ConfigEntry {
        name: "AXEYUM_LRA_DISEQ_SPLIT",
        module: "crates/axeyum-solver/src/lra_online.rs",
        value: "on",
        unit: "lever: 0 | off disarms it; anything else (unset included) is on -- splits a disequality the candidate model violates into its two strict halves for the SAT search",
        protects: Protects::Completeness,
        on_exceed: OnExceed::SearchEvent,
        signal: Signal::None,
        guarded_by: "lra_online::tests::a_violated_disequality_registers_its_two_strict_halves_once, lra_online::tests::both_halves_false_is_the_trichotomy_conflict_and_a_true_half_is_a_model, lra_online::tests::a_strict_half_against_its_equality_conflicts_at_assert (the three trichotomy edges), lra_online::tests::with_the_split_off_a_violated_disequality_registers_nothing (the OFF arm), lra_theory::tests::a_split_never_manufactures_a_model_for_an_infeasible_disequality and lra_theory::tests::a_split_lemma_must_keep_the_equality_or_it_refutes_a_satisfiable_query (the soundness-negative pair, both verdict sides), lra_theory::tests::an_unsat_after_a_split_lemma_still_carries_the_two_stream_artifact (the certificate route), qf_lra_differential_fuzz with its `distinct` and disequality seed classes under both env arms",
        env_override: Some("AXEYUM_LRA_DISEQ_SPLIT"),
        justification: dated(
            "docs/research/09-decisions/adr-2147-a-violated-disequality-is-a-case-split-not-an-unknown.md",
            "2026-09-17",
            None,
            &[
                sym(
                ...
                sym(
                    "crates/axeyum-solver/src/lra_online.rs",
                    "split_violated_disequalities",
                ),
            ],
            &[adr("ADR-2147")],
        ),
        note: "ADR-2147. `LraTheory::assert` drops an equality atom asserted FALSE (`(AtomKind::Equality { .. }, false) => Vec::new()`) under a comment that said the driver never sets one -- true of the offline `check_qf_lra_online` driver it named, false of the CDCL(T) driver, whose `is_lra_atom` admits every real `Op::Eq`: the census counted 371 of 776 equality atoms asserted false on sc-25 and 11 pinned files losing their verdict to the drop at the replay gate. The repair is cvc5's shape (`splitDisequalities`, theory_arith_private.cpp:4251-4286; the lemma at constraint.cpp:1265): at a FEASIBLE complete check, evaluate each false equality at the point, and for one on its hyperplane register `e < c` and `e > c` as fresh atoms (once, ever) riding the equality's own two tableau rows at the strict relation, so no row is added and none is bounded twice -- `{eq,lt}`, `{eq,gt}`, `{lt,gt}` fire at `assert` from the form-bound crossing and `{~eq,~lt,~gt}` at the next complete check. z3 does the same split eagerly at internalization (arith_eq_adapter.cpp:208-210). The native core polls `take_new_atoms` once more after a `Sat` final check so the registration is not lost behind the verdict. An `unsat` that used a split lemma carries the ADR-1704 two-stream artifact exactly as before, graded `SatRefutationModuloTheory`; the extended formula is widened to the fresh variables, which `TheoryRefutation::from_cnf_and_lemmas` used to panic on. SHIPS ON (ADR-2147 accepted): one binary, env arms interleaved per file on idle s5/s6, 24 s / 8 GiB -- pinned QF_LRA 200: 107 -> 113, 5 STABLE-GAIN (sc-7/9/11/13/15) + 1 UNSTABLE (sc-17, 2/3 at 20 s) after 3x recheck, 0 losses; held-out QF_LRA 200 (ADR-2132's draw, disjoint): 93 -> 97, all four rechecked stable; QF_UFLRA 200: 148 -> 150, both stable; QF_IDL and QF_RDL controls unmoved; 0 flips, 0 `:status` disagreements, 0 rc-134 in any arm. `the_shipped_route_decides_a_bare_disequality` is the default-carrying test.",
    },
    ConfigEntry {
    ...
    ConfigEntry {
        name: "CERT_MAX_DEGREE",
        module: "crates/axeyum-solver/src/nra_cell_cert.rs",
        value: "64",
        unit: "polynomial degree, for every Sturm chain the CHECKER builds",
        protects: Protects::Time,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: None,
        justification: undated(
            "crates/axeyum-solver/src/nra_cell_cert.rs (CERT_MAX_DEGREE doc comment)",
        ),
        note: "Matches `nra_real_root`'s own `MAX_DEGREE` on purpose: a polynomial the PRODUCER could form must be one this checker can examine, and a checker that refuses what the producer routinely builds would turn every such verdict into a decline. Crossing it is `CellCheckFailure::ArithmeticExhausted`, which REJECTS the refutation -- the verdict is dropped, never weakened -- so the direction of a wrong value here is lost completeness and not a wrong `unsat`. UNDATED: nobody has measured the degree distribution of coverings on the corpus, and the honest reason the two constants agree is that one was copied from the other.",
    },
    ConfigEntry {
        name: "DELINEABILITY_SAMPLES",
        module: "crates/axeyum-solver/src/nra_cell_cert.rs",
        value: "3",
        unit: "probe points per cell in the delineability CROSS-CHECK (check 6b)",
        protects: Protects::Time,
        on_exceed: OnExceed::SearchEvent,
        signal: Signal::NotApplicable,
        guarded_by: "",
        env_override: None,
        justification: dated(
            "docs/research/09-decisions/adr-2126-exact-delineability-and-the-clause-loop-for-nra.md",
            "2026-09-16",
            None,
            &[sym(
            ...
            &[live(
                "delineability_exact_tests",
                "crates/axeyum-solver/src/nra_cell_cert.rs",
            )],
        ),
        note: "READ THE DATE AGAINST WHAT IT NOW GOVERNS. Before ADR-2126 this was the sample count of a SOUNDNESS-CRITICAL test: delineability was established by sampling, which can falsify it but never prove it, and this number was how much evidence an `unsat` rested on. ADR-2126 made check 6a exact and demoted the probe to an independent cross-check, so a wrong value here can now only cost a redundant disagreement between two implementations of one property -- never a verdict. That is why `protects` is `Time` and not `Soundness`. `guarded_by` is deliberately EMPTY rather than naming check 6a: that field is reserved for an unsignalled relaxation naming what stops it becoming a wrong verdict, and this is not a relaxation -- 6a is not a guard on this sample, it REPLACED it as the load-bearing test. A lane raising it is buying nothing; a lane lowering it to 0 should instead delete 6b and say so.",
    },
    ConfigEntry {
        name: "REFINE_DEPTH",
        module: "crates/axeyum-solver/src/nra_cell_cert.rs",
        value: "64",
        unit: "bisections to isolate one root, separate two, or clear a bracket",
        protects: Protects::Termination,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: None,
        justification: dated(
            "docs/research/09-decisions/adr-2131-the-clause-loop-measured-and-certified.md",
            "2026-09-16",
            None,
            &[sym(
                "crates/axeyum-solver/src/nra_cell_cert.rs",
        ...
        note: "REGISTERED BECAUSE TWO BUGS HID BEHIND IT, and that is the useful thing to know about this bound. Exceeding it is `RefinementExhausted`, a REJECTION -- so a covering that cannot be checked in 64 bisections is dropped, which is safe. But ADR-2131 found two shapes that could never terminate at ANY depth and therefore presented as this bound being too small: a root of the closing atom sitting exactly ON the cell boundary (the bracket always contains it, so the sliver count never falls), and an unbounded cell whose Cauchy bound does not clear its finite endpoint (the counting interval is empty and there is nothing on the infinite side to refine). `x > 1 ∧ x < 0` hit the first. Both are fixed by exact arguments rather than by more depth. So before raising this, check that the case actually CONVERGES -- a non-terminating shape reads exactly like an insufficient budget, and raising the number would have hidden both.",
    },
    ConfigEntry {
        name: "MAX_ENCODE_STEPS",
        module: "crates/axeyum-solver/src/nra_clause_cert.rs",
        value: "1 << 16",
        unit: "term/literal pairs the certificate's faithfulness walk verifies",
        protects: Protects::Termination,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: None,
        justification: undated(
            "crates/axeyum-solver/src/nra_clause_cert.rs (MAX_ENCODE_STEPS doc comment)",
        ),
        note: "The walk is over a term DAG with memoisation, so it is linear in the term count and this cannot be reached by any query the 48-atom cap admits. Registered anyway, and kept as a CHECKED bound rather than an argument, because `EncodingBudget` is a rejection: if it ever fires the certificate is refused and the route declines, which is the safe direction. UNDATED because no query has come near it -- the largest admissible file in ADR-2131's sizing has 12 atoms.",
    },
    ConfigEntry {
        name: "MAX_CLAUSE_ATOMS",
        module: "crates/axeyum-solver/src/nra_clause_loop.rs",
        value: "48",
        unit: "distinct polynomial comparisons the clause loop will abstract",
        protects: Protects::Time,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        ...
        note: "THIS IS NOT A LEVER ON THE CORPUS, and the measurement says so precisely. ADR-2131 parsed every `QF_NRA` file whose single-cell decline is `non-conjunctive` (118 on the pinned 200-file draw) and found `n_atoms` sharply BIMODAL: fifty-five files at 3..12 atoms, exactly ONE at 103, then sixty-two files from 125 to 63,180. The cap sits in the empty gap, so moving it anywhere from 13 to 102 changes nothing at all, and raising it to reach the upper mode would admit files needing 125+ atoms -- an exponential number of Boolean models per theory call. A lane briefed to 'raise the cap' would measure zero and have no way to tell that from a cap that was already right. The theory call per Boolean model is the expensive part and the model count is exponential in the atom count, so this bounds the whole loop rather than any one step.",
    },
    ConfigEntry {
        name: "MAX_CLAUSE_LOOP_ROUNDS",
        module: "crates/axeyum-solver/src/nra_clause_loop.rs",
        value: "256",
        unit: "theory calls (Boolean models) per clause-loop decision",
        protects: Protects::Time,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: None,
        justification: undated(
            "crates/axeyum-solver/src/nra_clause_loop.rs (MAX_CLAUSE_LOOP_ROUNDS doc comment)",
        ),
        note: "What bounds the SEARCH, as opposed to the abstraction. The blocking clauses the loop learns are the atoms a covering CITED, which is a real unsat core and usually much shorter than the assignment -- but nothing guarantees that, so the round cap is the bound rather than an argument about how fast it converges. Crossing it is `ClauseLoopBudget`, a decline, so the cost is a forgone decision. UNDATED: ADR-2131 measured the ceiling of files this loop can ADMIT (16 on the pinned draw) but not the round distribution on them.",
    },
    ConfigEntry {
        name: "FBBT_DEFAULT",
    ...
    ConfigEntry {
        ...
        justification: dated(
            ...
            "2026-09-16",
            None,
            // RE-DATED by ADR-2126, and the re-dating is a re-measurement and not
            // a refresh: that ADR ran the SAME interleaved A/B protocol on the
            // same 200-file QF_NRA list with this entry's value as arm A, and arm
            // A scored 121 -- the number ADR-2121 shipped this default on. A
            // basis that still reproduces is the only thing that makes a date
            // move mean anything.
            //
            // Three measurements now. ADR-2110: "on the 83 QF_NRA files the
            ...
            &[
                ...
                doc("bench-results/nra-algebraic-witness-20260916/README.md"),
            ],
        ),
        note: "Whether the N-variable CAD gets the shipped cell budget or 16x it. DEFAULT is the shipped arm and is byte-identical to the pre-ADR-2110 engine (`CadPolicy::DEFAULT.cell_cap` IS `MAX_CAD_CELLS`), so the A/B is one binary and one env var. Raising the cap can only let the decomposition VISIT more cells before declining, and a definite verdict is returned only after COMPLETE coverage of the arrangement -- so `wide` can turn an `unknown` into a verdict and can never flip one; `the_wide_arm_only_raises_the_cap` holds that the two arms differ in exactly the cap and nothing else. It ships OFF because the cost was unmeasured when it landed, not because the verdict was in doubt. The lever exists because the ADR-2110 census could not otherwise tell a cell-cap decline from a projection decline: measured 2026-09-15, the exact decider's OWN recorded cause on the largest bucket is `non-conjunctive` and `projection`, NOT `cell-budget`, which is what this lever was built to test and is why the A/B is expected to move little. Read the A/B in `bench-results/nra-trace-20260915/README.md` before raising the default. ADR-2121 adds a THIRD arm, `single-cell`, which leaves the cap at `MAX_CAD_CELLS` and instead offers the query to `nra_single_cell::decide_single_cell` -- the model-constructing CDCAC route that builds one cell per conflict instead of enumerating the arrangement -- before the enumerative decomposition. Its `cell_cap` is IDENTICAL to `default`'s, so an A/B between `default` and `single-cell` isolates the ROUTE and not the budget, and the two levers do not interact. It also ships OFF, and its `unsat` is emitted only after `nra_cell_cert::check_cell_refutation` accepts the covering, so a bug in it declines rather than answers. Read `bench-results/nra-single-cell-20260915/README.md` for that arm's A/B. ADR-2121 then MOVED THIS DEFAULT, from `CadPolicy::DEFAULT` to `CadPolicy::SINGLE_CELL_SAT`, which is the first time this entry's shipped value has changed. The measured basis: QF_NRA 117 -> 121 (+4), 4 STABLE-GAIN and 0 STABLE-LOSS on a three-pass recheck, 0 sat<->unsat flips across QF_NRA + QF_NIA + a QF_LRA control that moved 0 rows, and 0 disagreements against declared `:status` over 593 comparable verdicts. The decisive fact is not the +4 but WHICH verdicts it is: all four are `sat`, a rational model replayed through the ground evaluator against the original assertions, which is exact. The route's `unsat` half was withheld on this arm as `CadDecline::UnsatWithheldByArm` (ADR-2121 spelled that cause `UnsatWithheldSampledDelineability`, after the reason it withheld) and reaches no default. The cell cap is unchanged, so this is a route change and not a budget change, and `AXEYUM_NRA_CAD=default` still selects the pre-ADR-2121 engine through an EXPLICIT arm in `parse_cad_arm` rather than the catch-all. ADR-2126 then changed WHAT THE WITHHELD HALF IS WORTH WITHHOLDING, without touching any arm: `nra_cell_cert`'s delineability test is no longer sampling. Check 6a proves delineability over the whole cell exactly -- leading coefficients, discriminants and pairwise resultants required to be root-free on the cell by Sturm counting against its ALGEBRAIC endpoints -- the old probe survives as 6b, an independent cross-check that can now only fire on a disagreement between two implementations of one property, and 6c names the argument's scope boundary and REJECTS a generalisation resting on another generalisation (`NestedOpenGeneralization`), which the sampling check silently assumed. So `single-cell`'s `unsat` no longer rests on a finite sample, and the A/B between `single-cell-sat` and `single-cell` prices that half on its own. ADR-2126 also adds a FIFTH arm, `clause-loop`, which differs from `single-cell-sat` in exactly one field (`clause_loop`) and offers the query to `nra_clause_loop::decide_clause_loop` -- CDCL(T) over the sign atoms -- only where the conjunctive route just recorded `non-conjunctive`, which is 12 of ADR-2121's 24 in-bounds files. Its `sat` is a model replayed against the ORIGINAL assertions, so nothing in the Boolean layer is load-bearing for it; its `unsat` is withheld as `ClauseLoopUnsatUncertified` because it would rest on the Tseitin encoding, the blocking clauses and the SAT core's refutation, none of which a checker in this tree can yet read. Read `bench-results/nra-cell-exact-20260916/README.md`. ADR-2134 adds a SIXTH arm, `algebraic-witness`, which differs from `single-cell` in exactly one field (`algebraic_witness`) and lets the route accept a cell whose representative point is an IRRATIONAL root as the FINAL coordinate of a model -- the cause `CadDecline::AlgebraicWitness`, measured at 7 of the pinned 200 QF_NRA files on the shipped default with 6 of them `unknown`, which is that arm's whole ceiling. It is accepted ONLY at the last level, where the sample is complete and no deeper level has to substitute the coordinate into a polynomial, and only when the exact replay through the ground evaluator accepts every ORIGINAL assertion at that point -- the same gate every other `sat` from this route passes, with the coordinate evaluated by exact algebraic field arithmetic (ADR-0038) rather than rational arithmetic. It adds no `unsat`, so it cannot flip a verdict; the acceptance sits on the branch that otherwise returned `None`, so it is strictly additive by construction. A replay that does not resolve is `AlgebraicReplayUndecided` and one that resolves FALSE is `AlgebraicReplayRefuted` -- named apart because the second is a producer disagreement and not a budget. The model value prints as `(root-obj p k)` and NEVER as a rounded rational: a rounded coordinate names a point that does not satisfy the query. Note the `value` field above still reads `CadPolicy::SINGLE_CELL_SAT` while the code has said `CadPolicy::SINGLE_CELL` since ADR-2126; that staleness predates this arm and is recorded here rather than silently corrected. Read `bench-results/nra-algebraic-witness-20260916/README.md`.",
    },
    ConfigEntry {
    ...
    ConfigEntry {
        name: "MAX_CELL_DEGREE",
        module: "crates/axeyum-solver/src/nra_single_cell.rs",
        value: "8",
        unit: "total degree of any monomial in an admitted atom",
        protects: Protects::Time,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        ...
        note: "Half of the route's DECLARED SLICE, refused up front rather than discovered halfway through a decomposition -- which is what makes `SliceBounds` an attributable cause instead of a timeout. Re-measured by ADR-2131's sizing: `slice-bounds` is 22 of the 76 undecided files on the pinned QF_NRA draw, the second-largest bucket after `non-conjunctive`, so this pair IS a lever, unlike `MAX_CLAUSE_ATOMS`. What the sizing does NOT say is how the 22 split between the degree bound and the variable bound; a lane raising either should split that first, because ADR-2110's whole lesson is that a bundled cause is an upper bound on each of its members and a measurement of none.",
    },
    ConfigEntry {
        name: "MAX_CELL_VARS",
        module: "crates/axeyum-solver/src/nra_single_cell.rs",
        value: "4",
        unit: "distinct variables in an admitted atom set",
        protects: Protects::Time,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        ...
        note: "The other half of the declared slice; see `MAX_CELL_DEGREE` for the shared measurement and for why the 22-file `slice-bounds` bucket has not been split between them. This bound is the one with a superlinear cost behind it: the decomposition recurses one level per variable and each level's arrangement is built over the projections of the level above, so raising it is not the same size of change as raising the degree.",
    },
    ConfigEntry {
        name: "MAX_REFINE_ROUNDS",
        module: "crates/axeyum-solver/src/nra_single_cell.rs",
        value: "24",
        unit: "bisections when isolating or ordering a critical value",
        protects: Protects::Termination,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: None,
        justification: undated(
            "crates/axeyum-solver/src/nra_single_cell.rs (MAX_REFINE_ROUNDS doc comment)",
        ),
        note: "The PRODUCER's refinement budget, and it is a different number from the CHECKER's `nra_cell_cert.rs::REFINE_DEPTH` (64) on purpose -- the producer may give up early and decline, while the checker giving up rejects a verdict the producer already built, so the checker is the side that should try harder. Crossing it is `RootOrdering` or `RootIsolation`, both declines, both attributable. Read `REFINE_DEPTH`'s note before raising this: ADR-2131 found two shapes that never converge at any depth and present exactly as an insufficient budget.",
    },
    ConfigEntry {
        name: "MAX_TOTAL_CELLS",
        module: "crates/axeyum-solver/src/nra_single_cell.rs",
        value: "4096",
        unit: "cells visited across one single-cell decision",
        protects: Protects::Time,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: None,
        justification: undated(
            "crates/axeyum-solver/src/nra_single_cell.rs (MAX_TOTAL_CELLS doc comment)",
        ),
        note: "Bounds the whole decision rather than any one level, which is what a per-level cap would fail to do: the arrangement is a product across levels, so a per-level bound that looks generous multiplies. Distinct from `nra_real_root.rs::MAX_CAD_CELLS`, which bounds the ENUMERATIVE decomposition, and the two are not linked -- a lane moving one should say why the other stays. Crossing it is `CellBudget`, a decline. UNDATED: ADR-2131's sizing shows `cell-budget` at ZERO of the 76 undecided files on the pinned QF_NRA draw, so nothing on that corpus reaches it and no measurement here would have a population.",
    },
    ConfigEntry {
    ...
    ConfigEntry {
        name: "GENERATION_LADDER_BUDGET_DIVISOR",
        module: "crates/axeyum-solver/src/qinst_egraph.rs",
        value: "4",
        unit: "divisor of the remaining deadline",
        protects: Protects::Time,
        on_exceed: OnExceed::Truncate,
        signal: Signal::None,
        guarded_by: "the ladder's whole spend is `remaining / 4`, split equally across the layers still to run, and exhausting it breaks out to the unchanged full check rather than returning a verdict",
        env_override: None,
        justification: dated(
            "docs/research/09-decisions/adr-2133-generation-bounded-instance-selection.md",
            "2026-09-16",
            ...
            &[sym(
                "crates/axeyum-solver/src/qinst_egraph.rs",
                "GENERATION_LADDER_BUDGET_DIVISOR",
            )],
            &[doc(
                "docs/research/09-decisions/adr-2133-generation-bounded-instance-selection.md",
            )],
        ),
        note: "Deliberately the same value as MID_LOOP_CHECK_BUDGET_DIVISOR, which is what the shipped single-layer pre-check already spends, so turning the lever ON costs no more wall clock before the full check than the shipped path already could. The per-layer share is the ladder's remaining budget divided by the layers still to run, so one expensive shallow layer cannot consume the ladder.",
    },
    ConfigEntry {
        name: "GENERATION_LADDER_LEVEL",
        module: "crates/axeyum-solver/src/qinst_egraph.rs",
        value: "0",
        unit: "ladder level (0 = off)",
        protects: Protects::Completeness,
        on_exceed: OnExceed::Truncate,
        signal: Signal::None,
        guarded_by: "the ladder only ever tries SUBSETS of the conjunction the unchanged full check takes, and every member of that conjunction is an original assertion or an admitted instance of an asserted universal -- so a layer's `unsat` refutes the whole set, a layer's non-`unsat` is discarded rather than believed, and no layer can stop the full check from running (`generation_ladder_check` returns `Option`, and `None` means only \"no layer refuted\")",
        env_override: Some("AXEYUM_QINST_GEN_LADDER"),
        justification: dated(
            "docs/research/09-decisions/adr-2133-generation-bounded-instance-selection.md",
            "2026-09-16",
            None,
    ...
    ConfigEntry {
        ...
        on_exceed: OnExceed::Truncate,
        signal: Signal::None,
        guarded_by: "capping the number of layers can only leave a refutation for the unchanged full check to find; it never admits a layer whose `unsat` would not have refuted the whole set",
        env_override: None,
        justification: dated(
            "docs/research/09-decisions/adr-2133-generation-bounded-instance-selection.md",
            "2026-09-16",
            ...
            &[sym(
                "crates/axeyum-solver/src/qinst_egraph.rs",
                "GENERATION_LADDER_MAX_GENERATION",
            )],
            &[doc(
                "docs/research/09-decisions/adr-2133-generation-bounded-instance-selection.md",
            )],
    ...
    ConfigEntry {
        ...
        module: "crates/axeyum-solver/src/qinst_egraph.rs",
        value: "0",
        unit: "0 = the session refuses any ground set with a Boolean-position term its EUF encoder has no arm for; 1 = it ABSTRACTS such a term to a free propositional variable; 2 = it HOSTS the arithmetic ones in a LIA sub-theory and abstracts the rest",
        protects: Protects::Time,
        on_exceed: OnExceed::Truncate,
        signal: Signal::None,
        guarded_by: "the abstraction is a WEAKENING, so it cannot manufacture a refutation: replacing an atom by a free propositional variable only ADDS models (every model of the original extends to one of the skeleton by giving the variable the atom's truth value), `unsat` of the skeleton therefore transfers back and `sat` says nothing. SECOND AND INDEPENDENT: the session's `Unsat` is never the verdict -- `scoped_candidate_fixpoint_step` reaches `Refuted` only through `replay_online_refutation`, which re-establishes the refutation over the same ground set with the ordinary cold quantifier-free route. `quant_ground_session_soundness.rs` tries to REFUTE the rule over SATISFIABLE UFLIA queries at levels 0 and 1, including two comparisons a collapsing abstraction would merge, and requires the two levels never to disagree on a DECIDED verdict. LEVEL 2 IS GUARDED DIFFERENTLY BECAUSE IT IS A DIFFERENT KIND OF CLAIM (ADR-2130): a hosted atom is not a weakening, it is a real constraint, so the abstraction argument does not cover it. What covers it is (a) the arithmetic sub-theory is `LiaTheory`, whose conflict core is a Farkas support over asserted literals and whose `assert`/`push`/`pop` are the same ones the shipped QF_LIA online route drives; (b) the combination propagates NO interface equalities, which is an incompleteness and can only LOSE conflicts; (c) `new_with_opaque_apps` treats an Int-sorted UF application as its own variable, an over-approximation of the model space that likewise can only lose conflicts; and (d) the session's `Unsat` is still never the verdict. The retraction of an arithmetic bound on backjump has no bound trail to get wrong -- `IntSimplexEngine::sync` re-derives the imposed bounds from the live assignment set on every check, so `LiaTheory::pop` unassigning the atom IS the retraction, and `pop_retracts_an_arithmetic_bound_a_backjump_undid` fails if that forwarding is removed",
        ...
        justification: dated(
            ...
            &[
                ...
                sym(
                    "crates/axeyum-solver/src/qinst_egraph.rs",
                    "ground_session_hosts_arithmetic",
                ),
                sym(
                    "crates/axeyum-solver/src/qinst_session_theory.rs",
                    "session_lia_hostable_atom",
                ),
            ],
            &[
                doc(
                ...
                doc(
                    "docs/research/09-decisions/adr-2130-the-quantifier-session-hosts-arithmetic.md",
                ),
                live(
                    "EufLiaSessionTheory",
                    "crates/axeyum-solver/src/qinst_session_theory.rs",
    ...
    ConfigEntry {
        ...
        env_override: Some("AXEYUM_QINST_ROUNDS"),
        justification: undated("doc comment"),
        note: "Hard ceiling on `for round in 0..instantiation_round_ceiling()`. The wall-clock deadline and MAX_GROUND_TERMS are the practical bounds in ordinary operation; this is the backstop when neither is configured. WIRED AS A LEVER 2026-09-13 (`AXEYUM_QINST_ROUNDS`) so the question ADR-1950 opened -- is the 177-file `round budget` family reachable by raising this? -- can be asked without a patch. READ THE GIVE-UP DETAIL FIRST: three different loop exits used to share one string naming only this ceiling, and `InstantiationLoopExit` now splits them, so a row that ends `reached fixpoint` or `could not fit another round` is NOT bound by this value.",
    },
    ConfigEntry {
    ...
    ConfigEntry {
        ...
        signal: Signal::NotApplicable,
        guarded_by: "",
        env_override: Some("AXEYUM_QINST_CADENCE"),
        justification: dated(
            "doc comment",
            "2026-09-16",
            None,
            &[
                sym(
                    "crates/axeyum-solver/src/qinst_egraph.rs",
                    "ground_check_split_round",
                ),
                sym(
                    "crates/axeyum-solver/src/qinst_egraph.rs",
                    "interleaved_check_due",
                ),
            ],
            &[
                doc(
        ...
        note: "Former hard round budget, now purely the cadence anchor for the interleaved ground check. THE CADENCE SELECTS BETWEEN TWO SCHEDULES, AND WHAT THAT SELECTION DOES WAS STATED WRONGLY HERE, IN THE SOURCE, AND IN ADR-2124's FIRST DRAFT, ALL AT ONCE. The two are now named (`GroundCheckSchedule`, ADR-2130): `PerRoundThenExponential` fires on every round of the window and then on power-of-two rounds (0,1,2,3,4,5,6, then 7,15,31,...); `ExponentialOnly` fires on the power-of-two rounds alone (7,15,31,...). `online_clauses.is_none()` picks between them. They differ on EXACTLY the rounds below `ground_check_split_round()` = `instantiation_cadence() - 1` = 7 at this value, and AGREE from there on -- so a live session does NOT suppress the check, it skips the first seven per-round re-solves. Measured 2026-09-16 (ADR-2124): 38 of 53 cores ran an IDENTICAL number of cold checks in both arms, which suppression could not produce. Both arms of `GroundCheckSchedule::due` are written in terms of the split so the seven-round gap is structural rather than two conditions that happen to line up, and `the_two_ground_check_schedules_differ_by_exactly_the_leading_rounds` DERIVES the differing set by running both schedules rather than asserting a literal list -- a literal would have measured the maintainer's memory of this constant, which is what was wrong. The previous version of this note cited `qinst_egraph.rs:1677-1682` for `interleaved_check_due`, which had not been true for some time; the justification now names the symbol instead of a line.",
    },
    ConfigEntry {
    ...
    ConfigEntry {
        ...
        env_override: Some("AXEYUM_QINST_ROUND_HEADROOM"),
        justification: undated("doc comment"),
    ...
    ConfigEntry {
        name: "MAX_ONLINE_TABLEAU_NONZEROS",
        module: "crates/axeyum-solver/src/simplex.rs",
        value: "400_000",
        unit: "stored nonzeros at construction (~40 B each, so 16 MiB)",
        protects: Protects::Memory,
        on_exceed: OnExceed::DeclineRoute,
        signal: Signal::ToCaller,
        guarded_by: "",
        ...
        note: "Pinned by: lra_online::tests::the_nonzero_admission_builds_the_tableau_the_dense_cap_refuses (the value is asserted against the fixture's nonzero count in a const block). ADR-2146: the ONLINE engine's door in the currency the tableau is stored in, consulted only under `AXEYUM_LRA_ADMIT_NONZEROS`. Deliberately the same figure as `MAX_WARM_CUBE_NONZEROS` (ADR-2125's door for the offline cube decider): two doors onto one structure should agree about what it costs, and both are ADR-2111's `TableauReserve::Sparse` figure -- 11.6x ADR-2055's measured median of 34,555 nonzeros on the route and 2.7x its extreme. A separate constant because the two call sites are separate levers with separate A/Bs. What it does NOT bound is fill-in, which `MAX_TABLEAU_FILL_NONZEROS` does at run time.",
    },
    ConfigEntry {
        name: "MAX_ONLINE_TABLEAU_ROWS",
        module: "crates/axeyum-solver/src/simplex.rs",
        value: "65_536",
        unit: "tableau rows",
        protects: Protects::Time,
        ...
        justification: dated(
            ...
            &[sym(
                "crates/axeyum-solver/src/simplex.rs",
                "MAX_ONLINE_TABLEAU_ROWS",
            )],
            &[adr("ADR-2146")],
        ),
        note: "Pinned by: lra_online::tests::the_online_admission_refuses_on_rows_alone (one row over is refused, at the ceiling is admitted, with the fixture under the nonzero cap so the row cap is what refuses). ADR-2146. The dense-cell cap bounded rows implicitly -- `m x (nvars+m) <= 4,000,000` caps `m` at 2,000 even with no problem variables -- and a nonzero count does not: a row costs its nonzeros in storage but a whole slot in every per-row scan (`Tableau::run`'s leaving-variable scan, `SimplexEngine::row_bounded`, the bound vectors). 65,536 is 32x the dense cap's implicit maximum and about 2x the largest row count the census population would reach (LassoRanker `queens`, 15,647 atoms, at most two rows each). It refuses the absurd, not the measured; no file in the pinned 200 crosses it.",
    },
    ConfigEntry {
        name: "MAX_PIVOTS",
        module: "crates/axeyum-solver/src/simplex.rs",
    ...
    ConfigEntry {
        name: "MAX_TABLEAU_FILL_NONZEROS",
        module: "crates/axeyum-solver/src/simplex.rs",
        value: "4_000_000",
        unit: "live stored nonzeros during pivoting (~48 B each with the column index, so ~190 MiB)",
        protects: Protects::Memory,
        on_exceed: OnExceed::RefuseUnknown,
        signal: Signal::ToCaller,
        guarded_by: "",
        ...
        justification: dated(
            ...
            &[adr("ADR-2146"), adr("ADR-2132")],
        ),
        note: "Pinned by: simplex::tests::the_fill_cap_declines_before_the_pivot_that_would_cross_it -- the same system under `fill_cap = None` is decided and under a cap below its fill-in answers `Unknown` with `fill_cap_declines = 1`, and the live count is checked against a full recount after every pivot. ADR-2146: `MAX_TABLEAU_CELLS` restated in the currency the tableau is stored in, and the half that makes a nonzero admission a MEMORY bound rather than a hope. Under the dense admission `nnz <= cells <= 4,000,000` held by construction, so fill-in could never exceed what admission had priced; under a nonzero admission it can -- ADR-2132 measured sc-39 entering at 4,688 nonzeros and peaking at 98,572, 21x -- so the bound moves to where the growth happens: `Tableau::run` refuses BEFORE a pivot whose worst-case fill-in (`col_nnz[entering] x nnz(pivot row)`) would carry the live count past the cap, and answers `Unknown`, which every caller already treats as a sound don't-know. Checked only on a tableau built by `Incremental::with_online_admission` (`Tableau::fill_cap` is `None` everywhere else), so the shipped OFF arm runs the identical pivot loop. The harness's `ulimit -v 8G` is invisible to the process (ADR-2045), so this and not the watchdog is what stands between a nonzero-admitted tableau and an rc=134 abort.",
    },
    ConfigEntry {
    ...
    ConfigEntry {
        ...
        justification: dated(
            "bench-results/ufbv-caps-ab-20260912/README.md",
            "2026-09-13",
            None,
            &[sym(
                "crates/axeyum-solver/src/ufbv_online.rs",
            ...
            &[doc("bench-results/ufbv-caps-ab-20260912/README.md")],
        ),
        note: "Admits `abv-online-cdclt`, the FIRST route every array query tries. ADR-1945 A/B'd it and the value is HELD, now for a measured reason rather than a doc comment: raising it to 2x, 4x and 16x decides **+0** of the 200 pinned QF_UFBV files at every value (the atom cap is the next gate and catches every file this admits), and costs 2 of 800 already-decided control files -- two QF_ABVFP queries go from a 0.4-0.6 s `array-fast-path` verdict to a 25 s Watchdog, deterministically, in all four node-raising arms. This cap is currently the guard in front of `abv-online-cdclt`'s measured non-progress defect (0 CEGAR rounds at a 600 s budget).",
    },
    ConfigEntry {
    ...
    ConfigEntry {
        name: "MAX_SCALAR_THEORY_ATOMS",
        module: "crates/axeyum-solver/src/ufbv_online.rs",
        value: "4_096",
        unit: "semantic atoms on the SCALAR path (admit_arrays == false)",
        protects: Protects::Memory,
        on_exceed: OnExceed::RefuseUnknown,
        signal: Signal::ToCaller,
        guarded_by: "",
        env_override: Some("AXEYUM_UFBV_MAX_SCALAR_THEORY_ATOMS"),
        justification: dated(
            "bench-results/ufbv-caps-ab-20260912/README.md",
            "2026-09-13",
            None,
        ...
        note: "ADR-1945. 1_800 runs over the pinned 200-file QF_UFBV division at nine cap settings: 1_024 -> 2_048 is +31 files, -> 4_096 is +44, -> 8_192 is +45 for 11 % more wall, so 4_096 is where the curve flattens. 0 losses on QF_UFBV at any value. Scalar-only because the array path's twin `MAX_THEORY_ATOMS` costs `abv-online-cdclt` time it does not recover; `check_qf_ufbv_online_cdclt` is the only caller that reaches this value.",
    },
    ConfigEntry {
        name: "MAX_THEORY_ATOMS",
        module: "crates/axeyum-solver/src/ufbv_online.rs",
        value: "1_024",
        unit: "semantic atoms on the ARRAY path (admit_arrays == true)",
        protects: Protects::Memory,
        on_exceed: OnExceed::RefuseUnknown,
        signal: Signal::ToCaller,
        guarded_by: "",
        ...
        note: "Since ADR-1945 this is the ARRAY path's cap only; the scalar path reads `MAX_SCALAR_THEORY_ATOMS` through `theory_atom_cap(admit_arrays)`. Held at 1_024 because raising it where arrays are admitted costs `abv-online-cdclt` real time: `QF_ABV/dwp_formulas/try3_sameret_functions_dwp_pr.hold_file.il.dwp.smt2` goes from a 4.2 s `array-fast-path` `sat` to 22.2 s at 4_096 on an IDLE box, and to a 24 s loss on a shared one. Two sites read the cap: the static pre-check and the dynamic retained-search check, which takes the value the static check admitted under rather than re-reading the lever.",
    },
    ConfigEntry {
