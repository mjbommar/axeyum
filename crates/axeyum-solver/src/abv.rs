...
fn scalar_repair_support_gain(
    ...
    let target_support = scalar_select_support_score(arena, originals, projected, target_symbol)?;
    ...
    Ok(source_support.saturating_sub(target_support))
}
        ...
        for &assertion in originals {
            ...
            for &conjunct in &conjuncts {
                ...
                for (symbol, value_term) in choices {
                    ...
                    let support_gain = scalar_repair_support_gain(
                        arena, originals, projected, symbol, value_term,
                    )?;
                    if support_gain > 0 {
                        stats.scalar_support_candidates += 1;
                    }
                    let mut false_count = positive_replay_false_count(arena, originals, &trial)?;
                    stabilize_supported_scalar_trial(
                        arena,
                        originals,
                        current_false,
                        support_gain,
...
fn repair_projected_returned_or_array_store_literal(
    arena: &TermArena,
    originals: &[TermId],
    projected: &Assignment,
) -> Result<Option<(usize, ProjectionRepairStats, Assignment)>, SolverError> {
    let ir = |e: axeyum_ir::IrError| {
        SolverError::Backend(format!(
            "lazy-ext scalar returned-OR stabilization failed: {e}"
        ))
    };
    let current_false = positive_replay_false_count(arena, originals, projected)?;
    let Some(failure) = first_projected_replay_failure(
    ...
    let mut trial = projected.clone();
    let mut stats = ProjectionRepairStats::default();
    let repaired = repair_projected_branch_literal_in_branch(
        arena,
        originals,
        or_failure.best_branch_term,
        false_literal,
    ...
    if !repaired || eval(arena, failure.conjunct_term, &trial).map_err(ir)? != Value::Bool(true) {
        return Ok(None);
    }
    let total_false = positive_replay_false_count(arena, originals, &trial)?;
    if total_false <= current_false {
        Ok(Some((total_false, stats, trial)))
    } else {
        Ok(None)
    }
}

fn stabilize_scalar_trial_after_returned_or_array_store(
    arena: &TermArena,
    originals: &[TermId],
    trial: &mut Assignment,
    stats: &mut ProjectionRepairStats,
    total_false: &mut usize,
) -> Result<bool, SolverError> {
    let Some((stabilized_false, stabilized_stats, stabilized_trial)) =
        repair_projected_returned_or_array_store_literal(arena, originals, trial)?
    else {
        return Ok(false);
    };
    if stabilized_false >= *total_false {
        return Ok(false);
    }
    *trial = stabilized_trial;
    stats.absorb(stabilized_stats);
    *total_false = stabilized_false;
    Ok(true)
}

fn stabilize_supported_scalar_trial(
    arena: &TermArena,
    originals: &[TermId],
    current_false: usize,
    support_gain: usize,
    trial: &mut Assignment,
    trial_stats: &mut ProjectionRepairStats,
    total_false: &mut usize,
) -> Result<(), SolverError> {
    if support_gain == 0 {
        return Ok(());
    ...
    if *total_false >= current_false {
        trial_stats.scalar_stabilized_trials += 1;
        let stabilize_stats = stabilize_projected_after_scalar_trial(arena, originals, trial)?;
        trial_stats.absorb(stabilize_stats);
        *total_false = positive_replay_false_count(arena, originals, trial)?;
    } else {
        const MAX_RETURNED_OR_STABILIZATION_CONJUNCTS: usize = 64;
        if positive_replay_conjunct_count(arena, originals)
            > MAX_RETURNED_OR_STABILIZATION_CONJUNCTS
        {
            return Ok(());
        }
        trial_stats.scalar_stabilized_trials += 1;
        let _ = stabilize_scalar_trial_after_returned_or_array_store(
            arena,
            originals,
            trial,
            trial_stats,
...
fn write_scalar_followup_or_diagnostic(
    ...
    let _ = write!(
        ...
        followup.status,
        followup.repair_changes,
        followup.closure_steps.len()
    );
    if let Some(kind) = &followup.repair_kind {
        let _ = write!(note, ",followup_kind={kind}");
    }
    if !followup.closure_steps.is_empty() {
        let _ = write!(note, ",followup_closure_step_details=");
        write_scalar_choice_effect_list(note, &followup.closure_steps);
    }
    if let Some(final_branch_false) = followup.final_branch_false_literals {
        let _ = write!(note, ",followup_final_branch_false={final_branch_false}");
    }
    if let Some(total_false) = followup.total_false_conjuncts {
        let _ = write!(note, ",followup_total_false={total_false}");
    }
    if let Some(ordinal) = followup.first_global_false_ordinal {
        let _ = write!(note, ",followup_global_false_ordinal={ordinal}");
    }
    if let Some(term) = followup.first_global_false_term {
        let _ = write!(note, ",followup_global_false_term={}", term.index());
    }
    if let Some(eq) = &followup.first_global_false_eq {
        let _ = write!(
            note,
            ",followup_global_false_lhs_term={},followup_global_false_rhs_term={},\
             followup_global_false_lhs_value={},followup_global_false_rhs_value={}",
            eq.lhs_term.index(),
            eq.rhs_term.index(),
            eq.lhs_value,
            eq.rhs_value
    ...
    if let Some(closure_branch_false) = followup.closure_branch_false_literals {
        let _ = write!(
            note,
            ",followup_closure_branch_false={closure_branch_false}"
        );
    }
    if let Some(closure_total_false) = followup.closure_total_false_conjuncts {
        let _ = write!(note, ",followup_closure_total_false={closure_total_false}");
    }
    if let Some(ordinal) = followup.closure_global_false_ordinal {
        let _ = write!(note, ",followup_closure_global_false_ordinal={ordinal}");
    }
    if let Some(term) = followup.closure_global_false_term {
        let _ = write!(note, ",followup_closure_global_false_term={}", term.index());
    }
    if let Some(eq) = &followup.closure_global_false_eq {
        let _ = write!(
            note,
            ",followup_closure_global_false_lhs_term={},\
             followup_closure_global_false_rhs_term={},\
             followup_closure_global_false_lhs_value={},\
             followup_closure_global_false_rhs_value={}",
            eq.lhs_term.index(),
            eq.rhs_term.index(),
            eq.lhs_value,
            eq.rhs_value
...
fn scalar_followup_or_status(total_false: usize, scalar_total_false: usize) -> &'static str {
    if total_false == 0 {
        "closes"
    } else if total_false < scalar_total_false {
        "improves"
    } else if total_false == scalar_total_false {
        "same"
    } else {
...
fn replay_scalar_followup_or_diagnostic(
    arena: &TermArena,
    originals: &[TermId],
    scalar_total_false: usize,
    global_failure: Option<&ReplayFailure>,
    scalar_trial: &Assignment,
) -> Result<Option<ReplayScalarFollowupOrDiagnostic>, SolverError> {
    let Some(global_failure) = global_failure else {
        return Ok(None);
    };
    let Some(or_failure) = &global_failure.failed_or else {
    ...
    let mut followup_trial = scalar_trial.clone();
    let Some((repair_kind, repair_stats)) = repair_projected_branch_best_candidate(
        arena,
        originals,
        or_failure.best_branch_term,
        ...
        return Ok(Some(ReplayScalarFollowupOrDiagnostic {
            or_ordinal: global_failure.conjunct_ordinal,
            or_term: global_failure.conjunct_term,
            branch_ordinal: or_failure.best_branch_ordinal,
            branch_term: or_failure.best_branch_term,
    ...
    let total_false = positive_replay_false_count(arena, originals, &followup_trial)?;
    let final_branch_false =
        branch_false_literal_count(arena, or_failure.best_branch_term, &followup_trial)?;
    let followup_failure = first_projected_replay_failure(
        arena,
        originals,
        &followup_trial,
        ProjectionRepairStats::default(),
    ...
    let status = if let Some((closure_total_false, closure_branch_false, closure_failure)) =
        &closure_summary
    {
        if *closure_total_false >= scalar_total_false
            && *closure_branch_false > 0
            && closure_failure
                .as_ref()
                .is_some_and(|failure| failure.conjunct_term == global_failure.conjunct_term)
        {
            "guarded_loop"
        } else {
    ...
    Ok(Some(ReplayScalarFollowupOrDiagnostic {
        ...
        final_branch_false_literals: Some(final_branch_false),
        total_false_conjuncts: Some(total_false),
        first_global_false_ordinal: followup_failure
            .as_ref()
            .map(|failure| failure.conjunct_ordinal),
        first_global_false_term: followup_failure
            .as_ref()
            .map(|failure| failure.conjunct_term),
...
fn replay_scalar_candidate_diagnostics(
    ...
    for (target_symbol, value_term) in choices.into_iter().take(MAX_SCALAR_CANDIDATE_DIAGNOSTICS) {
        ...
        let support_gain =
            scalar_repair_support_gain(arena, originals, projected, target_symbol, value_term)?;
        let literal_true =
        ...
        let followup_or = replay_scalar_followup_or_diagnostic(
            arena,
            originals,
            total_false,
...
fn repair_projected_replay_scalar_failure(
    ...
    for (ordinal, (symbol, value_term)) in choices.into_iter().enumerate() {
        ...
        let support_gain =
            scalar_repair_support_gain(arena, originals, projected, symbol, value_term)?;
        if support_gain > 0 {
            stats.scalar_support_candidates += 1;
        }
        let mut total_false = positive_replay_false_count(arena, originals, &trial)?;
