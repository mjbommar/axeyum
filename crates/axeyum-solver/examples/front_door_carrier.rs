...
//! actually runs. Until 2026-08-18 the answer was "the `Real` package", whose 30
...
//! `CReal`" is worth nothing without "non-zero over `Real`" beside it. A run in
//! which the `Real` control came back empty would mean the measurement is broken,
//! not that `Real` became free — so the exit status checks for that too.
//!
//! # Usage
//!
//! ```text
...
//! footprint, the `Real` control is non-empty on every fixture, and the shipped
...
/// small: a `Real` module states the carrier laws it uses as axioms and stops,
...
fn run(require_axiom_free: bool) -> Result<(), String> {
    ...
    for &(label, _, build) in &fixtures {
        ...
        // `CReal.` also matches a `contains("Real.")` test, so the carrier is
        // decided by the carrier DECLARATION, which is unambiguous.
        let constructed = source.contains("inductive CReal") || source.contains("axiom CReal");
        let axiomatized = source.contains("axiom Real : Sort");
        println!(
    ...
    for &(label, route, build) in &fixtures {
        ...
        // (a) the control: the axiomatized `Real` package.
        let mut real_ctx = LraReconstructCtx::try_new()
            .map_err(|e| format!("{label}: the Real package did not build: {e:?}"))?;
        let real_proof = reconstruct(&mut real_ctx, route, &arena, &assertions)
            .map_err(|e| format!("{label}: Real reconstruction failed: {e:?}"))?;
        let real_footprint = refutation_axiom_footprint(&mut real_ctx, real_proof)
            .map_err(|e| format!("{label}: Real footprint failed: {e:?}"))?;
        ...
        println!("  --- {label}");
        println!(
            "    over Real  : footprint {} of which {} are CARRIER axioms; module {real_bytes} \
    ...
    println!("the Real control is non-vacuous: {control_is_live}");
        ...
        && !(all_free && control_is_live && front_door_is_constructed && module_matches_kernel)
    {
        return Err(
            "--require-axiom-free was given but the shipped front door does not \
                    reconstruct over an axiom-free carrier, the Real control came back \
                    empty (which would mean the measurement is broken, not that Real is \
