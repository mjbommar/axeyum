...
fn run() -> Result<(), String> {
    ...
    for (label, route, build) in fixtures {
        ...
        all_empty &= full.footprint.is_empty() && tight.footprint.is_empty();
    ...
    if require_empty && !(all_empty && all_recovered) {
        return Err(
            "--require-empty was given but a refutation was not axiom-free, or an \
             instantiation did not recover the original statement"
                .to_owned(),
        );
    }
    Ok(())
