...
fn main() {
    ...
    let Ok(parsed) = axeyum_smtlib::parse_script(&text) else {
        eprintln!("parse {path}: failed");
        std::process::exit(2);
    };
    ...
    if let Some(s) = axeyum_solver::single_cell_last_check() {
        println!(
        ...
        println!("{verdict}\t(no certificate check recorded)\t{path}");
    }
}
