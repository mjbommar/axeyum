...
use axeyum_ir::{Assignment, Op, Rational, Sort, TermArena, TermId, TermNode, Value, eval};
use axeyum_solver::{
...
/// Coverage guard for `cdclt_lra_vs_sibling_online_differential_fuzz` (same seed
...
///
/// Not guarded here because it is structural, not an LCG artefact: the
/// conjunction fuzz draws only integer-valued `rconst(i128)` coefficients and
/// constants, so a non-integer rational such as `(1/2)·x0 <= 1/3` is unreachable
/// by construction.
#[test]
fn the_generator_reaches_mixed_polarity_clauses() {
    ...
    eprintln!(
        "cdclt-lra generator coverage: mixed-polarity width-2 clauses {mixed}/{width2}, \
