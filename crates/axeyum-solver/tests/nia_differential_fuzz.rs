...
struct AxeyumOutcome {
    ...
    /// (captured only for the opt-in `NIA_DUMP_UNKNOWN` capability-gap dump).
...
fn nia_differential_fuzz_disagree_zero() {
    ...
    for seed in 0..INSTANCES {
        ...
        if ax_label == Verdict::Unknown {
            ...
            // single machine-greppable line per gap when `NIA_DUMP_UNKNOWN` is set;
            // zero behavior change (no output) when the env var is unset.
            if std::env::var_os("NIA_DUMP_UNKNOWN").is_some() {
