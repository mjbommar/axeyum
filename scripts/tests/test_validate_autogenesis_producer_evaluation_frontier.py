...
class Controls(unittest.TestCase):
 ...
 def test_control_outside_frontier_rejected(self):
  d=copy.deepcopy(self.data);d['must_decline_control_fact_ids'].append('F:invented');d['must_decline_control_fact_ids'].sort();d['census']['must_decline_controls']+=1;d['census']['candidate_facts']-=1;self.assertTrue(any('outside evaluation' in x for x in EF.validate(d)))
