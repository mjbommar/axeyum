...
ROOT=pathlib.Path(__file__).resolve().parents[1];AUTO=ROOT/'artifacts/autogenesis';OUT=AUTO/'producer-evaluation-frontier-v1.json';NURSERY=AUTO/'nursery-v1.json';CATALOG=AUTO/'mathlib-nat-int-fact-catalog-v1.json';FRONTIER=ROOT/'scripts/fact-frontier.py'
...
def build():
 m=module()
 try:
  facts=m.load();machine=m.build_machine_frontier(facts);nursery=json.loads(NURSERY.read_text());catalog=json.loads(CATALOG.read_text())
 ...
 return {'schema_version':1,'kind':'axeyum-autogenesis-producer-evaluation-frontier','derivation':{'source':'scripts/fact-frontier.py build_machine_frontier intersected with nursery train/development','frontier_sha256':machine['frontier_sha256'],'ledger_sha256':machine['ledger']['ledger_sha256'],'nursery_sha256':sha(NURSERY),'reviewed_fact_catalog_sha256':sha(CATALOG),'evaluation_partitions':['development','train'],'trust_boundary':'deterministic train/development producer input only; never held-out inspection, operation registration, proof, or admission authority'},'census':{'global_dependency_ready_facts':len(ready),'evaluation_dependency_ready_facts':len(selected),'by_partition':by_partition,'excluded_held_out_ready_facts':len(held),'excluded_outside_evaluation_ready_facts':len(outside),'groups':len(groups)},'groups':groups}
def main():
