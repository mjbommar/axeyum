#!/usr/bin/env python3
"""Validate the twice-reconstructed exact Nat.fib_gcd capsule."""
...
def validate() -> dict:
    ...
    if (
        ...
        or result.get("authority") != {"target_credit": 1, "fact_status_changes": 0, "ledger_writes": 0}
    ):
        raise RuntimeError("V3 construction identity, evidence, execution, or authority changed")
    return result
...
def main() -> int:
    try:
        ...
        print(f"autogenesis-nat-fib-gcd-construction-result-v3: FAIL: {error}", file=sys.stderr)
        return 1
    print("AUTOGENESIS_NAT_FIB_GCD_CONSTRUCTION_RESULT_V3_OK|runs=2|imports=4|footprint=0|target_credit=1")
    return 0
