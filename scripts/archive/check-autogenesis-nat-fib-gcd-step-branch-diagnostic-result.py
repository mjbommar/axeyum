#!/usr/bin/env python3
"""Validate the retained zero-pass, successor-fail step diagnostic."""
...
class ResultError(RuntimeError):
    """The branch diagnostic evidence or zero authority changed."""
...
def validate() -> dict:
    ...
    if (
        result.get("state") != "zero-branch-infers-successor-branch-internal-type-mismatch"
        ...
        or observation.get("zero_branch_inference_succeeded") is not True
        or observation.get("successor_branch_inference_succeeded") is not False
        ...
        raise ResultError("branch diagnostic identity, evidence, execution, or authority changed")
    return result
...
def main() -> int:
    try:
        ...
        print(f"autogenesis-nat-fib-gcd-step-branch-diagnostic-result: FAIL: {error}", file=sys.stderr)
        return 1
    print("AUTOGENESIS_NAT_FIB_GCD_STEP_BRANCH_DIAGNOSTIC_RESULT_OK|zero=pass|successor=fail|target=0")
    return 0
