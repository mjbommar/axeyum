...
def solver_reuse_disposition_dashboard(
    packs: list[dict[str, Any]],
    pack_gates: dict[str, int],
) -> str:
    status_rank = {
    ...
    for pack in packs:
        ...
        reuse = metadata.get("solver_reuse") or {}
        status = reuse.get("status", "unclassified")
        evidence_rows = reuse.get("evidence", [])
        if status == "promoted":
            promoted_evidence_rows += len(evidence_rows)
        status_counts[status] += 1
        rows.append(
            {
                "status": status,
                "pack_id": metadata["id"],
                "gate": gate_label(pack_gates[metadata["id"]]),
                "next_gate": next_gate_label(pack_gates[metadata["id"]]),
                "fields": md_list(metadata["field_ids"]),
                "fragments": md_list(metadata["axeyum_fragments"]),
                "proof_statuses": count_text(proof_counts(checks, include_checked=True)),
                "pressure_buckets": md_list(
                    [bucket["label"] for bucket in pressure_buckets_for_pack(pack)]
                ),
                "target": reuse.get("target", "-"),
                "evidence_rows": md_list(evidence_rows),
                "next_step": reuse.get(
                    "next_step",
                    "Add `solver_reuse.status` with target/evidence/next_step, "
                    "or classify the pack as non-benchmark-horizon.",
                ),
                "validator": metadata["validator_command"],
                "unsat_checks": md_list(unsat_check_ids(checks)),
            }
        )

    rows.sort(key=lambda row: (status_rank.get(row["status"], 99), row["pack_id"]))
    statuses = sorted(
        set(status_counts) | set(status_rank),
        key=lambda status: (status_rank.get(status, 99), status),
    )
    unclassified = [row for row in rows if row["status"] == "unclassified"]
    ...
    lines = [
        ...
        "| Metric | Count |",
        "|---|---:|",
        f"| Non-template math packs | {len(packs)} |",
        f"| Promoted evidence row links | {promoted_evidence_rows} |",
        f"| Unclassified packs | {len(unclassified)} |",
        "",
        "## Status Totals",
        "",
        "| Status | Packs |",
    ...
    if not unclassified:
        lines.append("No unclassified solver-reuse packs.")
    else:
        lines.extend(
            [
                "| Pack | Gate | Next Gate | Fields | Fragments | Proof Statuses | Unsat Checks | Validator | Suggested Next Step |",
                "|---|---|---|---|---|---|---|---|---|",
            ]
        )
        for row in unclassified:
            lines.append(
                ...
                f"{row['fields']} | "
                f"{row['fragments']} | "
                f"{row['proof_statuses']} | "
                f"{row['unsat_checks']} | "
                f"`{table_cell(row['validator'])}` | "
                f"{table_cell(row['next_step'])} |"
            )
    ...
    for row in rows:
        lines.append(
            ...
            f"{row['fragments']} | "
            f"{row['proof_statuses']} | "
            f"{row['pressure_buckets']} | "
            f"{table_cell(row['target'])} | "
            f"{row['evidence_rows']} | "
            f"{table_cell(row['next_step'])} | "
            f"`{table_cell(row['validator'])}` |"
        )
...
def main() -> int:
    ...
    write(
        OUT_DIR / "solver-reuse-disposition-audit.md",
        solver_reuse_disposition_dashboard(packs, pack_gates),
    )
