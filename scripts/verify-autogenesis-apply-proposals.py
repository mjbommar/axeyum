#!/usr/bin/env python3
"""Verify a sandboxed apply-proposal bundle against its exact input catalog."""
...
PROPOSER = ROOT / "scripts/autogenesis-apply-proposer.py"
...
def load_proposer():
    spec = importlib.util.spec_from_file_location("autogenesis_apply_proposer", PROPOSER)
...
def verify(catalog: dict, bundle: dict, tsv: str) -> None:
    ...
    denied = set(catalog.get("denied_theorems") or [])
    proposed = {plan["theorem"] for plan in bundle["plans"]}
    leaked = sorted(denied.intersection(proposed))
    if leaked:
        raise ProposalError(f"proposal bundle contains denied theorems: {leaked}")
