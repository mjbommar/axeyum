...
This is intentionally a committed snapshot. It measures populations, static
gate reachability, and—when present—the latest provider-captured CI receipt.
The receipt remains bound to its tested commit; it never turns an ancestor's
result into a claim that the current checkout passed.
"""
...
RUNTIME_RECEIPT = ROOT / "artifacts/runtime/ci-latest-v1.json"
...
def _runtime_gate_status() -> dict[str, Any]:
    if not RUNTIME_RECEIPT.exists():
        return {
            "state": "not-recorded",
            "meaning": "No provider-captured CI receipt is committed; static wiring is not runtime evidence.",
        }
    receipt = _load(RUNTIME_RECEIPT)
    run = receipt.get("run")
    jobs = receipt.get("jobs")
    if (
        receipt.get("kind") != "axeyum-github-actions-runtime-receipt"
        or not isinstance(run, dict)
        or not isinstance(jobs, list)
        or run.get("status") != "completed"
    ):
        raise HealthError("runtime receipt is not a completed canonical receipt")
    commit = run.get("head_sha")
    if not isinstance(commit, str):
        raise HealthError("runtime receipt has no tested commit")
    ...
    if ancestor.returncode:
        raise HealthError("runtime receipt commit is not an ancestor of HEAD")
    failed = sorted(
        str(job.get("name"))
        for job in jobs
    ...
    state = "passed-ancestor" if conclusion == "success" else "failed-ancestor"
    return {
        "state": state,
...
def build() -> dict[str, Any]:
    ...
    if RUNTIME_RECEIPT.exists():
        sources.append(RUNTIME_RECEIPT)
    ...
    return {
        ...
        "runtime_gate_status": _runtime_gate_status(),
...
def render(document: dict[str, Any]) -> str:
    ...
    runtime = document["runtime_gate_status"]
