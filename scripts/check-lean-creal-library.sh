...
#
# COMMAND_FLOOR is the MEASURED published count, not the replay census's
# kernel-accepted count (3,542, ADR-1661). The first draft of this gate assumed
# the source route must carry at least the wire route's count, because a
# Type-valued proof the wire cannot call a theorem is an ordinary `def` in
# source. Measured 2026-09-05 on leanprover/lean4:v4.34.0-rc1, that assumption
# is FALSE in the other direction: Lean's ELABORATOR refuses 19 declarations
# from source that its KERNEL accepts over the wire (ADR-0517's two checkers),
# and their dependency closure is 103 declarations -- among them the FTC in
# both directions, `CReal.e` and `CReal.pi`
# (`artifacts/measurements/lean-creal-elaborator-refusals-2026-09-05.json`,
# ADR-1675). So the package publishes 3,617 - 103 - 117 (regenerated by Lean
# from `inductive` commands) = 3,397 commands, and that is the floor. A drop
# below it is a regression; a rise is what a refusal being fixed looks like,
# and should raise the floor and lower the ceiling together.
#
