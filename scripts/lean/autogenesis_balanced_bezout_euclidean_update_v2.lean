...
def BalancedBezoutUpdateV2 (m n g : Nat) : Prop :=
  ∃ mp mn np nn : Nat,
    g + m * mn + n * nn = m * mp + n * np

private theorem rotateLastFiveV2 (a b c d e : Nat) :
...
private theorem rotateFourthThenSwapV2 (a b c d : Nat) :
    ((a + b) + c) + d = ((d + a) + c) + b := by
  calc
    ((a + b) + c) + d = ((a + b) + d) + c :=
...
theorem balancedBezoutEuclideanUpdateV2
    ...
    BalancedBezoutUpdateV2 divisor dividend common := by
  ...
  calc
    ...
    _ = (((common + rmn) + dnn) + dqmp) + dqmn :=
      rotateLastFiveV2 common dnn dqmp dqmn rmn
    _ = (((rmp + dnp) + dqmp) + dqmn) := recursiveLifted
    _ = ((dnp + dqmn) + dqmp) + rmp :=
      (rotateFourthThenSwapV2 dnp dqmn dqmp rmp).symm
    _ = (dnp + dqmn) + (dqmp + rmp) :=
