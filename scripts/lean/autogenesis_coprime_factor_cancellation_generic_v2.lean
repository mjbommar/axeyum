...
def BalancedBezoutCancellationV2 (a c g : Nat) : Prop :=
  ∃ mp mn np nn : Nat,
    g + a * mn + c * nn = a * mp + c * np

theorem coprimeFactorDivisibilityCancellationGenericV2
    (balancedBezout : ∀ a c : Nat,
      BalancedBezoutCancellationV2 a c (Nat.gcd a c))
    (a c b d : Nat)
    (coprime : Nat.gcd a c = 1)
    (dividesA : d ∣ a)
    (dividesProduct : d ∣ c * b) :
    d ∣ b := by
  have certificate : BalancedBezoutCancellationV2 a c 1 :=
    Eq.mp
      (congrArg (BalancedBezoutCancellationV2 a c) coprime)
      (balancedBezout a c)
