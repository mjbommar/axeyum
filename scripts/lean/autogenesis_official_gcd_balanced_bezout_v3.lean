...
def OfficialBalancedBezoutV3 (m n g : Nat) : Prop :=
  ∃ mp mn np nn : Nat,
    g + m * mn + n * nn = m * mp + n * np

theorem modQuotientWitnessV3 (m n : Nat) (hm : 0 < m) :
...
theorem officialGcdBalancedBezoutV3
  ...
  · intro m n hm ih
    ...
    | succ m =>
        rw [gcdSucc m n]
        rcases modQuotientWitnessV3 (Nat.succ m) n (Nat.zero_lt_succ m) with
          ⟨q, hq⟩
        rcases ih with ⟨mp, mn, np, nn, hbezout⟩
        change Nat.gcd (Nat.mod n (Nat.succ m)) (Nat.succ m) +
            Nat.mod n (Nat.succ m) * mn + Nat.succ m * nn =
          Nat.mod n (Nat.succ m) * mp + Nat.succ m * np at hbezout
        have hqMn :
            (Nat.succ m * q + Nat.mod n (Nat.succ m)) * mn = n * mn :=
          congrArg (fun value => value * mn) hq
        have hqMp :
            (Nat.succ m * q + Nat.mod n (Nat.succ m)) * mp = n * mp :=
          congrArg (fun value => value * mp) hq
