...
def classify(demands: Counter[str], accepted: int, measured: int) -> tuple[int, str, str]:
    if accepted >= 3:
        return 0, "operation-integration-ready", "register the unchanged producer over at least three checked targets"
    if accepted:
        return 1, "expand-unchanged-producer", "convert two more siblings with the already accepted producer contract"
    if demands["missing-rewrite-or-induction-plan"] >= 3:
        return 2, "shared-proof-composition", "add one target-agnostic rewrite or induction plan and rerun the family"
    if demands["non-equality-terminal-family"] >= 3:
        return 3, "shared-producer-grammar", "add one reusable terminal relation family and rerun unchanged retrieval"
    if demands["type-slice-generalization"] >= 3:
        return 4, "shared-statement-contract", "complete one shared semantic contract and rerun the family"
    if measured == 0:
        return 5, "measurement-missing", "run the current producer protocol over every ready sibling before implementation"
    return 6, "fragmented-obstructions", "retain the measured declines until one obstruction reaches three siblings"
...
def build() -> dict[str, Any]:
    ...
    for family, facts in family_facts.items():
        if not facts:
            continue
        measured_rows = [strategy_by_fact[fact] for fact in sorted(facts) if fact in strategy_by_fact]
        demands = Counter(row["capability_demand"] for row in measured_rows)
        results = Counter(row["result"] for row in measured_rows)
        accepted_ids = sorted(row["fact_id"] for row in measured_rows if row["result"] == "accepted")
        ...
        rows.append(
            {
                ...
                "target_owned_semantic_analogue_fact_ids": sorted(facts.intersection(analogue_facts)),
