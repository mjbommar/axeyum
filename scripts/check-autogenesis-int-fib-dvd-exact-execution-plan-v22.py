...
def main():
 try:
  ...
  assert p["state"]=="preregistered-one-dependent-motive-execution" and sha256(ROOT/pred["path"])==pred["sha256"] and sha256(impl)==p["implementation_sha256"] and not pathlib.Path(p["output"]).exists()
