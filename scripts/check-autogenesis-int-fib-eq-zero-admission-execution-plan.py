#!/usr/bin/env python3
"""Validate crash-safe Int.fib_eq_zero admission execution authority."""
import hashlib,json,pathlib,subprocess,sys
ROOT=pathlib.Path(__file__).resolve().parents[1]; PLAN=ROOT/"artifacts/autogenesis/mathlib-int-fib-eq-zero-admission-execution-plan-v1.json"
...
def main():
 try:
  ...
  assert p["selection"]=={"fact_id":"F:ml430-int-fib-eq-zero-8193c7cb","operation_id":"authoritative-mathlib-int-fib-eq-zero-kernel-capsule-v1","unique_admissible":True}
 ...
 except (AssertionError,OSError,ValueError,KeyError,TypeError,subprocess.CalledProcessError) as error: print(f"autogenesis-int-fib-eq-zero-admission-execution-plan: FAIL: {error}",file=sys.stderr); return 1
 print("autogenesis-int-fib-eq-zero-admission-execution-plan: PASS: faults=0/1|recoveries=0/1|writes=0/1"); return 0
